import {
  bT as _t,
  bN as ft,
  bU as Ye,
  bV as ur,
  bW as it,
  bX as Lt,
  bY as nt,
  bD as z,
  bZ as lc,
  b_ as Dt,
  b$ as uc,
  c0 as vc,
  c1 as cc,
  c2 as L,
  c3 as au,
  c4 as nu,
  c5 as ho,
  c6 as hc,
  c7 as fc,
  c8 as St,
  bE as Ct,
  c9 as Ht,
  bF as lt,
  ca as iu,
  cb as pe,
  cc as _e,
  cd as yr,
  ce as pc,
  cf as fo,
  cg as O,
  ch as dc,
  bL as ou,
  ci as et,
  cj as gc,
  ck as yc,
  cl as W,
  cm as su,
  cn as Qt,
  co as lu,
  cp as uu,
  cq as mc,
  cr as wt,
  cs as vu,
  ct as _a,
  cu as Sc,
  cv as xc,
  cw as bc,
  cx as _c,
  cy as wc,
  cz as Ac,
  cA as Tc,
  cB as Ic,
  cC as Dc,
  cD as Lc,
  cE as Cc,
  cF as ea,
  cG as Pc,
  cH as Gt,
  cI as Mc,
  cJ as cu,
  cK as ci,
  cL as Ec,
  cM as Rc,
  cN as Nc,
  cO as H,
  cP as Ge,
  cQ as po,
  cR as kc,
  cS as qe,
  cT as k,
  cU as Je,
  cV as gt,
  bG as Z,
  bO as ie,
  cW as wa,
  cX as Aa,
  cY as pt,
  cZ as q,
  c_ as Vc,
  c$ as Gc,
  d0 as ve,
  d1 as we,
  d2 as Se,
  d3 as Q,
  d4 as zt,
  d5 as Ut,
  d6 as Pt,
  d7 as ct,
  d8 as Qe,
  d9 as Dr,
  da as Ta,
  db as at,
  dc as Lr,
  dd as zc,
  de as Mt,
  df as Yt,
  dg as $t,
  dh as te,
  di as Ia,
  dj as Oc,
  dk as Xt,
  dl as Ne,
  bP as Cr,
  dm as hu,
  dn as Kt,
  dp as ee,
  dq as oe,
  dr as Vn,
  ds as Bc,
  dt as Da,
  bJ as Et,
  du as bt,
  dv as Fc,
  dw as Wc,
  dx as jt,
  dy as Hc,
  bR as fu,
  dz as Uc,
  dA as go,
  dB as La,
  dC as pu,
  dD as ke,
  dE as tr,
  dF as tt,
  dG as Yc,
  dH as $c,
  dI as Xc,
  dJ as Zc,
  dK as Ca,
  dL as mr,
  dM as Ke,
  dN as hi,
  dO as fi,
  dP as qc,
  dQ as Kc,
  dR as Pr,
  dS as du,
  dT as ot,
  dU as gu,
  dV as Gn,
  dW as jc,
  dX as Jc,
  dY as It,
  dZ as er,
  d_ as pi,
  d$ as Ft,
  e0 as se,
  e1 as Qc,
  e2 as th,
  e3 as Pa,
  e4 as eh,
  e5 as rh,
  e6 as ah,
  e7 as Ma,
  e8 as yu,
  e9 as ra,
  ea as nh,
  eb as aa,
  ec as mu,
  ed as Mr,
  ee as di,
  bA as Vt,
  ef as zn,
  eg as Jt,
  eh as ih,
  ei as oh,
  bK as kt,
  ej as xt,
  ek as je,
  el as Su,
  em as yo,
  en as Gr,
  eo as mo,
  ep as xu,
  eq as sh,
  er as bu,
  es as On,
  et as _u,
  eu as So,
  ev as Er,
  ew as ye,
  ex as Rr,
  ey as lh,
  ez as uh,
  eA as $e,
  eB as xo,
  eC as Bn,
  eD as vh,
  eE as qt,
  eF as Fn,
  eG as gi,
  eH as yi,
  eI as mi,
  eJ as Nr,
  eK as Ea,
  eL as ch,
  eM as wu,
  eN as hh,
  eO as Au,
  eP as fh,
  eQ as ph,
  eR as bo,
  eS as Ve,
  eT as _o,
  eU as dh,
  eV as gh,
  eW as de,
  eX as yh,
  eY as mh,
  eZ as Tu,
  e_ as Sh,
  e$ as Si,
  f0 as na,
  f1 as xi,
  f2 as xh,
  f3 as Sr,
  f4 as Iu,
  f5 as bi,
  f6 as bh,
  f7 as _h,
  f8 as wh,
  f9 as ia,
  fa as Wn,
  fb as Hn,
  fc as Ah,
  fd as Oa,
  fe as Du,
  ff as vt,
  fg as _i,
  fh as Ra,
  fi as Ba,
  fj as Th,
  fk as Ih,
  fl as vr,
  fm as Dh,
  fn as wo,
  fo as Lh,
  fp as Ao,
  fq as Na,
  fr as Un,
  fs as Ch,
  ft as Lu,
  fu as To,
  fv as Ph,
  fw as Mh,
  fx as Eh,
  fy as Io,
  fz as Rh,
  fA as Nh,
  fB as kh,
  fC as wi,
  fD as Do,
  fE as Lo,
  fF as Vh,
  fG as Gh,
  bM as zh,
  fH as Oh,
  fI as Bh,
  fJ as Fh,
  fK as Wh,
  fL as Hh,
  fM as Uh,
  fN as Cu,
  fO as Yh,
  fP as Pu,
  fQ as Co,
  fR as Mu,
  fS as $h,
  fT as Po,
  fU as kr,
  fV as Eu,
  fW as Ai,
  fX as Xh,
  fY as Zh,
  fZ as qh,
  f_ as Kh,
  f$ as jh,
  g0 as Jh,
  g1 as Qh,
  g2 as tf,
  g3 as ef,
  g4 as rf,
  g5 as Mo,
  g6 as af,
  g7 as ar,
  g8 as zr,
  g9 as nf,
  ga as of,
  gb as sf,
  gc as lf,
  gd as uf,
  ge as vf,
  gf as cf,
  gg as Ti,
  bH as Fa,
  gh as hf,
  gi as ff,
  gj as pf,
  gk as df,
  gl as gf,
  gm as oa,
  gn as yf,
  go as mf,
  gp as Sf,
  gq as Ii,
  gr as xf,
  gs as bf,
  gt as Ru,
  gu as _f,
  gv as wf,
  gw as Af,
  gx as Tf,
  gy as If,
  gz as Eo,
  gA as Df,
  gB as sa,
  gC as Di,
  gD as Lf,
  gE as Li,
  gF as Wa,
  gG as Cf,
  gH as Pf,
  gI as Mf,
  gJ as Ef,
  gK as Rf,
  gL as Nf,
  gM as kf,
  gN as Vf,
  gO as Gf,
  gP as zf,
  gQ as Of,
  gR as Bf,
  gS as Ff,
  gT as Wf,
  gU as Hf,
  gV as Uf,
  gW as Yf
} from "./index-DuV_pBtV.js";
function $f(a) {
  if (a) {
    for (var e = [], t = 0; t < a.length; t++) e.push(a[t].slice());
    return e;
  }
}
function Xf(a, e) {
  var t = a.label,
    r = e && e.getTextGuideLine();
  return {
    dataIndex: a.dataIndex,
    dataType: a.dataType,
    seriesIndex: a.seriesModel.seriesIndex,
    text: a.label.style.text,
    rect: a.hostRect,
    labelRect: a.rect,
    align: t.style.align,
    verticalAlign: t.style.verticalAlign,
    labelLinePoints: $f(r && r.shape.points)
  };
}
var Ro = ["align", "verticalAlign", "width", "height", "fontSize"],
  Tt = new Ye(),
  Ha = _t(),
  Zf = _t();
function Or(a, e, t) {
  for (var r = 0; r < t.length; r++) {
    var n = t[r];
    e[n] != null && (a[n] = e[n]);
  }
}
var Br = ["x", "y", "rotation"],
  qf = (function () {
    function a() {
      (this._labelList = []), (this._chartViewList = []);
    }
    return (
      (a.prototype.clearLabels = function () {
        (this._labelList = []), (this._chartViewList = []);
      }),
      (a.prototype._addLabel = function (e, t, r, n, i) {
        var o = n.style,
          s = n.__hostTarget,
          l = s.textConfig || {},
          u = n.getComputedTransform(),
          v = n.getBoundingRect().plain();
        ft.applyTransform(v, v, u),
          u
            ? Tt.setLocalTransform(u)
            : ((Tt.x = Tt.y = Tt.rotation = Tt.originX = Tt.originY = 0),
              (Tt.scaleX = Tt.scaleY = 1)),
          (Tt.rotation = ur(Tt.rotation));
        var c = n.__hostTarget,
          h;
        if (c) {
          h = c.getBoundingRect().plain();
          var f = c.getComputedTransform();
          ft.applyTransform(h, h, f);
        }
        var p = h && c.getTextGuideLine();
        this._labelList.push({
          label: n,
          labelLine: p,
          seriesModel: r,
          dataIndex: e,
          dataType: t,
          layoutOption: i,
          computedLayoutOption: null,
          rect: v,
          hostRect: h,
          priority: h ? h.width * h.height : 0,
          defaultAttr: {
            ignore: n.ignore,
            labelGuideIgnore: p && p.ignore,
            x: Tt.x,
            y: Tt.y,
            scaleX: Tt.scaleX,
            scaleY: Tt.scaleY,
            rotation: Tt.rotation,
            style: {
              x: o.x,
              y: o.y,
              align: o.align,
              verticalAlign: o.verticalAlign,
              width: o.width,
              height: o.height,
              fontSize: o.fontSize
            },
            cursor: n.cursor,
            attachedPos: l.position,
            attachedRot: l.rotation
          }
        });
      }),
      (a.prototype.addLabelsOfSeries = function (e) {
        var t = this;
        this._chartViewList.push(e);
        var r = e.__model,
          n = r.get("labelLayout");
        (it(n) || Lt(n).length) &&
          e.group.traverse(function (i) {
            if (i.ignore) return !0;
            var o = i.getTextContent(),
              s = nt(i);
            o &&
              !o.disableLabelLayout &&
              t._addLabel(s.dataIndex, s.dataType, r, o, n);
          });
      }),
      (a.prototype.updateLayoutConfig = function (e) {
        var t = e.getWidth(),
          r = e.getHeight();
        function n(m, x) {
          return function () {
            ho(m, x);
          };
        }
        for (var i = 0; i < this._labelList.length; i++) {
          var o = this._labelList[i],
            s = o.label,
            l = s.__hostTarget,
            u = o.defaultAttr,
            v = void 0;
          it(o.layoutOption)
            ? (v = o.layoutOption(Xf(o, l)))
            : (v = o.layoutOption),
            (v = v || {}),
            (o.computedLayoutOption = v);
          var c = Math.PI / 180;
          l &&
            l.setTextConfig({
              local: !1,
              position: v.x != null || v.y != null ? null : u.attachedPos,
              rotation: v.rotate != null ? v.rotate * c : u.attachedRot,
              offset: [v.dx || 0, v.dy || 0]
            });
          var h = !1;
          if (
            (v.x != null
              ? ((s.x = z(v.x, t)), s.setStyle("x", 0), (h = !0))
              : ((s.x = u.x), s.setStyle("x", u.style.x)),
            v.y != null
              ? ((s.y = z(v.y, r)), s.setStyle("y", 0), (h = !0))
              : ((s.y = u.y), s.setStyle("y", u.style.y)),
            v.labelLinePoints)
          ) {
            var f = l.getTextGuideLine();
            f && (f.setShape({ points: v.labelLinePoints }), (h = !1));
          }
          var p = Ha(s);
          (p.needsUpdateLabelLine = h),
            (s.rotation = v.rotate != null ? v.rotate * c : u.rotation),
            (s.scaleX = u.scaleX),
            (s.scaleY = u.scaleY);
          for (var d = 0; d < Ro.length; d++) {
            var g = Ro[d];
            s.setStyle(g, v[g] != null ? v[g] : u.style[g]);
          }
          if (v.draggable) {
            if (((s.draggable = !0), (s.cursor = "move"), l)) {
              var y = o.seriesModel;
              if (o.dataIndex != null) {
                var S = o.seriesModel.getData(o.dataType);
                y = S.getItemModel(o.dataIndex);
              }
              s.on("drag", n(l, y.getModel("labelLine")));
            }
          } else s.off("drag"), (s.cursor = u.cursor);
        }
      }),
      (a.prototype.layout = function (e) {
        var t = e.getWidth(),
          r = e.getHeight(),
          n = lc(this._labelList),
          i = Dt(n, function (l) {
            return l.layoutOption.moveOverlap === "shiftX";
          }),
          o = Dt(n, function (l) {
            return l.layoutOption.moveOverlap === "shiftY";
          });
        uc(i, 0, t), vc(o, 0, r);
        var s = Dt(n, function (l) {
          return l.layoutOption.hideOverlap;
        });
        cc(s);
      }),
      (a.prototype.processLabelsOverall = function () {
        var e = this;
        L(this._chartViewList, function (t) {
          var r = t.__model,
            n = t.ignoreLabelLineUpdate,
            i = r.isAnimationEnabled();
          t.group.traverse(function (o) {
            if (o.ignore && !o.forceLabelAnimation) return !0;
            var s = !n,
              l = o.getTextContent();
            !s && l && (s = Ha(l).needsUpdateLabelLine),
              s && e._updateLabelLine(o, r),
              i && e._animateLabels(o, r);
          });
        });
      }),
      (a.prototype._updateLabelLine = function (e, t) {
        var r = e.getTextContent(),
          n = nt(e),
          i = n.dataIndex;
        if (r && i != null) {
          var o = t.getData(n.dataType),
            s = o.getItemModel(i),
            l = {},
            u = o.getItemVisual(i, "style");
          if (u) {
            var v = o.getVisual("drawType");
            l.stroke = u[v];
          }
          var c = s.getModel("labelLine");
          au(e, nu(s), l), ho(e, c);
        }
      }),
      (a.prototype._animateLabels = function (e, t) {
        var r = e.getTextContent(),
          n = e.getTextGuideLine();
        if (
          r &&
          (e.forceLabelAnimation ||
            (!r.ignore && !r.invisible && !e.disableLabelAnimation && !hc(e)))
        ) {
          var i = Ha(r),
            o = i.oldLayout,
            s = nt(e),
            l = s.dataIndex,
            u = { x: r.x, y: r.y, rotation: r.rotation },
            v = t.getData(s.dataType);
          if (o) {
            r.attr(o);
            var h = e.prevStates;
            h &&
              (Ht(h, "select") >= 0 && r.attr(i.oldLayoutSelect),
              Ht(h, "emphasis") >= 0 && r.attr(i.oldLayoutEmphasis)),
              lt(r, u, t, l);
          } else if ((r.attr(u), !fc(r).valueAnimation)) {
            var c = St(r.style.opacity, 1);
            (r.style.opacity = 0), Ct(r, { style: { opacity: c } }, t, l);
          }
          if (((i.oldLayout = u), r.states.select)) {
            var f = (i.oldLayoutSelect = {});
            Or(f, u, Br), Or(f, r.states.select, Br);
          }
          if (r.states.emphasis) {
            var p = (i.oldLayoutEmphasis = {});
            Or(p, u, Br), Or(p, r.states.emphasis, Br);
          }
          iu(r, l, v, t, t);
        }
        if (n && !n.ignore && !n.invisible) {
          var i = Zf(n),
            o = i.oldLayout,
            d = { points: n.shape.points };
          o
            ? (n.attr({ shape: o }), lt(n, { shape: d }, t))
            : (n.setShape(d),
              (n.style.strokePercent = 0),
              Ct(n, { style: { strokePercent: 1 } }, t)),
            (i.oldLayout = d);
        }
      }),
      a
    );
  })(),
  Ua = _t();
function Kf(a) {
  a.registerUpdateLifecycle("series:beforeupdate", function (e, t, r) {
    var n = Ua(t).labelManager;
    n || (n = Ua(t).labelManager = new qf()), n.clearLabels();
  }),
    a.registerUpdateLifecycle("series:layoutlabels", function (e, t, r) {
      var n = Ua(t).labelManager;
      r.updatedSeries.forEach(function (i) {
        n.addLabelsOfSeries(t.getViewOfSeriesModel(i));
      }),
        n.updateLayoutConfig(t),
        n.layout(t),
        n.processLabelsOverall();
    });
}
var Ya = Math.sin,
  $a = Math.cos,
  Nu = Math.PI,
  Te = Math.PI * 2,
  jf = 180 / Nu,
  ku = (function () {
    function a() {}
    return (
      (a.prototype.reset = function (e) {
        (this._start = !0),
          (this._d = []),
          (this._str = ""),
          (this._p = Math.pow(10, e || 4));
      }),
      (a.prototype.moveTo = function (e, t) {
        this._add("M", e, t);
      }),
      (a.prototype.lineTo = function (e, t) {
        this._add("L", e, t);
      }),
      (a.prototype.bezierCurveTo = function (e, t, r, n, i, o) {
        this._add("C", e, t, r, n, i, o);
      }),
      (a.prototype.quadraticCurveTo = function (e, t, r, n) {
        this._add("Q", e, t, r, n);
      }),
      (a.prototype.arc = function (e, t, r, n, i, o) {
        this.ellipse(e, t, r, r, 0, n, i, o);
      }),
      (a.prototype.ellipse = function (e, t, r, n, i, o, s, l) {
        var u = s - o,
          v = !l,
          c = Math.abs(u),
          h = pe(c - Te) || (v ? u >= Te : -u >= Te),
          f = u > 0 ? u % Te : (u % Te) + Te,
          p = !1;
        h ? (p = !0) : pe(c) ? (p = !1) : (p = f >= Nu == !!v);
        var d = e + r * $a(o),
          g = t + n * Ya(o);
        this._start && this._add("M", d, g);
        var y = Math.round(i * jf);
        if (h) {
          var S = 1 / this._p,
            m = (v ? 1 : -1) * (Te - S);
          this._add("A", r, n, y, 1, +v, e + r * $a(o + m), t + n * Ya(o + m)),
            S > 0.01 && this._add("A", r, n, y, 0, +v, d, g);
        } else {
          var x = e + r * $a(s),
            b = t + n * Ya(s);
          this._add("A", r, n, y, +p, +v, x, b);
        }
      }),
      (a.prototype.rect = function (e, t, r, n) {
        this._add("M", e, t),
          this._add("l", r, 0),
          this._add("l", 0, n),
          this._add("l", -r, 0),
          this._add("Z");
      }),
      (a.prototype.closePath = function () {
        this._d.length > 0 && this._add("Z");
      }),
      (a.prototype._add = function (e, t, r, n, i, o, s, l, u) {
        for (var v = [], c = this._p, h = 1; h < arguments.length; h++) {
          var f = arguments[h];
          if (isNaN(f)) {
            this._invalid = !0;
            return;
          }
          v.push(Math.round(f * c) / c);
        }
        this._d.push(e + v.join(" ")), (this._start = e === "Z");
      }),
      (a.prototype.generateStr = function () {
        (this._str = this._invalid ? "" : this._d.join("")), (this._d = []);
      }),
      (a.prototype.getStr = function () {
        return this._str;
      }),
      a
    );
  })(),
  Ci = "none",
  Jf = Math.round;
function Qf(a) {
  var e = a.fill;
  return e != null && e !== Ci;
}
function tp(a) {
  var e = a.stroke;
  return e != null && e !== Ci;
}
var Yn = ["lineCap", "miterLimit", "lineJoin"],
  ep = O(Yn, function (a) {
    return "stroke-" + a.toLowerCase();
  });
function rp(a, e, t, r) {
  var n = e.opacity == null ? 1 : e.opacity;
  if (t instanceof _e) {
    a("opacity", n);
    return;
  }
  if (Qf(e)) {
    var i = yr(e.fill);
    a("fill", i.color);
    var o =
      e.fillOpacity != null ? e.fillOpacity * i.opacity * n : i.opacity * n;
    o < 1 && a("fill-opacity", o);
  } else a("fill", Ci);
  if (tp(e)) {
    var s = yr(e.stroke);
    a("stroke", s.color);
    var l = e.strokeNoScale ? t.getLineScale() : 1,
      u = l ? (e.lineWidth || 0) / l : 0,
      v =
        e.strokeOpacity != null
          ? e.strokeOpacity * s.opacity * n
          : s.opacity * n,
      c = e.strokeFirst;
    if (
      (u !== 1 && a("stroke-width", u),
      c && a("paint-order", c ? "stroke" : "fill"),
      v < 1 && a("stroke-opacity", v),
      e.lineDash)
    ) {
      var h = pc(t),
        f = h[0],
        p = h[1];
      f &&
        ((p = Jf(p || 0)),
        a("stroke-dasharray", f.join(",")),
        (p || r) && a("stroke-dashoffset", p));
    }
    for (var d = 0; d < Yn.length; d++) {
      var g = Yn[d];
      if (e[g] !== fo[g]) {
        var y = e[g] || fo[g];
        y && a(ep[d], y);
      }
    }
  }
}
var Vu = "http://www.w3.org/2000/svg",
  Gu = "http://www.w3.org/1999/xlink",
  ap = "http://www.w3.org/2000/xmlns/",
  np = "http://www.w3.org/XML/1998/namespace",
  No = "ecmeta_";
function zu(a) {
  return document.createElementNS(Vu, a);
}
function ht(a, e, t, r, n) {
  return { tag: a, attrs: t || {}, children: r, text: n, key: e };
}
function ip(a, e) {
  var t = [];
  if (e)
    for (var r in e) {
      var n = e[r],
        i = r;
      n !== !1 && (n !== !0 && n != null && (i += '="' + n + '"'), t.push(i));
    }
  return "<" + a + " " + t.join(" ") + ">";
}
function op(a) {
  return "</" + a + ">";
}
function Pi(a, e) {
  e = e || {};
  var t = e.newline
    ? `
`
    : "";
  function r(n) {
    var i = n.children,
      o = n.tag,
      s = n.attrs,
      l = n.text;
    return (
      ip(o, s) +
      (o !== "style" ? dc(l) : l || "") +
      (i
        ? "" +
          t +
          O(i, function (u) {
            return r(u);
          }).join(t) +
          t
        : "") +
      op(o)
    );
  }
  return r(a);
}
function sp(a, e, t) {
  t = t || {};
  var r = t.newline
      ? `
`
      : "",
    n = " {" + r,
    i = r + "}",
    o = O(Lt(a), function (l) {
      return (
        l +
        n +
        O(Lt(a[l]), function (u) {
          return u + ":" + a[l][u] + ";";
        }).join(r) +
        i
      );
    }).join(r),
    s = O(Lt(e), function (l) {
      return (
        "@keyframes " +
        l +
        n +
        O(Lt(e[l]), function (u) {
          return (
            u +
            n +
            O(Lt(e[l][u]), function (v) {
              var c = e[l][u][v];
              return v === "d" && (c = 'path("' + c + '")'), v + ":" + c + ";";
            }).join(r) +
            i
          );
        }).join(r) +
        i
      );
    }).join(r);
  return !o && !s ? "" : ["<![CDATA[", o, s, "]]>"].join(r);
}
function $n(a) {
  return {
    zrId: a,
    shadowCache: {},
    patternCache: {},
    gradientCache: {},
    clipPathCache: {},
    defs: {},
    cssNodes: {},
    cssAnims: {},
    cssStyleCache: {},
    cssAnimIdx: 0,
    shadowIdx: 0,
    gradientIdx: 0,
    patternIdx: 0,
    clipPathIdx: 0
  };
}
function ko(a, e, t, r) {
  return ht(
    "svg",
    "root",
    {
      width: a,
      height: e,
      xmlns: Vu,
      "xmlns:xlink": Gu,
      version: "1.1",
      baseProfile: "full",
      viewBox: r ? "0 0 " + a + " " + e : !1
    },
    t
  );
}
var lp = 0;
function Ou() {
  return lp++;
}
var Vo = {
    cubicIn: "0.32,0,0.67,0",
    cubicOut: "0.33,1,0.68,1",
    cubicInOut: "0.65,0,0.35,1",
    quadraticIn: "0.11,0,0.5,0",
    quadraticOut: "0.5,1,0.89,1",
    quadraticInOut: "0.45,0,0.55,1",
    quarticIn: "0.5,0,0.75,0",
    quarticOut: "0.25,1,0.5,1",
    quarticInOut: "0.76,0,0.24,1",
    quinticIn: "0.64,0,0.78,0",
    quinticOut: "0.22,1,0.36,1",
    quinticInOut: "0.83,0,0.17,1",
    sinusoidalIn: "0.12,0,0.39,0",
    sinusoidalOut: "0.61,1,0.88,1",
    sinusoidalInOut: "0.37,0,0.63,1",
    exponentialIn: "0.7,0,0.84,0",
    exponentialOut: "0.16,1,0.3,1",
    exponentialInOut: "0.87,0,0.13,1",
    circularIn: "0.55,0,1,0.45",
    circularOut: "0,0.55,0.45,1",
    circularInOut: "0.85,0,0.15,1"
  },
  De = "transform-origin";
function up(a, e, t) {
  var r = W({}, a.shape);
  W(r, e), a.buildPath(t, r);
  var n = new ku();
  return n.reset(lu(a)), t.rebuildPath(n, 1), n.generateStr(), n.getStr();
}
function vp(a, e) {
  var t = e.originX,
    r = e.originY;
  (t || r) && (a[De] = t + "px " + r + "px");
}
var cp = {
  fill: "fill",
  opacity: "opacity",
  lineWidth: "stroke-width",
  lineDashOffset: "stroke-dashoffset"
};
function Bu(a, e) {
  var t = e.zrId + "-ani-" + e.cssAnimIdx++;
  return (e.cssAnims[t] = a), t;
}
function hp(a, e, t) {
  var r = a.shape.paths,
    n = {},
    i,
    o;
  if (
    (L(r, function (l) {
      var u = $n(t.zrId);
      (u.animation = !0), ka(l, {}, u, !0);
      var v = u.cssAnims,
        c = u.cssNodes,
        h = Lt(v),
        f = h.length;
      if (f) {
        o = h[f - 1];
        var p = v[o];
        for (var d in p) {
          var g = p[d];
          (n[d] = n[d] || { d: "" }), (n[d].d += g.d || "");
        }
        for (var y in c) {
          var S = c[y].animation;
          S.indexOf(o) >= 0 && (i = S);
        }
      }
    }),
    !!i)
  ) {
    e.d = !1;
    var s = Bu(n, t);
    return i.replace(o, s);
  }
}
function Go(a) {
  return et(a) ? (Vo[a] ? "cubic-bezier(" + Vo[a] + ")" : gc(a) ? a : "") : "";
}
function ka(a, e, t, r) {
  var n = a.animators,
    i = n.length,
    o = [];
  if (a instanceof ou) {
    var s = hp(a, e, t);
    if (s) o.push(s);
    else if (!i) return;
  } else if (!i) return;
  for (var l = {}, u = 0; u < i; u++) {
    var v = n[u],
      c = [v.getMaxTime() / 1e3 + "s"],
      h = Go(v.getClip().easing),
      f = v.getDelay();
    h ? c.push(h) : c.push("linear"),
      f && c.push(f / 1e3 + "s"),
      v.getLoop() && c.push("infinite");
    var p = c.join(" ");
    (l[p] = l[p] || [p, []]), l[p][1].push(v);
  }
  function d(S) {
    var m = S[1],
      x = m.length,
      b = {},
      _ = {},
      w = {},
      A = "animation-timing-function";
    function I(X, U, rt) {
      for (
        var J = X.getTracks(), st = X.getMaxTime(), dt = 0;
        dt < J.length;
        dt++
      ) {
        var At = J[dt];
        if (At.needsAnimate()) {
          var ut = At.keyframes,
            mt = At.propName;
          if ((rt && (mt = rt(mt)), mt))
            for (var Rt = 0; Rt < ut.length; Rt++) {
              var Zt = ut[Rt],
                Ae = Math.round((Zt.time / st) * 100) + "%",
                rr = Go(Zt.easing),
                co = Zt.rawValue;
              (et(co) || Qt(co)) &&
                ((U[Ae] = U[Ae] || {}),
                (U[Ae][mt] = Zt.rawValue),
                rr && (U[Ae][A] = rr));
            }
        }
      }
    }
    for (var T = 0; T < x; T++) {
      var D = m[T],
        M = D.targetName;
      M ? M === "shape" && I(D, _) : !r && I(D, b);
    }
    for (var C in b) {
      var P = {};
      yc(P, a), W(P, b[C]);
      var E = su(P),
        R = b[C][A];
      (w[C] = E ? { transform: E } : {}), vp(w[C], P), R && (w[C][A] = R);
    }
    var N,
      V = !0;
    for (var C in _) {
      w[C] = w[C] || {};
      var G = !N,
        R = _[C][A];
      G && (N = new uu());
      var B = N.len();
      N.reset(), (w[C].d = up(a, _[C], N));
      var F = N.len();
      if (!G && B !== F) {
        V = !1;
        break;
      }
      R && (w[C][A] = R);
    }
    if (!V) for (var C in w) delete w[C].d;
    if (!r)
      for (var T = 0; T < x; T++) {
        var D = m[T],
          M = D.targetName;
        M === "style" &&
          I(D, w, function (J) {
            return cp[J];
          });
      }
    for (var Y = Lt(w), K = !0, $, T = 1; T < Y.length; T++) {
      var j = Y[T - 1],
        yt = Y[T];
      if (w[j][De] !== w[yt][De]) {
        K = !1;
        break;
      }
      $ = w[j][De];
    }
    if (K && $) {
      for (var C in w) w[C][De] && delete w[C][De];
      e[De] = $;
    }
    if (
      Dt(Y, function (X) {
        return Lt(w[X]).length > 0;
      }).length
    ) {
      var re = Bu(w, t);
      return re + " " + S[0] + " both";
    }
  }
  for (var g in l) {
    var s = d(l[g]);
    s && o.push(s);
  }
  if (o.length) {
    var y = t.zrId + "-cls-" + Ou();
    (t.cssNodes["." + y] = { animation: o.join(",") }), (e.class = y);
  }
}
function fp(a, e, t) {
  if (!a.ignore)
    if (a.isSilent()) {
      var r = { "pointer-events": "none" };
      zo(r, e, t);
    } else {
      var n =
          a.states.emphasis && a.states.emphasis.style
            ? a.states.emphasis.style
            : {},
        i = n.fill;
      if (!i) {
        var o = a.style && a.style.fill,
          s =
            a.states.select &&
            a.states.select.style &&
            a.states.select.style.fill,
          l = (a.currentStates.indexOf("select") >= 0 && s) || o;
        l && (i = mc(l));
      }
      var u = n.lineWidth;
      if (u) {
        var v = !n.strokeNoScale && a.transform ? a.transform[0] : 1;
        u = u / v;
      }
      var r = { cursor: "pointer" };
      i && (r.fill = i),
        n.stroke && (r.stroke = n.stroke),
        u && (r["stroke-width"] = u),
        zo(r, e, t);
    }
}
function zo(a, e, t, r) {
  var n = JSON.stringify(a),
    i = t.cssStyleCache[n];
  i ||
    ((i = t.zrId + "-cls-" + Ou()),
    (t.cssStyleCache[n] = i),
    (t.cssNodes["." + i + ":hover"] = a)),
    (e.class = e.class ? e.class + " " + i : i);
}
var xr = Math.round;
function Fu(a) {
  return a && et(a.src);
}
function Wu(a) {
  return a && it(a.toDataURL);
}
function Mi(a, e, t, r) {
  rp(
    function (n, i) {
      var o = n === "fill" || n === "stroke";
      o && cu(i) ? Uu(e, a, n, r) : o && ci(i) ? Yu(t, a, n, r) : (a[n] = i),
        o && r.ssr && i === "none" && (a["pointer-events"] = "visible");
    },
    e,
    t,
    !1
  ),
    xp(t, a, r);
}
function Ei(a, e) {
  var t = Ec(e);
  t &&
    (t.each(function (r, n) {
      r != null && (a[(No + n).toLowerCase()] = r + "");
    }),
    e.isSilent() && (a[No + "silent"] = "true"));
}
function Oo(a) {
  return pe(a[0] - 1) && pe(a[1]) && pe(a[2]) && pe(a[3] - 1);
}
function pp(a) {
  return pe(a[4]) && pe(a[5]);
}
function Ri(a, e, t) {
  if (e && !(pp(e) && Oo(e))) {
    var r = 1e4;
    a.transform = Oo(e)
      ? "translate(" + xr(e[4] * r) / r + " " + xr(e[5] * r) / r + ")"
      : Mc(e);
  }
}
function Bo(a, e, t) {
  for (var r = a.points, n = [], i = 0; i < r.length; i++)
    n.push(xr(r[i][0] * t) / t), n.push(xr(r[i][1] * t) / t);
  e.points = n.join(" ");
}
function Fo(a) {
  return !a.smooth;
}
function dp(a) {
  var e = O(a, function (t) {
    return typeof t == "string" ? [t, t] : t;
  });
  return function (t, r, n) {
    for (var i = 0; i < e.length; i++) {
      var o = e[i],
        s = t[o[0]];
      s != null && (r[o[1]] = xr(s * n) / n);
    }
  };
}
var gp = {
  circle: [dp(["cx", "cy", "r"])],
  polyline: [Bo, Fo],
  polygon: [Bo, Fo]
};
function yp(a) {
  for (var e = a.animators, t = 0; t < e.length; t++)
    if (e[t].targetName === "shape") return !0;
  return !1;
}
function Hu(a, e) {
  var t = a.style,
    r = a.shape,
    n = gp[a.type],
    i = {},
    o = e.animation,
    s = "path",
    l = a.style.strokePercent,
    u = (e.compress && lu(a)) || 4;
  if (n && !e.willUpdate && !(n[1] && !n[1](r)) && !(o && yp(a)) && !(l < 1)) {
    s = a.type;
    var v = Math.pow(10, u);
    n[0](r, i, v);
  } else {
    var c = !a.path || a.shapeChanged();
    a.path || a.createPathProxy();
    var h = a.path;
    c && (h.beginPath(), a.buildPath(h, a.shape), a.pathUpdated());
    var f = h.getVersion(),
      p = a,
      d = p.__svgPathBuilder;
    (p.__svgPathVersion !== f || !d || l !== p.__svgPathStrokePercent) &&
      (d || (d = p.__svgPathBuilder = new ku()),
      d.reset(u),
      h.rebuildPath(d, l),
      d.generateStr(),
      (p.__svgPathVersion = f),
      (p.__svgPathStrokePercent = l)),
      (i.d = d.getStr());
  }
  return (
    Ri(i, a.transform),
    Mi(i, t, a, e),
    Ei(i, a),
    e.animation && ka(a, i, e),
    e.emphasis && fp(a, i, e),
    ht(s, a.id + "", i)
  );
}
function mp(a, e) {
  var t = a.style,
    r = t.image;
  if (
    (r && !et(r) && (Fu(r) ? (r = r.src) : Wu(r) && (r = r.toDataURL())), !!r)
  ) {
    var n = t.x || 0,
      i = t.y || 0,
      o = t.width,
      s = t.height,
      l = { href: r, width: o, height: s };
    return (
      n && (l.x = n),
      i && (l.y = i),
      Ri(l, a.transform),
      Mi(l, t, a, e),
      Ei(l, a),
      e.animation && ka(a, l, e),
      ht("image", a.id + "", l)
    );
  }
}
function Sp(a, e) {
  var t = a.style,
    r = t.text;
  if ((r != null && (r += ""), !(!r || isNaN(t.x) || isNaN(t.y)))) {
    var n = t.font || Sc,
      i = t.x || 0,
      o = xc(t.y || 0, bc(n), t.textBaseline),
      s = _c[t.textAlign] || t.textAlign,
      l = { "dominant-baseline": "central", "text-anchor": s };
    if (wc(t)) {
      var u = "",
        v = t.fontStyle,
        c = Ac(t.fontSize);
      if (!parseFloat(c)) return;
      var h = t.fontFamily || Tc,
        f = t.fontWeight;
      (u += "font-size:" + c + ";font-family:" + h + ";"),
        v && v !== "normal" && (u += "font-style:" + v + ";"),
        f && f !== "normal" && (u += "font-weight:" + f + ";"),
        (l.style = u);
    } else l.style = "font: " + n;
    return (
      r.match(/\s/) && (l["xml:space"] = "preserve"),
      i && (l.x = i),
      o && (l.y = o),
      Ri(l, a.transform),
      Mi(l, t, a, e),
      Ei(l, a),
      e.animation && ka(a, l, e),
      ht("text", a.id + "", l, void 0, r)
    );
  }
}
function Wo(a, e) {
  if (a instanceof wt) return Hu(a, e);
  if (a instanceof _e) return mp(a, e);
  if (a instanceof vu) return Sp(a, e);
}
function xp(a, e, t) {
  var r = a.style;
  if (Rc(r)) {
    var n = Nc(a),
      i = t.shadowCache,
      o = i[n];
    if (!o) {
      var s = a.getGlobalScale(),
        l = s[0],
        u = s[1];
      if (!l || !u) return;
      var v = r.shadowOffsetX || 0,
        c = r.shadowOffsetY || 0,
        h = r.shadowBlur,
        f = yr(r.shadowColor),
        p = f.opacity,
        d = f.color,
        g = h / 2 / l,
        y = h / 2 / u,
        S = g + " " + y;
      (o = t.zrId + "-s" + t.shadowIdx++),
        (t.defs[o] = ht(
          "filter",
          o,
          { id: o, x: "-100%", y: "-100%", width: "300%", height: "300%" },
          [
            ht("feDropShadow", "", {
              dx: v / l,
              dy: c / u,
              stdDeviation: S,
              "flood-color": d,
              "flood-opacity": p
            })
          ]
        )),
        (i[n] = o);
    }
    e.filter = _a(o);
  }
}
function Uu(a, e, t, r) {
  var n = a[t],
    i,
    o = { gradientUnits: n.global ? "userSpaceOnUse" : "objectBoundingBox" };
  if (Ic(n))
    (i = "linearGradient"),
      (o.x1 = n.x),
      (o.y1 = n.y),
      (o.x2 = n.x2),
      (o.y2 = n.y2);
  else if (Dc(n))
    (i = "radialGradient"),
      (o.cx = St(n.x, 0.5)),
      (o.cy = St(n.y, 0.5)),
      (o.r = St(n.r, 0.5));
  else return;
  for (var s = n.colorStops, l = [], u = 0, v = s.length; u < v; ++u) {
    var c = Lc(s[u].offset) * 100 + "%",
      h = s[u].color,
      f = yr(h),
      p = f.color,
      d = f.opacity,
      g = { offset: c };
    (g["stop-color"] = p),
      d < 1 && (g["stop-opacity"] = d),
      l.push(ht("stop", u + "", g));
  }
  var y = ht(i, "", o, l),
    S = Pi(y),
    m = r.gradientCache,
    x = m[S];
  x ||
    ((x = r.zrId + "-g" + r.gradientIdx++),
    (m[S] = x),
    (o.id = x),
    (r.defs[x] = ht(i, x, o, l))),
    (e[t] = _a(x));
}
function Yu(a, e, t, r) {
  var n = a.style[t],
    i = a.getBoundingRect(),
    o = {},
    s = n.repeat,
    l = s === "no-repeat",
    u = s === "repeat-x",
    v = s === "repeat-y",
    c;
  if (Cc(n)) {
    var h = n.imageWidth,
      f = n.imageHeight,
      p = void 0,
      d = n.image;
    if (
      (et(d) ? (p = d) : Fu(d) ? (p = d.src) : Wu(d) && (p = d.toDataURL()),
      typeof Image > "u")
    ) {
      var g =
        "Image width/height must been given explictly in svg-ssr renderer.";
      ea(h, g), ea(f, g);
    } else if (h == null || f == null) {
      var y = function (T, D) {
          if (T) {
            var M = T.elm,
              C = h || D.width,
              P = f || D.height;
            T.tag === "pattern" &&
              (u ? ((P = 1), (C /= i.width)) : v && ((C = 1), (P /= i.height))),
              (T.attrs.width = C),
              (T.attrs.height = P),
              M && (M.setAttribute("width", C), M.setAttribute("height", P));
          }
        },
        S = Pc(p, null, a, function (T) {
          l || y(_, T), y(c, T);
        });
      S && S.width && S.height && ((h = h || S.width), (f = f || S.height));
    }
    (c = ht("image", "img", { href: p, width: h, height: f })),
      (o.width = h),
      (o.height = f);
  } else
    n.svgElement &&
      ((c = Gt(n.svgElement)),
      (o.width = n.svgWidth),
      (o.height = n.svgHeight));
  if (c) {
    var m, x;
    l
      ? (m = x = 1)
      : u
      ? ((x = 1), (m = o.width / i.width))
      : v
      ? ((m = 1), (x = o.height / i.height))
      : (o.patternUnits = "userSpaceOnUse"),
      m != null && !isNaN(m) && (o.width = m),
      x != null && !isNaN(x) && (o.height = x);
    var b = su(n);
    b && (o.patternTransform = b);
    var _ = ht("pattern", "", o, [c]),
      w = Pi(_),
      A = r.patternCache,
      I = A[w];
    I ||
      ((I = r.zrId + "-p" + r.patternIdx++),
      (A[w] = I),
      (o.id = I),
      (_ = r.defs[I] = ht("pattern", I, o, [c]))),
      (e[t] = _a(I));
  }
}
function bp(a, e, t) {
  var r = t.clipPathCache,
    n = t.defs,
    i = r[a.id];
  if (!i) {
    i = t.zrId + "-c" + t.clipPathIdx++;
    var o = { id: i };
    (r[a.id] = i), (n[i] = ht("clipPath", i, o, [Hu(a, t)]));
  }
  e["clip-path"] = _a(i);
}
function Ho(a) {
  return document.createTextNode(a);
}
function Ee(a, e, t) {
  a.insertBefore(e, t);
}
function Uo(a, e) {
  a.removeChild(e);
}
function Yo(a, e) {
  a.appendChild(e);
}
function $u(a) {
  return a.parentNode;
}
function Xu(a) {
  return a.nextSibling;
}
function Xa(a, e) {
  a.textContent = e;
}
var $o = 58,
  _p = 120,
  wp = ht("", "");
function Xn(a) {
  return a === void 0;
}
function ne(a) {
  return a !== void 0;
}
function Ap(a, e, t) {
  for (var r = {}, n = e; n <= t; ++n) {
    var i = a[n].key;
    i !== void 0 && (r[i] = n);
  }
  return r;
}
function cr(a, e) {
  var t = a.key === e.key,
    r = a.tag === e.tag;
  return r && t;
}
function br(a) {
  var e,
    t = a.children,
    r = a.tag;
  if (ne(r)) {
    var n = (a.elm = zu(r));
    if ((Ni(wp, a), H(t)))
      for (e = 0; e < t.length; ++e) {
        var i = t[e];
        i != null && Yo(n, br(i));
      }
    else ne(a.text) && !Ge(a.text) && Yo(n, Ho(a.text));
  } else a.elm = Ho(a.text);
  return a.elm;
}
function Zu(a, e, t, r, n) {
  for (; r <= n; ++r) {
    var i = t[r];
    i != null && Ee(a, br(i), e);
  }
}
function la(a, e, t, r) {
  for (; t <= r; ++t) {
    var n = e[t];
    if (n != null)
      if (ne(n.tag)) {
        var i = $u(n.elm);
        Uo(i, n.elm);
      } else Uo(a, n.elm);
  }
}
function Ni(a, e) {
  var t,
    r = e.elm,
    n = (a && a.attrs) || {},
    i = e.attrs || {};
  if (n !== i) {
    for (t in i) {
      var o = i[t],
        s = n[t];
      s !== o &&
        (o === !0
          ? r.setAttribute(t, "")
          : o === !1
          ? r.removeAttribute(t)
          : t === "style"
          ? (r.style.cssText = o)
          : t.charCodeAt(0) !== _p
          ? r.setAttribute(t, o)
          : t === "xmlns:xlink" || t === "xmlns"
          ? r.setAttributeNS(ap, t, o)
          : t.charCodeAt(3) === $o
          ? r.setAttributeNS(np, t, o)
          : t.charCodeAt(5) === $o
          ? r.setAttributeNS(Gu, t, o)
          : r.setAttribute(t, o));
    }
    for (t in n) t in i || r.removeAttribute(t);
  }
}
function Tp(a, e, t) {
  for (
    var r = 0,
      n = 0,
      i = e.length - 1,
      o = e[0],
      s = e[i],
      l = t.length - 1,
      u = t[0],
      v = t[l],
      c,
      h,
      f,
      p;
    r <= i && n <= l;

  )
    o == null
      ? (o = e[++r])
      : s == null
      ? (s = e[--i])
      : u == null
      ? (u = t[++n])
      : v == null
      ? (v = t[--l])
      : cr(o, u)
      ? (Ue(o, u), (o = e[++r]), (u = t[++n]))
      : cr(s, v)
      ? (Ue(s, v), (s = e[--i]), (v = t[--l]))
      : cr(o, v)
      ? (Ue(o, v), Ee(a, o.elm, Xu(s.elm)), (o = e[++r]), (v = t[--l]))
      : cr(s, u)
      ? (Ue(s, u), Ee(a, s.elm, o.elm), (s = e[--i]), (u = t[++n]))
      : (Xn(c) && (c = Ap(e, r, i)),
        (h = c[u.key]),
        Xn(h)
          ? Ee(a, br(u), o.elm)
          : ((f = e[h]),
            f.tag !== u.tag
              ? Ee(a, br(u), o.elm)
              : (Ue(f, u), (e[h] = void 0), Ee(a, f.elm, o.elm))),
        (u = t[++n]));
  (r <= i || n <= l) &&
    (r > i
      ? ((p = t[l + 1] == null ? null : t[l + 1].elm), Zu(a, p, t, n, l))
      : la(a, e, r, i));
}
function Ue(a, e) {
  var t = (e.elm = a.elm),
    r = a.children,
    n = e.children;
  a !== e &&
    (Ni(a, e),
    Xn(e.text)
      ? ne(r) && ne(n)
        ? r !== n && Tp(t, r, n)
        : ne(n)
        ? (ne(a.text) && Xa(t, ""), Zu(t, null, n, 0, n.length - 1))
        : ne(r)
        ? la(t, r, 0, r.length - 1)
        : ne(a.text) && Xa(t, "")
      : a.text !== e.text &&
        (ne(r) && la(t, r, 0, r.length - 1), Xa(t, e.text)));
}
function Ip(a, e) {
  if (cr(a, e)) Ue(a, e);
  else {
    var t = a.elm,
      r = $u(t);
    br(e), r !== null && (Ee(r, e.elm, Xu(t)), la(r, [a], 0, 0));
  }
  return e;
}
var Dp = 0,
  Lp = (function () {
    function a(e, t, r) {
      if (
        ((this.type = "svg"),
        (this.refreshHover = Xo()),
        (this.configLayer = Xo()),
        (this.storage = t),
        (this._opts = r = W({}, r)),
        (this.root = e),
        (this._id = "zr" + Dp++),
        (this._oldVNode = ko(r.width, r.height)),
        e && !r.ssr)
      ) {
        var n = (this._viewport = document.createElement("div"));
        n.style.cssText = "position:relative;overflow:hidden";
        var i = (this._svgDom = this._oldVNode.elm = zu("svg"));
        Ni(null, this._oldVNode), n.appendChild(i), e.appendChild(n);
      }
      this.resize(r.width, r.height);
    }
    return (
      (a.prototype.getType = function () {
        return this.type;
      }),
      (a.prototype.getViewportRoot = function () {
        return this._viewport;
      }),
      (a.prototype.getViewportRootOffset = function () {
        var e = this.getViewportRoot();
        if (e)
          return { offsetLeft: e.offsetLeft || 0, offsetTop: e.offsetTop || 0 };
      }),
      (a.prototype.getSvgDom = function () {
        return this._svgDom;
      }),
      (a.prototype.refresh = function () {
        if (this.root) {
          var e = this.renderToVNode({ willUpdate: !0 });
          (e.attrs.style = "position:absolute;left:0;top:0;user-select:none"),
            Ip(this._oldVNode, e),
            (this._oldVNode = e);
        }
      }),
      (a.prototype.renderOneToVNode = function (e) {
        return Wo(e, $n(this._id));
      }),
      (a.prototype.renderToVNode = function (e) {
        e = e || {};
        var t = this.storage.getDisplayList(!0),
          r = this._width,
          n = this._height,
          i = $n(this._id);
        (i.animation = e.animation),
          (i.willUpdate = e.willUpdate),
          (i.compress = e.compress),
          (i.emphasis = e.emphasis),
          (i.ssr = this._opts.ssr);
        var o = [],
          s = (this._bgVNode = Cp(r, n, this._backgroundColor, i));
        s && o.push(s);
        var l = e.compress ? null : (this._mainVNode = ht("g", "main", {}, []));
        this._paintList(t, i, l ? l.children : o), l && o.push(l);
        var u = O(Lt(i.defs), function (h) {
          return i.defs[h];
        });
        if ((u.length && o.push(ht("defs", "defs", {}, u)), e.animation)) {
          var v = sp(i.cssNodes, i.cssAnims, { newline: !0 });
          if (v) {
            var c = ht("style", "stl", {}, [], v);
            o.push(c);
          }
        }
        return ko(r, n, o, e.useViewBox);
      }),
      (a.prototype.renderToString = function (e) {
        return (
          (e = e || {}),
          Pi(
            this.renderToVNode({
              animation: St(e.cssAnimation, !0),
              emphasis: St(e.cssEmphasis, !0),
              willUpdate: !1,
              compress: !0,
              useViewBox: St(e.useViewBox, !0)
            }),
            { newline: !0 }
          )
        );
      }),
      (a.prototype.setBackgroundColor = function (e) {
        this._backgroundColor = e;
      }),
      (a.prototype.getSvgRoot = function () {
        return this._mainVNode && this._mainVNode.elm;
      }),
      (a.prototype._paintList = function (e, t, r) {
        for (var n = e.length, i = [], o = 0, s, l, u = 0, v = 0; v < n; v++) {
          var c = e[v];
          if (!c.invisible) {
            var h = c.__clipPaths,
              f = (h && h.length) || 0,
              p = (l && l.length) || 0,
              d = void 0;
            for (
              d = Math.max(f - 1, p - 1);
              d >= 0 && !(h && l && h[d] === l[d]);
              d--
            );
            for (var g = p - 1; g > d; g--) o--, (s = i[o - 1]);
            for (var y = d + 1; y < f; y++) {
              var S = {};
              bp(h[y], S, t);
              var m = ht("g", "clip-g-" + u++, S, []);
              (s ? s.children : r).push(m), (i[o++] = m), (s = m);
            }
            l = h;
            var x = Wo(c, t);
            x && (s ? s.children : r).push(x);
          }
        }
      }),
      (a.prototype.resize = function (e, t) {
        var r = this._opts,
          n = this.root,
          i = this._viewport;
        if (
          (e != null && (r.width = e),
          t != null && (r.height = t),
          n &&
            i &&
            ((i.style.display = "none"),
            (e = po(n, 0, r)),
            (t = po(n, 1, r)),
            (i.style.display = "")),
          this._width !== e || this._height !== t)
        ) {
          if (((this._width = e), (this._height = t), i)) {
            var o = i.style;
            (o.width = e + "px"), (o.height = t + "px");
          }
          if (ci(this._backgroundColor)) this.refresh();
          else {
            var s = this._svgDom;
            s && (s.setAttribute("width", e), s.setAttribute("height", t));
            var l = this._bgVNode && this._bgVNode.elm;
            l && (l.setAttribute("width", e), l.setAttribute("height", t));
          }
        }
      }),
      (a.prototype.getWidth = function () {
        return this._width;
      }),
      (a.prototype.getHeight = function () {
        return this._height;
      }),
      (a.prototype.dispose = function () {
        this.root && (this.root.innerHTML = ""),
          (this._svgDom =
            this._viewport =
            this.storage =
            this._oldVNode =
            this._bgVNode =
            this._mainVNode =
              null);
      }),
      (a.prototype.clear = function () {
        this._svgDom && (this._svgDom.innerHTML = null),
          (this._oldVNode = null);
      }),
      (a.prototype.toDataURL = function (e) {
        var t = this.renderToString(),
          r = "data:image/svg+xml;";
        return e
          ? ((t = kc(t)), t && r + "base64," + t)
          : r + "charset=UTF-8," + encodeURIComponent(t);
      }),
      a
    );
  })();
function Xo(a) {
  return function () {};
}
function Cp(a, e, t, r) {
  var n;
  if (t && t !== "none")
    if (
      ((n = ht("rect", "bg", { width: a, height: e, x: "0", y: "0" })), cu(t))
    )
      Uu({ fill: t }, n.attrs, "fill", r);
    else if (ci(t))
      Yu(
        {
          style: { fill: t },
          dirty: qe,
          getBoundingRect: function () {
            return { width: a, height: e };
          }
        },
        n.attrs,
        "fill",
        r
      );
    else {
      var i = yr(t),
        o = i.color,
        s = i.opacity;
      (n.attrs.fill = o), s < 1 && (n.attrs["fill-opacity"] = s);
    }
  return n;
}
function Pp(a) {
  a.registerPainter("svg", Lp);
}
var Mp = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.hasSymbolVisual = !0), t;
    }
    return (
      (e.prototype.getInitialData = function (t, r) {
        return Je(null, this, { useEncodeDefaulter: !0 });
      }),
      (e.prototype.getProgressive = function () {
        var t = this.option.progressive;
        return t ?? (this.option.large ? 5e3 : this.get("progressive"));
      }),
      (e.prototype.getProgressiveThreshold = function () {
        var t = this.option.progressiveThreshold;
        return (
          t ?? (this.option.large ? 1e4 : this.get("progressiveThreshold"))
        );
      }),
      (e.prototype.brushSelector = function (t, r, n) {
        return n.point(r.getItemLayout(t));
      }),
      (e.prototype.getZLevelKey = function () {
        return this.getData().count() > this.getProgressiveThreshold()
          ? this.id
          : "";
      }),
      (e.type = "series.scatter"),
      (e.dependencies = ["grid", "polar", "geo", "singleAxis", "calendar"]),
      (e.defaultOption = {
        coordinateSystem: "cartesian2d",
        z: 2,
        legendHoverLink: !0,
        symbolSize: 10,
        large: !1,
        largeThreshold: 2e3,
        itemStyle: { opacity: 0.8 },
        emphasis: { scale: !0 },
        clip: !0,
        select: { itemStyle: { borderColor: "#212121" } },
        universalTransition: { divideShape: "clone" }
      }),
      e
    );
  })(gt),
  qu = 4,
  Ep = (function () {
    function a() {}
    return a;
  })(),
  Rp = (function (a) {
    k(e, a);
    function e(t) {
      var r = a.call(this, t) || this;
      return (r._off = 0), (r.hoverDataIdx = -1), r;
    }
    return (
      (e.prototype.getDefaultShape = function () {
        return new Ep();
      }),
      (e.prototype.reset = function () {
        (this.notClear = !1), (this._off = 0);
      }),
      (e.prototype.buildPath = function (t, r) {
        var n = r.points,
          i = r.size,
          o = this.symbolProxy,
          s = o.shape,
          l = t.getContext ? t.getContext() : t,
          u = l && i[0] < qu,
          v = this.softClipShape,
          c;
        if (u) {
          this._ctx = l;
          return;
        }
        for (this._ctx = null, c = this._off; c < n.length; ) {
          var h = n[c++],
            f = n[c++];
          isNaN(h) ||
            isNaN(f) ||
            (v && !v.contain(h, f)) ||
            ((s.x = h - i[0] / 2),
            (s.y = f - i[1] / 2),
            (s.width = i[0]),
            (s.height = i[1]),
            o.buildPath(t, s, !0));
        }
        this.incremental && ((this._off = c), (this.notClear = !0));
      }),
      (e.prototype.afterBrush = function () {
        var t = this.shape,
          r = t.points,
          n = t.size,
          i = this._ctx,
          o = this.softClipShape,
          s;
        if (i) {
          for (s = this._off; s < r.length; ) {
            var l = r[s++],
              u = r[s++];
            isNaN(l) ||
              isNaN(u) ||
              (o && !o.contain(l, u)) ||
              i.fillRect(l - n[0] / 2, u - n[1] / 2, n[0], n[1]);
          }
          this.incremental && ((this._off = s), (this.notClear = !0));
        }
      }),
      (e.prototype.findDataIndex = function (t, r) {
        for (
          var n = this.shape,
            i = n.points,
            o = n.size,
            s = Math.max(o[0], 4),
            l = Math.max(o[1], 4),
            u = i.length / 2 - 1;
          u >= 0;
          u--
        ) {
          var v = u * 2,
            c = i[v] - s / 2,
            h = i[v + 1] - l / 2;
          if (t >= c && r >= h && t <= c + s && r <= h + l) return u;
        }
        return -1;
      }),
      (e.prototype.contain = function (t, r) {
        var n = this.transformCoordToLocal(t, r),
          i = this.getBoundingRect();
        if (((t = n[0]), (r = n[1]), i.contain(t, r))) {
          var o = (this.hoverDataIdx = this.findDataIndex(t, r));
          return o >= 0;
        }
        return (this.hoverDataIdx = -1), !1;
      }),
      (e.prototype.getBoundingRect = function () {
        var t = this._rect;
        if (!t) {
          for (
            var r = this.shape,
              n = r.points,
              i = r.size,
              o = i[0],
              s = i[1],
              l = 1 / 0,
              u = 1 / 0,
              v = -1 / 0,
              c = -1 / 0,
              h = 0;
            h < n.length;

          ) {
            var f = n[h++],
              p = n[h++];
            (l = Math.min(f, l)),
              (v = Math.max(f, v)),
              (u = Math.min(p, u)),
              (c = Math.max(p, c));
          }
          t = this._rect = new ft(l - o / 2, u - s / 2, v - l + o, c - u + s);
        }
        return t;
      }),
      e
    );
  })(wt),
  Np = (function () {
    function a() {
      this.group = new Z();
    }
    return (
      (a.prototype.updateData = function (e, t) {
        this._clear();
        var r = this._create();
        r.setShape({ points: e.getLayout("points") }), this._setCommon(r, e, t);
      }),
      (a.prototype.updateLayout = function (e) {
        var t = e.getLayout("points");
        this.group.eachChild(function (r) {
          if (r.startIndex != null) {
            var n = (r.endIndex - r.startIndex) * 2,
              i = r.startIndex * 4 * 2;
            t = new Float32Array(t.buffer, i, n);
          }
          r.setShape("points", t), r.reset();
        });
      }),
      (a.prototype.incrementalPrepareUpdate = function (e) {
        this._clear();
      }),
      (a.prototype.incrementalUpdate = function (e, t, r) {
        var n = this._newAdded[0],
          i = t.getLayout("points"),
          o = n && n.shape.points;
        if (o && o.length < 2e4) {
          var s = o.length,
            l = new Float32Array(s + i.length);
          l.set(o),
            l.set(i, s),
            (n.endIndex = e.end),
            n.setShape({ points: l });
        } else {
          this._newAdded = [];
          var u = this._create();
          (u.startIndex = e.start),
            (u.endIndex = e.end),
            (u.incremental = !0),
            u.setShape({ points: i }),
            this._setCommon(u, t, r);
        }
      }),
      (a.prototype.eachRendered = function (e) {
        this._newAdded[0] && e(this._newAdded[0]);
      }),
      (a.prototype._create = function () {
        var e = new Rp({ cursor: "default" });
        return (
          (e.ignoreCoarsePointer = !0),
          this.group.add(e),
          this._newAdded.push(e),
          e
        );
      }),
      (a.prototype._setCommon = function (e, t, r) {
        var n = t.hostModel;
        r = r || {};
        var i = t.getVisual("symbolSize");
        e.setShape("size", i instanceof Array ? i : [i, i]),
          (e.softClipShape = r.clipShape || null),
          (e.symbolProxy = ie(t.getVisual("symbol"), 0, 0, 0, 0)),
          (e.setColor = e.symbolProxy.setColor);
        var o = e.shape.size[0] < qu;
        e.useStyle(
          n
            .getModel("itemStyle")
            .getItemStyle(
              o ? ["color", "shadowBlur", "shadowColor"] : ["color"]
            )
        );
        var s = t.getVisual("style"),
          l = s && s.fill;
        l && e.setColor(l);
        var u = nt(e);
        (u.seriesIndex = n.seriesIndex),
          e.on("mousemove", function (v) {
            u.dataIndex = null;
            var c = e.hoverDataIdx;
            c >= 0 && (u.dataIndex = c + (e.startIndex || 0));
          });
      }),
      (a.prototype.remove = function () {
        this._clear();
      }),
      (a.prototype._clear = function () {
        (this._newAdded = []), this.group.removeAll();
      }),
      a
    );
  })(),
  kp = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i = t.getData(),
          o = this._updateSymbolDraw(i, t);
        o.updateData(i, { clipShape: this._getClipShape(t) }),
          (this._finished = !0);
      }),
      (e.prototype.incrementalPrepareRender = function (t, r, n) {
        var i = t.getData(),
          o = this._updateSymbolDraw(i, t);
        o.incrementalPrepareUpdate(i), (this._finished = !1);
      }),
      (e.prototype.incrementalRender = function (t, r, n) {
        this._symbolDraw.incrementalUpdate(t, r.getData(), {
          clipShape: this._getClipShape(r)
        }),
          (this._finished = t.end === r.getData().count());
      }),
      (e.prototype.updateTransform = function (t, r, n) {
        var i = t.getData();
        if ((this.group.dirty(), !this._finished || i.count() > 1e4))
          return { update: !0 };
        var o = wa("").reset(t, r, n);
        o.progress &&
          o.progress({ start: 0, end: i.count(), count: i.count() }, i),
          this._symbolDraw.updateLayout(i);
      }),
      (e.prototype.eachRendered = function (t) {
        this._symbolDraw && this._symbolDraw.eachRendered(t);
      }),
      (e.prototype._getClipShape = function (t) {
        if (t.get("clip", !0)) {
          var r = t.coordinateSystem;
          return r && r.getArea && r.getArea(0.1);
        }
      }),
      (e.prototype._updateSymbolDraw = function (t, r) {
        var n = this._symbolDraw,
          i = r.pipelineContext,
          o = i.large;
        return (
          (!n || o !== this._isLargeDraw) &&
            (n && n.remove(),
            (n = this._symbolDraw = o ? new Np() : new Aa()),
            (this._isLargeDraw = o),
            this.group.removeAll()),
          this.group.add(n.group),
          n
        );
      }),
      (e.prototype.remove = function (t, r) {
        this._symbolDraw && this._symbolDraw.remove(!0),
          (this._symbolDraw = null);
      }),
      (e.prototype.dispose = function () {}),
      (e.type = "scatter"),
      e
    );
  })(pt);
function Vp(a) {
  q(Vc),
    a.registerSeriesModel(Mp),
    a.registerChartView(kp),
    a.registerLayout(wa("scatter"));
}
function Gp(a) {
  a.eachSeriesByType("radar", function (e) {
    var t = e.getData(),
      r = [],
      n = e.coordinateSystem;
    if (n) {
      var i = n.getIndicatorAxes();
      L(i, function (o, s) {
        t.each(t.mapDimension(i[s].dim), function (l, u) {
          r[u] = r[u] || [];
          var v = n.dataToPoint(l, s);
          r[u][s] = Zo(v) ? v : qo(n);
        });
      }),
        t.each(function (o) {
          var s =
            Gc(r[o], function (l) {
              return Zo(l);
            }) || qo(n);
          r[o].push(s.slice()), t.setItemLayout(o, r[o]);
        });
    }
  });
}
function Zo(a) {
  return !isNaN(a[0]) && !isNaN(a[1]);
}
function qo(a) {
  return [a.cx, a.cy];
}
function zp(a) {
  var e = a.polar;
  if (e) {
    H(e) || (e = [e]);
    var t = [];
    L(e, function (r, n) {
      r.indicator
        ? (r.type && !r.shape && (r.shape = r.type),
          (a.radar = a.radar || []),
          H(a.radar) || (a.radar = [a.radar]),
          a.radar.push(r))
        : t.push(r);
    }),
      (a.polar = t);
  }
  L(a.series, function (r) {
    r && r.type === "radar" && r.polarIndex && (r.radarIndex = r.polarIndex);
  });
}
var Op = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i = t.coordinateSystem,
          o = this.group,
          s = t.getData(),
          l = this._data;
        function u(h, f) {
          var p = h.getItemVisual(f, "symbol") || "circle";
          if (p !== "none") {
            var d = Dr(h.getItemVisual(f, "symbolSize")),
              g = ie(p, -1, -1, 2, 2),
              y = h.getItemVisual(f, "symbolRotate") || 0;
            return (
              g.attr({
                style: { strokeNoScale: !0 },
                z2: 100,
                scaleX: d[0] / 2,
                scaleY: d[1] / 2,
                rotation: (y * Math.PI) / 180 || 0
              }),
              g
            );
          }
        }
        function v(h, f, p, d, g, y) {
          p.removeAll();
          for (var S = 0; S < f.length - 1; S++) {
            var m = u(d, g);
            m &&
              ((m.__dimIdx = S),
              h[S]
                ? (m.setPosition(h[S]),
                  Qe[y ? "initProps" : "updateProps"](
                    m,
                    { x: f[S][0], y: f[S][1] },
                    t,
                    g
                  ))
                : m.setPosition(f[S]),
              p.add(m));
          }
        }
        function c(h) {
          return O(h, function (f) {
            return [i.cx, i.cy];
          });
        }
        s
          .diff(l)
          .add(function (h) {
            var f = s.getItemLayout(h);
            if (f) {
              var p = new ve(),
                d = new we(),
                g = { shape: { points: f } };
              (p.shape.points = c(f)),
                (d.shape.points = c(f)),
                Ct(p, g, t, h),
                Ct(d, g, t, h);
              var y = new Z(),
                S = new Z();
              y.add(d),
                y.add(p),
                y.add(S),
                v(d.shape.points, f, S, s, h, !0),
                s.setItemGraphicEl(h, y);
            }
          })
          .update(function (h, f) {
            var p = l.getItemGraphicEl(f),
              d = p.childAt(0),
              g = p.childAt(1),
              y = p.childAt(2),
              S = { shape: { points: s.getItemLayout(h) } };
            S.shape.points &&
              (v(d.shape.points, S.shape.points, y, s, h, !1),
              Se(g),
              Se(d),
              lt(d, S, t),
              lt(g, S, t),
              s.setItemGraphicEl(h, p));
          })
          .remove(function (h) {
            o.remove(l.getItemGraphicEl(h));
          })
          .execute(),
          s.eachItemGraphicEl(function (h, f) {
            var p = s.getItemModel(f),
              d = h.childAt(0),
              g = h.childAt(1),
              y = h.childAt(2),
              S = s.getItemVisual(f, "style"),
              m = S.fill;
            o.add(h),
              d.useStyle(
                Q(p.getModel("lineStyle").getLineStyle(), {
                  fill: "none",
                  stroke: m
                })
              ),
              zt(d, p, "lineStyle"),
              zt(g, p, "areaStyle");
            var x = p.getModel("areaStyle"),
              b = x.isEmpty() && x.parentModel.isEmpty();
            (g.ignore = b),
              L(["emphasis", "select", "blur"], function (A) {
                var I = p.getModel([A, "areaStyle"]),
                  T = I.isEmpty() && I.parentModel.isEmpty();
                g.ensureState(A).ignore = T && b;
              }),
              g.useStyle(
                Q(x.getAreaStyle(), { fill: m, opacity: 0.7, decal: S.decal })
              );
            var _ = p.getModel("emphasis"),
              w = _.getModel("itemStyle").getItemStyle();
            y.eachChild(function (A) {
              if (A instanceof _e) {
                var I = A.style;
                A.useStyle(
                  W(
                    {
                      image: I.image,
                      x: I.x,
                      y: I.y,
                      width: I.width,
                      height: I.height
                    },
                    S
                  )
                );
              } else A.useStyle(S), A.setColor(m), (A.style.strokeNoScale = !0);
              var T = A.ensureState("emphasis");
              T.style = Gt(w);
              var D = s.getStore().get(s.getDimensionIndex(A.__dimIdx), f);
              (D == null || isNaN(D)) && (D = ""),
                Ut(A, Pt(p), {
                  labelFetcher: s.hostModel,
                  labelDataIndex: f,
                  labelDimIndex: A.__dimIdx,
                  defaultText: D,
                  inheritColor: m,
                  defaultOpacity: S.opacity
                });
            }),
              ct(h, _.get("focus"), _.get("blurScope"), _.get("disabled"));
          }),
          (this._data = s);
      }),
      (e.prototype.remove = function () {
        this.group.removeAll(), (this._data = null);
      }),
      (e.type = "radar"),
      e
    );
  })(pt),
  Bp = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.hasSymbolVisual = !0), t;
    }
    return (
      (e.prototype.init = function (t) {
        a.prototype.init.apply(this, arguments),
          (this.legendVisualProvider = new Ta(
            at(this.getData, this),
            at(this.getRawData, this)
          ));
      }),
      (e.prototype.getInitialData = function (t, r) {
        return Lr(this, {
          generateCoord: "indicator_",
          generateCoordCount: 1 / 0
        });
      }),
      (e.prototype.formatTooltip = function (t, r, n) {
        var i = this.getData(),
          o = this.coordinateSystem,
          s = o.getIndicatorAxes(),
          l = this.getData().getName(t),
          u = l === "" ? this.name : l,
          v = zc(this, t);
        return Mt("section", {
          header: u,
          sortBlocks: !0,
          blocks: O(s, function (c) {
            var h = i.get(i.mapDimension(c.dim), t);
            return Mt("nameValue", {
              markerType: "subItem",
              markerColor: v,
              name: c.name,
              value: h,
              sortParam: h
            });
          })
        });
      }),
      (e.prototype.getTooltipPosition = function (t) {
        if (t != null) {
          for (
            var r = this.getData(),
              n = this.coordinateSystem,
              i = r.getValues(
                O(n.dimensions, function (u) {
                  return r.mapDimension(u);
                }),
                t
              ),
              o = 0,
              s = i.length;
            o < s;
            o++
          )
            if (!isNaN(i[o])) {
              var l = n.getIndicatorAxes();
              return n.coordToPoint(l[o].dataToCoord(i[o]), o);
            }
        }
      }),
      (e.type = "series.radar"),
      (e.dependencies = ["radar"]),
      (e.defaultOption = {
        z: 2,
        colorBy: "data",
        coordinateSystem: "radar",
        legendHoverLink: !0,
        radarIndex: 0,
        lineStyle: { width: 2, type: "solid", join: "round" },
        label: { position: "top" },
        symbolSize: 8
      }),
      e
    );
  })(gt),
  nr = Oc.value;
function Fr(a, e) {
  return Q({ show: e }, a);
}
var Fp = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.optionUpdated = function () {
        var t = this.get("boundaryGap"),
          r = this.get("splitNumber"),
          n = this.get("scale"),
          i = this.get("axisLine"),
          o = this.get("axisTick"),
          s = this.get("axisLabel"),
          l = this.get("axisName"),
          u = this.get(["axisName", "show"]),
          v = this.get(["axisName", "formatter"]),
          c = this.get("axisNameGap"),
          h = this.get("triggerEvent"),
          f = O(
            this.get("indicator") || [],
            function (p) {
              p.max != null && p.max > 0 && !p.min
                ? (p.min = 0)
                : p.min != null && p.min < 0 && !p.max && (p.max = 0);
              var d = l;
              p.color != null && (d = Q({ color: p.color }, l));
              var g = Yt(
                Gt(p),
                {
                  boundaryGap: t,
                  splitNumber: r,
                  scale: n,
                  axisLine: i,
                  axisTick: o,
                  axisLabel: s,
                  name: p.text,
                  showName: u,
                  nameLocation: "end",
                  nameGap: c,
                  nameTextStyle: d,
                  triggerEvent: h
                },
                !1
              );
              if (et(v)) {
                var y = g.name;
                g.name = v.replace("{value}", y ?? "");
              } else it(v) && (g.name = v(g.name, g));
              var S = new $t(g, null, this.ecModel);
              return (
                te(S, Ia.prototype),
                (S.mainType = "radar"),
                (S.componentIndex = this.componentIndex),
                S
              );
            },
            this
          );
        this._indicatorModels = f;
      }),
      (e.prototype.getIndicatorModels = function () {
        return this._indicatorModels;
      }),
      (e.type = "radar"),
      (e.defaultOption = {
        z: 0,
        center: ["50%", "50%"],
        radius: "75%",
        startAngle: 90,
        axisName: { show: !0 },
        boundaryGap: [0, 0],
        splitNumber: 5,
        axisNameGap: 15,
        scale: !1,
        shape: "polygon",
        axisLine: Yt({ lineStyle: { color: "#bbb" } }, nr.axisLine),
        axisLabel: Fr(nr.axisLabel, !1),
        axisTick: Fr(nr.axisTick, !1),
        splitLine: Fr(nr.splitLine, !0),
        splitArea: Fr(nr.splitArea, !0),
        indicator: []
      }),
      e
    );
  })(Xt),
  Wp = ["axisLine", "axisTickLabel", "axisName"],
  Hp = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i = this.group;
        i.removeAll(), this._buildAxes(t), this._buildSplitLineAndArea(t);
      }),
      (e.prototype._buildAxes = function (t) {
        var r = t.coordinateSystem,
          n = r.getIndicatorAxes(),
          i = O(n, function (o) {
            var s = o.model.get("showName") ? o.name : "",
              l = new Ne(o.model, {
                axisName: s,
                position: [r.cx, r.cy],
                rotation: o.angle,
                labelDirection: -1,
                tickDirection: -1,
                nameDirection: 1
              });
            return l;
          });
        L(
          i,
          function (o) {
            L(Wp, o.add, o), this.group.add(o.getGroup());
          },
          this
        );
      }),
      (e.prototype._buildSplitLineAndArea = function (t) {
        var r = t.coordinateSystem,
          n = r.getIndicatorAxes();
        if (!n.length) return;
        var i = t.get("shape"),
          o = t.getModel("splitLine"),
          s = t.getModel("splitArea"),
          l = o.getModel("lineStyle"),
          u = s.getModel("areaStyle"),
          v = o.get("show"),
          c = s.get("show"),
          h = l.get("color"),
          f = u.get("color"),
          p = H(h) ? h : [h],
          d = H(f) ? f : [f],
          g = [],
          y = [];
        function S(E, R, N) {
          var V = N % R.length;
          return (E[V] = E[V] || []), V;
        }
        if (i === "circle")
          for (
            var m = n[0].getTicksCoords(), x = r.cx, b = r.cy, _ = 0;
            _ < m.length;
            _++
          ) {
            if (v) {
              var w = S(g, p, _);
              g[w].push(new Cr({ shape: { cx: x, cy: b, r: m[_].coord } }));
            }
            if (c && _ < m.length - 1) {
              var w = S(y, d, _);
              y[w].push(
                new hu({
                  shape: { cx: x, cy: b, r0: m[_].coord, r: m[_ + 1].coord }
                })
              );
            }
          }
        else
          for (
            var A,
              I = O(n, function (E, R) {
                var N = E.getTicksCoords();
                return (
                  (A = A == null ? N.length - 1 : Math.min(N.length - 1, A)),
                  O(N, function (V) {
                    return r.coordToPoint(V.coord, R);
                  })
                );
              }),
              T = [],
              _ = 0;
            _ <= A;
            _++
          ) {
            for (var D = [], M = 0; M < n.length; M++) D.push(I[M][_]);
            if ((D[0] && D.push(D[0].slice()), v)) {
              var w = S(g, p, _);
              g[w].push(new we({ shape: { points: D } }));
            }
            if (c && T) {
              var w = S(y, d, _ - 1);
              y[w].push(new ve({ shape: { points: D.concat(T) } }));
            }
            T = D.slice().reverse();
          }
        var C = l.getLineStyle(),
          P = u.getAreaStyle();
        L(
          y,
          function (E, R) {
            this.group.add(
              Kt(E, {
                style: Q({ stroke: "none", fill: d[R % d.length] }, P),
                silent: !0
              })
            );
          },
          this
        ),
          L(
            g,
            function (E, R) {
              this.group.add(
                Kt(E, {
                  style: Q({ fill: "none", stroke: p[R % p.length] }, C),
                  silent: !0
                })
              );
            },
            this
          );
      }),
      (e.type = "radar"),
      e
    );
  })(ee),
  Up = (function (a) {
    k(e, a);
    function e(t, r, n) {
      var i = a.call(this, t, r, n) || this;
      return (i.type = "value"), (i.angle = 0), (i.name = ""), i;
    }
    return e;
  })(oe),
  Yp = (function () {
    function a(e, t, r) {
      (this.dimensions = []),
        (this._model = e),
        (this._indicatorAxes = O(
          e.getIndicatorModels(),
          function (n, i) {
            var o = "indicator_" + i,
              s = new Up(o, new Vn());
            return (
              (s.name = n.get("name")),
              (s.model = n),
              (n.axis = s),
              this.dimensions.push(o),
              s
            );
          },
          this
        )),
        this.resize(e, r);
    }
    return (
      (a.prototype.getIndicatorAxes = function () {
        return this._indicatorAxes;
      }),
      (a.prototype.dataToPoint = function (e, t) {
        var r = this._indicatorAxes[t];
        return this.coordToPoint(r.dataToCoord(e), t);
      }),
      (a.prototype.coordToPoint = function (e, t) {
        var r = this._indicatorAxes[t],
          n = r.angle,
          i = this.cx + e * Math.cos(n),
          o = this.cy - e * Math.sin(n);
        return [i, o];
      }),
      (a.prototype.pointToData = function (e) {
        var t = e[0] - this.cx,
          r = e[1] - this.cy,
          n = Math.sqrt(t * t + r * r);
        (t /= n), (r /= n);
        for (
          var i = Math.atan2(-r, t), o = 1 / 0, s, l = -1, u = 0;
          u < this._indicatorAxes.length;
          u++
        ) {
          var v = this._indicatorAxes[u],
            c = Math.abs(i - v.angle);
          c < o && ((s = v), (l = u), (o = c));
        }
        return [l, +(s && s.coordToData(n))];
      }),
      (a.prototype.resize = function (e, t) {
        var r = e.get("center"),
          n = t.getWidth(),
          i = t.getHeight(),
          o = Math.min(n, i) / 2;
        (this.cx = z(r[0], n)),
          (this.cy = z(r[1], i)),
          (this.startAngle = (e.get("startAngle") * Math.PI) / 180);
        var s = e.get("radius");
        (et(s) || Qt(s)) && (s = [0, s]),
          (this.r0 = z(s[0], o)),
          (this.r = z(s[1], o)),
          L(
            this._indicatorAxes,
            function (l, u) {
              l.setExtent(this.r0, this.r);
              var v =
                this.startAngle +
                (u * Math.PI * 2) / this._indicatorAxes.length;
              (v = Math.atan2(Math.sin(v), Math.cos(v))), (l.angle = v);
            },
            this
          );
      }),
      (a.prototype.update = function (e, t) {
        var r = this._indicatorAxes,
          n = this._model;
        L(r, function (s) {
          s.scale.setExtent(1 / 0, -1 / 0);
        }),
          e.eachSeriesByType(
            "radar",
            function (s, l) {
              if (
                !(
                  s.get("coordinateSystem") !== "radar" ||
                  e.getComponent("radar", s.get("radarIndex")) !== n
                )
              ) {
                var u = s.getData();
                L(r, function (v) {
                  v.scale.unionExtentFromData(u, u.mapDimension(v.dim));
                });
              }
            },
            this
          );
        var i = n.get("splitNumber"),
          o = new Vn();
        o.setExtent(0, i),
          o.setInterval(1),
          L(r, function (s, l) {
            Bc(s.scale, s.model, o);
          });
      }),
      (a.prototype.convertToPixel = function (e, t, r) {
        return console.warn("Not implemented."), null;
      }),
      (a.prototype.convertFromPixel = function (e, t, r) {
        return console.warn("Not implemented."), null;
      }),
      (a.prototype.containPoint = function (e) {
        return console.warn("Not implemented."), !1;
      }),
      (a.create = function (e, t) {
        var r = [];
        return (
          e.eachComponent("radar", function (n) {
            var i = new a(n, e, t);
            r.push(i), (n.coordinateSystem = i);
          }),
          e.eachSeriesByType("radar", function (n) {
            n.get("coordinateSystem") === "radar" &&
              (n.coordinateSystem = r[n.get("radarIndex") || 0]);
          }),
          r
        );
      }),
      (a.dimensions = []),
      a
    );
  })();
function $p(a) {
  a.registerCoordinateSystem("radar", Yp),
    a.registerComponentModel(Fp),
    a.registerComponentView(Hp),
    a.registerVisual({
      seriesType: "radar",
      reset: function (e) {
        var t = e.getData();
        t.each(function (r) {
          t.setItemVisual(r, "legendIcon", "roundRect");
        }),
          t.setVisual("legendIcon", "roundRect");
      }
    });
}
function Xp(a) {
  q($p),
    a.registerChartView(Op),
    a.registerSeriesModel(Bp),
    a.registerLayout(Gp),
    a.registerProcessor(Da("radar")),
    a.registerPreprocessor(zp);
}
function ki(a, e, t) {
  var r = a.target;
  (r.x += e), (r.y += t), r.dirty();
}
function Vi(a, e, t, r) {
  var n = a.target,
    i = a.zoomLimit,
    o = (a.zoom = a.zoom || 1);
  if (((o *= e), i)) {
    var s = i.min || 0,
      l = i.max || 1 / 0;
    o = Math.max(Math.min(l, o), s);
  }
  var u = o / a.zoom;
  (a.zoom = o),
    (n.x -= (t - n.x) * (u - 1)),
    (n.y -= (r - n.y) * (u - 1)),
    (n.scaleX *= u),
    (n.scaleY *= u),
    n.dirty();
}
function Ku(a) {
  if (et(a)) {
    var e = new DOMParser();
    a = e.parseFromString(a, "text/xml");
  }
  var t = a;
  for (
    t.nodeType === 9 && (t = t.firstChild);
    t.nodeName.toLowerCase() !== "svg" || t.nodeType !== 1;

  )
    t = t.nextSibling;
  return t;
}
var Za,
  ua = {
    fill: "fill",
    stroke: "stroke",
    "stroke-width": "lineWidth",
    opacity: "opacity",
    "fill-opacity": "fillOpacity",
    "stroke-opacity": "strokeOpacity",
    "stroke-dasharray": "lineDash",
    "stroke-dashoffset": "lineDashOffset",
    "stroke-linecap": "lineCap",
    "stroke-linejoin": "lineJoin",
    "stroke-miterlimit": "miterLimit",
    "font-family": "fontFamily",
    "font-size": "fontSize",
    "font-style": "fontStyle",
    "font-weight": "fontWeight",
    "text-anchor": "textAlign",
    visibility: "visibility",
    display: "display"
  },
  Ko = Lt(ua),
  va = { "alignment-baseline": "textBaseline", "stop-color": "stopColor" },
  jo = Lt(va),
  Zp = (function () {
    function a() {
      (this._defs = {}), (this._root = null);
    }
    return (
      (a.prototype.parse = function (e, t) {
        t = t || {};
        var r = Ku(e);
        this._defsUsePending = [];
        var n = new Z();
        this._root = n;
        var i = [],
          o = r.getAttribute("viewBox") || "",
          s = parseFloat(r.getAttribute("width") || t.width),
          l = parseFloat(r.getAttribute("height") || t.height);
        isNaN(s) && (s = null), isNaN(l) && (l = null), Nt(r, n, null, !0, !1);
        for (var u = r.firstChild; u; )
          this._parseNode(u, n, i, null, !1, !1), (u = u.nextSibling);
        jp(this._defs, this._defsUsePending), (this._defsUsePending = []);
        var v, c;
        if (o) {
          var h = Va(o);
          h.length >= 4 &&
            (v = {
              x: parseFloat(h[0] || 0),
              y: parseFloat(h[1] || 0),
              width: parseFloat(h[2]),
              height: parseFloat(h[3])
            });
        }
        if (
          v &&
          s != null &&
          l != null &&
          ((c = Ju(v, { x: 0, y: 0, width: s, height: l })), !t.ignoreViewBox)
        ) {
          var f = n;
          (n = new Z()),
            n.add(f),
            (f.scaleX = f.scaleY = c.scale),
            (f.x = c.x),
            (f.y = c.y);
        }
        return (
          !t.ignoreRootClip &&
            s != null &&
            l != null &&
            n.setClipPath(
              new Et({ shape: { x: 0, y: 0, width: s, height: l } })
            ),
          {
            root: n,
            width: s,
            height: l,
            viewBoxRect: v,
            viewBoxTransform: c,
            named: i
          }
        );
      }),
      (a.prototype._parseNode = function (e, t, r, n, i, o) {
        var s = e.nodeName.toLowerCase(),
          l,
          u = n;
        if (
          (s === "defs" && (i = !0),
          s === "text" && (o = !0),
          s === "defs" || s === "switch")
        )
          l = t;
        else {
          if (!i) {
            var v = Za[s];
            if (v && bt(Za, s)) {
              l = v.call(this, e, t);
              var c = e.getAttribute("name");
              if (c) {
                var h = { name: c, namedFrom: null, svgNodeTagLower: s, el: l };
                r.push(h), s === "g" && (u = h);
              } else
                n &&
                  r.push({
                    name: n.name,
                    namedFrom: n,
                    svgNodeTagLower: s,
                    el: l
                  });
              t.add(l);
            }
          }
          var f = Jo[s];
          if (f && bt(Jo, s)) {
            var p = f.call(this, e),
              d = e.getAttribute("id");
            d && (this._defs[d] = p);
          }
        }
        if (l && l.isGroup)
          for (var g = e.firstChild; g; )
            g.nodeType === 1
              ? this._parseNode(g, l, r, u, i, o)
              : g.nodeType === 3 && o && this._parseText(g, l),
              (g = g.nextSibling);
      }),
      (a.prototype._parseText = function (e, t) {
        var r = new vu({
          style: { text: e.textContent },
          silent: !0,
          x: this._textX || 0,
          y: this._textY || 0
        });
        Ot(t, r), Nt(e, r, this._defsUsePending, !1, !1), qp(r, t);
        var n = r.style,
          i = n.fontSize;
        i &&
          i < 9 &&
          ((n.fontSize = 9), (r.scaleX *= i / 9), (r.scaleY *= i / 9));
        var o =
          (n.fontSize || n.fontFamily) &&
          [
            n.fontStyle,
            n.fontWeight,
            (n.fontSize || 12) + "px",
            n.fontFamily || "sans-serif"
          ].join(" ");
        n.font = o;
        var s = r.getBoundingRect();
        return (this._textX += s.width), t.add(r), r;
      }),
      (a.internalField = (function () {
        Za = {
          g: function (e, t) {
            var r = new Z();
            return Ot(t, r), Nt(e, r, this._defsUsePending, !1, !1), r;
          },
          rect: function (e, t) {
            var r = new Et();
            return (
              Ot(t, r),
              Nt(e, r, this._defsUsePending, !1, !1),
              r.setShape({
                x: parseFloat(e.getAttribute("x") || "0"),
                y: parseFloat(e.getAttribute("y") || "0"),
                width: parseFloat(e.getAttribute("width") || "0"),
                height: parseFloat(e.getAttribute("height") || "0")
              }),
              (r.silent = !0),
              r
            );
          },
          circle: function (e, t) {
            var r = new Cr();
            return (
              Ot(t, r),
              Nt(e, r, this._defsUsePending, !1, !1),
              r.setShape({
                cx: parseFloat(e.getAttribute("cx") || "0"),
                cy: parseFloat(e.getAttribute("cy") || "0"),
                r: parseFloat(e.getAttribute("r") || "0")
              }),
              (r.silent = !0),
              r
            );
          },
          line: function (e, t) {
            var r = new jt();
            return (
              Ot(t, r),
              Nt(e, r, this._defsUsePending, !1, !1),
              r.setShape({
                x1: parseFloat(e.getAttribute("x1") || "0"),
                y1: parseFloat(e.getAttribute("y1") || "0"),
                x2: parseFloat(e.getAttribute("x2") || "0"),
                y2: parseFloat(e.getAttribute("y2") || "0")
              }),
              (r.silent = !0),
              r
            );
          },
          ellipse: function (e, t) {
            var r = new Wc();
            return (
              Ot(t, r),
              Nt(e, r, this._defsUsePending, !1, !1),
              r.setShape({
                cx: parseFloat(e.getAttribute("cx") || "0"),
                cy: parseFloat(e.getAttribute("cy") || "0"),
                rx: parseFloat(e.getAttribute("rx") || "0"),
                ry: parseFloat(e.getAttribute("ry") || "0")
              }),
              (r.silent = !0),
              r
            );
          },
          polygon: function (e, t) {
            var r = e.getAttribute("points"),
              n;
            r && (n = es(r));
            var i = new ve({ shape: { points: n || [] }, silent: !0 });
            return Ot(t, i), Nt(e, i, this._defsUsePending, !1, !1), i;
          },
          polyline: function (e, t) {
            var r = e.getAttribute("points"),
              n;
            r && (n = es(r));
            var i = new we({ shape: { points: n || [] }, silent: !0 });
            return Ot(t, i), Nt(e, i, this._defsUsePending, !1, !1), i;
          },
          image: function (e, t) {
            var r = new _e();
            return (
              Ot(t, r),
              Nt(e, r, this._defsUsePending, !1, !1),
              r.setStyle({
                image: e.getAttribute("xlink:href") || e.getAttribute("href"),
                x: +e.getAttribute("x"),
                y: +e.getAttribute("y"),
                width: +e.getAttribute("width"),
                height: +e.getAttribute("height")
              }),
              (r.silent = !0),
              r
            );
          },
          text: function (e, t) {
            var r = e.getAttribute("x") || "0",
              n = e.getAttribute("y") || "0",
              i = e.getAttribute("dx") || "0",
              o = e.getAttribute("dy") || "0";
            (this._textX = parseFloat(r) + parseFloat(i)),
              (this._textY = parseFloat(n) + parseFloat(o));
            var s = new Z();
            return Ot(t, s), Nt(e, s, this._defsUsePending, !1, !0), s;
          },
          tspan: function (e, t) {
            var r = e.getAttribute("x"),
              n = e.getAttribute("y");
            r != null && (this._textX = parseFloat(r)),
              n != null && (this._textY = parseFloat(n));
            var i = e.getAttribute("dx") || "0",
              o = e.getAttribute("dy") || "0",
              s = new Z();
            return (
              Ot(t, s),
              Nt(e, s, this._defsUsePending, !1, !0),
              (this._textX += parseFloat(i)),
              (this._textY += parseFloat(o)),
              s
            );
          },
          path: function (e, t) {
            var r = e.getAttribute("d") || "",
              n = Fc(r);
            return (
              Ot(t, n),
              Nt(e, n, this._defsUsePending, !1, !1),
              (n.silent = !0),
              n
            );
          }
        };
      })()),
      a
    );
  })(),
  Jo = {
    lineargradient: function (a) {
      var e = parseInt(a.getAttribute("x1") || "0", 10),
        t = parseInt(a.getAttribute("y1") || "0", 10),
        r = parseInt(a.getAttribute("x2") || "10", 10),
        n = parseInt(a.getAttribute("y2") || "0", 10),
        i = new fu(e, t, r, n);
      return Qo(a, i), ts(a, i), i;
    },
    radialgradient: function (a) {
      var e = parseInt(a.getAttribute("cx") || "0", 10),
        t = parseInt(a.getAttribute("cy") || "0", 10),
        r = parseInt(a.getAttribute("r") || "0", 10),
        n = new Hc(e, t, r);
      return Qo(a, n), ts(a, n), n;
    }
  };
function Qo(a, e) {
  var t = a.getAttribute("gradientUnits");
  t === "userSpaceOnUse" && (e.global = !0);
}
function ts(a, e) {
  for (var t = a.firstChild; t; ) {
    if (t.nodeType === 1 && t.nodeName.toLocaleLowerCase() === "stop") {
      var r = t.getAttribute("offset"),
        n = void 0;
      r && r.indexOf("%") > 0
        ? (n = parseInt(r, 10) / 100)
        : r
        ? (n = parseFloat(r))
        : (n = 0);
      var i = {};
      ju(t, i, i);
      var o = i.stopColor || t.getAttribute("stop-color") || "#000000";
      e.colorStops.push({ offset: n, color: o });
    }
    t = t.nextSibling;
  }
}
function Ot(a, e) {
  a &&
    a.__inheritedStyle &&
    (e.__inheritedStyle || (e.__inheritedStyle = {}),
    Q(e.__inheritedStyle, a.__inheritedStyle));
}
function es(a) {
  for (var e = Va(a), t = [], r = 0; r < e.length; r += 2) {
    var n = parseFloat(e[r]),
      i = parseFloat(e[r + 1]);
    t.push([n, i]);
  }
  return t;
}
function Nt(a, e, t, r, n) {
  var i = e,
    o = (i.__inheritedStyle = i.__inheritedStyle || {}),
    s = {};
  a.nodeType === 1 && (td(a, e), ju(a, o, s), r || ed(a, o, s)),
    (i.style = i.style || {}),
    o.fill != null && (i.style.fill = rs(i, "fill", o.fill, t)),
    o.stroke != null && (i.style.stroke = rs(i, "stroke", o.stroke, t)),
    L(
      [
        "lineWidth",
        "opacity",
        "fillOpacity",
        "strokeOpacity",
        "miterLimit",
        "fontSize"
      ],
      function (l) {
        o[l] != null && (i.style[l] = parseFloat(o[l]));
      }
    ),
    L(
      [
        "lineDashOffset",
        "lineCap",
        "lineJoin",
        "fontWeight",
        "fontFamily",
        "fontStyle",
        "textAlign"
      ],
      function (l) {
        o[l] != null && (i.style[l] = o[l]);
      }
    ),
    n && (i.__selfStyle = s),
    o.lineDash &&
      (i.style.lineDash = O(Va(o.lineDash), function (l) {
        return parseFloat(l);
      })),
    (o.visibility === "hidden" || o.visibility === "collapse") &&
      (i.invisible = !0),
    o.display === "none" && (i.ignore = !0);
}
function qp(a, e) {
  var t = e.__selfStyle;
  if (t) {
    var r = t.textBaseline,
      n = r;
    !r || r === "auto" || r === "baseline"
      ? (n = "alphabetic")
      : r === "before-edge" || r === "text-before-edge"
      ? (n = "top")
      : r === "after-edge" || r === "text-after-edge"
      ? (n = "bottom")
      : (r === "central" || r === "mathematical") && (n = "middle"),
      (a.style.textBaseline = n);
  }
  var i = e.__inheritedStyle;
  if (i) {
    var o = i.textAlign,
      s = o;
    o && (o === "middle" && (s = "center"), (a.style.textAlign = s));
  }
}
var Kp = /^url\(\s*#(.*?)\)/;
function rs(a, e, t, r) {
  var n = t && t.match(Kp);
  if (n) {
    var i = Uc(n[1]);
    r.push([a, e, i]);
    return;
  }
  return t === "none" && (t = null), t;
}
function jp(a, e) {
  for (var t = 0; t < e.length; t++) {
    var r = e[t];
    r[0].style[r[1]] = a[r[2]];
  }
}
var Jp = /-?([0-9]*\.)?[0-9]+([eE]-?[0-9]+)?/g;
function Va(a) {
  return a.match(Jp) || [];
}
var Qp = /(translate|scale|rotate|skewX|skewY|matrix)\(([\-\s0-9\.eE,]*)\)/g,
  qa = Math.PI / 180;
function td(a, e) {
  var t = a.getAttribute("transform");
  if (t) {
    t = t.replace(/,/g, " ");
    var r = [],
      n = null;
    t.replace(Qp, function (c, h, f) {
      return r.push(h, f), "";
    });
    for (var i = r.length - 1; i > 0; i -= 2) {
      var o = r[i],
        s = r[i - 1],
        l = Va(o);
      switch (((n = n || tr()), s)) {
        case "translate":
          ke(n, n, [parseFloat(l[0]), parseFloat(l[1] || "0")]);
          break;
        case "scale":
          pu(n, n, [parseFloat(l[0]), parseFloat(l[1] || l[0])]);
          break;
        case "rotate":
          La(n, n, -parseFloat(l[0]) * qa, [
            parseFloat(l[1] || "0"),
            parseFloat(l[2] || "0")
          ]);
          break;
        case "skewX":
          var u = Math.tan(parseFloat(l[0]) * qa);
          go(n, [1, 0, u, 1, 0, 0], n);
          break;
        case "skewY":
          var v = Math.tan(parseFloat(l[0]) * qa);
          go(n, [1, v, 0, 1, 0, 0], n);
          break;
        case "matrix":
          (n[0] = parseFloat(l[0])),
            (n[1] = parseFloat(l[1])),
            (n[2] = parseFloat(l[2])),
            (n[3] = parseFloat(l[3])),
            (n[4] = parseFloat(l[4])),
            (n[5] = parseFloat(l[5]));
          break;
      }
    }
    e.setLocalTransform(n);
  }
}
var as = /([^\s:;]+)\s*:\s*([^:;]+)/g;
function ju(a, e, t) {
  var r = a.getAttribute("style");
  if (r) {
    as.lastIndex = 0;
    for (var n; (n = as.exec(r)) != null; ) {
      var i = n[1],
        o = bt(ua, i) ? ua[i] : null;
      o && (e[o] = n[2]);
      var s = bt(va, i) ? va[i] : null;
      s && (t[s] = n[2]);
    }
  }
}
function ed(a, e, t) {
  for (var r = 0; r < Ko.length; r++) {
    var n = Ko[r],
      i = a.getAttribute(n);
    i != null && (e[ua[n]] = i);
  }
  for (var r = 0; r < jo.length; r++) {
    var n = jo[r],
      i = a.getAttribute(n);
    i != null && (t[va[n]] = i);
  }
}
function Ju(a, e) {
  var t = e.width / a.width,
    r = e.height / a.height,
    n = Math.min(t, r);
  return {
    scale: n,
    x: -(a.x + a.width / 2) * n + (e.x + e.width / 2),
    y: -(a.y + a.height / 2) * n + (e.y + e.height / 2)
  };
}
function rd(a, e) {
  var t = new Zp();
  return t.parse(a, e);
}
var ad = tt([
    "rect",
    "circle",
    "line",
    "ellipse",
    "polygon",
    "polyline",
    "path",
    "text",
    "tspan",
    "g"
  ]),
  nd = (function () {
    function a(e, t) {
      (this.type = "geoSVG"),
        (this._usedGraphicMap = tt()),
        (this._freedGraphics = []),
        (this._mapName = e),
        (this._parsedXML = Ku(t));
    }
    return (
      (a.prototype.load = function () {
        var e = this._firstGraphic;
        if (!e) {
          (e = this._firstGraphic = this._buildGraphic(this._parsedXML)),
            this._freedGraphics.push(e),
            (this._boundingRect = this._firstGraphic.boundingRect.clone());
          var t = od(e.named),
            r = t.regions,
            n = t.regionsMap;
          (this._regions = r), (this._regionsMap = n);
        }
        return {
          boundingRect: this._boundingRect,
          regions: this._regions,
          regionsMap: this._regionsMap
        };
      }),
      (a.prototype._buildGraphic = function (e) {
        var t, r;
        try {
          (t = (e && rd(e, { ignoreViewBox: !0, ignoreRootClip: !0 })) || {}),
            (r = t.root),
            ea(r != null);
        } catch (g) {
          throw new Error(
            `Invalid svg format
` + g.message
          );
        }
        var n = new Z();
        n.add(r), (n.isGeoSVGGraphicRoot = !0);
        var i = t.width,
          o = t.height,
          s = t.viewBoxRect,
          l = this._boundingRect;
        if (!l) {
          var u = void 0,
            v = void 0,
            c = void 0,
            h = void 0;
          if (
            (i != null ? ((u = 0), (c = i)) : s && ((u = s.x), (c = s.width)),
            o != null ? ((v = 0), (h = o)) : s && ((v = s.y), (h = s.height)),
            u == null || v == null)
          ) {
            var f = r.getBoundingRect();
            u == null && ((u = f.x), (c = f.width)),
              v == null && ((v = f.y), (h = f.height));
          }
          l = this._boundingRect = new ft(u, v, c, h);
        }
        if (s) {
          var p = Ju(s, l);
          (r.scaleX = r.scaleY = p.scale), (r.x = p.x), (r.y = p.y);
        }
        n.setClipPath(new Et({ shape: l.plain() }));
        var d = [];
        return (
          L(t.named, function (g) {
            ad.get(g.svgNodeTagLower) != null && (d.push(g), id(g.el));
          }),
          { root: n, boundingRect: l, named: d }
        );
      }),
      (a.prototype.useGraphic = function (e) {
        var t = this._usedGraphicMap,
          r = t.get(e);
        return (
          r ||
          ((r =
            this._freedGraphics.pop() || this._buildGraphic(this._parsedXML)),
          t.set(e, r),
          r)
        );
      }),
      (a.prototype.freeGraphic = function (e) {
        var t = this._usedGraphicMap,
          r = t.get(e);
        r && (t.removeKey(e), this._freedGraphics.push(r));
      }),
      a
    );
  })();
function id(a) {
  (a.silent = !1),
    a.isGroup &&
      a.traverse(function (e) {
        e.silent = !1;
      });
}
function od(a) {
  var e = [],
    t = tt();
  return (
    L(a, function (r) {
      if (r.namedFrom == null) {
        var n = new Yc(r.name, r.el);
        e.push(n), t.set(r.name, n);
      }
    }),
    { regions: e, regionsMap: t }
  );
}
var Zn = [126, 25],
  ns = "南海诸岛",
  Le = [
    [
      [0, 3.5],
      [7, 11.2],
      [15, 11.9],
      [30, 7],
      [42, 0.7],
      [52, 0.7],
      [56, 7.7],
      [59, 0.7],
      [64, 0.7],
      [64, 0],
      [5, 0],
      [0, 3.5]
    ],
    [
      [13, 16.1],
      [19, 14.7],
      [16, 21.7],
      [11, 23.1],
      [13, 16.1]
    ],
    [
      [12, 32.2],
      [14, 38.5],
      [15, 38.5],
      [13, 32.2],
      [12, 32.2]
    ],
    [
      [16, 47.6],
      [12, 53.2],
      [13, 53.2],
      [18, 47.6],
      [16, 47.6]
    ],
    [
      [6, 64.4],
      [8, 70],
      [9, 70],
      [8, 64.4],
      [6, 64.4]
    ],
    [
      [23, 82.6],
      [29, 79.8],
      [30, 79.8],
      [25, 82.6],
      [23, 82.6]
    ],
    [
      [37, 70.7],
      [43, 62.3],
      [44, 62.3],
      [39, 70.7],
      [37, 70.7]
    ],
    [
      [48, 51.1],
      [51, 45.5],
      [53, 45.5],
      [50, 51.1],
      [48, 51.1]
    ],
    [
      [51, 35],
      [51, 28.7],
      [53, 28.7],
      [53, 35],
      [51, 35]
    ],
    [
      [52, 22.4],
      [55, 17.5],
      [56, 17.5],
      [53, 22.4],
      [52, 22.4]
    ],
    [
      [58, 12.6],
      [62, 7],
      [63, 7],
      [60, 12.6],
      [58, 12.6]
    ],
    [
      [0, 3.5],
      [0, 93.1],
      [64, 93.1],
      [64, 0],
      [63, 0],
      [63, 92.4],
      [1, 92.4],
      [1, 3.5],
      [0, 3.5]
    ]
  ];
for (var Ie = 0; Ie < Le.length; Ie++)
  for (var ze = 0; ze < Le[Ie].length; ze++)
    (Le[Ie][ze][0] /= 10.5),
      (Le[Ie][ze][1] /= -10.5 / 0.75),
      (Le[Ie][ze][0] += Zn[0]),
      (Le[Ie][ze][1] += Zn[1]);
function sd(a, e) {
  if (a === "china") {
    for (var t = 0; t < e.length; t++) if (e[t].name === ns) return;
    e.push(
      new $c(
        ns,
        O(Le, function (r) {
          return { type: "polygon", exterior: r };
        }),
        Zn
      )
    );
  }
}
var ld = {
  南海诸岛: [32, 80],
  广东: [0, -10],
  香港: [10, 5],
  澳门: [-10, 10],
  天津: [5, 5]
};
function ud(a, e) {
  if (a === "china") {
    var t = ld[e.name];
    if (t) {
      var r = e.getCenter();
      (r[0] += t[0] / 10.5), (r[1] += -t[1] / (10.5 / 0.75)), e.setCenter(r);
    }
  }
}
var vd = [
  [
    [123.45165252685547, 25.73527164402261],
    [123.49731445312499, 25.73527164402261],
    [123.49731445312499, 25.750734064600884],
    [123.45165252685547, 25.750734064600884],
    [123.45165252685547, 25.73527164402261]
  ]
];
function cd(a, e) {
  a === "china" &&
    e.name === "台湾" &&
    e.geometries.push({ type: "polygon", exterior: vd[0] });
}
var hd = "name",
  fd = (function () {
    function a(e, t, r) {
      (this.type = "geoJSON"),
        (this._parsedMap = tt()),
        (this._mapName = e),
        (this._specialAreas = r),
        (this._geoJSON = dd(t));
    }
    return (
      (a.prototype.load = function (e, t) {
        t = t || hd;
        var r = this._parsedMap.get(t);
        if (!r) {
          var n = this._parseToRegions(t);
          r = this._parsedMap.set(t, { regions: n, boundingRect: pd(n) });
        }
        var i = tt(),
          o = [];
        return (
          L(r.regions, function (s) {
            var l = s.name;
            e && bt(e, l) && (s = s.cloneShallow((l = e[l]))),
              o.push(s),
              i.set(l, s);
          }),
          {
            regions: o,
            boundingRect: r.boundingRect || new ft(0, 0, 0, 0),
            regionsMap: i
          }
        );
      }),
      (a.prototype._parseToRegions = function (e) {
        var t = this._mapName,
          r = this._geoJSON,
          n;
        try {
          n = r ? Xc(r, e) : [];
        } catch (i) {
          throw new Error(
            `Invalid geoJson format
` + i.message
          );
        }
        return (
          sd(t, n),
          L(
            n,
            function (i) {
              var o = i.name;
              ud(t, i), cd(t, i);
              var s = this._specialAreas && this._specialAreas[o];
              s && i.transformTo(s.left, s.top, s.width, s.height);
            },
            this
          ),
          n
        );
      }),
      (a.prototype.getMapForUser = function () {
        return {
          geoJson: this._geoJSON,
          geoJSON: this._geoJSON,
          specialAreas: this._specialAreas
        };
      }),
      a
    );
  })();
function pd(a) {
  for (var e, t = 0; t < a.length; t++) {
    var r = a[t].getBoundingRect();
    (e = e || r.clone()), e.union(r);
  }
  return e;
}
function dd(a) {
  return et(a)
    ? typeof JSON < "u" && JSON.parse
      ? JSON.parse(a)
      : new Function("return (" + a + ");")()
    : a;
}
var ir = tt();
const ce = {
  registerMap: function (a, e, t) {
    if (e.svg) {
      var r = new nd(a, e.svg);
      ir.set(a, r);
    } else {
      var n = e.geoJson || e.geoJSON;
      n && !e.features ? (t = e.specialAreas) : (n = e);
      var r = new fd(a, n, t);
      ir.set(a, r);
    }
  },
  getGeoResource: function (a) {
    return ir.get(a);
  },
  getMapForUser: function (a) {
    var e = ir.get(a);
    return e && e.type === "geoJSON" && e.getMapForUser();
  },
  load: function (a, e, t) {
    var r = ir.get(a);
    if (r) return r.load(e, t);
  }
};
var Gi = ["rect", "circle", "line", "ellipse", "polygon", "polyline", "path"],
  gd = tt(Gi),
  yd = tt(Gi.concat(["g"])),
  md = tt(Gi.concat(["g"])),
  Qu = _t();
function Wr(a) {
  var e = a.getItemStyle(),
    t = a.get("areaColor");
  return t != null && (e.fill = t), e;
}
function is(a) {
  var e = a.style;
  e && ((e.stroke = e.stroke || e.fill), (e.fill = null));
}
var tv = (function () {
  function a(e) {
    var t = new Z();
    (this.uid = Zc("ec_map_draw")),
      (this._controller = new Ca(e.getZr())),
      (this._controllerHost = { target: t }),
      (this.group = t),
      t.add((this._regionsGroup = new Z())),
      t.add((this._svgGroup = new Z()));
  }
  return (
    (a.prototype.draw = function (e, t, r, n, i) {
      var o = e.mainType === "geo",
        s = e.getData && e.getData();
      o &&
        t.eachComponent({ mainType: "series", subType: "map" }, function (y) {
          !s && y.getHostGeoModel() === e && (s = y.getData());
        });
      var l = e.coordinateSystem,
        u = this._regionsGroup,
        v = this.group,
        c = l.getTransformInfo(),
        h = c.raw,
        f = c.roam,
        p = !u.childAt(0) || i;
      p
        ? ((v.x = f.x),
          (v.y = f.y),
          (v.scaleX = f.scaleX),
          (v.scaleY = f.scaleY),
          v.dirty())
        : lt(v, f, e);
      var d =
          s &&
          s.getVisual("visualMeta") &&
          s.getVisual("visualMeta").length > 0,
        g = {
          api: r,
          geo: l,
          mapOrGeoModel: e,
          data: s,
          isVisualEncodedByVisualMap: d,
          isGeo: o,
          transformInfoRaw: h
        };
      l.resourceType === "geoJSON"
        ? this._buildGeoJSON(g)
        : l.resourceType === "geoSVG" && this._buildSVG(g),
        this._updateController(e, t, r),
        this._updateMapSelectHandler(e, u, r, n);
    }),
    (a.prototype._buildGeoJSON = function (e) {
      var t = (this._regionsGroupByName = tt()),
        r = tt(),
        n = this._regionsGroup,
        i = e.transformInfoRaw,
        o = e.mapOrGeoModel,
        s = e.data,
        l = e.geo.projection,
        u = l && l.stream;
      function v(f, p) {
        return (
          p && (f = p(f)), f && [f[0] * i.scaleX + i.x, f[1] * i.scaleY + i.y]
        );
      }
      function c(f) {
        for (var p = [], d = !u && l && l.project, g = 0; g < f.length; ++g) {
          var y = v(f[g], d);
          y && p.push(y);
        }
        return p;
      }
      function h(f) {
        return { shape: { points: c(f) } };
      }
      n.removeAll(),
        L(e.geo.regions, function (f) {
          var p = f.name,
            d = t.get(p),
            g = r.get(p) || {},
            y = g.dataIdx,
            S = g.regionModel;
          if (!d) {
            (d = t.set(p, new Z())),
              n.add(d),
              (y = s ? s.indexOfName(p) : null),
              (S = e.isGeo
                ? o.getRegionModel(p)
                : s
                ? s.getItemModel(y)
                : null);
            var m = S.get("silent", !0);
            m != null && (d.silent = m),
              r.set(p, { dataIdx: y, regionModel: S });
          }
          var x = [],
            b = [];
          L(f.geometries, function (A) {
            if (A.type === "polygon") {
              var I = [A.exterior].concat(A.interiors || []);
              u && (I = cs(I, u)),
                L(I, function (D) {
                  x.push(new ve(h(D)));
                });
            } else {
              var T = A.points;
              u && (T = cs(T, u, !0)),
                L(T, function (D) {
                  b.push(new we(h(D)));
                });
            }
          });
          var _ = v(f.getCenter(), l && l.project);
          function w(A, I) {
            if (A.length) {
              var T = new ou({
                culling: !0,
                segmentIgnoreThreshold: 1,
                shape: { paths: A }
              });
              d.add(T),
                os(e, T, y, S),
                ss(e, T, p, S, o, y, _),
                I && (is(T), L(T.states, is));
            }
          }
          w(x), w(b, !0);
        }),
        t.each(function (f, p) {
          var d = r.get(p),
            g = d.dataIdx,
            y = d.regionModel;
          ls(e, f, p, y, o, g), us(e, f, p, y, o), vs(e, f, p, y, o);
        }, this);
    }),
    (a.prototype._buildSVG = function (e) {
      var t = e.geo.map,
        r = e.transformInfoRaw;
      (this._svgGroup.x = r.x),
        (this._svgGroup.y = r.y),
        (this._svgGroup.scaleX = r.scaleX),
        (this._svgGroup.scaleY = r.scaleY),
        this._svgResourceChanged(t) && (this._freeSVG(), this._useSVG(t));
      var n = (this._svgDispatcherMap = tt()),
        i = !1;
      L(
        this._svgGraphicRecord.named,
        function (o) {
          var s = o.name,
            l = e.mapOrGeoModel,
            u = e.data,
            v = o.svgNodeTagLower,
            c = o.el,
            h = u ? u.indexOfName(s) : null,
            f = l.getRegionModel(s);
          gd.get(v) != null && c instanceof mr && os(e, c, h, f),
            c instanceof mr && (c.culling = !0);
          var p = f.get("silent", !0);
          if (
            (p != null && (c.silent = p),
            (c.z2EmphasisLift = 0),
            !o.namedFrom &&
              (md.get(v) != null && ss(e, c, s, f, l, h, null),
              ls(e, c, s, f, l, h),
              us(e, c, s, f, l),
              yd.get(v) != null))
          ) {
            var d = vs(e, c, s, f, l);
            d === "self" && (i = !0);
            var g = n.get(s) || n.set(s, []);
            g.push(c);
          }
        },
        this
      ),
        this._enableBlurEntireSVG(i, e);
    }),
    (a.prototype._enableBlurEntireSVG = function (e, t) {
      if (e && t.isGeo) {
        var r = t.mapOrGeoModel.getModel(["blur", "itemStyle"]).getItemStyle(),
          n = r.opacity;
        this._svgGraphicRecord.root.traverse(function (i) {
          if (!i.isGroup) {
            Ke(i);
            var o = i.ensureState("blur").style || {};
            o.opacity == null && n != null && (o.opacity = n),
              i.ensureState("emphasis");
          }
        });
      }
    }),
    (a.prototype.remove = function () {
      this._regionsGroup.removeAll(),
        (this._regionsGroupByName = null),
        this._svgGroup.removeAll(),
        this._freeSVG(),
        this._controller.dispose(),
        (this._controllerHost = null);
    }),
    (a.prototype.findHighDownDispatchers = function (e, t) {
      if (e == null) return [];
      var r = t.coordinateSystem;
      if (r.resourceType === "geoJSON") {
        var n = this._regionsGroupByName;
        if (n) {
          var i = n.get(e);
          return i ? [i] : [];
        }
      } else if (r.resourceType === "geoSVG")
        return (this._svgDispatcherMap && this._svgDispatcherMap.get(e)) || [];
    }),
    (a.prototype._svgResourceChanged = function (e) {
      return this._svgMapName !== e;
    }),
    (a.prototype._useSVG = function (e) {
      var t = ce.getGeoResource(e);
      if (t && t.type === "geoSVG") {
        var r = t.useGraphic(this.uid);
        this._svgGroup.add(r.root),
          (this._svgGraphicRecord = r),
          (this._svgMapName = e);
      }
    }),
    (a.prototype._freeSVG = function () {
      var e = this._svgMapName;
      if (e != null) {
        var t = ce.getGeoResource(e);
        t && t.type === "geoSVG" && t.freeGraphic(this.uid),
          (this._svgGraphicRecord = null),
          (this._svgDispatcherMap = null),
          this._svgGroup.removeAll(),
          (this._svgMapName = null);
      }
    }),
    (a.prototype._updateController = function (e, t, r) {
      var n = e.coordinateSystem,
        i = this._controller,
        o = this._controllerHost;
      (o.zoomLimit = e.get("scaleLimit")),
        (o.zoom = n.getZoom()),
        i.enable(e.get("roam") || !1);
      var s = e.mainType;
      function l() {
        var u = { type: "geoRoam", componentType: s };
        return (u[s + "Id"] = e.id), u;
      }
      i.off("pan").on(
        "pan",
        function (u) {
          (this._mouseDownFlag = !1),
            ki(o, u.dx, u.dy),
            r.dispatchAction(
              W(l(), { dx: u.dx, dy: u.dy, animation: { duration: 0 } })
            );
        },
        this
      ),
        i.off("zoom").on(
          "zoom",
          function (u) {
            (this._mouseDownFlag = !1),
              Vi(o, u.scale, u.originX, u.originY),
              r.dispatchAction(
                W(l(), {
                  totalZoom: o.zoom,
                  zoom: u.scale,
                  originX: u.originX,
                  originY: u.originY,
                  animation: { duration: 0 }
                })
              );
          },
          this
        ),
        i.setPointerChecker(function (u, v, c) {
          return n.containPoint([v, c]) && !hi(u, r, e);
        });
    }),
    (a.prototype.resetForLabelLayout = function () {
      this.group.traverse(function (e) {
        var t = e.getTextContent();
        t && (t.ignore = Qu(t).ignore);
      });
    }),
    (a.prototype._updateMapSelectHandler = function (e, t, r, n) {
      var i = this;
      t.off("mousedown"),
        t.off("click"),
        e.get("selectedMode") &&
          (t.on("mousedown", function () {
            i._mouseDownFlag = !0;
          }),
          t.on("click", function (o) {
            i._mouseDownFlag && (i._mouseDownFlag = !1);
          }));
    }),
    a
  );
})();
function os(a, e, t, r) {
  var n = r.getModel("itemStyle"),
    i = r.getModel(["emphasis", "itemStyle"]),
    o = r.getModel(["blur", "itemStyle"]),
    s = r.getModel(["select", "itemStyle"]),
    l = Wr(n),
    u = Wr(i),
    v = Wr(s),
    c = Wr(o),
    h = a.data;
  if (h) {
    var f = h.getItemVisual(t, "style"),
      p = h.getItemVisual(t, "decal");
    a.isVisualEncodedByVisualMap && f.fill && (l.fill = f.fill),
      p && (l.decal = fi(p, a.api));
  }
  e.setStyle(l),
    (e.style.strokeNoScale = !0),
    (e.ensureState("emphasis").style = u),
    (e.ensureState("select").style = v),
    (e.ensureState("blur").style = c),
    Ke(e);
}
function ss(a, e, t, r, n, i, o) {
  var s = a.data,
    l = a.isGeo,
    u = s && isNaN(s.get(s.mapDimension("value"), i)),
    v = s && s.getItemLayout(i);
  if (l || u || (v && v.showLabel)) {
    var c = l ? t : i,
      h = void 0;
    (!s || i >= 0) && (h = n);
    var f = o ? { normal: { align: "center", verticalAlign: "middle" } } : null;
    Ut(e, Pt(r), { labelFetcher: h, labelDataIndex: c, defaultText: t }, f);
    var p = e.getTextContent();
    if (p && ((Qu(p).ignore = p.ignore), e.textConfig && o)) {
      var d = e.getBoundingRect().clone();
      (e.textConfig.layoutRect = d),
        (e.textConfig.position = [
          ((o[0] - d.x) / d.width) * 100 + "%",
          ((o[1] - d.y) / d.height) * 100 + "%"
        ]);
    }
    e.disableLabelAnimation = !0;
  } else
    e.removeTextContent(),
      e.removeTextConfig(),
      (e.disableLabelAnimation = null);
}
function ls(a, e, t, r, n, i) {
  a.data
    ? a.data.setItemGraphicEl(i, e)
    : (nt(e).eventData = {
        componentType: "geo",
        componentIndex: n.componentIndex,
        geoIndex: n.componentIndex,
        name: t,
        region: (r && r.option) || {}
      });
}
function us(a, e, t, r, n) {
  a.data ||
    qc({
      el: e,
      componentModel: n,
      itemName: t,
      itemTooltipOption: r.get("tooltip")
    });
}
function vs(a, e, t, r, n) {
  e.highDownSilentOnTouch = !!n.get("selectedMode");
  var i = r.getModel("emphasis"),
    o = i.get("focus");
  return (
    ct(e, o, i.get("blurScope"), i.get("disabled")), a.isGeo && Kc(e, n, t), o
  );
}
function cs(a, e, t) {
  var r = [],
    n;
  function i() {
    n = [];
  }
  function o() {
    n.length && (r.push(n), (n = []));
  }
  var s = e({
    polygonStart: i,
    polygonEnd: o,
    lineStart: i,
    lineEnd: o,
    point: function (l, u) {
      isFinite(l) && isFinite(u) && n.push([l, u]);
    },
    sphere: function () {}
  });
  return (
    !t && s.polygonStart(),
    L(a, function (l) {
      s.lineStart();
      for (var u = 0; u < l.length; u++) s.point(l[u][0], l[u][1]);
      s.lineEnd();
    }),
    !t && s.polygonEnd(),
    r
  );
}
var Sd = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n, i) {
        if (!(i && i.type === "mapToggleSelect" && i.from === this.uid)) {
          var o = this.group;
          if ((o.removeAll(), !t.getHostGeoModel())) {
            if (
              (this._mapDraw &&
                i &&
                i.type === "geoRoam" &&
                this._mapDraw.resetForLabelLayout(),
              i &&
                i.type === "geoRoam" &&
                i.componentType === "series" &&
                i.seriesId === t.id)
            ) {
              var s = this._mapDraw;
              s && o.add(s.group);
            } else if (t.needsDrawMap) {
              var s = this._mapDraw || new tv(n);
              o.add(s.group), s.draw(t, r, n, this, i), (this._mapDraw = s);
            } else
              this._mapDraw && this._mapDraw.remove(), (this._mapDraw = null);
            t.get("showLegendSymbol") &&
              r.getComponent("legend") &&
              this._renderSymbols(t, r, n);
          }
        }
      }),
      (e.prototype.remove = function () {
        this._mapDraw && this._mapDraw.remove(),
          (this._mapDraw = null),
          this.group.removeAll();
      }),
      (e.prototype.dispose = function () {
        this._mapDraw && this._mapDraw.remove(), (this._mapDraw = null);
      }),
      (e.prototype._renderSymbols = function (t, r, n) {
        var i = t.originalData,
          o = this.group;
        i.each(i.mapDimension("value"), function (s, l) {
          if (!isNaN(s)) {
            var u = i.getItemLayout(l);
            if (!(!u || !u.point)) {
              var v = u.point,
                c = u.offset,
                h = new Cr({
                  style: { fill: t.getData().getVisual("style").fill },
                  shape: { cx: v[0] + c * 9, cy: v[1], r: 3 },
                  silent: !0,
                  z2: 8 + (c ? 0 : Pr + 1)
                });
              if (!c) {
                var f = t.mainSeries.getData(),
                  p = i.getName(l),
                  d = f.indexOfName(p),
                  g = i.getItemModel(l),
                  y = g.getModel("label"),
                  S = f.getItemGraphicEl(d);
                Ut(h, Pt(g), {
                  labelFetcher: {
                    getFormattedLabel: function (m, x) {
                      return t.getFormattedLabel(d, x);
                    }
                  },
                  defaultText: p
                }),
                  (h.disableLabelAnimation = !0),
                  y.get("position") || h.setTextConfig({ position: "bottom" }),
                  (S.onHoverStateChange = function (m) {
                    du(h, m);
                  });
              }
              o.add(h);
            }
          }
        });
      }),
      (e.type = "map"),
      e
    );
  })(pt),
  xd = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (
        (t.type = e.type),
        (t.needsDrawMap = !1),
        (t.seriesGroup = []),
        (t.getTooltipPosition = function (r) {
          if (r != null) {
            var n = this.getData().getName(r),
              i = this.coordinateSystem,
              o = i.getRegion(n);
            return o && i.dataToPoint(o.getCenter());
          }
        }),
        t
      );
    }
    return (
      (e.prototype.getInitialData = function (t) {
        for (
          var r = Lr(this, {
              coordDimensions: ["value"],
              encodeDefaulter: ot(gu, this)
            }),
            n = tt(),
            i = [],
            o = 0,
            s = r.count();
          o < s;
          o++
        ) {
          var l = r.getName(o);
          n.set(l, o);
        }
        var u = ce.load(
          this.getMapType(),
          this.option.nameMap,
          this.option.nameProperty
        );
        return (
          L(u.regions, function (v) {
            var c = v.name,
              h = n.get(c),
              f = v.properties && v.properties.echartsStyle,
              p;
            h == null
              ? ((p = { name: c }), i.push(p))
              : (p = r.getRawDataItem(h)),
              f && Yt(p, f);
          }),
          r.appendData(i),
          r
        );
      }),
      (e.prototype.getHostGeoModel = function () {
        var t = this.option.geoIndex;
        return t != null ? this.ecModel.getComponent("geo", t) : null;
      }),
      (e.prototype.getMapType = function () {
        return (this.getHostGeoModel() || this).option.map;
      }),
      (e.prototype.getRawValue = function (t) {
        var r = this.getData();
        return r.get(r.mapDimension("value"), t);
      }),
      (e.prototype.getRegionModel = function (t) {
        var r = this.getData();
        return r.getItemModel(r.indexOfName(t));
      }),
      (e.prototype.formatTooltip = function (t, r, n) {
        for (
          var i = this.getData(),
            o = this.getRawValue(t),
            s = i.getName(t),
            l = this.seriesGroup,
            u = [],
            v = 0;
          v < l.length;
          v++
        ) {
          var c = l[v].originalData.indexOfName(s),
            h = i.mapDimension("value");
          isNaN(l[v].originalData.get(h, c)) || u.push(l[v].name);
        }
        return Mt("section", {
          header: u.join(", "),
          noHeader: !u.length,
          blocks: [Mt("nameValue", { name: s, value: o })]
        });
      }),
      (e.prototype.setZoom = function (t) {
        this.option.zoom = t;
      }),
      (e.prototype.setCenter = function (t) {
        this.option.center = t;
      }),
      (e.prototype.getLegendIcon = function (t) {
        var r = t.icon || "roundRect",
          n = ie(r, 0, 0, t.itemWidth, t.itemHeight, t.itemStyle.fill);
        return (
          n.setStyle(t.itemStyle),
          (n.style.stroke = "none"),
          r.indexOf("empty") > -1 &&
            ((n.style.stroke = n.style.fill),
            (n.style.fill = "#fff"),
            (n.style.lineWidth = 2)),
          n
        );
      }),
      (e.type = "series.map"),
      (e.dependencies = ["geo"]),
      (e.layoutMode = "box"),
      (e.defaultOption = {
        z: 2,
        coordinateSystem: "geo",
        map: "",
        left: "center",
        top: "center",
        aspectScale: null,
        showLegendSymbol: !0,
        boundingCoords: null,
        center: null,
        zoom: 1,
        scaleLimit: null,
        selectedMode: !0,
        label: { show: !1, color: "#000" },
        itemStyle: { borderWidth: 0.5, borderColor: "#444", areaColor: "#eee" },
        emphasis: {
          label: { show: !0, color: "rgb(100,0,0)" },
          itemStyle: { areaColor: "rgba(255,215,0,0.8)" }
        },
        select: {
          label: { show: !0, color: "rgb(100,0,0)" },
          itemStyle: { color: "rgba(255,215,0,0.8)" }
        },
        nameProperty: "name"
      }),
      e
    );
  })(gt);
function bd(a, e) {
  var t = {};
  return (
    L(a, function (r) {
      r.each(r.mapDimension("value"), function (n, i) {
        var o = "ec-" + r.getName(i);
        (t[o] = t[o] || []), isNaN(n) || t[o].push(n);
      });
    }),
    a[0].map(a[0].mapDimension("value"), function (r, n) {
      for (
        var i = "ec-" + a[0].getName(n),
          o = 0,
          s = 1 / 0,
          l = -1 / 0,
          u = t[i].length,
          v = 0;
        v < u;
        v++
      )
        (s = Math.min(s, t[i][v])), (l = Math.max(l, t[i][v])), (o += t[i][v]);
      var c;
      return (
        e === "min"
          ? (c = s)
          : e === "max"
          ? (c = l)
          : e === "average"
          ? (c = o / u)
          : (c = o),
        u === 0 ? NaN : c
      );
    })
  );
}
function _d(a) {
  var e = {};
  a.eachSeriesByType("map", function (t) {
    var r = t.getHostGeoModel(),
      n = r ? "o" + r.id : "i" + t.getMapType();
    (e[n] = e[n] || []).push(t);
  }),
    L(e, function (t, r) {
      for (
        var n = bd(
            O(t, function (o) {
              return o.getData();
            }),
            t[0].get("mapValueCalculation")
          ),
          i = 0;
        i < t.length;
        i++
      )
        t[i].originalData = t[i].getData();
      for (var i = 0; i < t.length; i++)
        (t[i].seriesGroup = t),
          (t[i].needsDrawMap = i === 0 && !t[i].getHostGeoModel()),
          t[i].setData(n.cloneShallow()),
          (t[i].mainSeries = t[0]);
    });
}
function wd(a) {
  var e = {};
  a.eachSeriesByType("map", function (t) {
    var r = t.getMapType();
    if (!(t.getHostGeoModel() || e[r])) {
      var n = {};
      L(t.seriesGroup, function (o) {
        var s = o.coordinateSystem,
          l = o.originalData;
        o.get("showLegendSymbol") &&
          a.getComponent("legend") &&
          l.each(l.mapDimension("value"), function (u, v) {
            var c = l.getName(v),
              h = s.getRegion(c);
            if (!(!h || isNaN(u))) {
              var f = n[c] || 0,
                p = s.dataToPoint(h.getCenter());
              (n[c] = f + 1), l.setItemLayout(v, { point: p, offset: f });
            }
          });
      });
      var i = t.getData();
      i.each(function (o) {
        var s = i.getName(o),
          l = i.getItemLayout(o) || {};
        (l.showLabel = !n[s]), i.setItemLayout(o, l);
      }),
        (e[r] = !0);
    }
  });
}
var hs = Gn,
  Vr = (function (a) {
    k(e, a);
    function e(t) {
      var r = a.call(this) || this;
      return (
        (r.type = "view"),
        (r.dimensions = ["x", "y"]),
        (r._roamTransformable = new Ye()),
        (r._rawTransformable = new Ye()),
        (r.name = t),
        r
      );
    }
    return (
      (e.prototype.setBoundingRect = function (t, r, n, i) {
        return (this._rect = new ft(t, r, n, i)), this._rect;
      }),
      (e.prototype.getBoundingRect = function () {
        return this._rect;
      }),
      (e.prototype.setViewRect = function (t, r, n, i) {
        this._transformTo(t, r, n, i), (this._viewRect = new ft(t, r, n, i));
      }),
      (e.prototype._transformTo = function (t, r, n, i) {
        var o = this.getBoundingRect(),
          s = this._rawTransformable;
        s.transform = o.calculateTransform(new ft(t, r, n, i));
        var l = s.parent;
        (s.parent = null),
          s.decomposeTransform(),
          (s.parent = l),
          this._updateTransform();
      }),
      (e.prototype.setCenter = function (t, r) {
        t &&
          ((this._center = [z(t[0], r.getWidth()), z(t[1], r.getHeight())]),
          this._updateCenterAndZoom());
      }),
      (e.prototype.setZoom = function (t) {
        t = t || 1;
        var r = this.zoomLimit;
        r &&
          (r.max != null && (t = Math.min(r.max, t)),
          r.min != null && (t = Math.max(r.min, t))),
          (this._zoom = t),
          this._updateCenterAndZoom();
      }),
      (e.prototype.getDefaultCenter = function () {
        var t = this.getBoundingRect(),
          r = t.x + t.width / 2,
          n = t.y + t.height / 2;
        return [r, n];
      }),
      (e.prototype.getCenter = function () {
        return this._center || this.getDefaultCenter();
      }),
      (e.prototype.getZoom = function () {
        return this._zoom || 1;
      }),
      (e.prototype.getRoamTransform = function () {
        return this._roamTransformable.getLocalTransform();
      }),
      (e.prototype._updateCenterAndZoom = function () {
        var t = this._rawTransformable.getLocalTransform(),
          r = this._roamTransformable,
          n = this.getDefaultCenter(),
          i = this.getCenter(),
          o = this.getZoom();
        (i = Gn([], i, t)),
          (n = Gn([], n, t)),
          (r.originX = i[0]),
          (r.originY = i[1]),
          (r.x = n[0] - i[0]),
          (r.y = n[1] - i[1]),
          (r.scaleX = r.scaleY = o),
          this._updateTransform();
      }),
      (e.prototype._updateTransform = function () {
        var t = this._roamTransformable,
          r = this._rawTransformable;
        (r.parent = t),
          t.updateTransform(),
          r.updateTransform(),
          jc(this.transform || (this.transform = []), r.transform || tr()),
          (this._rawTransform = r.getLocalTransform()),
          (this.invTransform = this.invTransform || []),
          Jc(this.invTransform, this.transform),
          this.decomposeTransform();
      }),
      (e.prototype.getTransformInfo = function () {
        var t = this._rawTransformable,
          r = this._roamTransformable,
          n = new Ye();
        return (
          (n.transform = r.transform),
          n.decomposeTransform(),
          {
            roam: { x: n.x, y: n.y, scaleX: n.scaleX, scaleY: n.scaleY },
            raw: { x: t.x, y: t.y, scaleX: t.scaleX, scaleY: t.scaleY }
          }
        );
      }),
      (e.prototype.getViewRect = function () {
        return this._viewRect;
      }),
      (e.prototype.getViewRectAfterRoam = function () {
        var t = this.getBoundingRect().clone();
        return t.applyTransform(this.transform), t;
      }),
      (e.prototype.dataToPoint = function (t, r, n) {
        var i = r ? this._rawTransform : this.transform;
        return (n = n || []), i ? hs(n, t, i) : It(n, t);
      }),
      (e.prototype.pointToData = function (t) {
        var r = this.invTransform;
        return r ? hs([], t, r) : [t[0], t[1]];
      }),
      (e.prototype.convertToPixel = function (t, r, n) {
        var i = fs(r);
        return i === this ? i.dataToPoint(n) : null;
      }),
      (e.prototype.convertFromPixel = function (t, r, n) {
        var i = fs(r);
        return i === this ? i.pointToData(n) : null;
      }),
      (e.prototype.containPoint = function (t) {
        return this.getViewRectAfterRoam().contain(t[0], t[1]);
      }),
      (e.dimensions = ["x", "y"]),
      e
    );
  })(Ye);
function fs(a) {
  var e = a.seriesModel;
  return e ? e.coordinateSystem : null;
}
var Ad = {
    geoJSON: { aspectScale: 0.75, invertLongitute: !0 },
    geoSVG: { aspectScale: 1, invertLongitute: !1 }
  },
  ev = ["lng", "lat"],
  qn = (function (a) {
    k(e, a);
    function e(t, r, n) {
      var i = a.call(this, t) || this;
      (i.dimensions = ev),
        (i.type = "geo"),
        (i._nameCoordMap = tt()),
        (i.map = r);
      var o = n.projection,
        s = ce.load(r, n.nameMap, n.nameProperty),
        l = ce.getGeoResource(r);
      i.resourceType = l ? l.type : null;
      var u = (i.regions = s.regions),
        v = Ad[l.type];
      (i._regionsMap = s.regionsMap),
        (i.regions = s.regions),
        (i.projection = o);
      var c;
      if (o)
        for (var h = 0; h < u.length; h++) {
          var f = u[h].getBoundingRect(o);
          (c = c || f.clone()), c.union(f);
        }
      else c = s.boundingRect;
      return (
        i.setBoundingRect(c.x, c.y, c.width, c.height),
        (i.aspectScale = o ? 1 : St(n.aspectScale, v.aspectScale)),
        (i._invertLongitute = o ? !1 : v.invertLongitute),
        i
      );
    }
    return (
      (e.prototype._transformTo = function (t, r, n, i) {
        var o = this.getBoundingRect(),
          s = this._invertLongitute;
        (o = o.clone()), s && (o.y = -o.y - o.height);
        var l = this._rawTransformable;
        l.transform = o.calculateTransform(new ft(t, r, n, i));
        var u = l.parent;
        (l.parent = null),
          l.decomposeTransform(),
          (l.parent = u),
          s && (l.scaleY = -l.scaleY),
          this._updateTransform();
      }),
      (e.prototype.getRegion = function (t) {
        return this._regionsMap.get(t);
      }),
      (e.prototype.getRegionByCoord = function (t) {
        for (var r = this.regions, n = 0; n < r.length; n++) {
          var i = r[n];
          if (i.type === "geoJSON" && i.contain(t)) return r[n];
        }
      }),
      (e.prototype.addGeoCoord = function (t, r) {
        this._nameCoordMap.set(t, r);
      }),
      (e.prototype.getGeoCoord = function (t) {
        var r = this._regionsMap.get(t);
        return this._nameCoordMap.get(t) || (r && r.getCenter());
      }),
      (e.prototype.dataToPoint = function (t, r, n) {
        if ((et(t) && (t = this.getGeoCoord(t)), t)) {
          var i = this.projection;
          return i && (t = i.project(t)), t && this.projectedToPoint(t, r, n);
        }
      }),
      (e.prototype.pointToData = function (t) {
        var r = this.projection;
        return r && (t = r.unproject(t)), t && this.pointToProjected(t);
      }),
      (e.prototype.pointToProjected = function (t) {
        return a.prototype.pointToData.call(this, t);
      }),
      (e.prototype.projectedToPoint = function (t, r, n) {
        return a.prototype.dataToPoint.call(this, t, r, n);
      }),
      (e.prototype.convertToPixel = function (t, r, n) {
        var i = ps(r);
        return i === this ? i.dataToPoint(n) : null;
      }),
      (e.prototype.convertFromPixel = function (t, r, n) {
        var i = ps(r);
        return i === this ? i.pointToData(n) : null;
      }),
      e
    );
  })(Vr);
te(qn, Vr);
function ps(a) {
  var e = a.geoModel,
    t = a.seriesModel;
  return e
    ? e.coordinateSystem
    : t
    ? t.coordinateSystem ||
      (t.getReferringComponents("geo", er).models[0] || {}).coordinateSystem
    : null;
}
function ds(a, e) {
  var t = a.get("boundingCoords");
  if (t != null) {
    var r = t[0],
      n = t[1];
    if (isFinite(r[0]) && isFinite(r[1]) && isFinite(n[0]) && isFinite(n[1])) {
      var i = this.projection;
      if (i) {
        var o = r[0],
          s = r[1],
          l = n[0],
          u = n[1];
        (r = [1 / 0, 1 / 0]), (n = [-1 / 0, -1 / 0]);
        var v = function (_, w, A, I) {
          for (var T = A - _, D = I - w, M = 0; M <= 100; M++) {
            var C = M / 100,
              P = i.project([_ + T * C, w + D * C]);
            Qc(r, r, P), th(n, n, P);
          }
        };
        v(o, s, l, s), v(l, s, l, u), v(l, u, o, u), v(o, u, l, s);
      }
      this.setBoundingRect(r[0], r[1], n[0] - r[0], n[1] - r[1]);
    }
  }
  var c = this.getBoundingRect(),
    h = a.get("layoutCenter"),
    f = a.get("layoutSize"),
    p = e.getWidth(),
    d = e.getHeight(),
    g = (c.width / c.height) * this.aspectScale,
    y = !1,
    S,
    m;
  h &&
    f &&
    ((S = [z(h[0], p), z(h[1], d)]),
    (m = z(f, Math.min(p, d))),
    !isNaN(S[0]) && !isNaN(S[1]) && !isNaN(m) && (y = !0));
  var x;
  if (y)
    (x = {}),
      g > 1
        ? ((x.width = m), (x.height = m / g))
        : ((x.height = m), (x.width = m * g)),
      (x.y = S[1] - x.height / 2),
      (x.x = S[0] - x.width / 2);
  else {
    var b = a.getBoxLayoutParams();
    (b.aspect = g), (x = se(b, { width: p, height: d }));
  }
  this.setViewRect(x.x, x.y, x.width, x.height),
    this.setCenter(a.get("center"), e),
    this.setZoom(a.get("zoom"));
}
function Td(a, e) {
  L(e.get("geoCoord"), function (t, r) {
    a.addGeoCoord(r, t);
  });
}
var Id = (function () {
    function a() {
      this.dimensions = ev;
    }
    return (
      (a.prototype.create = function (e, t) {
        var r = [];
        function n(o) {
          return {
            nameProperty: o.get("nameProperty"),
            aspectScale: o.get("aspectScale"),
            projection: o.get("projection")
          };
        }
        e.eachComponent("geo", function (o, s) {
          var l = o.get("map"),
            u = new qn(l + s, l, W({ nameMap: o.get("nameMap") }, n(o)));
          (u.zoomLimit = o.get("scaleLimit")),
            r.push(u),
            (o.coordinateSystem = u),
            (u.model = o),
            (u.resize = ds),
            u.resize(o, t);
        }),
          e.eachSeries(function (o) {
            var s = o.get("coordinateSystem");
            if (s === "geo") {
              var l = o.get("geoIndex") || 0;
              o.coordinateSystem = r[l];
            }
          });
        var i = {};
        return (
          e.eachSeriesByType("map", function (o) {
            if (!o.getHostGeoModel()) {
              var s = o.getMapType();
              (i[s] = i[s] || []), i[s].push(o);
            }
          }),
          L(i, function (o, s) {
            var l = O(o, function (v) {
                return v.get("nameMap");
              }),
              u = new qn(s, s, W({ nameMap: pi(l) }, n(o[0])));
            (u.zoomLimit = Ft.apply(
              null,
              O(o, function (v) {
                return v.get("scaleLimit");
              })
            )),
              r.push(u),
              (u.resize = ds),
              u.resize(o[0], t),
              L(o, function (v) {
                (v.coordinateSystem = u), Td(u, v);
              });
          }),
          r
        );
      }),
      (a.prototype.getFilledRegions = function (e, t, r, n) {
        for (var i = (e || []).slice(), o = tt(), s = 0; s < i.length; s++)
          o.set(i[s].name, i[s]);
        var l = ce.load(t, r, n);
        return (
          L(l.regions, function (u) {
            var v = u.name,
              c = o.get(v),
              h = u.properties && u.properties.echartsStyle;
            c || ((c = { name: v }), i.push(c)), h && Yt(c, h);
          }),
          i
        );
      }),
      a
    );
  })(),
  rv = new Id(),
  Dd = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.init = function (t, r, n) {
        var i = ce.getGeoResource(t.map);
        if (i && i.type === "geoJSON") {
          var o = (t.itemStyle = t.itemStyle || {});
          "color" in o || (o.color = "#eee");
        }
        this.mergeDefaultAndTheme(t, n), Pa(t, "label", ["show"]);
      }),
      (e.prototype.optionUpdated = function () {
        var t = this,
          r = this.option;
        r.regions = rv.getFilledRegions(
          r.regions,
          r.map,
          r.nameMap,
          r.nameProperty
        );
        var n = {};
        (this._optionModelMap = eh(
          r.regions || [],
          function (i, o) {
            var s = o.name;
            return (
              s &&
                (i.set(s, new $t(o, t, t.ecModel)), o.selected && (n[s] = !0)),
              i
            );
          },
          tt()
        )),
          r.selectedMap || (r.selectedMap = n);
      }),
      (e.prototype.getRegionModel = function (t) {
        return this._optionModelMap.get(t) || new $t(null, this, this.ecModel);
      }),
      (e.prototype.getFormattedLabel = function (t, r) {
        var n = this.getRegionModel(t),
          i =
            r === "normal"
              ? n.get(["label", "formatter"])
              : n.get(["emphasis", "label", "formatter"]),
          o = { name: t };
        if (it(i)) return (o.status = r), i(o);
        if (et(i)) return i.replace("{a}", t ?? "");
      }),
      (e.prototype.setZoom = function (t) {
        this.option.zoom = t;
      }),
      (e.prototype.setCenter = function (t) {
        this.option.center = t;
      }),
      (e.prototype.select = function (t) {
        var r = this.option,
          n = r.selectedMode;
        if (n) {
          n !== "multiple" && (r.selectedMap = null);
          var i = r.selectedMap || (r.selectedMap = {});
          i[t] = !0;
        }
      }),
      (e.prototype.unSelect = function (t) {
        var r = this.option.selectedMap;
        r && (r[t] = !1);
      }),
      (e.prototype.toggleSelected = function (t) {
        this[this.isSelected(t) ? "unSelect" : "select"](t);
      }),
      (e.prototype.isSelected = function (t) {
        var r = this.option.selectedMap;
        return !!(r && r[t]);
      }),
      (e.type = "geo"),
      (e.layoutMode = "box"),
      (e.defaultOption = {
        z: 0,
        show: !0,
        left: "center",
        top: "center",
        aspectScale: null,
        silent: !1,
        map: "",
        boundingCoords: null,
        center: null,
        zoom: 1,
        scaleLimit: null,
        label: { show: !1, color: "#000" },
        itemStyle: { borderWidth: 0.5, borderColor: "#444" },
        emphasis: {
          label: { show: !0, color: "rgb(100,0,0)" },
          itemStyle: { color: "rgba(255,215,0,0.8)" }
        },
        select: {
          label: { show: !0, color: "rgb(100,0,0)" },
          itemStyle: { color: "rgba(255,215,0,0.8)" }
        },
        regions: []
      }),
      e
    );
  })(Xt);
function gs(a, e) {
  return a.pointToProjected ? a.pointToProjected(e) : a.pointToData(e);
}
function zi(a, e, t, r) {
  var n = a.getZoom(),
    i = a.getCenter(),
    o = e.zoom,
    s = a.projectedToPoint ? a.projectedToPoint(i) : a.dataToPoint(i);
  if (
    (e.dx != null &&
      e.dy != null &&
      ((s[0] -= e.dx), (s[1] -= e.dy), a.setCenter(gs(a, s), r)),
    o != null)
  ) {
    if (t) {
      var l = t.min || 0,
        u = t.max || 1 / 0;
      o = Math.max(Math.min(n * o, u), l) / n;
    }
    (a.scaleX *= o), (a.scaleY *= o);
    var v = (e.originX - a.x) * (o - 1),
      c = (e.originY - a.y) * (o - 1);
    (a.x -= v),
      (a.y -= c),
      a.updateTransform(),
      a.setCenter(gs(a, s), r),
      a.setZoom(o * n);
  }
  return { center: a.getCenter(), zoom: a.getZoom() };
}
var Ld = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), (t.focusBlurEnabled = !0), t;
  }
  return (
    (e.prototype.init = function (t, r) {
      this._api = r;
    }),
    (e.prototype.render = function (t, r, n, i) {
      if (((this._model = t), !t.get("show"))) {
        this._mapDraw && this._mapDraw.remove(), (this._mapDraw = null);
        return;
      }
      this._mapDraw || (this._mapDraw = new tv(n));
      var o = this._mapDraw;
      o.draw(t, r, n, this, i),
        o.group.on("click", this._handleRegionClick, this),
        (o.group.silent = t.get("silent")),
        this.group.add(o.group),
        this.updateSelectStatus(t, r, n);
    }),
    (e.prototype._handleRegionClick = function (t) {
      var r;
      rh(
        t.target,
        function (n) {
          return (r = nt(n).eventData) != null;
        },
        !0
      ),
        r &&
          this._api.dispatchAction({
            type: "geoToggleSelect",
            geoId: this._model.id,
            name: r.name
          });
    }),
    (e.prototype.updateSelectStatus = function (t, r, n) {
      var i = this;
      this._mapDraw.group.traverse(function (o) {
        var s = nt(o).eventData;
        if (s)
          return (
            i._model.isSelected(s.name) ? n.enterSelect(o) : n.leaveSelect(o),
            !0
          );
      });
    }),
    (e.prototype.findHighDownDispatchers = function (t) {
      return (
        this._mapDraw && this._mapDraw.findHighDownDispatchers(t, this._model)
      );
    }),
    (e.prototype.dispose = function () {
      this._mapDraw && this._mapDraw.remove();
    }),
    (e.type = "geo"),
    e
  );
})(ee);
function Cd(a, e, t) {
  ce.registerMap(a, e, t);
}
function av(a) {
  a.registerCoordinateSystem("geo", rv),
    a.registerComponentModel(Dd),
    a.registerComponentView(Ld),
    a.registerImpl("registerMap", Cd),
    a.registerImpl("getMap", function (t) {
      return ce.getMapForUser(t);
    });
  function e(t, r) {
    (r.update = "geo:updateSelectStatus"),
      a.registerAction(r, function (n, i) {
        var o = {},
          s = [];
        return (
          i.eachComponent({ mainType: "geo", query: n }, function (l) {
            l[t](n.name);
            var u = l.coordinateSystem;
            L(u.regions, function (c) {
              o[c.name] = l.isSelected(c.name) || !1;
            });
            var v = [];
            L(o, function (c, h) {
              o[h] && v.push(h);
            }),
              s.push({ geoIndex: l.componentIndex, name: v });
          }),
          { selected: o, allSelected: s, name: n.name }
        );
      });
  }
  e("toggleSelected", { type: "geoToggleSelect", event: "geoselectchanged" }),
    e("select", { type: "geoSelect", event: "geoselected" }),
    e("unSelect", { type: "geoUnSelect", event: "geounselected" }),
    a.registerAction(
      { type: "geoRoam", event: "geoRoam", update: "updateTransform" },
      function (t, r, n) {
        var i = t.componentType || "series";
        r.eachComponent({ mainType: i, query: t }, function (o) {
          var s = o.coordinateSystem;
          if (s.type === "geo") {
            var l = zi(s, t, o.get("scaleLimit"), n);
            o.setCenter && o.setCenter(l.center),
              o.setZoom && o.setZoom(l.zoom),
              i === "series" &&
                L(o.seriesGroup, function (u) {
                  u.setCenter(l.center), u.setZoom(l.zoom);
                });
          }
        });
      }
    );
}
function Pd(a) {
  q(av),
    a.registerChartView(Sd),
    a.registerSeriesModel(xd),
    a.registerLayout(wd),
    a.registerProcessor(a.PRIORITY.PROCESSOR.STATISTIC, _d),
    ah("map", a.registerAction);
}
function Md(a) {
  var e = a;
  e.hierNode = {
    defaultAncestor: null,
    ancestor: e,
    prelim: 0,
    modifier: 0,
    change: 0,
    shift: 0,
    i: 0,
    thread: null
  };
  for (var t = [e], r, n; (r = t.pop()); )
    if (((n = r.children), r.isExpand && n.length))
      for (var i = n.length, o = i - 1; o >= 0; o--) {
        var s = n[o];
        (s.hierNode = {
          defaultAncestor: null,
          ancestor: s,
          prelim: 0,
          modifier: 0,
          change: 0,
          shift: 0,
          i: o,
          thread: null
        }),
          t.push(s);
      }
}
function Ed(a, e) {
  var t = a.isExpand ? a.children : [],
    r = a.parentNode.children,
    n = a.hierNode.i ? r[a.hierNode.i - 1] : null;
  if (t.length) {
    kd(a);
    var i = (t[0].hierNode.prelim + t[t.length - 1].hierNode.prelim) / 2;
    n
      ? ((a.hierNode.prelim = n.hierNode.prelim + e(a, n)),
        (a.hierNode.modifier = a.hierNode.prelim - i))
      : (a.hierNode.prelim = i);
  } else n && (a.hierNode.prelim = n.hierNode.prelim + e(a, n));
  a.parentNode.hierNode.defaultAncestor = Vd(
    a,
    n,
    a.parentNode.hierNode.defaultAncestor || r[0],
    e
  );
}
function Rd(a) {
  var e = a.hierNode.prelim + a.parentNode.hierNode.modifier;
  a.setLayout({ x: e }, !0),
    (a.hierNode.modifier += a.parentNode.hierNode.modifier);
}
function ys(a) {
  return arguments.length ? a : Od;
}
function hr(a, e) {
  return (a -= Math.PI / 2), { x: e * Math.cos(a), y: e * Math.sin(a) };
}
function Nd(a, e) {
  return se(a.getBoxLayoutParams(), {
    width: e.getWidth(),
    height: e.getHeight()
  });
}
function kd(a) {
  for (var e = a.children, t = e.length, r = 0, n = 0; --t >= 0; ) {
    var i = e[t];
    (i.hierNode.prelim += r),
      (i.hierNode.modifier += r),
      (n += i.hierNode.change),
      (r += i.hierNode.shift + n);
  }
}
function Vd(a, e, t, r) {
  if (e) {
    for (
      var n = a,
        i = a,
        o = i.parentNode.children[0],
        s = e,
        l = n.hierNode.modifier,
        u = i.hierNode.modifier,
        v = o.hierNode.modifier,
        c = s.hierNode.modifier;
      (s = Ka(s)), (i = ja(i)), s && i;

    ) {
      (n = Ka(n)), (o = ja(o)), (n.hierNode.ancestor = a);
      var h = s.hierNode.prelim + c - i.hierNode.prelim - u + r(s, i);
      h > 0 && (zd(Gd(s, a, t), a, h), (u += h), (l += h)),
        (c += s.hierNode.modifier),
        (u += i.hierNode.modifier),
        (l += n.hierNode.modifier),
        (v += o.hierNode.modifier);
    }
    s && !Ka(n) && ((n.hierNode.thread = s), (n.hierNode.modifier += c - l)),
      i &&
        !ja(o) &&
        ((o.hierNode.thread = i), (o.hierNode.modifier += u - v), (t = a));
  }
  return t;
}
function Ka(a) {
  var e = a.children;
  return e.length && a.isExpand ? e[e.length - 1] : a.hierNode.thread;
}
function ja(a) {
  var e = a.children;
  return e.length && a.isExpand ? e[0] : a.hierNode.thread;
}
function Gd(a, e, t) {
  return a.hierNode.ancestor.parentNode === e.parentNode
    ? a.hierNode.ancestor
    : t;
}
function zd(a, e, t) {
  var r = t / (e.hierNode.i - a.hierNode.i);
  (e.hierNode.change -= r),
    (e.hierNode.shift += t),
    (e.hierNode.modifier += t),
    (e.hierNode.prelim += t),
    (a.hierNode.change += r);
}
function Od(a, e) {
  return a.parentNode === e.parentNode ? 1 : 2;
}
var Bd = (function () {
    function a() {
      (this.parentPoint = []), (this.childPoints = []);
    }
    return a;
  })(),
  Fd = (function (a) {
    k(e, a);
    function e(t) {
      return a.call(this, t) || this;
    }
    return (
      (e.prototype.getDefaultStyle = function () {
        return { stroke: "#000", fill: null };
      }),
      (e.prototype.getDefaultShape = function () {
        return new Bd();
      }),
      (e.prototype.buildPath = function (t, r) {
        var n = r.childPoints,
          i = n.length,
          o = r.parentPoint,
          s = n[0],
          l = n[i - 1];
        if (i === 1) {
          t.moveTo(o[0], o[1]), t.lineTo(s[0], s[1]);
          return;
        }
        var u = r.orient,
          v = u === "TB" || u === "BT" ? 0 : 1,
          c = 1 - v,
          h = z(r.forkPosition, 1),
          f = [];
        (f[v] = o[v]),
          (f[c] = o[c] + (l[c] - o[c]) * h),
          t.moveTo(o[0], o[1]),
          t.lineTo(f[0], f[1]),
          t.moveTo(s[0], s[1]),
          (f[v] = s[v]),
          t.lineTo(f[0], f[1]),
          (f[v] = l[v]),
          t.lineTo(f[0], f[1]),
          t.lineTo(l[0], l[1]);
        for (var p = 1; p < i - 1; p++) {
          var d = n[p];
          t.moveTo(d[0], d[1]), (f[v] = d[v]), t.lineTo(f[0], f[1]);
        }
      }),
      e
    );
  })(wt),
  Wd = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t._mainGroup = new Z()), t;
    }
    return (
      (e.prototype.init = function (t, r) {
        (this._controller = new Ca(r.getZr())),
          (this._controllerHost = { target: this.group }),
          this.group.add(this._mainGroup);
      }),
      (e.prototype.render = function (t, r, n) {
        var i = t.getData(),
          o = t.layoutInfo,
          s = this._mainGroup,
          l = t.get("layout");
        l === "radial"
          ? ((s.x = o.x + o.width / 2), (s.y = o.y + o.height / 2))
          : ((s.x = o.x), (s.y = o.y)),
          this._updateViewCoordSys(t, n),
          this._updateController(t, r, n);
        var u = this._data;
        i
          .diff(u)
          .add(function (v) {
            ms(i, v) && Ss(i, v, null, s, t);
          })
          .update(function (v, c) {
            var h = u.getItemGraphicEl(c);
            if (!ms(i, v)) {
              h && bs(u, c, h, s, t);
              return;
            }
            Ss(i, v, h, s, t);
          })
          .remove(function (v) {
            var c = u.getItemGraphicEl(v);
            c && bs(u, v, c, s, t);
          })
          .execute(),
          (this._nodeScaleRatio = t.get("nodeScaleRatio")),
          this._updateNodeAndLinkScale(t),
          t.get("expandAndCollapse") === !0 &&
            i.eachItemGraphicEl(function (v, c) {
              v.off("click").on("click", function () {
                n.dispatchAction({
                  type: "treeExpandAndCollapse",
                  seriesId: t.id,
                  dataIndex: c
                });
              });
            }),
          (this._data = i);
      }),
      (e.prototype._updateViewCoordSys = function (t, r) {
        var n = t.getData(),
          i = [];
        n.each(function (c) {
          var h = n.getItemLayout(c);
          h && !isNaN(h.x) && !isNaN(h.y) && i.push([+h.x, +h.y]);
        });
        var o = [],
          s = [];
        Ma(i, o, s);
        var l = this._min,
          u = this._max;
        s[0] - o[0] === 0 &&
          ((o[0] = l ? l[0] : o[0] - 1), (s[0] = u ? u[0] : s[0] + 1)),
          s[1] - o[1] === 0 &&
            ((o[1] = l ? l[1] : o[1] - 1), (s[1] = u ? u[1] : s[1] + 1));
        var v = (t.coordinateSystem = new Vr());
        (v.zoomLimit = t.get("scaleLimit")),
          v.setBoundingRect(o[0], o[1], s[0] - o[0], s[1] - o[1]),
          v.setCenter(t.get("center"), r),
          v.setZoom(t.get("zoom")),
          this.group.attr({
            x: v.x,
            y: v.y,
            scaleX: v.scaleX,
            scaleY: v.scaleY
          }),
          (this._min = o),
          (this._max = s);
      }),
      (e.prototype._updateController = function (t, r, n) {
        var i = this,
          o = this._controller,
          s = this._controllerHost,
          l = this.group;
        o.setPointerChecker(function (u, v, c) {
          var h = l.getBoundingRect();
          return h.applyTransform(l.transform), h.contain(v, c) && !hi(u, n, t);
        }),
          o.enable(t.get("roam")),
          (s.zoomLimit = t.get("scaleLimit")),
          (s.zoom = t.coordinateSystem.getZoom()),
          o
            .off("pan")
            .off("zoom")
            .on("pan", function (u) {
              ki(s, u.dx, u.dy),
                n.dispatchAction({
                  seriesId: t.id,
                  type: "treeRoam",
                  dx: u.dx,
                  dy: u.dy
                });
            })
            .on("zoom", function (u) {
              Vi(s, u.scale, u.originX, u.originY),
                n.dispatchAction({
                  seriesId: t.id,
                  type: "treeRoam",
                  zoom: u.scale,
                  originX: u.originX,
                  originY: u.originY
                }),
                i._updateNodeAndLinkScale(t),
                n.updateLabelLayout();
            });
      }),
      (e.prototype._updateNodeAndLinkScale = function (t) {
        var r = t.getData(),
          n = this._getNodeGlobalScale(t);
        r.eachItemGraphicEl(function (i, o) {
          i.setSymbolScale(n);
        });
      }),
      (e.prototype._getNodeGlobalScale = function (t) {
        var r = t.coordinateSystem;
        if (r.type !== "view") return 1;
        var n = this._nodeScaleRatio,
          i = r.scaleX || 1,
          o = r.getZoom(),
          s = (o - 1) * n + 1;
        return s / i;
      }),
      (e.prototype.dispose = function () {
        this._controller && this._controller.dispose(),
          (this._controllerHost = null);
      }),
      (e.prototype.remove = function () {
        this._mainGroup.removeAll(), (this._data = null);
      }),
      (e.type = "tree"),
      e
    );
  })(pt);
function ms(a, e) {
  var t = a.getItemLayout(e);
  return t && !isNaN(t.x) && !isNaN(t.y);
}
function Ss(a, e, t, r, n) {
  var i = !t,
    o = a.tree.getNodeByDataIndex(e),
    s = o.getModel(),
    l = o.getVisual("style").fill,
    u = o.isExpand === !1 && o.children.length !== 0 ? l : "#fff",
    v = a.tree.root,
    c = o.parentNode === v ? o : o.parentNode || o,
    h = a.getItemGraphicEl(c.dataIndex),
    f = c.getLayout(),
    p = h
      ? {
          x: h.__oldX,
          y: h.__oldY,
          rawX: h.__radialOldRawX,
          rawY: h.__radialOldRawY
        }
      : f,
    d = o.getLayout();
  i
    ? ((t = new yu(a, e, null, { symbolInnerColor: u, useNameLabel: !0 })),
      (t.x = p.x),
      (t.y = p.y))
    : t.updateData(a, e, null, { symbolInnerColor: u, useNameLabel: !0 }),
    (t.__radialOldRawX = t.__radialRawX),
    (t.__radialOldRawY = t.__radialRawY),
    (t.__radialRawX = d.rawX),
    (t.__radialRawY = d.rawY),
    r.add(t),
    a.setItemGraphicEl(e, t),
    (t.__oldX = t.x),
    (t.__oldY = t.y),
    lt(t, { x: d.x, y: d.y }, n);
  var g = t.getSymbolPath();
  if (n.get("layout") === "radial") {
    var y = v.children[0],
      S = y.getLayout(),
      m = y.children.length,
      x = void 0,
      b = void 0;
    if (d.x === S.x && o.isExpand === !0 && y.children.length) {
      var _ = {
        x: (y.children[0].getLayout().x + y.children[m - 1].getLayout().x) / 2,
        y: (y.children[0].getLayout().y + y.children[m - 1].getLayout().y) / 2
      };
      (x = Math.atan2(_.y - S.y, _.x - S.x)),
        x < 0 && (x = Math.PI * 2 + x),
        (b = _.x < S.x),
        b && (x = x - Math.PI);
    } else
      (x = Math.atan2(d.y - S.y, d.x - S.x)),
        x < 0 && (x = Math.PI * 2 + x),
        o.children.length === 0 ||
        (o.children.length !== 0 && o.isExpand === !1)
          ? ((b = d.x < S.x), b && (x = x - Math.PI))
          : ((b = d.x > S.x), b || (x = x - Math.PI));
    var w = b ? "left" : "right",
      A = s.getModel("label"),
      I = A.get("rotate"),
      T = I * (Math.PI / 180),
      D = g.getTextContent();
    D &&
      (g.setTextConfig({
        position: A.get("position") || w,
        rotation: I == null ? -x : T,
        origin: "center"
      }),
      D.setStyle("verticalAlign", "middle"));
  }
  var M = s.get(["emphasis", "focus"]),
    C =
      M === "relative"
        ? ra(o.getAncestorsIndices(), o.getDescendantIndices())
        : M === "ancestor"
        ? o.getAncestorsIndices()
        : M === "descendant"
        ? o.getDescendantIndices()
        : null;
  C && (nt(t).focus = C),
    Hd(n, o, v, t, p, f, d, r),
    t.__edge &&
      (t.onHoverStateChange = function (P) {
        if (P !== "blur") {
          var E = o.parentNode && a.getItemGraphicEl(o.parentNode.dataIndex);
          (E && E.hoverState === nh) || du(t.__edge, P);
        }
      });
}
function Hd(a, e, t, r, n, i, o, s) {
  var l = e.getModel(),
    u = a.get("edgeShape"),
    v = a.get("layout"),
    c = a.getOrient(),
    h = a.get(["lineStyle", "curveness"]),
    f = a.get("edgeForkPosition"),
    p = l.getModel("lineStyle").getLineStyle(),
    d = r.__edge;
  if (u === "curve")
    e.parentNode &&
      e.parentNode !== t &&
      (d || (d = r.__edge = new mu({ shape: Kn(v, c, h, n, n) })),
      lt(d, { shape: Kn(v, c, h, i, o) }, a));
  else if (
    u === "polyline" &&
    v === "orthogonal" &&
    e !== t &&
    e.children &&
    e.children.length !== 0 &&
    e.isExpand === !0
  ) {
    for (var g = e.children, y = [], S = 0; S < g.length; S++) {
      var m = g[S].getLayout();
      y.push([m.x, m.y]);
    }
    d ||
      (d = r.__edge =
        new Fd({
          shape: {
            parentPoint: [o.x, o.y],
            childPoints: [[o.x, o.y]],
            orient: c,
            forkPosition: f
          }
        })),
      lt(d, { shape: { parentPoint: [o.x, o.y], childPoints: y } }, a);
  }
  d &&
    !(u === "polyline" && !e.isExpand) &&
    (d.useStyle(Q({ strokeNoScale: !0, fill: null }, p)),
    zt(d, l, "lineStyle"),
    Ke(d),
    s.add(d));
}
function xs(a, e, t, r, n) {
  var i = e.tree.root,
    o = nv(i, a),
    s = o.source,
    l = o.sourceLayout,
    u = e.getItemGraphicEl(a.dataIndex);
  if (u) {
    var v = e.getItemGraphicEl(s.dataIndex),
      c = v.__edge,
      h =
        u.__edge || (s.isExpand === !1 || s.children.length === 1 ? c : void 0),
      f = r.get("edgeShape"),
      p = r.get("layout"),
      d = r.get("orient"),
      g = r.get(["lineStyle", "curveness"]);
    h &&
      (f === "curve"
        ? aa(h, { shape: Kn(p, d, g, l, l), style: { opacity: 0 } }, r, {
            cb: function () {
              t.remove(h);
            },
            removeOpt: n
          })
        : f === "polyline" &&
          r.get("layout") === "orthogonal" &&
          aa(
            h,
            {
              shape: { parentPoint: [l.x, l.y], childPoints: [[l.x, l.y]] },
              style: { opacity: 0 }
            },
            r,
            {
              cb: function () {
                t.remove(h);
              },
              removeOpt: n
            }
          ));
  }
}
function nv(a, e) {
  for (
    var t = e.parentNode === a ? e : e.parentNode || e, r;
    (r = t.getLayout()), r == null;

  )
    t = t.parentNode === a ? t : t.parentNode || t;
  return { source: t, sourceLayout: r };
}
function bs(a, e, t, r, n) {
  var i = a.tree.getNodeByDataIndex(e),
    o = a.tree.root,
    s = nv(o, i).sourceLayout,
    l = {
      duration: n.get("animationDurationUpdate"),
      easing: n.get("animationEasingUpdate")
    };
  aa(t, { x: s.x + 1, y: s.y + 1 }, n, {
    cb: function () {
      r.remove(t), a.setItemGraphicEl(e, null);
    },
    removeOpt: l
  }),
    t.fadeOut(null, a.hostModel, { fadeLabel: !0, animation: l }),
    i.children.forEach(function (u) {
      xs(u, a, r, n, l);
    }),
    xs(i, a, r, n, l);
}
function Kn(a, e, t, r, n) {
  var i, o, s, l, u, v, c, h;
  if (a === "radial") {
    (u = r.rawX), (c = r.rawY), (v = n.rawX), (h = n.rawY);
    var f = hr(u, c),
      p = hr(u, c + (h - c) * t),
      d = hr(v, h + (c - h) * t),
      g = hr(v, h);
    return {
      x1: f.x || 0,
      y1: f.y || 0,
      x2: g.x || 0,
      y2: g.y || 0,
      cpx1: p.x || 0,
      cpy1: p.y || 0,
      cpx2: d.x || 0,
      cpy2: d.y || 0
    };
  } else
    (u = r.x),
      (c = r.y),
      (v = n.x),
      (h = n.y),
      (e === "LR" || e === "RL") &&
        ((i = u + (v - u) * t), (o = c), (s = v + (u - v) * t), (l = h)),
      (e === "TB" || e === "BT") &&
        ((i = u), (o = c + (h - c) * t), (s = v), (l = h + (c - h) * t));
  return { x1: u, y1: c, x2: v, y2: h, cpx1: i, cpy1: o, cpx2: s, cpy2: l };
}
var Wt = _t();
function iv(a) {
  var e = a.mainData,
    t = a.datas;
  t || ((t = { main: e }), (a.datasAttr = { main: "data" })),
    (a.datas = a.mainData = null),
    ov(e, t, a),
    L(t, function (r) {
      L(e.TRANSFERABLE_METHODS, function (n) {
        r.wrapMethod(n, ot(Ud, a));
      });
    }),
    e.wrapMethod("cloneShallow", ot($d, a)),
    L(e.CHANGABLE_METHODS, function (r) {
      e.wrapMethod(r, ot(Yd, a));
    }),
    ea(t[e.dataType] === e);
}
function Ud(a, e) {
  if (qd(this)) {
    var t = W({}, Wt(this).datas);
    (t[this.dataType] = e), ov(e, t, a);
  } else Oi(e, this.dataType, Wt(this).mainData, a);
  return e;
}
function Yd(a, e) {
  return a.struct && a.struct.update(), e;
}
function $d(a, e) {
  return (
    L(Wt(e).datas, function (t, r) {
      t !== e && Oi(t.cloneShallow(), r, e, a);
    }),
    e
  );
}
function Xd(a) {
  var e = Wt(this).mainData;
  return a == null || e == null ? e : Wt(e).datas[a];
}
function Zd() {
  var a = Wt(this).mainData;
  return a == null
    ? [{ data: a }]
    : O(Lt(Wt(a).datas), function (e) {
        return { type: e, data: Wt(a).datas[e] };
      });
}
function qd(a) {
  return Wt(a).mainData === a;
}
function ov(a, e, t) {
  (Wt(a).datas = {}),
    L(e, function (r, n) {
      Oi(r, n, a, t);
    });
}
function Oi(a, e, t, r) {
  (Wt(t).datas[e] = a),
    (Wt(a).mainData = t),
    (a.dataType = e),
    r.struct && ((a[r.structAttr] = r.struct), (r.struct[r.datasAttr[e]] = a)),
    (a.getLinkedData = Xd),
    (a.getLinkedDataAll = Zd);
}
var Kd = (function () {
    function a(e, t) {
      (this.depth = 0),
        (this.height = 0),
        (this.dataIndex = -1),
        (this.children = []),
        (this.viewChildren = []),
        (this.isExpand = !1),
        (this.name = e || ""),
        (this.hostTree = t);
    }
    return (
      (a.prototype.isRemoved = function () {
        return this.dataIndex < 0;
      }),
      (a.prototype.eachNode = function (e, t, r) {
        it(e) && ((r = t), (t = e), (e = null)),
          (e = e || {}),
          et(e) && (e = { order: e });
        var n = e.order || "preorder",
          i = this[e.attr || "children"],
          o;
        n === "preorder" && (o = t.call(r, this));
        for (var s = 0; !o && s < i.length; s++) i[s].eachNode(e, t, r);
        n === "postorder" && t.call(r, this);
      }),
      (a.prototype.updateDepthAndHeight = function (e) {
        var t = 0;
        this.depth = e;
        for (var r = 0; r < this.children.length; r++) {
          var n = this.children[r];
          n.updateDepthAndHeight(e + 1), n.height > t && (t = n.height);
        }
        this.height = t + 1;
      }),
      (a.prototype.getNodeById = function (e) {
        if (this.getId() === e) return this;
        for (var t = 0, r = this.children, n = r.length; t < n; t++) {
          var i = r[t].getNodeById(e);
          if (i) return i;
        }
      }),
      (a.prototype.contains = function (e) {
        if (e === this) return !0;
        for (var t = 0, r = this.children, n = r.length; t < n; t++) {
          var i = r[t].contains(e);
          if (i) return i;
        }
      }),
      (a.prototype.getAncestors = function (e) {
        for (var t = [], r = e ? this : this.parentNode; r; )
          t.push(r), (r = r.parentNode);
        return t.reverse(), t;
      }),
      (a.prototype.getAncestorsIndices = function () {
        for (var e = [], t = this; t; ) e.push(t.dataIndex), (t = t.parentNode);
        return e.reverse(), e;
      }),
      (a.prototype.getDescendantIndices = function () {
        var e = [];
        return (
          this.eachNode(function (t) {
            e.push(t.dataIndex);
          }),
          e
        );
      }),
      (a.prototype.getValue = function (e) {
        var t = this.hostTree.data;
        return t
          .getStore()
          .get(t.getDimensionIndex(e || "value"), this.dataIndex);
      }),
      (a.prototype.setLayout = function (e, t) {
        this.dataIndex >= 0 &&
          this.hostTree.data.setItemLayout(this.dataIndex, e, t);
      }),
      (a.prototype.getLayout = function () {
        return this.hostTree.data.getItemLayout(this.dataIndex);
      }),
      (a.prototype.getModel = function (e) {
        if (!(this.dataIndex < 0)) {
          var t = this.hostTree,
            r = t.data.getItemModel(this.dataIndex);
          return r.getModel(e);
        }
      }),
      (a.prototype.getLevelModel = function () {
        return (this.hostTree.levelModels || [])[this.depth];
      }),
      (a.prototype.setVisual = function (e, t) {
        this.dataIndex >= 0 &&
          this.hostTree.data.setItemVisual(this.dataIndex, e, t);
      }),
      (a.prototype.getVisual = function (e) {
        return this.hostTree.data.getItemVisual(this.dataIndex, e);
      }),
      (a.prototype.getRawIndex = function () {
        return this.hostTree.data.getRawIndex(this.dataIndex);
      }),
      (a.prototype.getId = function () {
        return this.hostTree.data.getId(this.dataIndex);
      }),
      (a.prototype.getChildIndex = function () {
        if (this.parentNode) {
          for (var e = this.parentNode.children, t = 0; t < e.length; ++t)
            if (e[t] === this) return t;
          return -1;
        }
        return -1;
      }),
      (a.prototype.isAncestorOf = function (e) {
        for (var t = e.parentNode; t; ) {
          if (t === this) return !0;
          t = t.parentNode;
        }
        return !1;
      }),
      (a.prototype.isDescendantOf = function (e) {
        return e !== this && e.isAncestorOf(this);
      }),
      a
    );
  })(),
  Bi = (function () {
    function a(e) {
      (this.type = "tree"), (this._nodes = []), (this.hostModel = e);
    }
    return (
      (a.prototype.eachNode = function (e, t, r) {
        this.root.eachNode(e, t, r);
      }),
      (a.prototype.getNodeByDataIndex = function (e) {
        var t = this.data.getRawIndex(e);
        return this._nodes[t];
      }),
      (a.prototype.getNodeById = function (e) {
        return this.root.getNodeById(e);
      }),
      (a.prototype.update = function () {
        for (
          var e = this.data, t = this._nodes, r = 0, n = t.length;
          r < n;
          r++
        )
          t[r].dataIndex = -1;
        for (var r = 0, n = e.count(); r < n; r++)
          t[e.getRawIndex(r)].dataIndex = r;
      }),
      (a.prototype.clearLayouts = function () {
        this.data.clearItemLayouts();
      }),
      (a.createTree = function (e, t, r) {
        var n = new a(t),
          i = [],
          o = 1;
        s(e);
        function s(v, c) {
          var h = v.value;
          (o = Math.max(o, H(h) ? h.length : 1)), i.push(v);
          var f = new Kd(Mr(v.name, ""), n);
          c ? jd(f, c) : (n.root = f), n._nodes.push(f);
          var p = v.children;
          if (p) for (var d = 0; d < p.length; d++) s(p[d], f);
        }
        n.root.updateDepthAndHeight(0);
        var l = di(i, {
            coordDimensions: ["value"],
            dimensionsCount: o
          }).dimensions,
          u = new Vt(l, t);
        return (
          u.initData(i),
          r && r(u),
          iv({ mainData: u, struct: n, structAttr: "tree" }),
          n.update(),
          n
        );
      }),
      a
    );
  })();
function jd(a, e) {
  var t = e.children;
  a.parentNode !== e && (t.push(a), (a.parentNode = e));
}
function _r(a, e, t) {
  if (a && Ht(e, a.type) >= 0) {
    var r = t.getData().tree.root,
      n = a.targetNode;
    if ((et(n) && (n = r.getNodeById(n)), n && r.contains(n)))
      return { node: n };
    var i = a.targetNodeId;
    if (i != null && (n = r.getNodeById(i))) return { node: n };
  }
}
function sv(a) {
  for (var e = []; a; ) (a = a.parentNode), a && e.push(a);
  return e.reverse();
}
function Fi(a, e) {
  var t = sv(a);
  return Ht(t, e) >= 0;
}
function Ga(a, e) {
  for (var t = []; a; ) {
    var r = a.dataIndex;
    t.push({ name: a.name, dataIndex: r, value: e.getRawValue(r) }),
      (a = a.parentNode);
  }
  return t.reverse(), t;
}
var Jd = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.hasSymbolVisual = !0), (t.ignoreStyleOnData = !0), t;
  }
  return (
    (e.prototype.getInitialData = function (t) {
      var r = { name: t.name, children: t.data },
        n = t.leaves || {},
        i = new $t(n, this, this.ecModel),
        o = Bi.createTree(r, this, s);
      function s(c) {
        c.wrapMethod("getItemModel", function (h, f) {
          var p = o.getNodeByDataIndex(f);
          return (
            (p && p.children.length && p.isExpand) || (h.parentModel = i), h
          );
        });
      }
      var l = 0;
      o.eachNode("preorder", function (c) {
        c.depth > l && (l = c.depth);
      });
      var u = t.expandAndCollapse,
        v = u && t.initialTreeDepth >= 0 ? t.initialTreeDepth : l;
      return (
        o.root.eachNode("preorder", function (c) {
          var h = c.hostTree.data.getRawDataItem(c.dataIndex);
          c.isExpand = h && h.collapsed != null ? !h.collapsed : c.depth <= v;
        }),
        o.data
      );
    }),
    (e.prototype.getOrient = function () {
      var t = this.get("orient");
      return (
        t === "horizontal" ? (t = "LR") : t === "vertical" && (t = "TB"), t
      );
    }),
    (e.prototype.setZoom = function (t) {
      this.option.zoom = t;
    }),
    (e.prototype.setCenter = function (t) {
      this.option.center = t;
    }),
    (e.prototype.formatTooltip = function (t, r, n) {
      for (
        var i = this.getData().tree,
          o = i.root.children[0],
          s = i.getNodeByDataIndex(t),
          l = s.getValue(),
          u = s.name;
        s && s !== o;

      )
        (u = s.parentNode.name + "." + u), (s = s.parentNode);
      return Mt("nameValue", {
        name: u,
        value: l,
        noValue: isNaN(l) || l == null
      });
    }),
    (e.prototype.getDataParams = function (t) {
      var r = a.prototype.getDataParams.apply(this, arguments),
        n = this.getData().tree.getNodeByDataIndex(t);
      return (r.treeAncestors = Ga(n, this)), (r.collapsed = !n.isExpand), r;
    }),
    (e.type = "series.tree"),
    (e.layoutMode = "box"),
    (e.defaultOption = {
      z: 2,
      coordinateSystem: "view",
      left: "12%",
      top: "12%",
      right: "12%",
      bottom: "12%",
      layout: "orthogonal",
      edgeShape: "curve",
      edgeForkPosition: "50%",
      roam: !1,
      nodeScaleRatio: 0.4,
      center: null,
      zoom: 1,
      orient: "LR",
      symbol: "emptyCircle",
      symbolSize: 7,
      expandAndCollapse: !0,
      initialTreeDepth: 2,
      lineStyle: { color: "#ccc", width: 1.5, curveness: 0.5 },
      itemStyle: { color: "lightsteelblue", borderWidth: 1.5 },
      label: { show: !0 },
      animationEasing: "linear",
      animationDuration: 700,
      animationDurationUpdate: 500
    }),
    e
  );
})(gt);
function Qd(a, e, t) {
  for (var r = [a], n = [], i; (i = r.pop()); )
    if ((n.push(i), i.isExpand)) {
      var o = i.children;
      if (o.length) for (var s = 0; s < o.length; s++) r.push(o[s]);
    }
  for (; (i = n.pop()); ) e(i, t);
}
function or(a, e) {
  for (var t = [a], r; (r = t.pop()); )
    if ((e(r), r.isExpand)) {
      var n = r.children;
      if (n.length) for (var i = n.length - 1; i >= 0; i--) t.push(n[i]);
    }
}
function tg(a, e) {
  a.eachSeriesByType("tree", function (t) {
    eg(t, e);
  });
}
function eg(a, e) {
  var t = Nd(a, e);
  a.layoutInfo = t;
  var r = a.get("layout"),
    n = 0,
    i = 0,
    o = null;
  r === "radial"
    ? ((n = 2 * Math.PI),
      (i = Math.min(t.height, t.width) / 2),
      (o = ys(function (m, x) {
        return (m.parentNode === x.parentNode ? 1 : 2) / m.depth;
      })))
    : ((n = t.width), (i = t.height), (o = ys()));
  var s = a.getData().tree.root,
    l = s.children[0];
  if (l) {
    Md(s), Qd(l, Ed, o), (s.hierNode.modifier = -l.hierNode.prelim), or(l, Rd);
    var u = l,
      v = l,
      c = l;
    or(l, function (m) {
      var x = m.getLayout().x;
      x < u.getLayout().x && (u = m),
        x > v.getLayout().x && (v = m),
        m.depth > c.depth && (c = m);
    });
    var h = u === v ? 1 : o(u, v) / 2,
      f = h - u.getLayout().x,
      p = 0,
      d = 0,
      g = 0,
      y = 0;
    if (r === "radial")
      (p = n / (v.getLayout().x + h + f)),
        (d = i / (c.depth - 1 || 1)),
        or(l, function (m) {
          (g = (m.getLayout().x + f) * p), (y = (m.depth - 1) * d);
          var x = hr(g, y);
          m.setLayout({ x: x.x, y: x.y, rawX: g, rawY: y }, !0);
        });
    else {
      var S = a.getOrient();
      S === "RL" || S === "LR"
        ? ((d = i / (v.getLayout().x + h + f)),
          (p = n / (c.depth - 1 || 1)),
          or(l, function (m) {
            (y = (m.getLayout().x + f) * d),
              (g = S === "LR" ? (m.depth - 1) * p : n - (m.depth - 1) * p),
              m.setLayout({ x: g, y }, !0);
          }))
        : (S === "TB" || S === "BT") &&
          ((p = n / (v.getLayout().x + h + f)),
          (d = i / (c.depth - 1 || 1)),
          or(l, function (m) {
            (g = (m.getLayout().x + f) * p),
              (y = S === "TB" ? (m.depth - 1) * d : i - (m.depth - 1) * d),
              m.setLayout({ x: g, y }, !0);
          }));
    }
  }
}
function rg(a) {
  a.eachSeriesByType("tree", function (e) {
    var t = e.getData(),
      r = t.tree;
    r.eachNode(function (n) {
      var i = n.getModel(),
        o = i.getModel("itemStyle").getItemStyle(),
        s = t.ensureUniqueItemVisual(n.dataIndex, "style");
      W(s, o);
    });
  });
}
function ag(a) {
  a.registerAction(
    {
      type: "treeExpandAndCollapse",
      event: "treeExpandAndCollapse",
      update: "update"
    },
    function (e, t) {
      t.eachComponent(
        { mainType: "series", subType: "tree", query: e },
        function (r) {
          var n = e.dataIndex,
            i = r.getData().tree,
            o = i.getNodeByDataIndex(n);
          o.isExpand = !o.isExpand;
        }
      );
    }
  ),
    a.registerAction(
      { type: "treeRoam", event: "treeRoam", update: "none" },
      function (e, t, r) {
        t.eachComponent(
          { mainType: "series", subType: "tree", query: e },
          function (n) {
            var i = n.coordinateSystem,
              o = zi(i, e, void 0, r);
            n.setCenter && n.setCenter(o.center),
              n.setZoom && n.setZoom(o.zoom);
          }
        );
      }
    );
}
function ng(a) {
  a.registerChartView(Wd),
    a.registerSeriesModel(Jd),
    a.registerLayout(tg),
    a.registerVisual(rg),
    ag(a);
}
var _s = ["treemapZoomToNode", "treemapRender", "treemapMove"];
function ig(a) {
  for (var e = 0; e < _s.length; e++)
    a.registerAction({ type: _s[e], update: "updateView" }, qe);
  a.registerAction(
    { type: "treemapRootToNode", update: "updateView" },
    function (t, r) {
      r.eachComponent({ mainType: "series", subType: "treemap", query: t }, n);
      function n(i, o) {
        var s = ["treemapZoomToNode", "treemapRootToNode"],
          l = _r(t, s, i);
        if (l) {
          var u = i.getViewRoot();
          u && (t.direction = Fi(u, l.node) ? "rollUp" : "drillDown"),
            i.resetViewRoot(l.node);
        }
      }
    }
  );
}
function lv(a) {
  var e = a.getData(),
    t = e.tree,
    r = {};
  t.eachNode(function (n) {
    for (var i = n; i && i.depth > 1; ) i = i.parentNode;
    var o = zn(a.ecModel, i.name || i.dataIndex + "", r);
    n.setVisual("decal", o);
  });
}
var og = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), (t.preventUsingHoverLayer = !0), t;
  }
  return (
    (e.prototype.getInitialData = function (t, r) {
      var n = { name: t.name, children: t.data };
      uv(n);
      var i = t.levels || [],
        o = (this.designatedVisualItemStyle = {}),
        s = new $t({ itemStyle: o }, this, r);
      i = t.levels = sg(i, r);
      var l = O(
          i || [],
          function (c) {
            return new $t(c, s, r);
          },
          this
        ),
        u = Bi.createTree(n, this, v);
      function v(c) {
        c.wrapMethod("getItemModel", function (h, f) {
          var p = u.getNodeByDataIndex(f),
            d = p ? l[p.depth] : null;
          return (h.parentModel = d || s), h;
        });
      }
      return u.data;
    }),
    (e.prototype.optionUpdated = function () {
      this.resetViewRoot();
    }),
    (e.prototype.formatTooltip = function (t, r, n) {
      var i = this.getData(),
        o = this.getRawValue(t),
        s = i.getName(t);
      return Mt("nameValue", { name: s, value: o });
    }),
    (e.prototype.getDataParams = function (t) {
      var r = a.prototype.getDataParams.apply(this, arguments),
        n = this.getData().tree.getNodeByDataIndex(t);
      return (
        (r.treeAncestors = Ga(n, this)), (r.treePathInfo = r.treeAncestors), r
      );
    }),
    (e.prototype.setLayoutInfo = function (t) {
      (this.layoutInfo = this.layoutInfo || {}), W(this.layoutInfo, t);
    }),
    (e.prototype.mapIdToIndex = function (t) {
      var r = this._idIndexMap;
      r || ((r = this._idIndexMap = tt()), (this._idIndexMapCount = 0));
      var n = r.get(t);
      return n == null && r.set(t, (n = this._idIndexMapCount++)), n;
    }),
    (e.prototype.getViewRoot = function () {
      return this._viewRoot;
    }),
    (e.prototype.resetViewRoot = function (t) {
      t ? (this._viewRoot = t) : (t = this._viewRoot);
      var r = this.getRawData().tree.root;
      (!t || (t !== r && !r.contains(t))) && (this._viewRoot = r);
    }),
    (e.prototype.enableAriaDecal = function () {
      lv(this);
    }),
    (e.type = "series.treemap"),
    (e.layoutMode = "box"),
    (e.defaultOption = {
      progressive: 0,
      left: "center",
      top: "middle",
      width: "80%",
      height: "80%",
      sort: !0,
      clipWindow: "origin",
      squareRatio: 0.5 * (1 + Math.sqrt(5)),
      leafDepth: null,
      drillDownIcon: "▶",
      zoomToNodeRatio: 0.32 * 0.32,
      scaleLimit: null,
      roam: !0,
      nodeClick: "zoomToNode",
      animation: !0,
      animationDurationUpdate: 900,
      animationEasing: "quinticInOut",
      breadcrumb: {
        show: !0,
        height: 22,
        left: "center",
        top: "bottom",
        emptyItemWidth: 25,
        itemStyle: { color: "rgba(0,0,0,0.7)", textStyle: { color: "#fff" } },
        emphasis: { itemStyle: { color: "rgba(0,0,0,0.9)" } }
      },
      label: {
        show: !0,
        distance: 0,
        padding: 5,
        position: "inside",
        color: "#fff",
        overflow: "truncate"
      },
      upperLabel: {
        show: !1,
        position: [0, "50%"],
        height: 20,
        overflow: "truncate",
        verticalAlign: "middle"
      },
      itemStyle: {
        color: null,
        colorAlpha: null,
        colorSaturation: null,
        borderWidth: 0,
        gapWidth: 0,
        borderColor: "#fff",
        borderColorSaturation: null
      },
      emphasis: {
        upperLabel: {
          show: !0,
          position: [0, "50%"],
          overflow: "truncate",
          verticalAlign: "middle"
        }
      },
      visualDimension: 0,
      visualMin: null,
      visualMax: null,
      color: [],
      colorAlpha: null,
      colorSaturation: null,
      colorMappingBy: "index",
      visibleMin: 10,
      childrenVisibleMin: null,
      levels: []
    }),
    e
  );
})(gt);
function uv(a) {
  var e = 0;
  L(a.children, function (r) {
    uv(r);
    var n = r.value;
    H(n) && (n = n[0]), (e += n);
  });
  var t = a.value;
  H(t) && (t = t[0]),
    (t == null || isNaN(t)) && (t = e),
    t < 0 && (t = 0),
    H(a.value) ? (a.value[0] = t) : (a.value = t);
}
function sg(a, e) {
  var t = Jt(e.get("color")),
    r = Jt(e.get(["aria", "decal", "decals"]));
  if (t) {
    a = a || [];
    var n, i;
    L(a, function (s) {
      var l = new $t(s),
        u = l.get("color"),
        v = l.get("decal");
      (l.get(["itemStyle", "color"]) || (u && u !== "none")) && (n = !0),
        (l.get(["itemStyle", "decal"]) || (v && v !== "none")) && (i = !0);
    });
    var o = a[0] || (a[0] = {});
    return n || (o.color = t.slice()), !i && r && (o.decal = r.slice()), a;
  }
}
var lg = 8,
  ws = 8,
  Ja = 5,
  ug = (function () {
    function a(e) {
      (this.group = new Z()), e.add(this.group);
    }
    return (
      (a.prototype.render = function (e, t, r, n) {
        var i = e.getModel("breadcrumb"),
          o = this.group;
        if ((o.removeAll(), !(!i.get("show") || !r))) {
          var s = i.getModel("itemStyle"),
            l = i.getModel("emphasis"),
            u = s.getModel("textStyle"),
            v = l.getModel(["itemStyle", "textStyle"]),
            c = {
              pos: {
                left: i.get("left"),
                right: i.get("right"),
                top: i.get("top"),
                bottom: i.get("bottom")
              },
              box: { width: t.getWidth(), height: t.getHeight() },
              emptyItemWidth: i.get("emptyItemWidth"),
              totalWidth: 0,
              renderList: []
            };
          this._prepare(r, c, u),
            this._renderContent(e, c, s, l, u, v, n),
            ih(o, c.pos, c.box);
        }
      }),
      (a.prototype._prepare = function (e, t, r) {
        for (var n = e; n; n = n.parentNode) {
          var i = Mr(n.getModel().get("name"), ""),
            o = r.getTextRect(i),
            s = Math.max(o.width + lg * 2, t.emptyItemWidth);
          (t.totalWidth += s + ws),
            t.renderList.push({ node: n, text: i, width: s });
        }
      }),
      (a.prototype._renderContent = function (e, t, r, n, i, o, s) {
        for (
          var l = 0,
            u = t.emptyItemWidth,
            v = e.get(["breadcrumb", "height"]),
            c = oh(t.pos, t.box),
            h = t.totalWidth,
            f = t.renderList,
            p = n.getModel("itemStyle").getItemStyle(),
            d = f.length - 1;
          d >= 0;
          d--
        ) {
          var g = f[d],
            y = g.node,
            S = g.width,
            m = g.text;
          h > c.width && ((h -= S - u), (S = u), (m = null));
          var x = new ve({
            shape: { points: vg(l, 0, S, v, d === f.length - 1, d === 0) },
            style: Q(r.getItemStyle(), { lineJoin: "bevel" }),
            textContent: new kt({ style: xt(i, { text: m }) }),
            textConfig: { position: "inside" },
            z2: Pr * 1e4,
            onclick: ot(s, y)
          });
          (x.disableLabelAnimation = !0),
            (x.getTextContent().ensureState("emphasis").style = xt(o, {
              text: m
            })),
            (x.ensureState("emphasis").style = p),
            ct(x, n.get("focus"), n.get("blurScope"), n.get("disabled")),
            this.group.add(x),
            cg(x, e, y),
            (l += S + ws);
        }
      }),
      (a.prototype.remove = function () {
        this.group.removeAll();
      }),
      a
    );
  })();
function vg(a, e, t, r, n, i) {
  var o = [
    [n ? a : a - Ja, e],
    [a + t, e],
    [a + t, e + r],
    [n ? a : a - Ja, e + r]
  ];
  return (
    !i && o.splice(2, 0, [a + t + Ja, e + r / 2]),
    !n && o.push([a, e + r / 2]),
    o
  );
}
function cg(a, e, t) {
  nt(a).eventData = {
    componentType: "series",
    componentSubType: "treemap",
    componentIndex: e.componentIndex,
    seriesIndex: e.seriesIndex,
    seriesName: e.name,
    seriesType: "treemap",
    selfType: "breadcrumb",
    nodeData: { dataIndex: t && t.dataIndex, name: t && t.name },
    treePathInfo: t && Ga(t, e)
  };
}
var hg = (function () {
  function a() {
    (this._storage = []), (this._elExistsMap = {});
  }
  return (
    (a.prototype.add = function (e, t, r, n, i) {
      return this._elExistsMap[e.id]
        ? !1
        : ((this._elExistsMap[e.id] = !0),
          this._storage.push({
            el: e,
            target: t,
            duration: r,
            delay: n,
            easing: i
          }),
          !0);
    }),
    (a.prototype.finished = function (e) {
      return (this._finishedCallback = e), this;
    }),
    (a.prototype.start = function () {
      for (
        var e = this,
          t = this._storage.length,
          r = function () {
            t--,
              t <= 0 &&
                ((e._storage.length = 0),
                (e._elExistsMap = {}),
                e._finishedCallback && e._finishedCallback());
          },
          n = 0,
          i = this._storage.length;
        n < i;
        n++
      ) {
        var o = this._storage[n];
        o.el.animateTo(o.target, {
          duration: o.duration,
          delay: o.delay,
          easing: o.easing,
          setToFinal: !0,
          done: r,
          aborted: r
        });
      }
      return this;
    }),
    a
  );
})();
function fg() {
  return new hg();
}
var jn = Z,
  As = Et,
  Ts = 3,
  Is = "label",
  Ds = "upperLabel",
  pg = Pr * 10,
  dg = Pr * 2,
  gg = Pr * 3,
  Ce = xu([
    ["fill", "color"],
    ["stroke", "strokeColor"],
    ["lineWidth", "strokeWidth"],
    ["shadowBlur"],
    ["shadowOffsetX"],
    ["shadowOffsetY"],
    ["shadowColor"]
  ]),
  Ls = function (a) {
    var e = Ce(a);
    return (e.stroke = e.fill = e.lineWidth = null), e;
  },
  ca = _t(),
  yg = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t._state = "ready"), (t._storage = sr()), t;
    }
    return (
      (e.prototype.render = function (t, r, n, i) {
        var o = r.findComponents({
          mainType: "series",
          subType: "treemap",
          query: i
        });
        if (!(Ht(o, t) < 0)) {
          (this.seriesModel = t), (this.api = n), (this.ecModel = r);
          var s = ["treemapZoomToNode", "treemapRootToNode"],
            l = _r(i, s, t),
            u = i && i.type,
            v = t.layoutInfo,
            c = !this._oldTree,
            h = this._storage,
            f =
              u === "treemapRootToNode" && l && h
                ? {
                    rootNodeGroup: h.nodeGroup[l.node.getRawIndex()],
                    direction: i.direction
                  }
                : null,
            p = this._giveContainerGroup(v),
            d = t.get("animation"),
            g = this._doRender(p, t, f);
          d &&
          !c &&
          (!u || u === "treemapZoomToNode" || u === "treemapRootToNode")
            ? this._doAnimation(p, g, t, f)
            : g.renderFinally(),
            this._resetController(n),
            this._renderBreadcrumb(t, n, l);
        }
      }),
      (e.prototype._giveContainerGroup = function (t) {
        var r = this._containerGroup;
        return (
          r ||
            ((r = this._containerGroup = new jn()),
            this._initEvents(r),
            this.group.add(r)),
          (r.x = t.x),
          (r.y = t.y),
          r
        );
      }),
      (e.prototype._doRender = function (t, r, n) {
        var i = r.getData().tree,
          o = this._oldTree,
          s = sr(),
          l = sr(),
          u = this._storage,
          v = [];
        function c(S, m, x, b) {
          return mg(r, l, u, n, s, v, S, m, x, b);
        }
        d(
          i.root ? [i.root] : [],
          o && o.root ? [o.root] : [],
          t,
          i === o || !o,
          0
        );
        var h = g(u);
        if (((this._oldTree = i), (this._storage = l), this._controllerHost)) {
          var f = this.seriesModel.layoutInfo,
            p = i.root.getLayout();
          p.width === f.width &&
            p.height === f.height &&
            (this._controllerHost.zoom = 1);
        }
        return { lastsForAnimation: s, willDeleteEls: h, renderFinally: y };
        function d(S, m, x, b, _) {
          b
            ? ((m = S),
              L(S, function (I, T) {
                !I.isRemoved() && A(T, T);
              }))
            : new je(m, S, w, w).add(A).update(A).remove(ot(A, null)).execute();
          function w(I) {
            return I.getId();
          }
          function A(I, T) {
            var D = I != null ? S[I] : null,
              M = T != null ? m[T] : null,
              C = c(D, M, x, _);
            C &&
              d(
                (D && D.viewChildren) || [],
                (M && M.viewChildren) || [],
                C,
                b,
                _ + 1
              );
          }
        }
        function g(S) {
          var m = sr();
          return (
            S &&
              L(S, function (x, b) {
                var _ = m[b];
                L(x, function (w) {
                  w && (_.push(w), (ca(w).willDelete = !0));
                });
              }),
            m
          );
        }
        function y() {
          L(h, function (S) {
            L(S, function (m) {
              m.parent && m.parent.remove(m);
            });
          }),
            L(v, function (S) {
              (S.invisible = !0), S.dirty();
            });
        }
      }),
      (e.prototype._doAnimation = function (t, r, n, i) {
        var o = n.get("animationDurationUpdate"),
          s = n.get("animationEasing"),
          l = (it(o) ? 0 : o) || 0,
          u = (it(s) ? null : s) || "cubicOut",
          v = fg();
        L(r.willDeleteEls, function (c, h) {
          L(c, function (f, p) {
            if (!f.invisible) {
              var d = f.parent,
                g,
                y = ca(d);
              if (i && i.direction === "drillDown")
                g =
                  d === i.rootNodeGroup
                    ? {
                        shape: {
                          x: 0,
                          y: 0,
                          width: y.nodeWidth,
                          height: y.nodeHeight
                        },
                        style: { opacity: 0 }
                      }
                    : { style: { opacity: 0 } };
              else {
                var S = 0,
                  m = 0;
                y.willDelete || ((S = y.nodeWidth / 2), (m = y.nodeHeight / 2)),
                  (g =
                    h === "nodeGroup"
                      ? { x: S, y: m, style: { opacity: 0 } }
                      : {
                          shape: { x: S, y: m, width: 0, height: 0 },
                          style: { opacity: 0 }
                        });
              }
              g && v.add(f, g, l, 0, u);
            }
          });
        }),
          L(
            this._storage,
            function (c, h) {
              L(c, function (f, p) {
                var d = r.lastsForAnimation[h][p],
                  g = {};
                d &&
                  (f instanceof Z
                    ? d.oldX != null &&
                      ((g.x = f.x), (g.y = f.y), (f.x = d.oldX), (f.y = d.oldY))
                    : (d.oldShape &&
                        ((g.shape = W({}, f.shape)), f.setShape(d.oldShape)),
                      d.fadein
                        ? (f.setStyle("opacity", 0), (g.style = { opacity: 1 }))
                        : f.style.opacity !== 1 && (g.style = { opacity: 1 })),
                  v.add(f, g, l, 0, u));
              });
            },
            this
          ),
          (this._state = "animating"),
          v
            .finished(
              at(function () {
                (this._state = "ready"), r.renderFinally();
              }, this)
            )
            .start();
      }),
      (e.prototype._resetController = function (t) {
        var r = this._controller,
          n = this._controllerHost;
        n ||
          ((this._controllerHost = { target: this.group }),
          (n = this._controllerHost)),
          r ||
            ((r = this._controller = new Ca(t.getZr())),
            r.enable(this.seriesModel.get("roam")),
            (n.zoomLimit = this.seriesModel.get("scaleLimit")),
            (n.zoom = this.seriesModel.get("zoom")),
            r.on("pan", at(this._onPan, this)),
            r.on("zoom", at(this._onZoom, this)));
        var i = new ft(0, 0, t.getWidth(), t.getHeight());
        r.setPointerChecker(function (o, s, l) {
          return i.contain(s, l);
        });
      }),
      (e.prototype._clearController = function () {
        var t = this._controller;
        (this._controllerHost = null), t && (t.dispose(), (t = null));
      }),
      (e.prototype._onPan = function (t) {
        if (
          this._state !== "animating" &&
          (Math.abs(t.dx) > Ts || Math.abs(t.dy) > Ts)
        ) {
          var r = this.seriesModel.getData().tree.root;
          if (!r) return;
          var n = r.getLayout();
          if (!n) return;
          this.api.dispatchAction({
            type: "treemapMove",
            from: this.uid,
            seriesId: this.seriesModel.id,
            rootRect: {
              x: n.x + t.dx,
              y: n.y + t.dy,
              width: n.width,
              height: n.height
            }
          });
        }
      }),
      (e.prototype._onZoom = function (t) {
        var r = t.originX,
          n = t.originY,
          i = t.scale;
        if (this._state !== "animating") {
          var o = this.seriesModel.getData().tree.root;
          if (!o) return;
          var s = o.getLayout();
          if (!s) return;
          var l = new ft(s.x, s.y, s.width, s.height),
            u = null,
            v = this._controllerHost;
          u = v.zoomLimit;
          var c = (v.zoom = v.zoom || 1);
          if (((c *= i), u)) {
            var h = u.min || 0,
              f = u.max || 1 / 0;
            c = Math.max(Math.min(f, c), h);
          }
          var p = c / v.zoom;
          v.zoom = c;
          var d = this.seriesModel.layoutInfo;
          (r -= d.x), (n -= d.y);
          var g = tr();
          ke(g, g, [-r, -n]),
            pu(g, g, [p, p]),
            ke(g, g, [r, n]),
            l.applyTransform(g),
            this.api.dispatchAction({
              type: "treemapRender",
              from: this.uid,
              seriesId: this.seriesModel.id,
              rootRect: { x: l.x, y: l.y, width: l.width, height: l.height }
            });
        }
      }),
      (e.prototype._initEvents = function (t) {
        var r = this;
        t.on(
          "click",
          function (n) {
            if (r._state === "ready") {
              var i = r.seriesModel.get("nodeClick", !0);
              if (i) {
                var o = r.findTarget(n.offsetX, n.offsetY);
                if (o) {
                  var s = o.node;
                  if (s.getLayout().isLeafRoot) r._rootToNode(o);
                  else if (i === "zoomToNode") r._zoomToNode(o);
                  else if (i === "link") {
                    var l = s.hostTree.data.getItemModel(s.dataIndex),
                      u = l.get("link", !0),
                      v = l.get("target", !0) || "blank";
                    u && Su(u, v);
                  }
                }
              }
            }
          },
          this
        );
      }),
      (e.prototype._renderBreadcrumb = function (t, r, n) {
        var i = this;
        n ||
          ((n =
            t.get("leafDepth", !0) != null
              ? { node: t.getViewRoot() }
              : this.findTarget(r.getWidth() / 2, r.getHeight() / 2)),
          n || (n = { node: t.getData().tree.root })),
          (this._breadcrumb || (this._breadcrumb = new ug(this.group))).render(
            t,
            r,
            n.node,
            function (o) {
              i._state !== "animating" &&
                (Fi(t.getViewRoot(), o)
                  ? i._rootToNode({ node: o })
                  : i._zoomToNode({ node: o }));
            }
          );
      }),
      (e.prototype.remove = function () {
        this._clearController(),
          this._containerGroup && this._containerGroup.removeAll(),
          (this._storage = sr()),
          (this._state = "ready"),
          this._breadcrumb && this._breadcrumb.remove();
      }),
      (e.prototype.dispose = function () {
        this._clearController();
      }),
      (e.prototype._zoomToNode = function (t) {
        this.api.dispatchAction({
          type: "treemapZoomToNode",
          from: this.uid,
          seriesId: this.seriesModel.id,
          targetNode: t.node
        });
      }),
      (e.prototype._rootToNode = function (t) {
        this.api.dispatchAction({
          type: "treemapRootToNode",
          from: this.uid,
          seriesId: this.seriesModel.id,
          targetNode: t.node
        });
      }),
      (e.prototype.findTarget = function (t, r) {
        var n,
          i = this.seriesModel.getViewRoot();
        return (
          i.eachNode(
            { attr: "viewChildren", order: "preorder" },
            function (o) {
              var s = this._storage.background[o.getRawIndex()];
              if (s) {
                var l = s.transformCoordToLocal(t, r),
                  u = s.shape;
                if (
                  u.x <= l[0] &&
                  l[0] <= u.x + u.width &&
                  u.y <= l[1] &&
                  l[1] <= u.y + u.height
                )
                  n = { node: o, offsetX: l[0], offsetY: l[1] };
                else return !1;
              }
            },
            this
          ),
          n
        );
      }),
      (e.type = "treemap"),
      e
    );
  })(pt);
function sr() {
  return { nodeGroup: [], background: [], content: [] };
}
function mg(a, e, t, r, n, i, o, s, l, u) {
  if (!o) return;
  var v = o.getLayout(),
    c = a.getData(),
    h = o.getModel();
  if ((c.setItemGraphicEl(o.dataIndex, null), !v || !v.isInView)) return;
  var f = v.width,
    p = v.height,
    d = v.borderWidth,
    g = v.invisible,
    y = o.getRawIndex(),
    S = s && s.getRawIndex(),
    m = o.viewChildren,
    x = v.upperHeight,
    b = m && m.length,
    _ = h.getModel("itemStyle"),
    w = h.getModel(["emphasis", "itemStyle"]),
    A = h.getModel(["blur", "itemStyle"]),
    I = h.getModel(["select", "itemStyle"]),
    T = _.get("borderRadius") || 0,
    D = j("nodeGroup", jn);
  if (!D) return;
  if (
    (l.add(D),
    (D.x = v.x || 0),
    (D.y = v.y || 0),
    D.markRedraw(),
    (ca(D).nodeWidth = f),
    (ca(D).nodeHeight = p),
    v.isAboveViewRoot)
  )
    return D;
  var M = j("background", As, u, dg);
  M && B(D, M, b && v.upperLabelHeight);
  var C = h.getModel("emphasis"),
    P = C.get("focus"),
    E = C.get("blurScope"),
    R = C.get("disabled"),
    N =
      P === "ancestor"
        ? o.getAncestorsIndices()
        : P === "descendant"
        ? o.getDescendantIndices()
        : P;
  if (b)
    yo(D) && Gr(D, !1),
      M && (Gr(M, !R), c.setItemGraphicEl(o.dataIndex, M), mo(M, N, E));
  else {
    var V = j("content", As, u, gg);
    V && F(D, V),
      (M.disableMorphing = !0),
      M && yo(M) && Gr(M, !1),
      Gr(D, !R),
      c.setItemGraphicEl(o.dataIndex, D);
    var G = h.getShallow("cursor");
    G && V.attr("cursor", G), mo(D, N, E);
  }
  return D;
  function B(X, U, rt) {
    var J = nt(U);
    if (
      ((J.dataIndex = o.dataIndex),
      (J.seriesIndex = a.seriesIndex),
      U.setShape({ x: 0, y: 0, width: f, height: p, r: T }),
      g)
    )
      Y(U);
    else {
      U.invisible = !1;
      var st = o.getVisual("style"),
        dt = st.stroke,
        At = Ls(_);
      At.fill = dt;
      var ut = Ce(w);
      ut.fill = w.get("borderColor");
      var mt = Ce(A);
      mt.fill = A.get("borderColor");
      var Rt = Ce(I);
      if (((Rt.fill = I.get("borderColor")), rt)) {
        var Zt = f - 2 * d;
        K(U, dt, st.opacity, { x: d, y: 0, width: Zt, height: x });
      } else U.removeTextContent();
      U.setStyle(At),
        (U.ensureState("emphasis").style = ut),
        (U.ensureState("blur").style = mt),
        (U.ensureState("select").style = Rt),
        Ke(U);
    }
    X.add(U);
  }
  function F(X, U) {
    var rt = nt(U);
    (rt.dataIndex = o.dataIndex), (rt.seriesIndex = a.seriesIndex);
    var J = Math.max(f - 2 * d, 0),
      st = Math.max(p - 2 * d, 0);
    if (
      ((U.culling = !0),
      U.setShape({ x: d, y: d, width: J, height: st, r: T }),
      g)
    )
      Y(U);
    else {
      U.invisible = !1;
      var dt = o.getVisual("style"),
        At = dt.fill,
        ut = Ls(_);
      (ut.fill = At), (ut.decal = dt.decal);
      var mt = Ce(w),
        Rt = Ce(A),
        Zt = Ce(I);
      K(U, At, dt.opacity, null),
        U.setStyle(ut),
        (U.ensureState("emphasis").style = mt),
        (U.ensureState("blur").style = Rt),
        (U.ensureState("select").style = Zt),
        Ke(U);
    }
    X.add(U);
  }
  function Y(X) {
    !X.invisible && i.push(X);
  }
  function K(X, U, rt, J) {
    var st = h.getModel(J ? Ds : Is),
      dt = Mr(h.get("name"), null),
      At = st.getShallow("show");
    Ut(X, Pt(h, J ? Ds : Is), {
      defaultText: At ? dt : null,
      inheritColor: U,
      defaultOpacity: rt,
      labelFetcher: a,
      labelDataIndex: o.dataIndex
    });
    var ut = X.getTextContent();
    if (ut) {
      var mt = ut.style,
        Rt = sh(mt.padding || 0);
      J && (X.setTextConfig({ layoutRect: J }), (ut.disableLabelLayout = !0)),
        (ut.beforeUpdate = function () {
          var Ae = Math.max((J ? J.width : X.shape.width) - Rt[1] - Rt[3], 0),
            rr = Math.max((J ? J.height : X.shape.height) - Rt[0] - Rt[2], 0);
          (mt.width !== Ae || mt.height !== rr) &&
            ut.setStyle({ width: Ae, height: rr });
        }),
        (mt.truncateMinChar = 2),
        (mt.lineOverflow = "truncate"),
        $(mt, J, v);
      var Zt = ut.getState("emphasis");
      $(Zt ? Zt.style : null, J, v);
    }
  }
  function $(X, U, rt) {
    var J = X ? X.text : null;
    if (!U && rt.isLeafRoot && J != null) {
      var st = a.get("drillDownIcon", !0);
      X.text = st ? st + " " + J : J;
    }
  }
  function j(X, U, rt, J) {
    var st = S != null && t[X][S],
      dt = n[X];
    return (
      st
        ? ((t[X][S] = null), yt(dt, st))
        : g ||
          ((st = new U()), st instanceof mr && (st.z2 = Sg(rt, J)), re(dt, st)),
      (e[X][y] = st)
    );
  }
  function yt(X, U) {
    var rt = (X[y] = {});
    U instanceof jn
      ? ((rt.oldX = U.x), (rt.oldY = U.y))
      : (rt.oldShape = W({}, U.shape));
  }
  function re(X, U) {
    var rt = (X[y] = {}),
      J = o.parentNode,
      st = U instanceof Z;
    if (J && (!r || r.direction === "drillDown")) {
      var dt = 0,
        At = 0,
        ut = n.background[J.getRawIndex()];
      !r &&
        ut &&
        ut.oldShape &&
        ((dt = ut.oldShape.width), (At = ut.oldShape.height)),
        st
          ? ((rt.oldX = 0), (rt.oldY = At))
          : (rt.oldShape = { x: dt, y: At, width: 0, height: 0 });
    }
    rt.fadein = !st;
  }
}
function Sg(a, e) {
  return a * pg + e;
}
var xg = "itemStyle",
  vv = _t();
const bg = {
  seriesType: "treemap",
  reset: function (a) {
    var e = a.getData().tree,
      t = e.root;
    t.isRemoved() || cv(t, {}, a.getViewRoot().getAncestors(), a);
  }
};
function cv(a, e, t, r) {
  var n = a.getModel(),
    i = a.getLayout(),
    o = a.hostTree.data;
  if (!(!i || i.invisible || !i.isInView)) {
    var s = n.getModel(xg),
      l = _g(s, e, r),
      u = o.ensureUniqueItemVisual(a.dataIndex, "style"),
      v = s.get("borderColor"),
      c = s.get("borderColorSaturation"),
      h;
    c != null && ((h = Cs(l)), (v = wg(c, h))), (u.stroke = v);
    var f = a.viewChildren;
    if (!f || !f.length) (h = Cs(l)), (u.fill = h);
    else {
      var p = Ag(a, n, i, s, l, f);
      L(f, function (d, g) {
        if (d.depth >= t.length || d === t[d.depth]) {
          var y = Tg(n, l, d, g, p, r);
          cv(d, y, t, r);
        }
      });
    }
  }
}
function _g(a, e, t) {
  var r = W({}, e),
    n = t.designatedVisualItemStyle;
  return (
    L(["color", "colorAlpha", "colorSaturation"], function (i) {
      n[i] = e[i];
      var o = a.get(i);
      (n[i] = null), o != null && (r[i] = o);
    }),
    r
  );
}
function Cs(a) {
  var e = Qa(a, "color");
  if (e) {
    var t = Qa(a, "colorAlpha"),
      r = Qa(a, "colorSaturation");
    return r && (e = bu(e, null, null, r)), t && (e = On(e, t)), e;
  }
}
function wg(a, e) {
  return e != null ? bu(e, null, null, a) : null;
}
function Qa(a, e) {
  var t = a[e];
  if (t != null && t !== "none") return t;
}
function Ag(a, e, t, r, n, i) {
  if (!(!i || !i.length)) {
    var o =
      tn(e, "color") ||
      (n.color != null &&
        n.color !== "none" &&
        (tn(e, "colorAlpha") || tn(e, "colorSaturation")));
    if (o) {
      var s = e.get("visualMin"),
        l = e.get("visualMax"),
        u = t.dataExtent.slice();
      s != null && s < u[0] && (u[0] = s), l != null && l > u[1] && (u[1] = l);
      var v = e.get("colorMappingBy"),
        c = { type: o.name, dataExtent: u, visual: o.range };
      c.type === "color" && (v === "index" || v === "id")
        ? ((c.mappingMethod = "category"), (c.loop = !0))
        : (c.mappingMethod = "linear");
      var h = new _u(c);
      return (vv(h).drColorMappingBy = v), h;
    }
  }
}
function tn(a, e) {
  var t = a.get(e);
  return H(t) && t.length ? { name: e, range: t } : null;
}
function Tg(a, e, t, r, n, i) {
  var o = W({}, e);
  if (n) {
    var s = n.type,
      l = s === "color" && vv(n).drColorMappingBy,
      u =
        l === "index"
          ? r
          : l === "id"
          ? i.mapIdToIndex(t.getId())
          : t.getValue(a.get("visualDimension"));
    o[s] = n.mapValueToVisual(u);
  }
  return o;
}
var wr = Math.max,
  ha = Math.min,
  Ps = Ft,
  Wi = L,
  hv = ["itemStyle", "borderWidth"],
  Ig = ["itemStyle", "gapWidth"],
  Dg = ["upperLabel", "show"],
  Lg = ["upperLabel", "height"];
const Cg = {
  seriesType: "treemap",
  reset: function (a, e, t, r) {
    var n = t.getWidth(),
      i = t.getHeight(),
      o = a.option,
      s = se(a.getBoxLayoutParams(), {
        width: t.getWidth(),
        height: t.getHeight()
      }),
      l = o.size || [],
      u = z(Ps(s.width, l[0]), n),
      v = z(Ps(s.height, l[1]), i),
      c = r && r.type,
      h = ["treemapZoomToNode", "treemapRootToNode"],
      f = _r(r, h, a),
      p = c === "treemapRender" || c === "treemapMove" ? r.rootRect : null,
      d = a.getViewRoot(),
      g = sv(d);
    if (c !== "treemapMove") {
      var y =
          c === "treemapZoomToNode"
            ? kg(a, f, d, u, v)
            : p
            ? [p.width, p.height]
            : [u, v],
        S = o.sort;
      S && S !== "asc" && S !== "desc" && (S = "desc");
      var m = { squareRatio: o.squareRatio, sort: S, leafDepth: o.leafDepth };
      d.hostTree.clearLayouts();
      var x = { x: 0, y: 0, width: y[0], height: y[1], area: y[0] * y[1] };
      d.setLayout(x),
        fv(d, m, !1, 0),
        (x = d.getLayout()),
        Wi(g, function (_, w) {
          var A = (g[w + 1] || d).getValue();
          _.setLayout(
            W({ dataExtent: [A, A], borderWidth: 0, upperHeight: 0 }, x)
          );
        });
    }
    var b = a.getData().tree.root;
    b.setLayout(Vg(s, p, f), !0),
      a.setLayoutInfo(s),
      pv(b, new ft(-s.x, -s.y, n, i), g, d, 0);
  }
};
function fv(a, e, t, r) {
  var n, i;
  if (!a.isRemoved()) {
    var o = a.getLayout();
    (n = o.width), (i = o.height);
    var s = a.getModel(),
      l = s.get(hv),
      u = s.get(Ig) / 2,
      v = dv(s),
      c = Math.max(l, v),
      h = l - u,
      f = c - u;
    a.setLayout({ borderWidth: l, upperHeight: c, upperLabelHeight: v }, !0),
      (n = wr(n - 2 * h, 0)),
      (i = wr(i - h - f, 0));
    var p = n * i,
      d = Pg(a, s, p, e, t, r);
    if (d.length) {
      var g = { x: h, y: f, width: n, height: i },
        y = ha(n, i),
        S = 1 / 0,
        m = [];
      m.area = 0;
      for (var x = 0, b = d.length; x < b; ) {
        var _ = d[x];
        m.push(_), (m.area += _.getLayout().area);
        var w = Ng(m, y, e.squareRatio);
        w <= S
          ? (x++, (S = w))
          : ((m.area -= m.pop().getLayout().area),
            Ms(m, y, g, u, !1),
            (y = ha(g.width, g.height)),
            (m.length = m.area = 0),
            (S = 1 / 0));
      }
      if ((m.length && Ms(m, y, g, u, !0), !t)) {
        var A = s.get("childrenVisibleMin");
        A != null && p < A && (t = !0);
      }
      for (var x = 0, b = d.length; x < b; x++) fv(d[x], e, t, r + 1);
    }
  }
}
function Pg(a, e, t, r, n, i) {
  var o = a.children || [],
    s = r.sort;
  s !== "asc" && s !== "desc" && (s = null);
  var l = r.leafDepth != null && r.leafDepth <= i;
  if (n && !l) return (a.viewChildren = []);
  (o = Dt(o, function (f) {
    return !f.isRemoved();
  })),
    Eg(o, s);
  var u = Rg(e, o, s);
  if (u.sum === 0) return (a.viewChildren = []);
  if (((u.sum = Mg(e, t, u.sum, s, o)), u.sum === 0))
    return (a.viewChildren = []);
  for (var v = 0, c = o.length; v < c; v++) {
    var h = (o[v].getValue() / u.sum) * t;
    o[v].setLayout({ area: h });
  }
  return (
    l && (o.length && a.setLayout({ isLeafRoot: !0 }, !0), (o.length = 0)),
    (a.viewChildren = o),
    a.setLayout({ dataExtent: u.dataExtent }, !0),
    o
  );
}
function Mg(a, e, t, r, n) {
  if (!r) return t;
  for (
    var i = a.get("visibleMin"), o = n.length, s = o, l = o - 1;
    l >= 0;
    l--
  ) {
    var u = n[r === "asc" ? o - l - 1 : l].getValue();
    (u / t) * e < i && ((s = l), (t -= u));
  }
  return r === "asc" ? n.splice(0, o - s) : n.splice(s, o - s), t;
}
function Eg(a, e) {
  return (
    e &&
      a.sort(function (t, r) {
        var n =
          e === "asc"
            ? t.getValue() - r.getValue()
            : r.getValue() - t.getValue();
        return n === 0
          ? e === "asc"
            ? t.dataIndex - r.dataIndex
            : r.dataIndex - t.dataIndex
          : n;
      }),
    a
  );
}
function Rg(a, e, t) {
  for (var r = 0, n = 0, i = e.length; n < i; n++) r += e[n].getValue();
  var o = a.get("visualDimension"),
    s;
  return (
    !e || !e.length
      ? (s = [NaN, NaN])
      : o === "value" && t
      ? ((s = [e[e.length - 1].getValue(), e[0].getValue()]),
        t === "asc" && s.reverse())
      : ((s = [1 / 0, -1 / 0]),
        Wi(e, function (l) {
          var u = l.getValue(o);
          u < s[0] && (s[0] = u), u > s[1] && (s[1] = u);
        })),
    { sum: r, dataExtent: s }
  );
}
function Ng(a, e, t) {
  for (var r = 0, n = 1 / 0, i = 0, o = void 0, s = a.length; i < s; i++)
    (o = a[i].getLayout().area), o && (o < n && (n = o), o > r && (r = o));
  var l = a.area * a.area,
    u = e * e * t;
  return l ? wr((u * r) / l, l / (u * n)) : 1 / 0;
}
function Ms(a, e, t, r, n) {
  var i = e === t.width ? 0 : 1,
    o = 1 - i,
    s = ["x", "y"],
    l = ["width", "height"],
    u = t[s[i]],
    v = e ? a.area / e : 0;
  (n || v > t[l[o]]) && (v = t[l[o]]);
  for (var c = 0, h = a.length; c < h; c++) {
    var f = a[c],
      p = {},
      d = v ? f.getLayout().area / v : 0,
      g = (p[l[o]] = wr(v - 2 * r, 0)),
      y = t[s[i]] + t[l[i]] - u,
      S = c === h - 1 || y < d ? y : d,
      m = (p[l[i]] = wr(S - 2 * r, 0));
    (p[s[o]] = t[s[o]] + ha(r, g / 2)),
      (p[s[i]] = u + ha(r, m / 2)),
      (u += S),
      f.setLayout(p, !0);
  }
  (t[s[o]] += v), (t[l[o]] -= v);
}
function kg(a, e, t, r, n) {
  var i = (e || {}).node,
    o = [r, n];
  if (!i || i === t) return o;
  for (
    var s, l = r * n, u = l * a.option.zoomToNodeRatio;
    (s = i.parentNode);

  ) {
    for (var v = 0, c = s.children, h = 0, f = c.length; h < f; h++)
      v += c[h].getValue();
    var p = i.getValue();
    if (p === 0) return o;
    u *= v / p;
    var d = s.getModel(),
      g = d.get(hv),
      y = Math.max(g, dv(d));
    (u += 4 * g * g + (3 * g + y) * Math.pow(u, 0.5)),
      u > So && (u = So),
      (i = s);
  }
  u < l && (u = l);
  var S = Math.pow(u / l, 0.5);
  return [r * S, n * S];
}
function Vg(a, e, t) {
  if (e) return { x: e.x, y: e.y };
  var r = { x: 0, y: 0 };
  if (!t) return r;
  var n = t.node,
    i = n.getLayout();
  if (!i) return r;
  for (var o = [i.width / 2, i.height / 2], s = n; s; ) {
    var l = s.getLayout();
    (o[0] += l.x), (o[1] += l.y), (s = s.parentNode);
  }
  return { x: a.width / 2 - o[0], y: a.height / 2 - o[1] };
}
function pv(a, e, t, r, n) {
  var i = a.getLayout(),
    o = t[n],
    s = o && o === a;
  if (!((o && !s) || (n === t.length && a !== r))) {
    a.setLayout(
      { isInView: !0, invisible: !s && !e.intersect(i), isAboveViewRoot: s },
      !0
    );
    var l = new ft(e.x - i.x, e.y - i.y, e.width, e.height);
    Wi(a.viewChildren || [], function (u) {
      pv(u, l, t, r, n + 1);
    });
  }
}
function dv(a) {
  return a.get(Dg) ? a.get(Lg) : 0;
}
function Gg(a) {
  a.registerSeriesModel(og),
    a.registerChartView(yg),
    a.registerVisual(bg),
    a.registerLayout(Cg),
    ig(a);
}
function zg(a) {
  var e = a.findComponents({ mainType: "legend" });
  !e ||
    !e.length ||
    a.eachSeriesByType("graph", function (t) {
      var r = t.getCategoriesData(),
        n = t.getGraph(),
        i = n.data,
        o = r.mapArray(r.getName);
      i.filterSelf(function (s) {
        var l = i.getItemModel(s),
          u = l.getShallow("category");
        if (u != null) {
          Qt(u) && (u = o[u]);
          for (var v = 0; v < e.length; v++) if (!e[v].isSelected(u)) return !1;
        }
        return !0;
      });
    });
}
function Og(a) {
  var e = {};
  a.eachSeriesByType("graph", function (t) {
    var r = t.getCategoriesData(),
      n = t.getData(),
      i = {};
    r.each(function (o) {
      var s = r.getName(o);
      i["ec-" + s] = o;
      var l = r.getItemModel(o),
        u = l.getModel("itemStyle").getItemStyle();
      u.fill || (u.fill = t.getColorFromPalette(s, e)),
        r.setItemVisual(o, "style", u);
      for (
        var v = ["symbol", "symbolSize", "symbolKeepAspect"], c = 0;
        c < v.length;
        c++
      ) {
        var h = l.getShallow(v[c], !0);
        h != null && r.setItemVisual(o, v[c], h);
      }
    }),
      r.count() &&
        n.each(function (o) {
          var s = n.getItemModel(o),
            l = s.getShallow("category");
          if (l != null) {
            et(l) && (l = i["ec-" + l]);
            var u = r.getItemVisual(l, "style"),
              v = n.ensureUniqueItemVisual(o, "style");
            W(v, u);
            for (
              var c = ["symbol", "symbolSize", "symbolKeepAspect"], h = 0;
              h < c.length;
              h++
            )
              n.setItemVisual(o, c[h], r.getItemVisual(l, c[h]));
          }
        });
  });
}
function Hr(a) {
  return a instanceof Array || (a = [a, a]), a;
}
function Bg(a) {
  a.eachSeriesByType("graph", function (e) {
    var t = e.getGraph(),
      r = e.getEdgeData(),
      n = Hr(e.get("edgeSymbol")),
      i = Hr(e.get("edgeSymbolSize"));
    r.setVisual("fromSymbol", n && n[0]),
      r.setVisual("toSymbol", n && n[1]),
      r.setVisual("fromSymbolSize", i && i[0]),
      r.setVisual("toSymbolSize", i && i[1]),
      r.setVisual("style", e.getModel("lineStyle").getLineStyle()),
      r.each(function (o) {
        var s = r.getItemModel(o),
          l = t.getEdgeByIndex(o),
          u = Hr(s.getShallow("symbol", !0)),
          v = Hr(s.getShallow("symbolSize", !0)),
          c = s.getModel("lineStyle").getLineStyle(),
          h = r.ensureUniqueItemVisual(o, "style");
        switch ((W(h, c), h.stroke)) {
          case "source": {
            var f = l.node1.getVisual("style");
            h.stroke = f && f.fill;
            break;
          }
          case "target": {
            var f = l.node2.getVisual("style");
            h.stroke = f && f.fill;
            break;
          }
        }
        u[0] && l.setVisual("fromSymbol", u[0]),
          u[1] && l.setVisual("toSymbol", u[1]),
          v[0] && l.setVisual("fromSymbolSize", v[0]),
          v[1] && l.setVisual("toSymbolSize", v[1]);
      });
  });
}
var Jn = "-->",
  za = function (a) {
    return a.get("autoCurveness") || null;
  },
  gv = function (a, e) {
    var t = za(a),
      r = 20,
      n = [];
    if (Qt(t)) r = t;
    else if (H(t)) {
      a.__curvenessList = t;
      return;
    }
    e > r && (r = e);
    var i = r % 2 ? r + 2 : r + 3;
    n = [];
    for (var o = 0; o < i; o++)
      n.push(((o % 2 ? o + 1 : o) / 10) * (o % 2 ? -1 : 1));
    a.__curvenessList = n;
  },
  Ar = function (a, e, t) {
    var r = [a.id, a.dataIndex].join("."),
      n = [e.id, e.dataIndex].join(".");
    return [t.uid, r, n].join(Jn);
  },
  yv = function (a) {
    var e = a.split(Jn);
    return [e[0], e[2], e[1]].join(Jn);
  },
  Fg = function (a, e) {
    var t = Ar(a.node1, a.node2, e);
    return e.__edgeMap[t];
  },
  Wg = function (a, e) {
    var t = Qn(Ar(a.node1, a.node2, e), e),
      r = Qn(Ar(a.node2, a.node1, e), e);
    return t + r;
  },
  Qn = function (a, e) {
    var t = e.__edgeMap;
    return t[a] ? t[a].length : 0;
  };
function Hg(a) {
  za(a) && ((a.__curvenessList = []), (a.__edgeMap = {}), gv(a));
}
function Ug(a, e, t, r) {
  if (za(t)) {
    var n = Ar(a, e, t),
      i = t.__edgeMap,
      o = i[yv(n)];
    i[n] && !o
      ? (i[n].isForward = !0)
      : o && i[n] && ((o.isForward = !0), (i[n].isForward = !1)),
      (i[n] = i[n] || []),
      i[n].push(r);
  }
}
function Hi(a, e, t, r) {
  var n = za(e),
    i = H(n);
  if (!n) return null;
  var o = Fg(a, e);
  if (!o) return null;
  for (var s = -1, l = 0; l < o.length; l++)
    if (o[l] === t) {
      s = l;
      break;
    }
  var u = Wg(a, e);
  gv(e, u), (a.lineStyle = a.lineStyle || {});
  var v = Ar(a.node1, a.node2, e),
    c = e.__curvenessList,
    h = i || u % 2 ? 0 : 1;
  if (o.isForward) return c[h + s];
  var f = yv(v),
    p = Qn(f, e),
    d = c[s + p + h];
  return r
    ? i
      ? n && n[0] === 0
        ? (p + h) % 2
          ? d
          : -d
        : ((p % 2 ? 0 : 1) + h) % 2
        ? d
        : -d
      : (p + h) % 2
      ? d
      : -d
    : c[s + p + h];
}
function mv(a) {
  var e = a.coordinateSystem;
  if (!(e && e.type !== "view")) {
    var t = a.getGraph();
    t.eachNode(function (r) {
      var n = r.getModel();
      r.setLayout([+n.get("x"), +n.get("y")]);
    }),
      Ui(t, a);
  }
}
function Ui(a, e) {
  a.eachEdge(function (t, r) {
    var n = Er(
        t.getModel().get(["lineStyle", "curveness"]),
        -Hi(t, e, r, !0),
        0
      ),
      i = ye(t.node1.getLayout()),
      o = ye(t.node2.getLayout()),
      s = [i, o];
    +n &&
      s.push([
        (i[0] + o[0]) / 2 - (i[1] - o[1]) * n,
        (i[1] + o[1]) / 2 - (o[0] - i[0]) * n
      ]),
      t.setLayout(s);
  });
}
function Yg(a, e) {
  a.eachSeriesByType("graph", function (t) {
    var r = t.get("layout"),
      n = t.coordinateSystem;
    if (n && n.type !== "view") {
      var i = t.getData(),
        o = [];
      L(n.dimensions, function (h) {
        o = o.concat(i.mapDimensionsAll(h));
      });
      for (var s = 0; s < i.count(); s++) {
        for (var l = [], u = !1, v = 0; v < o.length; v++) {
          var c = i.get(o[v], s);
          isNaN(c) || (u = !0), l.push(c);
        }
        u
          ? i.setItemLayout(s, n.dataToPoint(l))
          : i.setItemLayout(s, [NaN, NaN]);
      }
      Ui(i.graph, t);
    } else (!r || r === "none") && mv(t);
  });
}
function fr(a) {
  var e = a.coordinateSystem;
  if (e.type !== "view") return 1;
  var t = a.option.nodeScaleRatio,
    r = e.scaleX,
    n = e.getZoom(),
    i = (n - 1) * t + 1;
  return i / r;
}
function pr(a) {
  var e = a.getVisual("symbolSize");
  return e instanceof Array && (e = (e[0] + e[1]) / 2), +e;
}
var Es = Math.PI,
  en = [];
function Yi(a, e, t, r) {
  var n = a.coordinateSystem;
  if (!(n && n.type !== "view")) {
    var i = n.getBoundingRect(),
      o = a.getData(),
      s = o.graph,
      l = i.width / 2 + i.x,
      u = i.height / 2 + i.y,
      v = Math.min(i.width, i.height) / 2,
      c = o.count();
    if ((o.setLayout({ cx: l, cy: u }), !!c)) {
      if (t) {
        var h = n.pointToData(r),
          f = h[0],
          p = h[1],
          d = [f - l, p - u];
        Rr(d, d), lh(d, d, v), t.setLayout([l + d[0], u + d[1]], !0);
        var g = a.get(["circular", "rotateLabel"]);
        Sv(t, g, l, u);
      }
      $g[e](a, s, o, v, l, u, c),
        s.eachEdge(function (y, S) {
          var m = Er(
              y.getModel().get(["lineStyle", "curveness"]),
              Hi(y, a, S),
              0
            ),
            x = ye(y.node1.getLayout()),
            b = ye(y.node2.getLayout()),
            _,
            w = (x[0] + b[0]) / 2,
            A = (x[1] + b[1]) / 2;
          +m && ((m *= 3), (_ = [l * m + w * (1 - m), u * m + A * (1 - m)])),
            y.setLayout([x, b, _]);
        });
    }
  }
}
var $g = {
  value: function (a, e, t, r, n, i, o) {
    var s = 0,
      l = t.getSum("value"),
      u = (Math.PI * 2) / (l || o);
    e.eachNode(function (v) {
      var c = v.getValue("value"),
        h = (u * (l ? c : 1)) / 2;
      (s += h),
        v.setLayout([r * Math.cos(s) + n, r * Math.sin(s) + i]),
        (s += h);
    });
  },
  symbolSize: function (a, e, t, r, n, i, o) {
    var s = 0;
    en.length = o;
    var l = fr(a);
    e.eachNode(function (c) {
      var h = pr(c);
      isNaN(h) && (h = 2), h < 0 && (h = 0), (h *= l);
      var f = Math.asin(h / 2 / r);
      isNaN(f) && (f = Es / 2), (en[c.dataIndex] = f), (s += f * 2);
    });
    var u = (2 * Es - s) / o / 2,
      v = 0;
    e.eachNode(function (c) {
      var h = u + en[c.dataIndex];
      (v += h),
        (!c.getLayout() || !c.getLayout().fixed) &&
          c.setLayout([r * Math.cos(v) + n, r * Math.sin(v) + i]),
        (v += h);
    });
  }
};
function Sv(a, e, t, r) {
  var n = a.getGraphicEl();
  if (n) {
    var i = a.getModel(),
      o = i.get(["label", "rotate"]) || 0,
      s = n.getSymbolPath();
    if (e) {
      var l = a.getLayout(),
        u = Math.atan2(l[1] - r, l[0] - t);
      u < 0 && (u = Math.PI * 2 + u);
      var v = l[0] < t;
      v && (u = u - Math.PI);
      var c = v ? "left" : "right";
      s.setTextConfig({ rotation: -u, position: c, origin: "center" });
      var h = s.ensureState("emphasis");
      W(h.textConfig || (h.textConfig = {}), { position: c });
    } else s.setTextConfig({ rotation: (o *= Math.PI / 180) });
  }
}
function Xg(a) {
  a.eachSeriesByType("graph", function (e) {
    e.get("layout") === "circular" && Yi(e, "symbolSize");
  });
}
var Oe = Bn;
function Zg(a, e, t) {
  for (
    var r = a,
      n = e,
      i = t.rect,
      o = i.width,
      s = i.height,
      l = [i.x + o / 2, i.y + s / 2],
      u = t.gravity == null ? 0.1 : t.gravity,
      v = 0;
    v < r.length;
    v++
  ) {
    var c = r[v];
    c.p ||
      (c.p = uh(
        o * (Math.random() - 0.5) + l[0],
        s * (Math.random() - 0.5) + l[1]
      )),
      (c.pp = ye(c.p)),
      (c.edges = null);
  }
  var h = t.friction == null ? 0.6 : t.friction,
    f = h,
    p,
    d;
  return {
    warmUp: function () {
      f = h * 0.8;
    },
    setFixed: function (g) {
      r[g].fixed = !0;
    },
    setUnfixed: function (g) {
      r[g].fixed = !1;
    },
    beforeStep: function (g) {
      p = g;
    },
    afterStep: function (g) {
      d = g;
    },
    step: function (g) {
      p && p(r, n);
      for (var y = [], S = r.length, m = 0; m < n.length; m++) {
        var x = n[m];
        if (!x.ignoreForceLayout) {
          var b = x.n1,
            _ = x.n2;
          $e(y, _.p, b.p);
          var w = xo(y) - x.d,
            A = _.w / (b.w + _.w);
          isNaN(A) && (A = 0),
            Rr(y, y),
            !b.fixed && Oe(b.p, b.p, y, A * w * f),
            !_.fixed && Oe(_.p, _.p, y, -(1 - A) * w * f);
        }
      }
      for (var m = 0; m < S; m++) {
        var I = r[m];
        I.fixed || ($e(y, l, I.p), Oe(I.p, I.p, y, u * f));
      }
      for (var m = 0; m < S; m++)
        for (var b = r[m], T = m + 1; T < S; T++) {
          var _ = r[T];
          $e(y, _.p, b.p);
          var w = xo(y);
          w === 0 && (vh(y, Math.random() - 0.5, Math.random() - 0.5), (w = 1));
          var D = (b.rep + _.rep) / w / w;
          !b.fixed && Oe(b.pp, b.pp, y, D), !_.fixed && Oe(_.pp, _.pp, y, -D);
        }
      for (var M = [], m = 0; m < S; m++) {
        var I = r[m];
        I.fixed || ($e(M, I.p, I.pp), Oe(I.p, I.p, M, f), It(I.pp, I.p));
      }
      f = f * 0.992;
      var C = f < 0.01;
      d && d(r, n, C), g && g(C);
    }
  };
}
function qg(a) {
  a.eachSeriesByType("graph", function (e) {
    var t = e.coordinateSystem;
    if (!(t && t.type !== "view"))
      if (e.get("layout") === "force") {
        var r = e.preservedPoints || {},
          n = e.getGraph(),
          i = n.data,
          o = n.edgeData,
          s = e.getModel("force"),
          l = s.get("initLayout");
        e.preservedPoints
          ? i.each(function (m) {
              var x = i.getId(m);
              i.setItemLayout(m, r[x] || [NaN, NaN]);
            })
          : !l || l === "none"
          ? mv(e)
          : l === "circular" && Yi(e, "value");
        var u = i.getDataExtent("value"),
          v = o.getDataExtent("value"),
          c = s.get("repulsion"),
          h = s.get("edgeLength"),
          f = H(c) ? c : [c, c],
          p = H(h) ? h : [h, h];
        p = [p[1], p[0]];
        var d = i.mapArray("value", function (m, x) {
            var b = i.getItemLayout(x),
              _ = qt(m, u, f);
            return (
              isNaN(_) && (_ = (f[0] + f[1]) / 2),
              {
                w: _,
                rep: _,
                fixed: i.getItemModel(x).get("fixed"),
                p: !b || isNaN(b[0]) || isNaN(b[1]) ? null : b
              }
            );
          }),
          g = o.mapArray("value", function (m, x) {
            var b = n.getEdgeByIndex(x),
              _ = qt(m, v, p);
            isNaN(_) && (_ = (p[0] + p[1]) / 2);
            var w = b.getModel(),
              A = Er(
                b.getModel().get(["lineStyle", "curveness"]),
                -Hi(b, e, x, !0),
                0
              );
            return {
              n1: d[b.node1.dataIndex],
              n2: d[b.node2.dataIndex],
              d: _,
              curveness: A,
              ignoreForceLayout: w.get("ignoreForceLayout")
            };
          }),
          y = t.getBoundingRect(),
          S = Zg(d, g, {
            rect: y,
            gravity: s.get("gravity"),
            friction: s.get("friction")
          });
        S.beforeStep(function (m, x) {
          for (var b = 0, _ = m.length; b < _; b++)
            m[b].fixed && It(m[b].p, n.getNodeByIndex(b).getLayout());
        }),
          S.afterStep(function (m, x, b) {
            for (var _ = 0, w = m.length; _ < w; _++)
              m[_].fixed || n.getNodeByIndex(_).setLayout(m[_].p),
                (r[i.getId(_)] = m[_].p);
            for (var _ = 0, w = x.length; _ < w; _++) {
              var A = x[_],
                I = n.getEdgeByIndex(_),
                T = A.n1.p,
                D = A.n2.p,
                M = I.getLayout();
              (M = M ? M.slice() : []),
                (M[0] = M[0] || []),
                (M[1] = M[1] || []),
                It(M[0], T),
                It(M[1], D),
                +A.curveness &&
                  (M[2] = [
                    (T[0] + D[0]) / 2 - (T[1] - D[1]) * A.curveness,
                    (T[1] + D[1]) / 2 - (D[0] - T[0]) * A.curveness
                  ]),
                I.setLayout(M);
            }
          }),
          (e.forceLayout = S),
          (e.preservedPoints = r),
          S.step();
      } else e.forceLayout = null;
  });
}
function Kg(a, e, t) {
  var r = W(a.getBoxLayoutParams(), { aspect: t });
  return se(r, { width: e.getWidth(), height: e.getHeight() });
}
function jg(a, e) {
  var t = [];
  return (
    a.eachSeriesByType("graph", function (r) {
      var n = r.get("coordinateSystem");
      if (!n || n === "view") {
        var i = r.getData(),
          o = i.mapArray(function (g) {
            var y = i.getItemModel(g);
            return [+y.get("x"), +y.get("y")];
          }),
          s = [],
          l = [];
        Ma(o, s, l),
          l[0] - s[0] === 0 && ((l[0] += 1), (s[0] -= 1)),
          l[1] - s[1] === 0 && ((l[1] += 1), (s[1] -= 1));
        var u = (l[0] - s[0]) / (l[1] - s[1]),
          v = Kg(r, e, u);
        isNaN(u) && ((s = [v.x, v.y]), (l = [v.x + v.width, v.y + v.height]));
        var c = l[0] - s[0],
          h = l[1] - s[1],
          f = v.width,
          p = v.height,
          d = (r.coordinateSystem = new Vr());
        (d.zoomLimit = r.get("scaleLimit")),
          d.setBoundingRect(s[0], s[1], c, h),
          d.setViewRect(v.x, v.y, f, p),
          d.setCenter(r.get("center"), e),
          d.setZoom(r.get("zoom")),
          t.push(d);
      }
    }),
    t
  );
}
var Rs = jt.prototype,
  rn = mu.prototype,
  xv = (function () {
    function a() {
      (this.x1 = 0),
        (this.y1 = 0),
        (this.x2 = 0),
        (this.y2 = 0),
        (this.percent = 1);
    }
    return a;
  })();
(function (a) {
  k(e, a);
  function e() {
    return (a !== null && a.apply(this, arguments)) || this;
  }
  return e;
})(xv);
function an(a) {
  return isNaN(+a.cpx1) || isNaN(+a.cpy1);
}
var Jg = (function (a) {
    k(e, a);
    function e(t) {
      var r = a.call(this, t) || this;
      return (r.type = "ec-line"), r;
    }
    return (
      (e.prototype.getDefaultStyle = function () {
        return { stroke: "#000", fill: null };
      }),
      (e.prototype.getDefaultShape = function () {
        return new xv();
      }),
      (e.prototype.buildPath = function (t, r) {
        an(r) ? Rs.buildPath.call(this, t, r) : rn.buildPath.call(this, t, r);
      }),
      (e.prototype.pointAt = function (t) {
        return an(this.shape)
          ? Rs.pointAt.call(this, t)
          : rn.pointAt.call(this, t);
      }),
      (e.prototype.tangentAt = function (t) {
        var r = this.shape,
          n = an(r) ? [r.x2 - r.x1, r.y2 - r.y1] : rn.tangentAt.call(this, t);
        return Rr(n, n);
      }),
      e
    );
  })(wt),
  nn = ["fromSymbol", "toSymbol"];
function Ns(a) {
  return "_" + a + "Type";
}
function ks(a, e, t) {
  var r = e.getItemVisual(t, a);
  if (!r || r === "none") return r;
  var n = e.getItemVisual(t, a + "Size"),
    i = e.getItemVisual(t, a + "Rotate"),
    o = e.getItemVisual(t, a + "Offset"),
    s = e.getItemVisual(t, a + "KeepAspect"),
    l = Dr(n),
    u = Nr(o || 0, l);
  return r + l + u + (i || "") + (s || "");
}
function Vs(a, e, t) {
  var r = e.getItemVisual(t, a);
  if (!(!r || r === "none")) {
    var n = e.getItemVisual(t, a + "Size"),
      i = e.getItemVisual(t, a + "Rotate"),
      o = e.getItemVisual(t, a + "Offset"),
      s = e.getItemVisual(t, a + "KeepAspect"),
      l = Dr(n),
      u = Nr(o || 0, l),
      v = ie(r, -l[0] / 2 + u[0], -l[1] / 2 + u[1], l[0], l[1], null, s);
    return (
      (v.__specifiedRotation =
        i == null || isNaN(i) ? void 0 : (+i * Math.PI) / 180 || 0),
      (v.name = a),
      v
    );
  }
}
function Qg(a) {
  var e = new Jg({ name: "line", subPixelOptimize: !0 });
  return ti(e.shape, a), e;
}
function ti(a, e) {
  (a.x1 = e[0][0]),
    (a.y1 = e[0][1]),
    (a.x2 = e[1][0]),
    (a.y2 = e[1][1]),
    (a.percent = 1);
  var t = e[2];
  t ? ((a.cpx1 = t[0]), (a.cpy1 = t[1])) : ((a.cpx1 = NaN), (a.cpy1 = NaN));
}
var $i = (function (a) {
    k(e, a);
    function e(t, r, n) {
      var i = a.call(this) || this;
      return i._createLine(t, r, n), i;
    }
    return (
      (e.prototype._createLine = function (t, r, n) {
        var i = t.hostModel,
          o = t.getItemLayout(r),
          s = Qg(o);
        (s.shape.percent = 0),
          Ct(s, { shape: { percent: 1 } }, i, r),
          this.add(s),
          L(
            nn,
            function (l) {
              var u = Vs(l, t, r);
              this.add(u), (this[Ns(l)] = ks(l, t, r));
            },
            this
          ),
          this._updateCommonStl(t, r, n);
      }),
      (e.prototype.updateData = function (t, r, n) {
        var i = t.hostModel,
          o = this.childOfName("line"),
          s = t.getItemLayout(r),
          l = { shape: {} };
        ti(l.shape, s),
          lt(o, l, i, r),
          L(
            nn,
            function (u) {
              var v = ks(u, t, r),
                c = Ns(u);
              if (this[c] !== v) {
                this.remove(this.childOfName(u));
                var h = Vs(u, t, r);
                this.add(h);
              }
              this[c] = v;
            },
            this
          ),
          this._updateCommonStl(t, r, n);
      }),
      (e.prototype.getLinePath = function () {
        return this.childAt(0);
      }),
      (e.prototype._updateCommonStl = function (t, r, n) {
        var i = t.hostModel,
          o = this.childOfName("line"),
          s = n && n.emphasisLineStyle,
          l = n && n.blurLineStyle,
          u = n && n.selectLineStyle,
          v = n && n.labelStatesModels,
          c = n && n.emphasisDisabled,
          h = n && n.focus,
          f = n && n.blurScope;
        if (!n || t.hasItemOption) {
          var p = t.getItemModel(r),
            d = p.getModel("emphasis");
          (s = d.getModel("lineStyle").getLineStyle()),
            (l = p.getModel(["blur", "lineStyle"]).getLineStyle()),
            (u = p.getModel(["select", "lineStyle"]).getLineStyle()),
            (c = d.get("disabled")),
            (h = d.get("focus")),
            (f = d.get("blurScope")),
            (v = Pt(p));
        }
        var g = t.getItemVisual(r, "style"),
          y = g.stroke;
        o.useStyle(g),
          (o.style.fill = null),
          (o.style.strokeNoScale = !0),
          (o.ensureState("emphasis").style = s),
          (o.ensureState("blur").style = l),
          (o.ensureState("select").style = u),
          L(
            nn,
            function (_) {
              var w = this.childOfName(_);
              if (w) {
                w.setColor(y), (w.style.opacity = g.opacity);
                for (var A = 0; A < Fn.length; A++) {
                  var I = Fn[A],
                    T = o.getState(I);
                  if (T) {
                    var D = T.style || {},
                      M = w.ensureState(I),
                      C = M.style || (M.style = {});
                    D.stroke != null &&
                      (C[w.__isEmptyBrush ? "stroke" : "fill"] = D.stroke),
                      D.opacity != null && (C.opacity = D.opacity);
                  }
                }
                w.markRedraw();
              }
            },
            this
          );
        var S = i.getRawValue(r);
        Ut(this, v, {
          labelDataIndex: r,
          labelFetcher: {
            getFormattedLabel: function (_, w) {
              return i.getFormattedLabel(_, w, t.dataType);
            }
          },
          inheritColor: y || "#000",
          defaultOpacity: g.opacity,
          defaultText: (S == null ? t.getName(r) : isFinite(S) ? gi(S) : S) + ""
        });
        var m = this.getTextContent();
        if (m) {
          var x = v.normal;
          (m.__align = m.style.align),
            (m.__verticalAlign = m.style.verticalAlign),
            (m.__position = x.get("position") || "middle");
          var b = x.get("distance");
          H(b) || (b = [b, b]), (m.__labelDistance = b);
        }
        this.setTextConfig({ position: null, local: !0, inside: !1 }),
          ct(this, h, f, c);
      }),
      (e.prototype.highlight = function () {
        yi(this);
      }),
      (e.prototype.downplay = function () {
        mi(this);
      }),
      (e.prototype.updateLayout = function (t, r) {
        this.setLinePoints(t.getItemLayout(r));
      }),
      (e.prototype.setLinePoints = function (t) {
        var r = this.childOfName("line");
        ti(r.shape, t), r.dirty();
      }),
      (e.prototype.beforeUpdate = function () {
        var t = this,
          r = t.childOfName("fromSymbol"),
          n = t.childOfName("toSymbol"),
          i = t.getTextContent();
        if (!r && !n && (!i || i.ignore)) return;
        for (var o = 1, s = this.parent; s; )
          s.scaleX && (o /= s.scaleX), (s = s.parent);
        var l = t.childOfName("line");
        if (!this.__dirty && !l.__dirty) return;
        var u = l.shape.percent,
          v = l.pointAt(0),
          c = l.pointAt(u),
          h = $e([], c, v);
        Rr(h, h);
        function f(T, D) {
          var M = T.__specifiedRotation;
          if (M == null) {
            var C = l.tangentAt(D);
            T.attr(
              "rotation",
              ((D === 1 ? -1 : 1) * Math.PI) / 2 - Math.atan2(C[1], C[0])
            );
          } else T.attr("rotation", M);
        }
        if (
          (r &&
            (r.setPosition(v),
            f(r, 0),
            (r.scaleX = r.scaleY = o * u),
            r.markRedraw()),
          n &&
            (n.setPosition(c),
            f(n, 1),
            (n.scaleX = n.scaleY = o * u),
            n.markRedraw()),
          i && !i.ignore)
        ) {
          (i.x = i.y = 0), (i.originX = i.originY = 0);
          var p = void 0,
            d = void 0,
            g = i.__labelDistance,
            y = g[0] * o,
            S = g[1] * o,
            m = u / 2,
            x = l.tangentAt(m),
            b = [x[1], -x[0]],
            _ = l.pointAt(m);
          b[1] > 0 && ((b[0] = -b[0]), (b[1] = -b[1]));
          var w = x[0] < 0 ? -1 : 1;
          if (i.__position !== "start" && i.__position !== "end") {
            var A = -Math.atan2(x[1], x[0]);
            c[0] < v[0] && (A = Math.PI + A), (i.rotation = A);
          }
          var I = void 0;
          switch (i.__position) {
            case "insideStartTop":
            case "insideMiddleTop":
            case "insideEndTop":
            case "middle":
              (I = -S), (d = "bottom");
              break;
            case "insideStartBottom":
            case "insideMiddleBottom":
            case "insideEndBottom":
              (I = S), (d = "top");
              break;
            default:
              (I = 0), (d = "middle");
          }
          switch (i.__position) {
            case "end":
              (i.x = h[0] * y + c[0]),
                (i.y = h[1] * S + c[1]),
                (p = h[0] > 0.8 ? "left" : h[0] < -0.8 ? "right" : "center"),
                (d = h[1] > 0.8 ? "top" : h[1] < -0.8 ? "bottom" : "middle");
              break;
            case "start":
              (i.x = -h[0] * y + v[0]),
                (i.y = -h[1] * S + v[1]),
                (p = h[0] > 0.8 ? "right" : h[0] < -0.8 ? "left" : "center"),
                (d = h[1] > 0.8 ? "bottom" : h[1] < -0.8 ? "top" : "middle");
              break;
            case "insideStartTop":
            case "insideStart":
            case "insideStartBottom":
              (i.x = y * w + v[0]),
                (i.y = v[1] + I),
                (p = x[0] < 0 ? "right" : "left"),
                (i.originX = -y * w),
                (i.originY = -I);
              break;
            case "insideMiddleTop":
            case "insideMiddle":
            case "insideMiddleBottom":
            case "middle":
              (i.x = _[0]), (i.y = _[1] + I), (p = "center"), (i.originY = -I);
              break;
            case "insideEndTop":
            case "insideEnd":
            case "insideEndBottom":
              (i.x = -y * w + c[0]),
                (i.y = c[1] + I),
                (p = x[0] >= 0 ? "right" : "left"),
                (i.originX = y * w),
                (i.originY = -I);
              break;
          }
          (i.scaleX = i.scaleY = o),
            i.setStyle({
              verticalAlign: i.__verticalAlign || d,
              align: i.__align || p
            });
        }
      }),
      e
    );
  })(Z),
  Xi = (function () {
    function a(e) {
      (this.group = new Z()), (this._LineCtor = e || $i);
    }
    return (
      (a.prototype.updateData = function (e) {
        var t = this;
        this._progressiveEls = null;
        var r = this,
          n = r.group,
          i = r._lineData;
        (r._lineData = e), i || n.removeAll();
        var o = Gs(e);
        e.diff(i)
          .add(function (s) {
            t._doAdd(e, s, o);
          })
          .update(function (s, l) {
            t._doUpdate(i, e, l, s, o);
          })
          .remove(function (s) {
            n.remove(i.getItemGraphicEl(s));
          })
          .execute();
      }),
      (a.prototype.updateLayout = function () {
        var e = this._lineData;
        e &&
          e.eachItemGraphicEl(function (t, r) {
            t.updateLayout(e, r);
          }, this);
      }),
      (a.prototype.incrementalPrepareUpdate = function (e) {
        (this._seriesScope = Gs(e)),
          (this._lineData = null),
          this.group.removeAll();
      }),
      (a.prototype.incrementalUpdate = function (e, t) {
        this._progressiveEls = [];
        function r(s) {
          !s.isGroup &&
            !ty(s) &&
            ((s.incremental = !0), (s.ensureState("emphasis").hoverLayer = !0));
        }
        for (var n = e.start; n < e.end; n++) {
          var i = t.getItemLayout(n);
          if (on(i)) {
            var o = new this._LineCtor(t, n, this._seriesScope);
            o.traverse(r),
              this.group.add(o),
              t.setItemGraphicEl(n, o),
              this._progressiveEls.push(o);
          }
        }
      }),
      (a.prototype.remove = function () {
        this.group.removeAll();
      }),
      (a.prototype.eachRendered = function (e) {
        Ea(this._progressiveEls || this.group, e);
      }),
      (a.prototype._doAdd = function (e, t, r) {
        var n = e.getItemLayout(t);
        if (on(n)) {
          var i = new this._LineCtor(e, t, r);
          e.setItemGraphicEl(t, i), this.group.add(i);
        }
      }),
      (a.prototype._doUpdate = function (e, t, r, n, i) {
        var o = e.getItemGraphicEl(r);
        if (!on(t.getItemLayout(n))) {
          this.group.remove(o);
          return;
        }
        o ? o.updateData(t, n, i) : (o = new this._LineCtor(t, n, i)),
          t.setItemGraphicEl(n, o),
          this.group.add(o);
      }),
      a
    );
  })();
function ty(a) {
  return a.animators && a.animators.length > 0;
}
function Gs(a) {
  var e = a.hostModel,
    t = e.getModel("emphasis");
  return {
    lineStyle: e.getModel("lineStyle").getLineStyle(),
    emphasisLineStyle: t.getModel(["lineStyle"]).getLineStyle(),
    blurLineStyle: e.getModel(["blur", "lineStyle"]).getLineStyle(),
    selectLineStyle: e.getModel(["select", "lineStyle"]).getLineStyle(),
    emphasisDisabled: t.get("disabled"),
    blurScope: t.get("blurScope"),
    focus: t.get("focus"),
    labelStatesModels: Pt(e)
  };
}
function zs(a) {
  return isNaN(a[0]) || isNaN(a[1]);
}
function on(a) {
  return a && !zs(a[0]) && !zs(a[1]);
}
var sn = [],
  ln = [],
  un = [],
  Be = wu,
  vn = hh,
  Os = Math.abs;
function Bs(a, e, t) {
  for (
    var r = a[0], n = a[1], i = a[2], o = 1 / 0, s, l = t * t, u = 0.1, v = 0.1;
    v <= 0.9;
    v += 0.1
  ) {
    (sn[0] = Be(r[0], n[0], i[0], v)), (sn[1] = Be(r[1], n[1], i[1], v));
    var c = Os(vn(sn, e) - l);
    c < o && ((o = c), (s = v));
  }
  for (var h = 0; h < 32; h++) {
    var f = s + u;
    (ln[0] = Be(r[0], n[0], i[0], s)),
      (ln[1] = Be(r[1], n[1], i[1], s)),
      (un[0] = Be(r[0], n[0], i[0], f)),
      (un[1] = Be(r[1], n[1], i[1], f));
    var c = vn(ln, e) - l;
    if (Os(c) < 0.01) break;
    var p = vn(un, e) - l;
    (u /= 2),
      c < 0
        ? p >= 0
          ? (s = s + u)
          : (s = s - u)
        : p >= 0
        ? (s = s - u)
        : (s = s + u);
  }
  return s;
}
function cn(a, e) {
  var t = [],
    r = ch,
    n = [[], [], []],
    i = [[], []],
    o = [];
  (e /= 2),
    a.eachEdge(function (s, l) {
      var u = s.getLayout(),
        v = s.getVisual("fromSymbol"),
        c = s.getVisual("toSymbol");
      u.__original ||
        ((u.__original = [ye(u[0]), ye(u[1])]),
        u[2] && u.__original.push(ye(u[2])));
      var h = u.__original;
      if (u[2] != null) {
        if (
          (It(n[0], h[0]), It(n[1], h[2]), It(n[2], h[1]), v && v !== "none")
        ) {
          var f = pr(s.node1),
            p = Bs(n, h[0], f * e);
          r(n[0][0], n[1][0], n[2][0], p, t),
            (n[0][0] = t[3]),
            (n[1][0] = t[4]),
            r(n[0][1], n[1][1], n[2][1], p, t),
            (n[0][1] = t[3]),
            (n[1][1] = t[4]);
        }
        if (c && c !== "none") {
          var f = pr(s.node2),
            p = Bs(n, h[1], f * e);
          r(n[0][0], n[1][0], n[2][0], p, t),
            (n[1][0] = t[1]),
            (n[2][0] = t[2]),
            r(n[0][1], n[1][1], n[2][1], p, t),
            (n[1][1] = t[1]),
            (n[2][1] = t[2]);
        }
        It(u[0], n[0]), It(u[1], n[2]), It(u[2], n[1]);
      } else {
        if (
          (It(i[0], h[0]),
          It(i[1], h[1]),
          $e(o, i[1], i[0]),
          Rr(o, o),
          v && v !== "none")
        ) {
          var f = pr(s.node1);
          Bn(i[0], i[0], o, f * e);
        }
        if (c && c !== "none") {
          var f = pr(s.node2);
          Bn(i[1], i[1], o, -f * e);
        }
        It(u[0], i[0]), It(u[1], i[1]);
      }
    });
}
function Fs(a) {
  return a.type === "view";
}
var ey = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), t;
  }
  return (
    (e.prototype.init = function (t, r) {
      var n = new Aa(),
        i = new Xi(),
        o = this.group;
      (this._controller = new Ca(r.getZr())),
        (this._controllerHost = { target: o }),
        o.add(n.group),
        o.add(i.group),
        (this._symbolDraw = n),
        (this._lineDraw = i),
        (this._firstRender = !0);
    }),
    (e.prototype.render = function (t, r, n) {
      var i = this,
        o = t.coordinateSystem;
      this._model = t;
      var s = this._symbolDraw,
        l = this._lineDraw,
        u = this.group;
      if (Fs(o)) {
        var v = { x: o.x, y: o.y, scaleX: o.scaleX, scaleY: o.scaleY };
        this._firstRender ? u.attr(v) : lt(u, v, t);
      }
      cn(t.getGraph(), fr(t));
      var c = t.getData();
      s.updateData(c);
      var h = t.getEdgeData();
      l.updateData(h),
        this._updateNodeAndLinkScale(),
        this._updateController(t, r, n),
        clearTimeout(this._layoutTimeout);
      var f = t.forceLayout,
        p = t.get(["force", "layoutAnimation"]);
      f && this._startForceLayoutIteration(f, p);
      var d = t.get("layout");
      c.graph.eachNode(function (m) {
        var x = m.dataIndex,
          b = m.getGraphicEl(),
          _ = m.getModel();
        if (b) {
          b.off("drag").off("dragend");
          var w = _.get("draggable");
          w &&
            b
              .on("drag", function (I) {
                switch (d) {
                  case "force":
                    f.warmUp(),
                      !i._layouting && i._startForceLayoutIteration(f, p),
                      f.setFixed(x),
                      c.setItemLayout(x, [b.x, b.y]);
                    break;
                  case "circular":
                    c.setItemLayout(x, [b.x, b.y]),
                      m.setLayout({ fixed: !0 }, !0),
                      Yi(t, "symbolSize", m, [I.offsetX, I.offsetY]),
                      i.updateLayout(t);
                    break;
                  case "none":
                  default:
                    c.setItemLayout(x, [b.x, b.y]),
                      Ui(t.getGraph(), t),
                      i.updateLayout(t);
                    break;
                }
              })
              .on("dragend", function () {
                f && f.setUnfixed(x);
              }),
            b.setDraggable(w, !!_.get("cursor"));
          var A = _.get(["emphasis", "focus"]);
          A === "adjacency" && (nt(b).focus = m.getAdjacentDataIndices());
        }
      }),
        c.graph.eachEdge(function (m) {
          var x = m.getGraphicEl(),
            b = m.getModel().get(["emphasis", "focus"]);
          x &&
            b === "adjacency" &&
            (nt(x).focus = {
              edge: [m.dataIndex],
              node: [m.node1.dataIndex, m.node2.dataIndex]
            });
        });
      var g =
          t.get("layout") === "circular" && t.get(["circular", "rotateLabel"]),
        y = c.getLayout("cx"),
        S = c.getLayout("cy");
      c.graph.eachNode(function (m) {
        Sv(m, g, y, S);
      }),
        (this._firstRender = !1);
    }),
    (e.prototype.dispose = function () {
      this.remove(),
        this._controller && this._controller.dispose(),
        (this._controllerHost = null);
    }),
    (e.prototype._startForceLayoutIteration = function (t, r) {
      var n = this;
      (function i() {
        t.step(function (o) {
          n.updateLayout(n._model),
            (n._layouting = !o) &&
              (r ? (n._layoutTimeout = setTimeout(i, 16)) : i());
        });
      })();
    }),
    (e.prototype._updateController = function (t, r, n) {
      var i = this,
        o = this._controller,
        s = this._controllerHost,
        l = this.group;
      if (
        (o.setPointerChecker(function (u, v, c) {
          var h = l.getBoundingRect();
          return h.applyTransform(l.transform), h.contain(v, c) && !hi(u, n, t);
        }),
        !Fs(t.coordinateSystem))
      ) {
        o.disable();
        return;
      }
      o.enable(t.get("roam")),
        (s.zoomLimit = t.get("scaleLimit")),
        (s.zoom = t.coordinateSystem.getZoom()),
        o
          .off("pan")
          .off("zoom")
          .on("pan", function (u) {
            ki(s, u.dx, u.dy),
              n.dispatchAction({
                seriesId: t.id,
                type: "graphRoam",
                dx: u.dx,
                dy: u.dy
              });
          })
          .on("zoom", function (u) {
            Vi(s, u.scale, u.originX, u.originY),
              n.dispatchAction({
                seriesId: t.id,
                type: "graphRoam",
                zoom: u.scale,
                originX: u.originX,
                originY: u.originY
              }),
              i._updateNodeAndLinkScale(),
              cn(t.getGraph(), fr(t)),
              i._lineDraw.updateLayout(),
              n.updateLabelLayout();
          });
    }),
    (e.prototype._updateNodeAndLinkScale = function () {
      var t = this._model,
        r = t.getData(),
        n = fr(t);
      r.eachItemGraphicEl(function (i, o) {
        i && i.setSymbolScale(n);
      });
    }),
    (e.prototype.updateLayout = function (t) {
      cn(t.getGraph(), fr(t)),
        this._symbolDraw.updateLayout(),
        this._lineDraw.updateLayout();
    }),
    (e.prototype.remove = function () {
      clearTimeout(this._layoutTimeout),
        (this._layouting = !1),
        (this._layoutTimeout = null),
        this._symbolDraw && this._symbolDraw.remove(),
        this._lineDraw && this._lineDraw.remove();
    }),
    (e.type = "graph"),
    e
  );
})(pt);
function Fe(a) {
  return "_EC_" + a;
}
var ry = (function () {
    function a(e) {
      (this.type = "graph"),
        (this.nodes = []),
        (this.edges = []),
        (this._nodesMap = {}),
        (this._edgesMap = {}),
        (this._directed = e || !1);
    }
    return (
      (a.prototype.isDirected = function () {
        return this._directed;
      }),
      (a.prototype.addNode = function (e, t) {
        e = e == null ? "" + t : "" + e;
        var r = this._nodesMap;
        if (!r[Fe(e)]) {
          var n = new Pe(e, t);
          return (n.hostGraph = this), this.nodes.push(n), (r[Fe(e)] = n), n;
        }
      }),
      (a.prototype.getNodeByIndex = function (e) {
        var t = this.data.getRawIndex(e);
        return this.nodes[t];
      }),
      (a.prototype.getNodeById = function (e) {
        return this._nodesMap[Fe(e)];
      }),
      (a.prototype.addEdge = function (e, t, r) {
        var n = this._nodesMap,
          i = this._edgesMap;
        if (
          (Qt(e) && (e = this.nodes[e]),
          Qt(t) && (t = this.nodes[t]),
          e instanceof Pe || (e = n[Fe(e)]),
          t instanceof Pe || (t = n[Fe(t)]),
          !(!e || !t))
        ) {
          var o = e.id + "-" + t.id,
            s = new bv(e, t, r);
          return (
            (s.hostGraph = this),
            this._directed && (e.outEdges.push(s), t.inEdges.push(s)),
            e.edges.push(s),
            e !== t && t.edges.push(s),
            this.edges.push(s),
            (i[o] = s),
            s
          );
        }
      }),
      (a.prototype.getEdgeByIndex = function (e) {
        var t = this.edgeData.getRawIndex(e);
        return this.edges[t];
      }),
      (a.prototype.getEdge = function (e, t) {
        e instanceof Pe && (e = e.id), t instanceof Pe && (t = t.id);
        var r = this._edgesMap;
        return this._directed
          ? r[e + "-" + t]
          : r[e + "-" + t] || r[t + "-" + e];
      }),
      (a.prototype.eachNode = function (e, t) {
        for (var r = this.nodes, n = r.length, i = 0; i < n; i++)
          r[i].dataIndex >= 0 && e.call(t, r[i], i);
      }),
      (a.prototype.eachEdge = function (e, t) {
        for (var r = this.edges, n = r.length, i = 0; i < n; i++)
          r[i].dataIndex >= 0 &&
            r[i].node1.dataIndex >= 0 &&
            r[i].node2.dataIndex >= 0 &&
            e.call(t, r[i], i);
      }),
      (a.prototype.breadthFirstTraverse = function (e, t, r, n) {
        if ((t instanceof Pe || (t = this._nodesMap[Fe(t)]), !!t)) {
          for (
            var i = r === "out" ? "outEdges" : r === "in" ? "inEdges" : "edges",
              o = 0;
            o < this.nodes.length;
            o++
          )
            this.nodes[o].__visited = !1;
          if (!e.call(n, t, null))
            for (var s = [t]; s.length; )
              for (var l = s.shift(), u = l[i], o = 0; o < u.length; o++) {
                var v = u[o],
                  c = v.node1 === l ? v.node2 : v.node1;
                if (!c.__visited) {
                  if (e.call(n, c, l)) return;
                  s.push(c), (c.__visited = !0);
                }
              }
        }
      }),
      (a.prototype.update = function () {
        for (
          var e = this.data,
            t = this.edgeData,
            r = this.nodes,
            n = this.edges,
            i = 0,
            o = r.length;
          i < o;
          i++
        )
          r[i].dataIndex = -1;
        for (var i = 0, o = e.count(); i < o; i++)
          r[e.getRawIndex(i)].dataIndex = i;
        t.filterSelf(function (s) {
          var l = n[t.getRawIndex(s)];
          return l.node1.dataIndex >= 0 && l.node2.dataIndex >= 0;
        });
        for (var i = 0, o = n.length; i < o; i++) n[i].dataIndex = -1;
        for (var i = 0, o = t.count(); i < o; i++)
          n[t.getRawIndex(i)].dataIndex = i;
      }),
      (a.prototype.clone = function () {
        for (
          var e = new a(this._directed), t = this.nodes, r = this.edges, n = 0;
          n < t.length;
          n++
        )
          e.addNode(t[n].id, t[n].dataIndex);
        for (var n = 0; n < r.length; n++) {
          var i = r[n];
          e.addEdge(i.node1.id, i.node2.id, i.dataIndex);
        }
        return e;
      }),
      a
    );
  })(),
  Pe = (function () {
    function a(e, t) {
      (this.inEdges = []),
        (this.outEdges = []),
        (this.edges = []),
        (this.dataIndex = -1),
        (this.id = e ?? ""),
        (this.dataIndex = t ?? -1);
    }
    return (
      (a.prototype.degree = function () {
        return this.edges.length;
      }),
      (a.prototype.inDegree = function () {
        return this.inEdges.length;
      }),
      (a.prototype.outDegree = function () {
        return this.outEdges.length;
      }),
      (a.prototype.getModel = function (e) {
        if (!(this.dataIndex < 0)) {
          var t = this.hostGraph,
            r = t.data.getItemModel(this.dataIndex);
          return r.getModel(e);
        }
      }),
      (a.prototype.getAdjacentDataIndices = function () {
        for (
          var e = { edge: [], node: [] }, t = 0;
          t < this.edges.length;
          t++
        ) {
          var r = this.edges[t];
          r.dataIndex < 0 ||
            (e.edge.push(r.dataIndex),
            e.node.push(r.node1.dataIndex, r.node2.dataIndex));
        }
        return e;
      }),
      (a.prototype.getTrajectoryDataIndices = function () {
        for (var e = tt(), t = tt(), r = 0; r < this.edges.length; r++) {
          var n = this.edges[r];
          if (!(n.dataIndex < 0)) {
            e.set(n.dataIndex, !0);
            for (var i = [n.node1], o = [n.node2], s = 0; s < i.length; ) {
              var l = i[s];
              s++, t.set(l.dataIndex, !0);
              for (var u = 0; u < l.inEdges.length; u++)
                e.set(l.inEdges[u].dataIndex, !0), i.push(l.inEdges[u].node1);
            }
            for (s = 0; s < o.length; ) {
              var v = o[s];
              s++, t.set(v.dataIndex, !0);
              for (var u = 0; u < v.outEdges.length; u++)
                e.set(v.outEdges[u].dataIndex, !0), o.push(v.outEdges[u].node2);
            }
          }
        }
        return { edge: e.keys(), node: t.keys() };
      }),
      a
    );
  })(),
  bv = (function () {
    function a(e, t, r) {
      (this.dataIndex = -1),
        (this.node1 = e),
        (this.node2 = t),
        (this.dataIndex = r ?? -1);
    }
    return (
      (a.prototype.getModel = function (e) {
        if (!(this.dataIndex < 0)) {
          var t = this.hostGraph,
            r = t.edgeData.getItemModel(this.dataIndex);
          return r.getModel(e);
        }
      }),
      (a.prototype.getAdjacentDataIndices = function () {
        return {
          edge: [this.dataIndex],
          node: [this.node1.dataIndex, this.node2.dataIndex]
        };
      }),
      (a.prototype.getTrajectoryDataIndices = function () {
        var e = tt(),
          t = tt();
        e.set(this.dataIndex, !0);
        for (var r = [this.node1], n = [this.node2], i = 0; i < r.length; ) {
          var o = r[i];
          i++, t.set(o.dataIndex, !0);
          for (var s = 0; s < o.inEdges.length; s++)
            e.set(o.inEdges[s].dataIndex, !0), r.push(o.inEdges[s].node1);
        }
        for (i = 0; i < n.length; ) {
          var l = n[i];
          i++, t.set(l.dataIndex, !0);
          for (var s = 0; s < l.outEdges.length; s++)
            e.set(l.outEdges[s].dataIndex, !0), n.push(l.outEdges[s].node2);
        }
        return { edge: e.keys(), node: t.keys() };
      }),
      a
    );
  })();
function _v(a, e) {
  return {
    getValue: function (t) {
      var r = this[a][e];
      return r
        .getStore()
        .get(r.getDimensionIndex(t || "value"), this.dataIndex);
    },
    setVisual: function (t, r) {
      this.dataIndex >= 0 && this[a][e].setItemVisual(this.dataIndex, t, r);
    },
    getVisual: function (t) {
      return this[a][e].getItemVisual(this.dataIndex, t);
    },
    setLayout: function (t, r) {
      this.dataIndex >= 0 && this[a][e].setItemLayout(this.dataIndex, t, r);
    },
    getLayout: function () {
      return this[a][e].getItemLayout(this.dataIndex);
    },
    getGraphicEl: function () {
      return this[a][e].getItemGraphicEl(this.dataIndex);
    },
    getRawIndex: function () {
      return this[a][e].getRawIndex(this.dataIndex);
    }
  };
}
te(Pe, _v("hostGraph", "data"));
te(bv, _v("hostGraph", "edgeData"));
function wv(a, e, t, r, n) {
  for (var i = new ry(r), o = 0; o < a.length; o++)
    i.addNode(Ft(a[o].id, a[o].name, o), o);
  for (var s = [], l = [], u = 0, o = 0; o < e.length; o++) {
    var v = e[o],
      c = v.source,
      h = v.target;
    i.addEdge(c, h, u) &&
      (l.push(v), s.push(Ft(Mr(v.id, null), c + " > " + h)), u++);
  }
  var f = t.get("coordinateSystem"),
    p;
  if (f === "cartesian2d" || f === "polar") p = Je(a, t);
  else {
    var d = Au.get(f),
      g = d ? d.dimensions || [] : [];
    Ht(g, "value") < 0 && g.concat(["value"]);
    var y = di(a, {
      coordDimensions: g,
      encodeDefine: t.getEncode()
    }).dimensions;
    (p = new Vt(y, t)), p.initData(a);
  }
  var S = new Vt(["value"], t);
  return (
    S.initData(l, s),
    n && n(p, S),
    iv({
      mainData: p,
      struct: i,
      structAttr: "graph",
      datas: { node: p, edge: S },
      datasAttr: { node: "data", edge: "edgeData" }
    }),
    i.update(),
    i
  );
}
var ay = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.hasSymbolVisual = !0), t;
    }
    return (
      (e.prototype.init = function (t) {
        a.prototype.init.apply(this, arguments);
        var r = this;
        function n() {
          return r._categoriesData;
        }
        (this.legendVisualProvider = new Ta(n, n)),
          this.fillDataTextStyle(t.edges || t.links),
          this._updateCategoriesData();
      }),
      (e.prototype.mergeOption = function (t) {
        a.prototype.mergeOption.apply(this, arguments),
          this.fillDataTextStyle(t.edges || t.links),
          this._updateCategoriesData();
      }),
      (e.prototype.mergeDefaultAndTheme = function (t) {
        a.prototype.mergeDefaultAndTheme.apply(this, arguments),
          Pa(t, "edgeLabel", ["show"]);
      }),
      (e.prototype.getInitialData = function (t, r) {
        var n = t.edges || t.links || [],
          i = t.data || t.nodes || [],
          o = this;
        if (i && n) {
          Hg(this);
          var s = wv(i, n, this, !0, l);
          return (
            L(
              s.edges,
              function (u) {
                Ug(u.node1, u.node2, this, u.dataIndex);
              },
              this
            ),
            s.data
          );
        }
        function l(u, v) {
          u.wrapMethod("getItemModel", function (p) {
            var d = o._categoriesModels,
              g = p.getShallow("category"),
              y = d[g];
            return (
              y && ((y.parentModel = p.parentModel), (p.parentModel = y)), p
            );
          });
          var c = $t.prototype.getModel;
          function h(p, d) {
            var g = c.call(this, p, d);
            return (g.resolveParentPath = f), g;
          }
          v.wrapMethod("getItemModel", function (p) {
            return (p.resolveParentPath = f), (p.getModel = h), p;
          });
          function f(p) {
            if (p && (p[0] === "label" || p[1] === "label")) {
              var d = p.slice();
              return (
                p[0] === "label"
                  ? (d[0] = "edgeLabel")
                  : p[1] === "label" && (d[1] = "edgeLabel"),
                d
              );
            }
            return p;
          }
        }
      }),
      (e.prototype.getGraph = function () {
        return this.getData().graph;
      }),
      (e.prototype.getEdgeData = function () {
        return this.getGraph().edgeData;
      }),
      (e.prototype.getCategoriesData = function () {
        return this._categoriesData;
      }),
      (e.prototype.formatTooltip = function (t, r, n) {
        if (n === "edge") {
          var i = this.getData(),
            o = this.getDataParams(t, n),
            s = i.graph.getEdgeByIndex(t),
            l = i.getName(s.node1.dataIndex),
            u = i.getName(s.node2.dataIndex),
            v = [];
          return (
            l != null && v.push(l),
            u != null && v.push(u),
            Mt("nameValue", {
              name: v.join(" > "),
              value: o.value,
              noValue: o.value == null
            })
          );
        }
        var c = fh({ series: this, dataIndex: t, multipleSeries: r });
        return c;
      }),
      (e.prototype._updateCategoriesData = function () {
        var t = O(this.option.categories || [], function (n) {
            return n.value != null ? n : W({ value: 0 }, n);
          }),
          r = new Vt(["value"], this);
        r.initData(t),
          (this._categoriesData = r),
          (this._categoriesModels = r.mapArray(function (n) {
            return r.getItemModel(n);
          }));
      }),
      (e.prototype.setZoom = function (t) {
        this.option.zoom = t;
      }),
      (e.prototype.setCenter = function (t) {
        this.option.center = t;
      }),
      (e.prototype.isAnimationEnabled = function () {
        return (
          a.prototype.isAnimationEnabled.call(this) &&
          !(
            this.get("layout") === "force" &&
            this.get(["force", "layoutAnimation"])
          )
        );
      }),
      (e.type = "series.graph"),
      (e.dependencies = ["grid", "polar", "geo", "singleAxis", "calendar"]),
      (e.defaultOption = {
        z: 2,
        coordinateSystem: "view",
        legendHoverLink: !0,
        layout: null,
        circular: { rotateLabel: !1 },
        force: {
          initLayout: null,
          repulsion: [0, 50],
          gravity: 0.1,
          friction: 0.6,
          edgeLength: 30,
          layoutAnimation: !0
        },
        left: "center",
        top: "center",
        symbol: "circle",
        symbolSize: 10,
        edgeSymbol: ["none", "none"],
        edgeSymbolSize: 10,
        edgeLabel: { position: "middle", distance: 5 },
        draggable: !1,
        roam: !1,
        center: null,
        zoom: 1,
        nodeScaleRatio: 0.6,
        label: { show: !1, formatter: "{b}" },
        itemStyle: {},
        lineStyle: { color: "#aaa", width: 1, opacity: 0.5 },
        emphasis: { scale: !0, label: { show: !0 } },
        select: { itemStyle: { borderColor: "#212121" } }
      }),
      e
    );
  })(gt),
  ny = { type: "graphRoam", event: "graphRoam", update: "none" };
function iy(a) {
  a.registerChartView(ey),
    a.registerSeriesModel(ay),
    a.registerProcessor(zg),
    a.registerVisual(Og),
    a.registerVisual(Bg),
    a.registerLayout(Yg),
    a.registerLayout(a.PRIORITY.VISUAL.POST_CHART_LAYOUT, Xg),
    a.registerLayout(qg),
    a.registerCoordinateSystem("graphView", {
      dimensions: Vr.dimensions,
      create: jg
    }),
    a.registerAction(
      {
        type: "focusNodeAdjacency",
        event: "focusNodeAdjacency",
        update: "series:focusNodeAdjacency"
      },
      qe
    ),
    a.registerAction(
      {
        type: "unfocusNodeAdjacency",
        event: "unfocusNodeAdjacency",
        update: "series:unfocusNodeAdjacency"
      },
      qe
    ),
    a.registerAction(ny, function (e, t, r) {
      t.eachComponent({ mainType: "series", query: e }, function (n) {
        var i = n.coordinateSystem,
          o = zi(i, e, void 0, r);
        n.setCenter && n.setCenter(o.center), n.setZoom && n.setZoom(o.zoom);
      });
    });
}
var oy = (function () {
    function a() {
      (this.angle = 0),
        (this.width = 10),
        (this.r = 10),
        (this.x = 0),
        (this.y = 0);
    }
    return a;
  })(),
  sy = (function (a) {
    k(e, a);
    function e(t) {
      var r = a.call(this, t) || this;
      return (r.type = "pointer"), r;
    }
    return (
      (e.prototype.getDefaultShape = function () {
        return new oy();
      }),
      (e.prototype.buildPath = function (t, r) {
        var n = Math.cos,
          i = Math.sin,
          o = r.r,
          s = r.width,
          l = r.angle,
          u = r.x - n(l) * s * (s >= o / 3 ? 1 : 2),
          v = r.y - i(l) * s * (s >= o / 3 ? 1 : 2);
        (l = r.angle - Math.PI / 2),
          t.moveTo(u, v),
          t.lineTo(r.x + n(l) * s, r.y + i(l) * s),
          t.lineTo(r.x + n(r.angle) * o, r.y + i(r.angle) * o),
          t.lineTo(r.x - n(l) * s, r.y - i(l) * s),
          t.lineTo(u, v);
      }),
      e
    );
  })(wt);
function ly(a, e) {
  var t = a.get("center"),
    r = e.getWidth(),
    n = e.getHeight(),
    i = Math.min(r, n),
    o = z(t[0], e.getWidth()),
    s = z(t[1], e.getHeight()),
    l = z(a.get("radius"), i / 2);
  return { cx: o, cy: s, r: l };
}
function Ur(a, e) {
  var t = a == null ? "" : a + "";
  return e && (et(e) ? (t = e.replace("{value}", t)) : it(e) && (t = e(a))), t;
}
var uy = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        this.group.removeAll();
        var i = t.get(["axisLine", "lineStyle", "color"]),
          o = ly(t, n);
        this._renderMain(t, r, n, i, o), (this._data = t.getData());
      }),
      (e.prototype.dispose = function () {}),
      (e.prototype._renderMain = function (t, r, n, i, o) {
        var s = this.group,
          l = t.get("clockwise"),
          u = (-t.get("startAngle") / 180) * Math.PI,
          v = (-t.get("endAngle") / 180) * Math.PI,
          c = t.getModel("axisLine"),
          h = c.get("roundCap"),
          f = h ? bo : Ve,
          p = c.get("show"),
          d = c.getModel("lineStyle"),
          g = d.get("width"),
          y = [u, v];
        ph(y, !l), (u = y[0]), (v = y[1]);
        for (var S = v - u, m = u, x = [], b = 0; p && b < i.length; b++) {
          var _ = Math.min(Math.max(i[b][0], 0), 1);
          v = u + S * _;
          var w = new f({
            shape: {
              startAngle: m,
              endAngle: v,
              cx: o.cx,
              cy: o.cy,
              clockwise: l,
              r0: o.r - g,
              r: o.r
            },
            silent: !0
          });
          w.setStyle({ fill: i[b][1] }),
            w.setStyle(d.getLineStyle(["color", "width"])),
            x.push(w),
            (m = v);
        }
        x.reverse(),
          L(x, function (I) {
            return s.add(I);
          });
        var A = function (I) {
          if (I <= 0) return i[0][1];
          var T;
          for (T = 0; T < i.length; T++)
            if (i[T][0] >= I && (T === 0 ? 0 : i[T - 1][0]) < I) return i[T][1];
          return i[T - 1][1];
        };
        this._renderTicks(t, r, n, A, o, u, v, l, g),
          this._renderTitleAndDetail(t, r, n, A, o),
          this._renderAnchor(t, o),
          this._renderPointer(t, r, n, A, o, u, v, l, g);
      }),
      (e.prototype._renderTicks = function (t, r, n, i, o, s, l, u, v) {
        for (
          var c = this.group,
            h = o.cx,
            f = o.cy,
            p = o.r,
            d = +t.get("min"),
            g = +t.get("max"),
            y = t.getModel("splitLine"),
            S = t.getModel("axisTick"),
            m = t.getModel("axisLabel"),
            x = t.get("splitNumber"),
            b = S.get("splitNumber"),
            _ = z(y.get("length"), p),
            w = z(S.get("length"), p),
            A = s,
            I = (l - s) / x,
            T = I / b,
            D = y.getModel("lineStyle").getLineStyle(),
            M = S.getModel("lineStyle").getLineStyle(),
            C = y.get("distance"),
            P,
            E,
            R = 0;
          R <= x;
          R++
        ) {
          if (((P = Math.cos(A)), (E = Math.sin(A)), y.get("show"))) {
            var N = C ? C + v : v,
              V = new jt({
                shape: {
                  x1: P * (p - N) + h,
                  y1: E * (p - N) + f,
                  x2: P * (p - _ - N) + h,
                  y2: E * (p - _ - N) + f
                },
                style: D,
                silent: !0
              });
            D.stroke === "auto" && V.setStyle({ stroke: i(R / x) }), c.add(V);
          }
          if (m.get("show")) {
            var N = m.get("distance") + C,
              G = Ur(gi((R / x) * (g - d) + d), m.get("formatter")),
              B = i(R / x),
              F = P * (p - _ - N) + h,
              Y = E * (p - _ - N) + f,
              K = m.get("rotate"),
              $ = 0;
            K === "radial"
              ? (($ = -A + 2 * Math.PI), $ > Math.PI / 2 && ($ += Math.PI))
              : K === "tangential"
              ? ($ = -A - Math.PI / 2)
              : Qt(K) && ($ = (K * Math.PI) / 180),
              $ === 0
                ? c.add(
                    new kt({
                      style: xt(
                        m,
                        {
                          text: G,
                          x: F,
                          y: Y,
                          verticalAlign:
                            E < -0.8 ? "top" : E > 0.8 ? "bottom" : "middle",
                          align:
                            P < -0.4 ? "left" : P > 0.4 ? "right" : "center"
                        },
                        { inheritColor: B }
                      ),
                      silent: !0
                    })
                  )
                : c.add(
                    new kt({
                      style: xt(
                        m,
                        {
                          text: G,
                          x: F,
                          y: Y,
                          verticalAlign: "middle",
                          align: "center"
                        },
                        { inheritColor: B }
                      ),
                      silent: !0,
                      originX: F,
                      originY: Y,
                      rotation: $
                    })
                  );
          }
          if (S.get("show") && R !== x) {
            var N = S.get("distance");
            N = N ? N + v : v;
            for (var j = 0; j <= b; j++) {
              (P = Math.cos(A)), (E = Math.sin(A));
              var yt = new jt({
                shape: {
                  x1: P * (p - N) + h,
                  y1: E * (p - N) + f,
                  x2: P * (p - w - N) + h,
                  y2: E * (p - w - N) + f
                },
                silent: !0,
                style: M
              });
              M.stroke === "auto" &&
                yt.setStyle({ stroke: i((R + j / b) / x) }),
                c.add(yt),
                (A += T);
            }
            A -= T;
          } else A += I;
        }
      }),
      (e.prototype._renderPointer = function (t, r, n, i, o, s, l, u, v) {
        var c = this.group,
          h = this._data,
          f = this._progressEls,
          p = [],
          d = t.get(["pointer", "show"]),
          g = t.getModel("progress"),
          y = g.get("show"),
          S = t.getData(),
          m = S.mapDimension("value"),
          x = +t.get("min"),
          b = +t.get("max"),
          _ = [x, b],
          w = [s, l];
        function A(T, D) {
          var M = S.getItemModel(T),
            C = M.getModel("pointer"),
            P = z(C.get("width"), o.r),
            E = z(C.get("length"), o.r),
            R = t.get(["pointer", "icon"]),
            N = C.get("offsetCenter"),
            V = z(N[0], o.r),
            G = z(N[1], o.r),
            B = C.get("keepAspect"),
            F;
          return (
            R
              ? (F = ie(R, V - P / 2, G - E, P, E, null, B))
              : (F = new sy({
                  shape: { angle: -Math.PI / 2, width: P, r: E, x: V, y: G }
                })),
            (F.rotation = -(D + Math.PI / 2)),
            (F.x = o.cx),
            (F.y = o.cy),
            F
          );
        }
        function I(T, D) {
          var M = g.get("roundCap"),
            C = M ? bo : Ve,
            P = g.get("overlap"),
            E = P ? g.get("width") : v / S.count(),
            R = P ? o.r - E : o.r - (T + 1) * E,
            N = P ? o.r : o.r - T * E,
            V = new C({
              shape: {
                startAngle: s,
                endAngle: D,
                cx: o.cx,
                cy: o.cy,
                clockwise: u,
                r0: R,
                r: N
              }
            });
          return P && (V.z2 = qt(S.get(m, T), [x, b], [100, 0], !0)), V;
        }
        (y || d) &&
          (S.diff(h)
            .add(function (T) {
              var D = S.get(m, T);
              if (d) {
                var M = A(T, s);
                Ct(
                  M,
                  {
                    rotation: -(
                      (isNaN(+D) ? w[0] : qt(D, _, w, !0)) +
                      Math.PI / 2
                    )
                  },
                  t
                ),
                  c.add(M),
                  S.setItemGraphicEl(T, M);
              }
              if (y) {
                var C = I(T, s),
                  P = g.get("clip");
                Ct(C, { shape: { endAngle: qt(D, _, w, P) } }, t),
                  c.add(C),
                  _o(t.seriesIndex, S.dataType, T, C),
                  (p[T] = C);
              }
            })
            .update(function (T, D) {
              var M = S.get(m, T);
              if (d) {
                var C = h.getItemGraphicEl(D),
                  P = C ? C.rotation : s,
                  E = A(T, P);
                (E.rotation = P),
                  lt(
                    E,
                    {
                      rotation: -(
                        (isNaN(+M) ? w[0] : qt(M, _, w, !0)) +
                        Math.PI / 2
                      )
                    },
                    t
                  ),
                  c.add(E),
                  S.setItemGraphicEl(T, E);
              }
              if (y) {
                var R = f[D],
                  N = R ? R.shape.endAngle : s,
                  V = I(T, N),
                  G = g.get("clip");
                lt(V, { shape: { endAngle: qt(M, _, w, G) } }, t),
                  c.add(V),
                  _o(t.seriesIndex, S.dataType, T, V),
                  (p[T] = V);
              }
            })
            .execute(),
          S.each(function (T) {
            var D = S.getItemModel(T),
              M = D.getModel("emphasis"),
              C = M.get("focus"),
              P = M.get("blurScope"),
              E = M.get("disabled");
            if (d) {
              var R = S.getItemGraphicEl(T),
                N = S.getItemVisual(T, "style"),
                V = N.fill;
              if (R instanceof _e) {
                var G = R.style;
                R.useStyle(
                  W(
                    {
                      image: G.image,
                      x: G.x,
                      y: G.y,
                      width: G.width,
                      height: G.height
                    },
                    N
                  )
                );
              } else R.useStyle(N), R.type !== "pointer" && R.setColor(V);
              R.setStyle(D.getModel(["pointer", "itemStyle"]).getItemStyle()),
                R.style.fill === "auto" &&
                  R.setStyle("fill", i(qt(S.get(m, T), _, [0, 1], !0))),
                (R.z2EmphasisLift = 0),
                zt(R, D),
                ct(R, C, P, E);
            }
            if (y) {
              var B = p[T];
              B.useStyle(S.getItemVisual(T, "style")),
                B.setStyle(
                  D.getModel(["progress", "itemStyle"]).getItemStyle()
                ),
                (B.z2EmphasisLift = 0),
                zt(B, D),
                ct(B, C, P, E);
            }
          }),
          (this._progressEls = p));
      }),
      (e.prototype._renderAnchor = function (t, r) {
        var n = t.getModel("anchor"),
          i = n.get("show");
        if (i) {
          var o = n.get("size"),
            s = n.get("icon"),
            l = n.get("offsetCenter"),
            u = n.get("keepAspect"),
            v = ie(
              s,
              r.cx - o / 2 + z(l[0], r.r),
              r.cy - o / 2 + z(l[1], r.r),
              o,
              o,
              null,
              u
            );
          (v.z2 = n.get("showAbove") ? 1 : 0),
            v.setStyle(n.getModel("itemStyle").getItemStyle()),
            this.group.add(v);
        }
      }),
      (e.prototype._renderTitleAndDetail = function (t, r, n, i, o) {
        var s = this,
          l = t.getData(),
          u = l.mapDimension("value"),
          v = +t.get("min"),
          c = +t.get("max"),
          h = new Z(),
          f = [],
          p = [],
          d = t.isAnimationEnabled(),
          g = t.get(["pointer", "showAbove"]);
        l
          .diff(this._data)
          .add(function (y) {
            (f[y] = new kt({ silent: !0 })), (p[y] = new kt({ silent: !0 }));
          })
          .update(function (y, S) {
            (f[y] = s._titleEls[S]), (p[y] = s._detailEls[S]);
          })
          .execute(),
          l.each(function (y) {
            var S = l.getItemModel(y),
              m = l.get(u, y),
              x = new Z(),
              b = i(qt(m, [v, c], [0, 1], !0)),
              _ = S.getModel("title");
            if (_.get("show")) {
              var w = _.get("offsetCenter"),
                A = o.cx + z(w[0], o.r),
                I = o.cy + z(w[1], o.r),
                T = f[y];
              T.attr({
                z2: g ? 0 : 2,
                style: xt(
                  _,
                  {
                    x: A,
                    y: I,
                    text: l.getName(y),
                    align: "center",
                    verticalAlign: "middle"
                  },
                  { inheritColor: b }
                )
              }),
                x.add(T);
            }
            var D = S.getModel("detail");
            if (D.get("show")) {
              var M = D.get("offsetCenter"),
                C = o.cx + z(M[0], o.r),
                P = o.cy + z(M[1], o.r),
                E = z(D.get("width"), o.r),
                R = z(D.get("height"), o.r),
                N = t.get(["progress", "show"])
                  ? l.getItemVisual(y, "style").fill
                  : b,
                T = p[y],
                V = D.get("formatter");
              T.attr({
                z2: g ? 0 : 2,
                style: xt(
                  D,
                  {
                    x: C,
                    y: P,
                    text: Ur(m, V),
                    width: isNaN(E) ? null : E,
                    height: isNaN(R) ? null : R,
                    align: "center",
                    verticalAlign: "middle"
                  },
                  { inheritColor: N }
                )
              }),
                dh(T, { normal: D }, m, function (B) {
                  return Ur(B, V);
                }),
                d &&
                  iu(T, y, l, t, {
                    getFormattedLabel: function (B, F, Y, K, $, j) {
                      return Ur(j ? j.interpolatedValue : m, V);
                    }
                  }),
                x.add(T);
            }
            h.add(x);
          }),
          this.group.add(h),
          (this._titleEls = f),
          (this._detailEls = p);
      }),
      (e.type = "gauge"),
      e
    );
  })(pt),
  vy = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.visualStyleAccessPath = "itemStyle"), t;
    }
    return (
      (e.prototype.getInitialData = function (t, r) {
        return Lr(this, ["value"]);
      }),
      (e.type = "series.gauge"),
      (e.defaultOption = {
        z: 2,
        colorBy: "data",
        center: ["50%", "50%"],
        legendHoverLink: !0,
        radius: "75%",
        startAngle: 225,
        endAngle: -45,
        clockwise: !0,
        min: 0,
        max: 100,
        splitNumber: 10,
        axisLine: {
          show: !0,
          roundCap: !1,
          lineStyle: { color: [[1, "#E6EBF8"]], width: 10 }
        },
        progress: { show: !1, overlap: !0, width: 10, roundCap: !1, clip: !0 },
        splitLine: {
          show: !0,
          length: 10,
          distance: 10,
          lineStyle: { color: "#63677A", width: 3, type: "solid" }
        },
        axisTick: {
          show: !0,
          splitNumber: 5,
          length: 6,
          distance: 10,
          lineStyle: { color: "#63677A", width: 1, type: "solid" }
        },
        axisLabel: {
          show: !0,
          distance: 15,
          color: "#464646",
          fontSize: 12,
          rotate: 0
        },
        pointer: {
          icon: null,
          offsetCenter: [0, 0],
          show: !0,
          showAbove: !0,
          length: "60%",
          width: 6,
          keepAspect: !1
        },
        anchor: {
          show: !1,
          showAbove: !1,
          size: 6,
          icon: "circle",
          offsetCenter: [0, 0],
          keepAspect: !1,
          itemStyle: { color: "#fff", borderWidth: 0, borderColor: "#5470c6" }
        },
        title: {
          show: !0,
          offsetCenter: [0, "20%"],
          color: "#464646",
          fontSize: 16,
          valueAnimation: !1
        },
        detail: {
          show: !0,
          backgroundColor: "rgba(0,0,0,0)",
          borderWidth: 0,
          borderColor: "#ccc",
          width: 100,
          height: null,
          padding: [5, 10],
          offsetCenter: [0, "40%"],
          color: "#464646",
          fontSize: 30,
          fontWeight: "bold",
          lineHeight: 30,
          valueAnimation: !1
        }
      }),
      e
    );
  })(gt);
function cy(a) {
  a.registerChartView(uy), a.registerSeriesModel(vy);
}
var hy = ["itemStyle", "opacity"],
  fy = (function (a) {
    k(e, a);
    function e(t, r) {
      var n = a.call(this) || this,
        i = n,
        o = new we(),
        s = new kt();
      return (
        i.setTextContent(s), n.setTextGuideLine(o), n.updateData(t, r, !0), n
      );
    }
    return (
      (e.prototype.updateData = function (t, r, n) {
        var i = this,
          o = t.hostModel,
          s = t.getItemModel(r),
          l = t.getItemLayout(r),
          u = s.getModel("emphasis"),
          v = s.get(hy);
        (v = v ?? 1),
          n || Se(i),
          i.useStyle(t.getItemVisual(r, "style")),
          (i.style.lineJoin = "round"),
          n
            ? (i.setShape({ points: l.points }),
              (i.style.opacity = 0),
              Ct(i, { style: { opacity: v } }, o, r))
            : lt(
                i,
                { style: { opacity: v }, shape: { points: l.points } },
                o,
                r
              ),
          zt(i, s),
          this._updateLabel(t, r),
          ct(this, u.get("focus"), u.get("blurScope"), u.get("disabled"));
      }),
      (e.prototype._updateLabel = function (t, r) {
        var n = this,
          i = this.getTextGuideLine(),
          o = n.getTextContent(),
          s = t.hostModel,
          l = t.getItemModel(r),
          u = t.getItemLayout(r),
          v = u.label,
          c = t.getItemVisual(r, "style"),
          h = c.fill;
        Ut(
          o,
          Pt(l),
          {
            labelFetcher: t.hostModel,
            labelDataIndex: r,
            defaultOpacity: c.opacity,
            defaultText: t.getName(r)
          },
          { normal: { align: v.textAlign, verticalAlign: v.verticalAlign } }
        ),
          n.setTextConfig({
            local: !0,
            inside: !!v.inside,
            insideStroke: h,
            outsideFill: h
          });
        var f = v.linePoints;
        i.setShape({ points: f }),
          (n.textGuideLineConfig = {
            anchor: f ? new de(f[0][0], f[0][1]) : null
          }),
          lt(o, { style: { x: v.x, y: v.y } }, s, r),
          o.attr({ rotation: v.rotation, originX: v.x, originY: v.y, z2: 10 }),
          au(n, nu(l), { stroke: h });
      }),
      e
    );
  })(ve),
  py = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.ignoreLabelLineUpdate = !0), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i = t.getData(),
          o = this._data,
          s = this.group;
        i
          .diff(o)
          .add(function (l) {
            var u = new fy(i, l);
            i.setItemGraphicEl(l, u), s.add(u);
          })
          .update(function (l, u) {
            var v = o.getItemGraphicEl(u);
            v.updateData(i, l), s.add(v), i.setItemGraphicEl(l, v);
          })
          .remove(function (l) {
            var u = o.getItemGraphicEl(l);
            gh(u, t, l);
          })
          .execute(),
          (this._data = i);
      }),
      (e.prototype.remove = function () {
        this.group.removeAll(), (this._data = null);
      }),
      (e.prototype.dispose = function () {}),
      (e.type = "funnel"),
      e
    );
  })(pt),
  dy = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.init = function (t) {
        a.prototype.init.apply(this, arguments),
          (this.legendVisualProvider = new Ta(
            at(this.getData, this),
            at(this.getRawData, this)
          )),
          this._defaultLabelLine(t);
      }),
      (e.prototype.getInitialData = function (t, r) {
        return Lr(this, {
          coordDimensions: ["value"],
          encodeDefaulter: ot(gu, this)
        });
      }),
      (e.prototype._defaultLabelLine = function (t) {
        Pa(t, "labelLine", ["show"]);
        var r = t.labelLine,
          n = t.emphasis.labelLine;
        (r.show = r.show && t.label.show),
          (n.show = n.show && t.emphasis.label.show);
      }),
      (e.prototype.getDataParams = function (t) {
        var r = this.getData(),
          n = a.prototype.getDataParams.call(this, t),
          i = r.mapDimension("value"),
          o = r.getSum(i);
        return (
          (n.percent = o ? +((r.get(i, t) / o) * 100).toFixed(2) : 0),
          n.$vars.push("percent"),
          n
        );
      }),
      (e.type = "series.funnel"),
      (e.defaultOption = {
        z: 2,
        legendHoverLink: !0,
        colorBy: "data",
        left: 80,
        top: 60,
        right: 80,
        bottom: 60,
        minSize: "0%",
        maxSize: "100%",
        sort: "descending",
        orient: "vertical",
        gap: 0,
        funnelAlign: "center",
        label: { show: !0, position: "outer" },
        labelLine: { show: !0, length: 20, lineStyle: { width: 1 } },
        itemStyle: { borderColor: "#fff", borderWidth: 1 },
        emphasis: { label: { show: !0 } },
        select: { itemStyle: { borderColor: "#212121" } }
      }),
      e
    );
  })(gt);
function gy(a, e) {
  return se(a.getBoxLayoutParams(), {
    width: e.getWidth(),
    height: e.getHeight()
  });
}
function yy(a, e) {
  for (
    var t = a.mapDimension("value"),
      r = a.mapArray(t, function (l) {
        return l;
      }),
      n = [],
      i = e === "ascending",
      o = 0,
      s = a.count();
    o < s;
    o++
  )
    n[o] = o;
  return (
    it(e)
      ? n.sort(e)
      : e !== "none" &&
        n.sort(function (l, u) {
          return i ? r[l] - r[u] : r[u] - r[l];
        }),
    n
  );
}
function my(a) {
  var e = a.hostModel,
    t = e.get("orient");
  a.each(function (r) {
    var n = a.getItemModel(r),
      i = n.getModel("label"),
      o = i.get("position"),
      s = n.getModel("labelLine"),
      l = a.getItemLayout(r),
      u = l.points,
      v =
        o === "inner" ||
        o === "inside" ||
        o === "center" ||
        o === "insideLeft" ||
        o === "insideRight",
      c,
      h,
      f,
      p;
    if (v)
      o === "insideLeft"
        ? ((h = (u[0][0] + u[3][0]) / 2 + 5),
          (f = (u[0][1] + u[3][1]) / 2),
          (c = "left"))
        : o === "insideRight"
        ? ((h = (u[1][0] + u[2][0]) / 2 - 5),
          (f = (u[1][1] + u[2][1]) / 2),
          (c = "right"))
        : ((h = (u[0][0] + u[1][0] + u[2][0] + u[3][0]) / 4),
          (f = (u[0][1] + u[1][1] + u[2][1] + u[3][1]) / 4),
          (c = "center")),
        (p = [
          [h, f],
          [h, f]
        ]);
    else {
      var d = void 0,
        g = void 0,
        y = void 0,
        S = void 0,
        m = s.get("length");
      o === "left"
        ? ((d = (u[3][0] + u[0][0]) / 2),
          (g = (u[3][1] + u[0][1]) / 2),
          (y = d - m),
          (h = y - 5),
          (c = "right"))
        : o === "right"
        ? ((d = (u[1][0] + u[2][0]) / 2),
          (g = (u[1][1] + u[2][1]) / 2),
          (y = d + m),
          (h = y + 5),
          (c = "left"))
        : o === "top"
        ? ((d = (u[3][0] + u[0][0]) / 2),
          (g = (u[3][1] + u[0][1]) / 2),
          (S = g - m),
          (f = S - 5),
          (c = "center"))
        : o === "bottom"
        ? ((d = (u[1][0] + u[2][0]) / 2),
          (g = (u[1][1] + u[2][1]) / 2),
          (S = g + m),
          (f = S + 5),
          (c = "center"))
        : o === "rightTop"
        ? ((d = t === "horizontal" ? u[3][0] : u[1][0]),
          (g = t === "horizontal" ? u[3][1] : u[1][1]),
          t === "horizontal"
            ? ((S = g - m), (f = S - 5), (c = "center"))
            : ((y = d + m), (h = y + 5), (c = "top")))
        : o === "rightBottom"
        ? ((d = u[2][0]),
          (g = u[2][1]),
          t === "horizontal"
            ? ((S = g + m), (f = S + 5), (c = "center"))
            : ((y = d + m), (h = y + 5), (c = "bottom")))
        : o === "leftTop"
        ? ((d = u[0][0]),
          (g = t === "horizontal" ? u[0][1] : u[1][1]),
          t === "horizontal"
            ? ((S = g - m), (f = S - 5), (c = "center"))
            : ((y = d - m), (h = y - 5), (c = "right")))
        : o === "leftBottom"
        ? ((d = t === "horizontal" ? u[1][0] : u[3][0]),
          (g = t === "horizontal" ? u[1][1] : u[2][1]),
          t === "horizontal"
            ? ((S = g + m), (f = S + 5), (c = "center"))
            : ((y = d - m), (h = y - 5), (c = "right")))
        : ((d = (u[1][0] + u[2][0]) / 2),
          (g = (u[1][1] + u[2][1]) / 2),
          t === "horizontal"
            ? ((S = g + m), (f = S + 5), (c = "center"))
            : ((y = d + m), (h = y + 5), (c = "left"))),
        t === "horizontal" ? ((y = d), (h = y)) : ((S = g), (f = S)),
        (p = [
          [d, g],
          [y, S]
        ]);
    }
    l.label = {
      linePoints: p,
      x: h,
      y: f,
      verticalAlign: "middle",
      textAlign: c,
      inside: v
    };
  });
}
function Sy(a, e) {
  a.eachSeriesByType("funnel", function (t) {
    var r = t.getData(),
      n = r.mapDimension("value"),
      i = t.get("sort"),
      o = gy(t, e),
      s = t.get("orient"),
      l = o.width,
      u = o.height,
      v = yy(r, i),
      c = o.x,
      h = o.y,
      f =
        s === "horizontal"
          ? [z(t.get("minSize"), u), z(t.get("maxSize"), u)]
          : [z(t.get("minSize"), l), z(t.get("maxSize"), l)],
      p = r.getDataExtent(n),
      d = t.get("min"),
      g = t.get("max");
    d == null && (d = Math.min(p[0], 0)), g == null && (g = p[1]);
    var y = t.get("funnelAlign"),
      S = t.get("gap"),
      m = s === "horizontal" ? l : u,
      x = (m - S * (r.count() - 1)) / r.count(),
      b = function (P, E) {
        if (s === "horizontal") {
          var R = r.get(n, P) || 0,
            N = qt(R, [d, g], f, !0),
            V = void 0;
          switch (y) {
            case "top":
              V = h;
              break;
            case "center":
              V = h + (u - N) / 2;
              break;
            case "bottom":
              V = h + (u - N);
              break;
          }
          return [
            [E, V],
            [E, V + N]
          ];
        }
        var G = r.get(n, P) || 0,
          B = qt(G, [d, g], f, !0),
          F;
        switch (y) {
          case "left":
            F = c;
            break;
          case "center":
            F = c + (l - B) / 2;
            break;
          case "right":
            F = c + l - B;
            break;
        }
        return [
          [F, E],
          [F + B, E]
        ];
      };
    i === "ascending" &&
      ((x = -x),
      (S = -S),
      s === "horizontal" ? (c += l) : (h += u),
      (v = v.reverse()));
    for (var _ = 0; _ < v.length; _++) {
      var w = v[_],
        A = v[_ + 1],
        I = r.getItemModel(w);
      if (s === "horizontal") {
        var T = I.get(["itemStyle", "width"]);
        T == null ? (T = x) : ((T = z(T, l)), i === "ascending" && (T = -T));
        var D = b(w, c),
          M = b(A, c + T);
        (c += T + S),
          r.setItemLayout(w, { points: D.concat(M.slice().reverse()) });
      } else {
        var C = I.get(["itemStyle", "height"]);
        C == null ? (C = x) : ((C = z(C, u)), i === "ascending" && (C = -C));
        var D = b(w, h),
          M = b(A, h + C);
        (h += C + S),
          r.setItemLayout(w, { points: D.concat(M.slice().reverse()) });
      }
    }
    my(r);
  });
}
function xy(a) {
  a.registerChartView(py),
    a.registerSeriesModel(dy),
    a.registerLayout(Sy),
    a.registerProcessor(Da("funnel"));
}
var by = 0.3,
  _y = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (
        (t.type = e.type), (t._dataGroup = new Z()), (t._initialized = !1), t
      );
    }
    return (
      (e.prototype.init = function () {
        this.group.add(this._dataGroup);
      }),
      (e.prototype.render = function (t, r, n, i) {
        this._progressiveEls = null;
        var o = this._dataGroup,
          s = t.getData(),
          l = this._data,
          u = t.coordinateSystem,
          v = u.dimensions,
          c = Hs(t);
        s.diff(l).add(h).update(f).remove(p).execute();
        function h(g) {
          var y = Ws(s, o, g, v, u);
          hn(y, s, g, c);
        }
        function f(g, y) {
          var S = l.getItemGraphicEl(y),
            m = Av(s, g, v, u);
          s.setItemGraphicEl(g, S),
            lt(S, { shape: { points: m } }, t, g),
            Se(S),
            hn(S, s, g, c);
        }
        function p(g) {
          var y = l.getItemGraphicEl(g);
          o.remove(y);
        }
        if (!this._initialized) {
          this._initialized = !0;
          var d = wy(u, t, function () {
            setTimeout(function () {
              o.removeClipPath();
            });
          });
          o.setClipPath(d);
        }
        this._data = s;
      }),
      (e.prototype.incrementalPrepareRender = function (t, r, n) {
        (this._initialized = !0),
          (this._data = null),
          this._dataGroup.removeAll();
      }),
      (e.prototype.incrementalRender = function (t, r, n) {
        for (
          var i = r.getData(),
            o = r.coordinateSystem,
            s = o.dimensions,
            l = Hs(r),
            u = (this._progressiveEls = []),
            v = t.start;
          v < t.end;
          v++
        ) {
          var c = Ws(i, this._dataGroup, v, s, o);
          (c.incremental = !0), hn(c, i, v, l), u.push(c);
        }
      }),
      (e.prototype.remove = function () {
        this._dataGroup && this._dataGroup.removeAll(), (this._data = null);
      }),
      (e.type = "parallel"),
      e
    );
  })(pt);
function wy(a, e, t) {
  var r = a.model,
    n = a.getRect(),
    i = new Et({ shape: { x: n.x, y: n.y, width: n.width, height: n.height } }),
    o = r.get("layout") === "horizontal" ? "width" : "height";
  return (
    i.setShape(o, 0),
    Ct(i, { shape: { width: n.width, height: n.height } }, e, t),
    i
  );
}
function Av(a, e, t, r) {
  for (var n = [], i = 0; i < t.length; i++) {
    var o = t[i],
      s = a.get(a.mapDimension(o), e);
    Ay(s, r.getAxis(o).type) || n.push(r.dataToPoint(s, o));
  }
  return n;
}
function Ws(a, e, t, r, n) {
  var i = Av(a, t, r, n),
    o = new we({ shape: { points: i }, z2: 10 });
  return e.add(o), a.setItemGraphicEl(t, o), o;
}
function Hs(a) {
  var e = a.get("smooth", !0);
  return e === !0 && (e = by), (e = yh(e)), mh(e) && (e = 0), { smooth: e };
}
function hn(a, e, t, r) {
  a.useStyle(e.getItemVisual(t, "style")),
    (a.style.fill = null),
    a.setShape("smooth", r.smooth);
  var n = e.getItemModel(t),
    i = n.getModel("emphasis");
  zt(a, n, "lineStyle"),
    ct(a, i.get("focus"), i.get("blurScope"), i.get("disabled"));
}
function Ay(a, e) {
  return e === "category" ? a == null : a == null || isNaN(a);
}
var Ty = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (
      (t.type = e.type),
      (t.visualStyleAccessPath = "lineStyle"),
      (t.visualDrawType = "stroke"),
      t
    );
  }
  return (
    (e.prototype.getInitialData = function (t, r) {
      return Je(null, this, { useEncodeDefaulter: at(Iy, null, this) });
    }),
    (e.prototype.getRawIndicesByActiveState = function (t) {
      var r = this.coordinateSystem,
        n = this.getData(),
        i = [];
      return (
        r.eachActiveState(n, function (o, s) {
          t === o && i.push(n.getRawIndex(s));
        }),
        i
      );
    }),
    (e.type = "series.parallel"),
    (e.dependencies = ["parallel"]),
    (e.defaultOption = {
      z: 2,
      coordinateSystem: "parallel",
      parallelIndex: 0,
      label: { show: !1 },
      inactiveOpacity: 0.05,
      activeOpacity: 1,
      lineStyle: { width: 1, opacity: 0.45, type: "solid" },
      emphasis: { label: { show: !1 } },
      progressive: 500,
      smooth: !1,
      animationEasing: "linear"
    }),
    e
  );
})(gt);
function Iy(a) {
  var e = a.ecModel.getComponent("parallel", a.get("parallelIndex"));
  if (e) {
    var t = {};
    return (
      L(e.dimensions, function (r) {
        var n = Dy(r);
        t[r] = n;
      }),
      t
    );
  }
}
function Dy(a) {
  return +a.replace("dim", "");
}
var Ly = ["lineStyle", "opacity"],
  Cy = {
    seriesType: "parallel",
    reset: function (a, e) {
      var t = a.coordinateSystem,
        r = {
          normal: a.get(["lineStyle", "opacity"]),
          active: a.get("activeOpacity"),
          inactive: a.get("inactiveOpacity")
        };
      return {
        progress: function (n, i) {
          t.eachActiveState(
            i,
            function (o, s) {
              var l = r[o];
              if (o === "normal" && i.hasItemOption) {
                var u = i.getItemModel(s).get(Ly, !0);
                u != null && (l = u);
              }
              var v = i.ensureUniqueItemVisual(s, "style");
              v.opacity = l;
            },
            n.start,
            n.end
          );
        }
      };
    }
  };
function Py(a) {
  My(a), Ey(a);
}
function My(a) {
  if (!a.parallel) {
    var e = !1;
    L(a.series, function (t) {
      t && t.type === "parallel" && (e = !0);
    }),
      e && (a.parallel = [{}]);
  }
}
function Ey(a) {
  var e = Jt(a.parallelAxis);
  L(e, function (t) {
    if (Ge(t)) {
      var r = t.parallelIndex || 0,
        n = Jt(a.parallel)[r];
      n && n.parallelAxisDefault && Yt(t, n.parallelAxisDefault, !1);
    }
  });
}
var Ry = 5,
  Ny = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        (this._model = t),
          (this._api = n),
          this._handlers ||
            ((this._handlers = {}),
            L(
              ky,
              function (i, o) {
                n.getZr().on(o, (this._handlers[o] = at(i, this)));
              },
              this
            )),
          Tu(
            this,
            "_throttledDispatchExpand",
            t.get("axisExpandRate"),
            "fixRate"
          );
      }),
      (e.prototype.dispose = function (t, r) {
        Sh(this, "_throttledDispatchExpand"),
          L(this._handlers, function (n, i) {
            r.getZr().off(i, n);
          }),
          (this._handlers = null);
      }),
      (e.prototype._throttledDispatchExpand = function (t) {
        this._dispatchExpand(t);
      }),
      (e.prototype._dispatchExpand = function (t) {
        t && this._api.dispatchAction(W({ type: "parallelAxisExpand" }, t));
      }),
      (e.type = "parallel"),
      e
    );
  })(ee),
  ky = {
    mousedown: function (a) {
      fn(this, "click") && (this._mouseDownPoint = [a.offsetX, a.offsetY]);
    },
    mouseup: function (a) {
      var e = this._mouseDownPoint;
      if (fn(this, "click") && e) {
        var t = [a.offsetX, a.offsetY],
          r = Math.pow(e[0] - t[0], 2) + Math.pow(e[1] - t[1], 2);
        if (r > Ry) return;
        var n = this._model.coordinateSystem.getSlidedAxisExpandWindow([
          a.offsetX,
          a.offsetY
        ]);
        n.behavior !== "none" &&
          this._dispatchExpand({ axisExpandWindow: n.axisExpandWindow });
      }
      this._mouseDownPoint = null;
    },
    mousemove: function (a) {
      if (!(this._mouseDownPoint || !fn(this, "mousemove"))) {
        var e = this._model,
          t = e.coordinateSystem.getSlidedAxisExpandWindow([
            a.offsetX,
            a.offsetY
          ]),
          r = t.behavior;
        r === "jump" &&
          this._throttledDispatchExpand.debounceNextCall(
            e.get("axisExpandDebounce")
          ),
          this._throttledDispatchExpand(
            r === "none"
              ? null
              : {
                  axisExpandWindow: t.axisExpandWindow,
                  animation: r === "jump" ? null : { duration: 0 }
                }
          );
      }
    }
  };
function fn(a, e) {
  var t = a._model;
  return t.get("axisExpandable") && t.get("axisExpandTriggerOn") === e;
}
var Vy = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.init = function () {
        a.prototype.init.apply(this, arguments), this.mergeOption({});
      }),
      (e.prototype.mergeOption = function (t) {
        var r = this.option;
        t && Yt(r, t, !0), this._initDimensions();
      }),
      (e.prototype.contains = function (t, r) {
        var n = t.get("parallelIndex");
        return n != null && r.getComponent("parallel", n) === this;
      }),
      (e.prototype.setAxisExpand = function (t) {
        L(
          [
            "axisExpandable",
            "axisExpandCenter",
            "axisExpandCount",
            "axisExpandWidth",
            "axisExpandWindow"
          ],
          function (r) {
            t.hasOwnProperty(r) && (this.option[r] = t[r]);
          },
          this
        );
      }),
      (e.prototype._initDimensions = function () {
        var t = (this.dimensions = []),
          r = (this.parallelAxisIndex = []),
          n = Dt(
            this.ecModel.queryComponents({ mainType: "parallelAxis" }),
            function (i) {
              return (i.get("parallelIndex") || 0) === this.componentIndex;
            },
            this
          );
        L(n, function (i) {
          t.push("dim" + i.get("dim")), r.push(i.componentIndex);
        });
      }),
      (e.type = "parallel"),
      (e.dependencies = ["parallelAxis"]),
      (e.layoutMode = "box"),
      (e.defaultOption = {
        z: 0,
        left: 80,
        top: 60,
        right: 80,
        bottom: 60,
        layout: "horizontal",
        axisExpandable: !1,
        axisExpandCenter: null,
        axisExpandCount: 0,
        axisExpandWidth: 50,
        axisExpandRate: 17,
        axisExpandDebounce: 50,
        axisExpandSlideTriggerArea: [-0.15, 0.05, 0.4],
        axisExpandTriggerOn: "click",
        parallelAxisDefault: null
      }),
      e
    );
  })(Xt),
  Gy = (function (a) {
    k(e, a);
    function e(t, r, n, i, o) {
      var s = a.call(this, t, r, n) || this;
      return (s.type = i || "value"), (s.axisIndex = o), s;
    }
    return (
      (e.prototype.isHorizontal = function () {
        return this.coordinateSystem.getModel().get("layout") !== "horizontal";
      }),
      e
    );
  })(oe),
  pn = L,
  Tv = Math.min,
  Iv = Math.max,
  Us = Math.floor,
  zy = Math.ceil,
  Ys = gi,
  Oy = Math.PI,
  By = (function () {
    function a(e, t, r) {
      (this.type = "parallel"),
        (this._axesMap = tt()),
        (this._axesLayout = {}),
        (this.dimensions = e.dimensions),
        (this._model = e),
        this._init(e, t, r);
    }
    return (
      (a.prototype._init = function (e, t, r) {
        var n = e.dimensions,
          i = e.parallelAxisIndex;
        pn(
          n,
          function (o, s) {
            var l = i[s],
              u = t.getComponent("parallelAxis", l),
              v = this._axesMap.set(
                o,
                new Gy(o, Si(u), [0, 0], u.get("type"), l)
              ),
              c = v.type === "category";
            (v.onBand = c && u.get("boundaryGap")),
              (v.inverse = u.get("inverse")),
              (u.axis = v),
              (v.model = u),
              (v.coordinateSystem = u.coordinateSystem = this);
          },
          this
        );
      }),
      (a.prototype.update = function (e, t) {
        this._updateAxesFromSeries(this._model, e);
      }),
      (a.prototype.containPoint = function (e) {
        var t = this._makeLayoutInfo(),
          r = t.axisBase,
          n = t.layoutBase,
          i = t.pixelDimIndex,
          o = e[1 - i],
          s = e[i];
        return (
          o >= r && o <= r + t.axisLength && s >= n && s <= n + t.layoutLength
        );
      }),
      (a.prototype.getModel = function () {
        return this._model;
      }),
      (a.prototype._updateAxesFromSeries = function (e, t) {
        t.eachSeries(function (r) {
          if (e.contains(r, t)) {
            var n = r.getData();
            pn(
              this.dimensions,
              function (i) {
                var o = this._axesMap.get(i);
                o.scale.unionExtentFromData(n, n.mapDimension(i)),
                  na(o.scale, o.model);
              },
              this
            );
          }
        }, this);
      }),
      (a.prototype.resize = function (e, t) {
        (this._rect = se(e.getBoxLayoutParams(), {
          width: t.getWidth(),
          height: t.getHeight()
        })),
          this._layoutAxes();
      }),
      (a.prototype.getRect = function () {
        return this._rect;
      }),
      (a.prototype._makeLayoutInfo = function () {
        var e = this._model,
          t = this._rect,
          r = ["x", "y"],
          n = ["width", "height"],
          i = e.get("layout"),
          o = i === "horizontal" ? 0 : 1,
          s = t[n[o]],
          l = [0, s],
          u = this.dimensions.length,
          v = Yr(e.get("axisExpandWidth"), l),
          c = Yr(e.get("axisExpandCount") || 0, [0, u]),
          h =
            e.get("axisExpandable") &&
            u > 3 &&
            u > c &&
            c > 1 &&
            v > 0 &&
            s > 0,
          f = e.get("axisExpandWindow"),
          p;
        if (f) (p = Yr(f[1] - f[0], l)), (f[1] = f[0] + p);
        else {
          p = Yr(v * (c - 1), l);
          var d = e.get("axisExpandCenter") || Us(u / 2);
          (f = [v * d - p / 2]), (f[1] = f[0] + p);
        }
        var g = (s - p) / (u - c);
        g < 3 && (g = 0);
        var y = [Us(Ys(f[0] / v, 1)) + 1, zy(Ys(f[1] / v, 1)) - 1],
          S = (g / v) * f[0];
        return {
          layout: i,
          pixelDimIndex: o,
          layoutBase: t[r[o]],
          layoutLength: s,
          axisBase: t[r[1 - o]],
          axisLength: t[n[1 - o]],
          axisExpandable: h,
          axisExpandWidth: v,
          axisCollapseWidth: g,
          axisExpandWindow: f,
          axisCount: u,
          winInnerIndices: y,
          axisExpandWindow0Pos: S
        };
      }),
      (a.prototype._layoutAxes = function () {
        var e = this._rect,
          t = this._axesMap,
          r = this.dimensions,
          n = this._makeLayoutInfo(),
          i = n.layout;
        t.each(function (o) {
          var s = [0, n.axisLength],
            l = o.inverse ? 1 : 0;
          o.setExtent(s[l], s[1 - l]);
        }),
          pn(
            r,
            function (o, s) {
              var l = (n.axisExpandable ? Wy : Fy)(s, n),
                u = {
                  horizontal: { x: l.position, y: n.axisLength },
                  vertical: { x: 0, y: l.position }
                },
                v = { horizontal: Oy / 2, vertical: 0 },
                c = [u[i].x + e.x, u[i].y + e.y],
                h = v[i],
                f = tr();
              La(f, f, h),
                ke(f, f, c),
                (this._axesLayout[o] = {
                  position: c,
                  rotation: h,
                  transform: f,
                  axisNameAvailableWidth: l.axisNameAvailableWidth,
                  axisLabelShow: l.axisLabelShow,
                  nameTruncateMaxWidth: l.nameTruncateMaxWidth,
                  tickDirection: 1,
                  labelDirection: 1
                });
            },
            this
          );
      }),
      (a.prototype.getAxis = function (e) {
        return this._axesMap.get(e);
      }),
      (a.prototype.dataToPoint = function (e, t) {
        return this.axisCoordToPoint(this._axesMap.get(t).dataToCoord(e), t);
      }),
      (a.prototype.eachActiveState = function (e, t, r, n) {
        r == null && (r = 0), n == null && (n = e.count());
        var i = this._axesMap,
          o = this.dimensions,
          s = [],
          l = [];
        L(o, function (g) {
          s.push(e.mapDimension(g)), l.push(i.get(g).model);
        });
        for (var u = this.hasAxisBrushed(), v = r; v < n; v++) {
          var c = void 0;
          if (!u) c = "normal";
          else {
            c = "active";
            for (var h = e.getValues(s, v), f = 0, p = o.length; f < p; f++) {
              var d = l[f].getActiveState(h[f]);
              if (d === "inactive") {
                c = "inactive";
                break;
              }
            }
          }
          t(c, v);
        }
      }),
      (a.prototype.hasAxisBrushed = function () {
        for (
          var e = this.dimensions,
            t = this._axesMap,
            r = !1,
            n = 0,
            i = e.length;
          n < i;
          n++
        )
          t.get(e[n]).model.getActiveState() !== "normal" && (r = !0);
        return r;
      }),
      (a.prototype.axisCoordToPoint = function (e, t) {
        var r = this._axesLayout[t];
        return xi([e, 0], r.transform);
      }),
      (a.prototype.getAxisLayout = function (e) {
        return Gt(this._axesLayout[e]);
      }),
      (a.prototype.getSlidedAxisExpandWindow = function (e) {
        var t = this._makeLayoutInfo(),
          r = t.pixelDimIndex,
          n = t.axisExpandWindow.slice(),
          i = n[1] - n[0],
          o = [0, t.axisExpandWidth * (t.axisCount - 1)];
        if (!this.containPoint(e))
          return { behavior: "none", axisExpandWindow: n };
        var s = e[r] - t.layoutBase - t.axisExpandWindow0Pos,
          l,
          u = "slide",
          v = t.axisCollapseWidth,
          c = this._model.get("axisExpandSlideTriggerArea"),
          h = c[0] != null;
        if (v)
          h && v && s < i * c[0]
            ? ((u = "jump"), (l = s - i * c[2]))
            : h && v && s > i * (1 - c[0])
            ? ((u = "jump"), (l = s - i * (1 - c[2])))
            : (l = s - i * c[1]) >= 0 &&
              (l = s - i * (1 - c[1])) <= 0 &&
              (l = 0),
            (l *= t.axisExpandWidth / v),
            l ? xh(l, n, o, "all") : (u = "none");
        else {
          var f = n[1] - n[0],
            p = (o[1] * s) / f;
          (n = [Iv(0, p - f / 2)]),
            (n[1] = Tv(o[1], n[0] + f)),
            (n[0] = n[1] - f);
        }
        return { axisExpandWindow: n, behavior: u };
      }),
      a
    );
  })();
function Yr(a, e) {
  return Tv(Iv(a, e[0]), e[1]);
}
function Fy(a, e) {
  var t = e.layoutLength / (e.axisCount - 1);
  return { position: t * a, axisNameAvailableWidth: t, axisLabelShow: !0 };
}
function Wy(a, e) {
  var t = e.layoutLength,
    r = e.axisExpandWidth,
    n = e.axisCount,
    i = e.axisCollapseWidth,
    o = e.winInnerIndices,
    s,
    l = i,
    u = !1,
    v;
  return (
    a < o[0]
      ? ((s = a * i), (v = i))
      : a <= o[1]
      ? ((s = e.axisExpandWindow0Pos + a * r - e.axisExpandWindow[0]),
        (l = r),
        (u = !0))
      : ((s = t - (n - 1 - a) * i), (v = i)),
    {
      position: s,
      axisNameAvailableWidth: l,
      axisLabelShow: u,
      nameTruncateMaxWidth: v
    }
  );
}
function Hy(a, e) {
  var t = [];
  return (
    a.eachComponent("parallel", function (r, n) {
      var i = new By(r, a, e);
      (i.name = "parallel_" + n),
        i.resize(r, e),
        (r.coordinateSystem = i),
        (i.model = r),
        t.push(i);
    }),
    a.eachSeries(function (r) {
      if (r.get("coordinateSystem") === "parallel") {
        var n = r.getReferringComponents("parallel", er).models[0];
        r.coordinateSystem = n.coordinateSystem;
      }
    }),
    t
  );
}
var Uy = { create: Hy },
  ei = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.activeIntervals = []), t;
    }
    return (
      (e.prototype.getAreaSelectStyle = function () {
        return xu([
          ["fill", "color"],
          ["lineWidth", "borderWidth"],
          ["stroke", "borderColor"],
          ["width", "width"],
          ["opacity", "opacity"]
        ])(this.getModel("areaSelectStyle"));
      }),
      (e.prototype.setActiveIntervals = function (t) {
        var r = (this.activeIntervals = Gt(t));
        if (r) for (var n = r.length - 1; n >= 0; n--) Sr(r[n]);
      }),
      (e.prototype.getActiveState = function (t) {
        var r = this.activeIntervals;
        if (!r.length) return "normal";
        if (t == null || isNaN(+t)) return "inactive";
        if (r.length === 1) {
          var n = r[0];
          if (n[0] <= t && t <= n[1]) return "active";
        } else
          for (var i = 0, o = r.length; i < o; i++)
            if (r[i][0] <= t && t <= r[i][1]) return "active";
        return "inactive";
      }),
      e
    );
  })(Xt);
te(ei, Ia);
var Yy = ["axisLine", "axisTickLabel", "axisName"],
  $y = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.init = function (t, r) {
        a.prototype.init.apply(this, arguments),
          (this._brushController = new Iu(r.getZr())).on(
            "brush",
            at(this._onBrush, this)
          );
      }),
      (e.prototype.render = function (t, r, n, i) {
        if (!Xy(t, r, i)) {
          (this.axisModel = t), (this.api = n), this.group.removeAll();
          var o = this._axisGroup;
          if (
            ((this._axisGroup = new Z()),
            this.group.add(this._axisGroup),
            !!t.get("show"))
          ) {
            var s = qy(t, r),
              l = s.coordinateSystem,
              u = t.getAreaSelectStyle(),
              v = u.width,
              c = t.axis.dim,
              h = l.getAxisLayout(c),
              f = W({ strokeContainThreshold: v }, h),
              p = new Ne(t, f);
            L(Yy, p.add, p),
              this._axisGroup.add(p.getGroup()),
              this._refreshBrushController(f, u, t, s, v, n),
              bi(o, this._axisGroup, t);
          }
        }
      }),
      (e.prototype._refreshBrushController = function (t, r, n, i, o, s) {
        var l = n.axis.getExtent(),
          u = l[1] - l[0],
          v = Math.min(30, Math.abs(u) * 0.1),
          c = ft.create({ x: l[0], y: -o / 2, width: u, height: o });
        (c.x -= v),
          (c.width += 2 * v),
          this._brushController
            .mount({
              enableGlobalPan: !0,
              rotation: t.rotation,
              x: t.position[0],
              y: t.position[1]
            })
            .setPanels([
              {
                panelId: "pl",
                clipPath: wh(c),
                isTargetByCursor: _h(c, s, i),
                getLinearBrushOtherExtent: bh(c, 0)
              }
            ])
            .enableBrush({
              brushType: "lineX",
              brushStyle: r,
              removeOnClick: !0
            })
            .updateCovers(Zy(n));
      }),
      (e.prototype._onBrush = function (t) {
        var r = t.areas,
          n = this.axisModel,
          i = n.axis,
          o = O(r, function (s) {
            return [
              i.coordToData(s.range[0], !0),
              i.coordToData(s.range[1], !0)
            ];
          });
        (!n.option.realtime === t.isEnd || t.removeOnClick) &&
          this.api.dispatchAction({
            type: "axisAreaSelect",
            parallelAxisId: n.id,
            intervals: o
          });
      }),
      (e.prototype.dispose = function () {
        this._brushController.dispose();
      }),
      (e.type = "parallelAxis"),
      e
    );
  })(ee);
function Xy(a, e, t) {
  return (
    t &&
    t.type === "axisAreaSelect" &&
    e.findComponents({ mainType: "parallelAxis", query: t })[0] === a
  );
}
function Zy(a) {
  var e = a.axis;
  return O(a.activeIntervals, function (t) {
    return {
      brushType: "lineX",
      panelId: "pl",
      range: [e.dataToCoord(t[0], !0), e.dataToCoord(t[1], !0)]
    };
  });
}
function qy(a, e) {
  return e.getComponent("parallel", a.get("parallelIndex"));
}
var Ky = { type: "axisAreaSelect", event: "axisAreaSelected" };
function jy(a) {
  a.registerAction(Ky, function (e, t) {
    t.eachComponent({ mainType: "parallelAxis", query: e }, function (r) {
      r.axis.model.setActiveIntervals(e.intervals);
    });
  }),
    a.registerAction("parallelAxisExpand", function (e, t) {
      t.eachComponent({ mainType: "parallel", query: e }, function (r) {
        r.setAxisExpand(e);
      });
    });
}
var Jy = {
  type: "value",
  areaSelectStyle: {
    width: 20,
    borderWidth: 1,
    borderColor: "rgba(160,197,232)",
    color: "rgba(160,197,232)",
    opacity: 0.3
  },
  realtime: !0,
  z: 10
};
function Dv(a) {
  a.registerComponentView(Ny),
    a.registerComponentModel(Vy),
    a.registerCoordinateSystem("parallel", Uy),
    a.registerPreprocessor(Py),
    a.registerComponentModel(ei),
    a.registerComponentView($y),
    ia(a, "parallel", ei, Jy),
    jy(a);
}
function Qy(a) {
  q(Dv),
    a.registerChartView(_y),
    a.registerSeriesModel(Ty),
    a.registerVisual(a.PRIORITY.VISUAL.BRUSH, Cy);
}
var tm = (function () {
    function a() {
      (this.x1 = 0),
        (this.y1 = 0),
        (this.x2 = 0),
        (this.y2 = 0),
        (this.cpx1 = 0),
        (this.cpy1 = 0),
        (this.cpx2 = 0),
        (this.cpy2 = 0),
        (this.extent = 0);
    }
    return a;
  })(),
  em = (function (a) {
    k(e, a);
    function e(t) {
      return a.call(this, t) || this;
    }
    return (
      (e.prototype.getDefaultShape = function () {
        return new tm();
      }),
      (e.prototype.buildPath = function (t, r) {
        var n = r.extent;
        t.moveTo(r.x1, r.y1),
          t.bezierCurveTo(r.cpx1, r.cpy1, r.cpx2, r.cpy2, r.x2, r.y2),
          r.orient === "vertical"
            ? (t.lineTo(r.x2 + n, r.y2),
              t.bezierCurveTo(
                r.cpx2 + n,
                r.cpy2,
                r.cpx1 + n,
                r.cpy1,
                r.x1 + n,
                r.y1
              ))
            : (t.lineTo(r.x2, r.y2 + n),
              t.bezierCurveTo(
                r.cpx2,
                r.cpy2 + n,
                r.cpx1,
                r.cpy1 + n,
                r.x1,
                r.y1 + n
              )),
          t.closePath();
      }),
      (e.prototype.highlight = function () {
        yi(this);
      }),
      (e.prototype.downplay = function () {
        mi(this);
      }),
      e
    );
  })(wt),
  rm = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t._focusAdjacencyDisabled = !1), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i = this,
          o = t.getGraph(),
          s = this.group,
          l = t.layoutInfo,
          u = l.width,
          v = l.height,
          c = t.getData(),
          h = t.getData("edge"),
          f = t.get("orient");
        (this._model = t),
          s.removeAll(),
          (s.x = l.x),
          (s.y = l.y),
          o.eachEdge(function (p) {
            var d = new em(),
              g = nt(d);
            (g.dataIndex = p.dataIndex),
              (g.seriesIndex = t.seriesIndex),
              (g.dataType = "edge");
            var y = p.getModel(),
              S = y.getModel("lineStyle"),
              m = S.get("curveness"),
              x = p.node1.getLayout(),
              b = p.node1.getModel(),
              _ = b.get("localX"),
              w = b.get("localY"),
              A = p.node2.getLayout(),
              I = p.node2.getModel(),
              T = I.get("localX"),
              D = I.get("localY"),
              M = p.getLayout(),
              C,
              P,
              E,
              R,
              N,
              V,
              G,
              B;
            (d.shape.extent = Math.max(1, M.dy)),
              (d.shape.orient = f),
              f === "vertical"
                ? ((C = (_ != null ? _ * u : x.x) + M.sy),
                  (P = (w != null ? w * v : x.y) + x.dy),
                  (E = (T != null ? T * u : A.x) + M.ty),
                  (R = D != null ? D * v : A.y),
                  (N = C),
                  (V = P * (1 - m) + R * m),
                  (G = E),
                  (B = P * m + R * (1 - m)))
                : ((C = (_ != null ? _ * u : x.x) + x.dx),
                  (P = (w != null ? w * v : x.y) + M.sy),
                  (E = T != null ? T * u : A.x),
                  (R = (D != null ? D * v : A.y) + M.ty),
                  (N = C * (1 - m) + E * m),
                  (V = P),
                  (G = C * m + E * (1 - m)),
                  (B = R)),
              d.setShape({
                x1: C,
                y1: P,
                x2: E,
                y2: R,
                cpx1: N,
                cpy1: V,
                cpx2: G,
                cpy2: B
              }),
              d.useStyle(S.getItemStyle()),
              $s(d.style, f, p);
            var F = "" + y.get("value"),
              Y = Pt(y, "edgeLabel");
            Ut(d, Y, {
              labelFetcher: {
                getFormattedLabel: function (j, yt, re, X, U, rt) {
                  return t.getFormattedLabel(
                    j,
                    yt,
                    "edge",
                    X,
                    Er(U, Y.normal && Y.normal.get("formatter"), F),
                    rt
                  );
                }
              },
              labelDataIndex: p.dataIndex,
              defaultText: F
            }),
              d.setTextConfig({ position: "inside" });
            var K = y.getModel("emphasis");
            zt(d, y, "lineStyle", function (j) {
              var yt = j.getItemStyle();
              return $s(yt, f, p), yt;
            }),
              s.add(d),
              h.setItemGraphicEl(p.dataIndex, d);
            var $ = K.get("focus");
            ct(
              d,
              $ === "adjacency"
                ? p.getAdjacentDataIndices()
                : $ === "trajectory"
                ? p.getTrajectoryDataIndices()
                : $,
              K.get("blurScope"),
              K.get("disabled")
            );
          }),
          o.eachNode(function (p) {
            var d = p.getLayout(),
              g = p.getModel(),
              y = g.get("localX"),
              S = g.get("localY"),
              m = g.getModel("emphasis"),
              x = g.get(["itemStyle", "borderRadius"]) || 0,
              b = new Et({
                shape: {
                  x: y != null ? y * u : d.x,
                  y: S != null ? S * v : d.y,
                  width: d.dx,
                  height: d.dy,
                  r: x
                },
                style: g.getModel("itemStyle").getItemStyle(),
                z2: 10
              });
            Ut(b, Pt(g), {
              labelFetcher: {
                getFormattedLabel: function (w, A) {
                  return t.getFormattedLabel(w, A, "node");
                }
              },
              labelDataIndex: p.dataIndex,
              defaultText: p.id
            }),
              (b.disableLabelAnimation = !0),
              b.setStyle("fill", p.getVisual("color")),
              b.setStyle("decal", p.getVisual("style").decal),
              zt(b, g),
              s.add(b),
              c.setItemGraphicEl(p.dataIndex, b),
              (nt(b).dataType = "node");
            var _ = m.get("focus");
            ct(
              b,
              _ === "adjacency"
                ? p.getAdjacentDataIndices()
                : _ === "trajectory"
                ? p.getTrajectoryDataIndices()
                : _,
              m.get("blurScope"),
              m.get("disabled")
            );
          }),
          c.eachItemGraphicEl(function (p, d) {
            var g = c.getItemModel(d);
            g.get("draggable") &&
              ((p.drift = function (y, S) {
                (i._focusAdjacencyDisabled = !0),
                  (this.shape.x += y),
                  (this.shape.y += S),
                  this.dirty(),
                  n.dispatchAction({
                    type: "dragNode",
                    seriesId: t.id,
                    dataIndex: c.getRawIndex(d),
                    localX: this.shape.x / u,
                    localY: this.shape.y / v
                  });
              }),
              (p.ondragend = function () {
                i._focusAdjacencyDisabled = !1;
              }),
              (p.draggable = !0),
              (p.cursor = "move"));
          }),
          !this._data &&
            t.isAnimationEnabled() &&
            s.setClipPath(
              am(s.getBoundingRect(), t, function () {
                s.removeClipPath();
              })
            ),
          (this._data = t.getData());
      }),
      (e.prototype.dispose = function () {}),
      (e.type = "sankey"),
      e
    );
  })(pt);
function $s(a, e, t) {
  switch (a.fill) {
    case "source":
      (a.fill = t.node1.getVisual("color")),
        (a.decal = t.node1.getVisual("style").decal);
      break;
    case "target":
      (a.fill = t.node2.getVisual("color")),
        (a.decal = t.node2.getVisual("style").decal);
      break;
    case "gradient":
      var r = t.node1.getVisual("color"),
        n = t.node2.getVisual("color");
      et(r) &&
        et(n) &&
        (a.fill = new fu(0, 0, +(e === "horizontal"), +(e === "vertical"), [
          { color: r, offset: 0 },
          { color: n, offset: 1 }
        ]));
  }
}
function am(a, e, t) {
  var r = new Et({
    shape: { x: a.x - 10, y: a.y - 10, width: 0, height: a.height + 20 }
  });
  return Ct(r, { shape: { width: a.width + 20 } }, e, t), r;
}
var nm = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), t;
  }
  return (
    (e.prototype.getInitialData = function (t, r) {
      var n = t.edges || t.links || [],
        i = t.data || t.nodes || [],
        o = t.levels || [];
      this.levelModels = [];
      for (var s = this.levelModels, l = 0; l < o.length; l++)
        o[l].depth != null &&
          o[l].depth >= 0 &&
          (s[o[l].depth] = new $t(o[l], this, r));
      var u = wv(i, n, this, !0, v);
      return u.data;
      function v(c, h) {
        c.wrapMethod("getItemModel", function (f, p) {
          var d = f.parentModel,
            g = d.getData().getItemLayout(p);
          if (g) {
            var y = g.depth,
              S = d.levelModels[y];
            S && (f.parentModel = S);
          }
          return f;
        }),
          h.wrapMethod("getItemModel", function (f, p) {
            var d = f.parentModel,
              g = d.getGraph().getEdgeByIndex(p),
              y = g.node1.getLayout();
            if (y) {
              var S = y.depth,
                m = d.levelModels[S];
              m && (f.parentModel = m);
            }
            return f;
          });
      }
    }),
    (e.prototype.setNodePosition = function (t, r) {
      var n = this.option.data || this.option.nodes,
        i = n[t];
      (i.localX = r[0]), (i.localY = r[1]);
    }),
    (e.prototype.getGraph = function () {
      return this.getData().graph;
    }),
    (e.prototype.getEdgeData = function () {
      return this.getGraph().edgeData;
    }),
    (e.prototype.formatTooltip = function (t, r, n) {
      function i(f) {
        return isNaN(f) || f == null;
      }
      if (n === "edge") {
        var o = this.getDataParams(t, n),
          s = o.data,
          l = o.value,
          u = s.source + " -- " + s.target;
        return Mt("nameValue", { name: u, value: l, noValue: i(l) });
      } else {
        var v = this.getGraph().getNodeByIndex(t),
          c = v.getLayout().value,
          h = this.getDataParams(t, n).data.name;
        return Mt("nameValue", {
          name: h != null ? h + "" : null,
          value: c,
          noValue: i(c)
        });
      }
    }),
    (e.prototype.optionUpdated = function () {}),
    (e.prototype.getDataParams = function (t, r) {
      var n = a.prototype.getDataParams.call(this, t, r);
      if (n.value == null && r === "node") {
        var i = this.getGraph().getNodeByIndex(t),
          o = i.getLayout().value;
        n.value = o;
      }
      return n;
    }),
    (e.type = "series.sankey"),
    (e.defaultOption = {
      z: 2,
      coordinateSystem: "view",
      left: "5%",
      top: "5%",
      right: "20%",
      bottom: "5%",
      orient: "horizontal",
      nodeWidth: 20,
      nodeGap: 8,
      draggable: !0,
      layoutIterations: 32,
      label: { show: !0, position: "right", fontSize: 12 },
      edgeLabel: { show: !1, fontSize: 12 },
      levels: [],
      nodeAlign: "justify",
      lineStyle: { color: "#314656", opacity: 0.2, curveness: 0.5 },
      emphasis: { label: { show: !0 }, lineStyle: { opacity: 0.5 } },
      select: { itemStyle: { borderColor: "#212121" } },
      animationEasing: "linear",
      animationDuration: 1e3
    }),
    e
  );
})(gt);
function im(a, e) {
  a.eachSeriesByType("sankey", function (t) {
    var r = t.get("nodeWidth"),
      n = t.get("nodeGap"),
      i = om(t, e);
    t.layoutInfo = i;
    var o = i.width,
      s = i.height,
      l = t.getGraph(),
      u = l.nodes,
      v = l.edges;
    lm(u);
    var c = Dt(u, function (d) {
        return d.getLayout().value === 0;
      }),
      h = c.length !== 0 ? 0 : t.get("layoutIterations"),
      f = t.get("orient"),
      p = t.get("nodeAlign");
    sm(u, v, r, n, o, s, h, f, p);
  });
}
function om(a, e) {
  return se(a.getBoxLayoutParams(), {
    width: e.getWidth(),
    height: e.getHeight()
  });
}
function sm(a, e, t, r, n, i, o, s, l) {
  um(a, e, t, n, i, s, l), fm(a, e, i, n, r, o, s), _m(a, s);
}
function lm(a) {
  L(a, function (e) {
    var t = me(e.outEdges, fa),
      r = me(e.inEdges, fa),
      n = e.getValue() || 0,
      i = Math.max(t, r, n);
    e.setLayout({ value: i }, !0);
  });
}
function um(a, e, t, r, n, i, o) {
  for (var s = [], l = [], u = [], v = [], c = 0, h = 0; h < e.length; h++)
    s[h] = 1;
  for (var h = 0; h < a.length; h++)
    (l[h] = a[h].inEdges.length), l[h] === 0 && u.push(a[h]);
  for (var f = -1; u.length; ) {
    for (var p = 0; p < u.length; p++) {
      var d = u[p],
        g = d.hostGraph.data.getRawDataItem(d.dataIndex),
        y = g.depth != null && g.depth >= 0;
      y && g.depth > f && (f = g.depth),
        d.setLayout({ depth: y ? g.depth : c }, !0),
        i === "vertical"
          ? d.setLayout({ dy: t }, !0)
          : d.setLayout({ dx: t }, !0);
      for (var S = 0; S < d.outEdges.length; S++) {
        var m = d.outEdges[S],
          x = e.indexOf(m);
        s[x] = 0;
        var b = m.node2,
          _ = a.indexOf(b);
        --l[_] === 0 && v.indexOf(b) < 0 && v.push(b);
      }
    }
    ++c, (u = v), (v = []);
  }
  for (var h = 0; h < s.length; h++)
    if (s[h] === 1)
      throw new Error("Sankey is a DAG, the original data has cycle!");
  var w = f > c - 1 ? f : c - 1;
  o && o !== "left" && vm(a, o, i, w);
  var A = i === "vertical" ? (n - t) / w : (r - t) / w;
  hm(a, A, i);
}
function Lv(a) {
  var e = a.hostGraph.data.getRawDataItem(a.dataIndex);
  return e.depth != null && e.depth >= 0;
}
function vm(a, e, t, r) {
  if (e === "right") {
    for (var n = [], i = a, o = 0; i.length; ) {
      for (var s = 0; s < i.length; s++) {
        var l = i[s];
        l.setLayout({ skNodeHeight: o }, !0);
        for (var u = 0; u < l.inEdges.length; u++) {
          var v = l.inEdges[u];
          n.indexOf(v.node1) < 0 && n.push(v.node1);
        }
      }
      (i = n), (n = []), ++o;
    }
    L(a, function (c) {
      Lv(c) ||
        c.setLayout({ depth: Math.max(0, r - c.getLayout().skNodeHeight) }, !0);
    });
  } else e === "justify" && cm(a, r);
}
function cm(a, e) {
  L(a, function (t) {
    !Lv(t) && !t.outEdges.length && t.setLayout({ depth: e }, !0);
  });
}
function hm(a, e, t) {
  L(a, function (r) {
    var n = r.getLayout().depth * e;
    t === "vertical" ? r.setLayout({ y: n }, !0) : r.setLayout({ x: n }, !0);
  });
}
function fm(a, e, t, r, n, i, o) {
  var s = pm(a, o);
  dm(s, e, t, r, n, o), dn(s, n, t, r, o);
  for (var l = 1; i > 0; i--)
    (l *= 0.99), gm(s, l, o), dn(s, n, t, r, o), bm(s, l, o), dn(s, n, t, r, o);
}
function pm(a, e) {
  var t = [],
    r = e === "vertical" ? "y" : "x",
    n = Wn(a, function (i) {
      return i.getLayout()[r];
    });
  return (
    n.keys.sort(function (i, o) {
      return i - o;
    }),
    L(n.keys, function (i) {
      t.push(n.buckets.get(i));
    }),
    t
  );
}
function dm(a, e, t, r, n, i) {
  var o = 1 / 0;
  L(a, function (s) {
    var l = s.length,
      u = 0;
    L(s, function (c) {
      u += c.getLayout().value;
    });
    var v = i === "vertical" ? (r - (l - 1) * n) / u : (t - (l - 1) * n) / u;
    v < o && (o = v);
  }),
    L(a, function (s) {
      L(s, function (l, u) {
        var v = l.getLayout().value * o;
        i === "vertical"
          ? (l.setLayout({ x: u }, !0), l.setLayout({ dx: v }, !0))
          : (l.setLayout({ y: u }, !0), l.setLayout({ dy: v }, !0));
      });
    }),
    L(e, function (s) {
      var l = +s.getValue() * o;
      s.setLayout({ dy: l }, !0);
    });
}
function dn(a, e, t, r, n) {
  var i = n === "vertical" ? "x" : "y";
  L(a, function (o) {
    o.sort(function (d, g) {
      return d.getLayout()[i] - g.getLayout()[i];
    });
    for (
      var s,
        l,
        u,
        v = 0,
        c = o.length,
        h = n === "vertical" ? "dx" : "dy",
        f = 0;
      f < c;
      f++
    )
      (l = o[f]),
        (u = v - l.getLayout()[i]),
        u > 0 &&
          ((s = l.getLayout()[i] + u),
          n === "vertical"
            ? l.setLayout({ x: s }, !0)
            : l.setLayout({ y: s }, !0)),
        (v = l.getLayout()[i] + l.getLayout()[h] + e);
    var p = n === "vertical" ? r : t;
    if (((u = v - e - p), u > 0)) {
      (s = l.getLayout()[i] - u),
        n === "vertical"
          ? l.setLayout({ x: s }, !0)
          : l.setLayout({ y: s }, !0),
        (v = s);
      for (var f = c - 2; f >= 0; --f)
        (l = o[f]),
          (u = l.getLayout()[i] + l.getLayout()[h] + e - v),
          u > 0 &&
            ((s = l.getLayout()[i] - u),
            n === "vertical"
              ? l.setLayout({ x: s }, !0)
              : l.setLayout({ y: s }, !0)),
          (v = l.getLayout()[i]);
    }
  });
}
function gm(a, e, t) {
  L(a.slice().reverse(), function (r) {
    L(r, function (n) {
      if (n.outEdges.length) {
        var i = me(n.outEdges, ym, t) / me(n.outEdges, fa);
        if (isNaN(i)) {
          var o = n.outEdges.length;
          i = o ? me(n.outEdges, mm, t) / o : 0;
        }
        if (t === "vertical") {
          var s = n.getLayout().x + (i - xe(n, t)) * e;
          n.setLayout({ x: s }, !0);
        } else {
          var l = n.getLayout().y + (i - xe(n, t)) * e;
          n.setLayout({ y: l }, !0);
        }
      }
    });
  });
}
function ym(a, e) {
  return xe(a.node2, e) * a.getValue();
}
function mm(a, e) {
  return xe(a.node2, e);
}
function Sm(a, e) {
  return xe(a.node1, e) * a.getValue();
}
function xm(a, e) {
  return xe(a.node1, e);
}
function xe(a, e) {
  return e === "vertical"
    ? a.getLayout().x + a.getLayout().dx / 2
    : a.getLayout().y + a.getLayout().dy / 2;
}
function fa(a) {
  return a.getValue();
}
function me(a, e, t) {
  for (var r = 0, n = a.length, i = -1; ++i < n; ) {
    var o = +e(a[i], t);
    isNaN(o) || (r += o);
  }
  return r;
}
function bm(a, e, t) {
  L(a, function (r) {
    L(r, function (n) {
      if (n.inEdges.length) {
        var i = me(n.inEdges, Sm, t) / me(n.inEdges, fa);
        if (isNaN(i)) {
          var o = n.inEdges.length;
          i = o ? me(n.inEdges, xm, t) / o : 0;
        }
        if (t === "vertical") {
          var s = n.getLayout().x + (i - xe(n, t)) * e;
          n.setLayout({ x: s }, !0);
        } else {
          var l = n.getLayout().y + (i - xe(n, t)) * e;
          n.setLayout({ y: l }, !0);
        }
      }
    });
  });
}
function _m(a, e) {
  var t = e === "vertical" ? "x" : "y";
  L(a, function (r) {
    r.outEdges.sort(function (n, i) {
      return n.node2.getLayout()[t] - i.node2.getLayout()[t];
    }),
      r.inEdges.sort(function (n, i) {
        return n.node1.getLayout()[t] - i.node1.getLayout()[t];
      });
  }),
    L(a, function (r) {
      var n = 0,
        i = 0;
      L(r.outEdges, function (o) {
        o.setLayout({ sy: n }, !0), (n += o.getLayout().dy);
      }),
        L(r.inEdges, function (o) {
          o.setLayout({ ty: i }, !0), (i += o.getLayout().dy);
        });
    });
}
function wm(a) {
  a.eachSeriesByType("sankey", function (e) {
    var t = e.getGraph(),
      r = t.nodes,
      n = t.edges;
    if (r.length) {
      var i = 1 / 0,
        o = -1 / 0;
      L(r, function (s) {
        var l = s.getLayout().value;
        l < i && (i = l), l > o && (o = l);
      }),
        L(r, function (s) {
          var l = new _u({
              type: "color",
              mappingMethod: "linear",
              dataExtent: [i, o],
              visual: e.get("color")
            }),
            u = l.mapValueToVisual(s.getLayout().value),
            v = s.getModel().get(["itemStyle", "color"]);
          v != null
            ? (s.setVisual("color", v), s.setVisual("style", { fill: v }))
            : (s.setVisual("color", u), s.setVisual("style", { fill: u }));
        });
    }
    n.length &&
      L(n, function (s) {
        var l = s.getModel().get("lineStyle");
        s.setVisual("style", l);
      });
  });
}
function Am(a) {
  a.registerChartView(rm),
    a.registerSeriesModel(nm),
    a.registerLayout(im),
    a.registerVisual(wm),
    a.registerAction(
      { type: "dragNode", event: "dragnode", update: "update" },
      function (e, t) {
        t.eachComponent(
          { mainType: "series", subType: "sankey", query: e },
          function (r) {
            r.setNodePosition(e.dataIndex, [e.localX, e.localY]);
          }
        );
      }
    );
}
var Cv = (function () {
    function a() {}
    return (
      (a.prototype._hasEncodeRule = function (e) {
        var t = this.getEncode();
        return t && t.get(e) != null;
      }),
      (a.prototype.getInitialData = function (e, t) {
        var r,
          n = t.getComponent("xAxis", this.get("xAxisIndex")),
          i = t.getComponent("yAxis", this.get("yAxisIndex")),
          o = n.get("type"),
          s = i.get("type"),
          l;
        o === "category"
          ? ((e.layout = "horizontal"),
            (r = n.getOrdinalMeta()),
            (l = !this._hasEncodeRule("x")))
          : s === "category"
          ? ((e.layout = "vertical"),
            (r = i.getOrdinalMeta()),
            (l = !this._hasEncodeRule("y")))
          : (e.layout = e.layout || "horizontal");
        var u = ["x", "y"],
          v = e.layout === "horizontal" ? 0 : 1,
          c = (this._baseAxisDim = u[v]),
          h = u[1 - v],
          f = [n, i],
          p = f[v].get("type"),
          d = f[1 - v].get("type"),
          g = e.data;
        if (g && l) {
          var y = [];
          L(g, function (x, b) {
            var _;
            H(x)
              ? ((_ = x.slice()), x.unshift(b))
              : H(x.value)
              ? ((_ = W({}, x)),
                (_.value = _.value.slice()),
                x.value.unshift(b))
              : (_ = x),
              y.push(_);
          }),
            (e.data = y);
        }
        var S = this.defaultValueDimensions,
          m = [
            {
              name: c,
              type: Hn(p),
              ordinalMeta: r,
              otherDims: { tooltip: !1, itemName: 0 },
              dimsDef: ["base"]
            },
            { name: h, type: Hn(d), dimsDef: S.slice() }
          ];
        return Lr(this, {
          coordDimensions: m,
          dimensionsCount: S.length + 1,
          encodeDefaulter: ot(Ah, m, this)
        });
      }),
      (a.prototype.getBaseAxis = function () {
        var e = this._baseAxisDim;
        return this.ecModel.getComponent(e + "Axis", this.get(e + "AxisIndex"))
          .axis;
      }),
      a
    );
  })(),
  Pv = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (
        (t.type = e.type),
        (t.defaultValueDimensions = [
          { name: "min", defaultTooltip: !0 },
          { name: "Q1", defaultTooltip: !0 },
          { name: "median", defaultTooltip: !0 },
          { name: "Q3", defaultTooltip: !0 },
          { name: "max", defaultTooltip: !0 }
        ]),
        (t.visualDrawType = "stroke"),
        t
      );
    }
    return (
      (e.type = "series.boxplot"),
      (e.dependencies = ["xAxis", "yAxis", "grid"]),
      (e.defaultOption = {
        z: 2,
        coordinateSystem: "cartesian2d",
        legendHoverLink: !0,
        layout: null,
        boxWidth: [7, 50],
        itemStyle: { color: "#fff", borderWidth: 1 },
        emphasis: {
          scale: !0,
          itemStyle: {
            borderWidth: 2,
            shadowBlur: 5,
            shadowOffsetX: 1,
            shadowOffsetY: 1,
            shadowColor: "rgba(0,0,0,0.2)"
          }
        },
        animationDuration: 800
      }),
      e
    );
  })(gt);
te(Pv, Cv, !0);
var Tm = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i = t.getData(),
          o = this.group,
          s = this._data;
        this._data || o.removeAll();
        var l = t.get("layout") === "horizontal" ? 1 : 0;
        i
          .diff(s)
          .add(function (u) {
            if (i.hasValue(u)) {
              var v = i.getItemLayout(u),
                c = Xs(v, i, u, l, !0);
              i.setItemGraphicEl(u, c), o.add(c);
            }
          })
          .update(function (u, v) {
            var c = s.getItemGraphicEl(v);
            if (!i.hasValue(u)) {
              o.remove(c);
              return;
            }
            var h = i.getItemLayout(u);
            c ? (Se(c), Mv(h, c, i, u)) : (c = Xs(h, i, u, l)),
              o.add(c),
              i.setItemGraphicEl(u, c);
          })
          .remove(function (u) {
            var v = s.getItemGraphicEl(u);
            v && o.remove(v);
          })
          .execute(),
          (this._data = i);
      }),
      (e.prototype.remove = function (t) {
        var r = this.group,
          n = this._data;
        (this._data = null),
          n &&
            n.eachItemGraphicEl(function (i) {
              i && r.remove(i);
            });
      }),
      (e.type = "boxplot"),
      e
    );
  })(pt),
  Im = (function () {
    function a() {}
    return a;
  })(),
  Dm = (function (a) {
    k(e, a);
    function e(t) {
      var r = a.call(this, t) || this;
      return (r.type = "boxplotBoxPath"), r;
    }
    return (
      (e.prototype.getDefaultShape = function () {
        return new Im();
      }),
      (e.prototype.buildPath = function (t, r) {
        var n = r.points,
          i = 0;
        for (t.moveTo(n[i][0], n[i][1]), i++; i < 4; i++)
          t.lineTo(n[i][0], n[i][1]);
        for (t.closePath(); i < n.length; i++)
          t.moveTo(n[i][0], n[i][1]), i++, t.lineTo(n[i][0], n[i][1]);
      }),
      e
    );
  })(wt);
function Xs(a, e, t, r, n) {
  var i = a.ends,
    o = new Dm({ shape: { points: n ? Lm(i, r, a) : i } });
  return Mv(a, o, e, t, n), o;
}
function Mv(a, e, t, r, n) {
  var i = t.hostModel,
    o = Qe[n ? "initProps" : "updateProps"];
  o(e, { shape: { points: a.ends } }, i, r),
    e.useStyle(t.getItemVisual(r, "style")),
    (e.style.strokeNoScale = !0),
    (e.z2 = 100);
  var s = t.getItemModel(r),
    l = s.getModel("emphasis");
  zt(e, s), ct(e, l.get("focus"), l.get("blurScope"), l.get("disabled"));
}
function Lm(a, e, t) {
  return O(a, function (r) {
    return (r = r.slice()), (r[e] = t.initBaseline), r;
  });
}
var dr = L;
function Cm(a) {
  var e = Pm(a);
  dr(e, function (t) {
    var r = t.seriesModels;
    r.length &&
      (Mm(t),
      dr(r, function (n, i) {
        Em(n, t.boxOffsetList[i], t.boxWidthList[i]);
      }));
  });
}
function Pm(a) {
  var e = [],
    t = [];
  return (
    a.eachSeriesByType("boxplot", function (r) {
      var n = r.getBaseAxis(),
        i = Ht(t, n);
      i < 0 &&
        ((i = t.length), (t[i] = n), (e[i] = { axis: n, seriesModels: [] })),
        e[i].seriesModels.push(r);
    }),
    e
  );
}
function Mm(a) {
  var e = a.axis,
    t = a.seriesModels,
    r = t.length,
    n = (a.boxWidthList = []),
    i = (a.boxOffsetList = []),
    o = [],
    s;
  if (e.type === "category") s = e.getBandWidth();
  else {
    var l = 0;
    dr(t, function (p) {
      l = Math.max(l, p.getData().count());
    });
    var u = e.getExtent();
    s = Math.abs(u[1] - u[0]) / l;
  }
  dr(t, function (p) {
    var d = p.get("boxWidth");
    H(d) || (d = [d, d]), o.push([z(d[0], s) || 0, z(d[1], s) || 0]);
  });
  var v = s * 0.8 - 2,
    c = (v / r) * 0.3,
    h = (v - c * (r - 1)) / r,
    f = h / 2 - v / 2;
  dr(t, function (p, d) {
    i.push(f), (f += c + h), n.push(Math.min(Math.max(h, o[d][0]), o[d][1]));
  });
}
function Em(a, e, t) {
  var r = a.coordinateSystem,
    n = a.getData(),
    i = t / 2,
    o = a.get("layout") === "horizontal" ? 0 : 1,
    s = 1 - o,
    l = ["x", "y"],
    u = n.mapDimension(l[o]),
    v = n.mapDimensionsAll(l[s]);
  if (u == null || v.length < 5) return;
  for (var c = 0; c < n.count(); c++) {
    var h = n.get(u, c),
      f = m(h, v[2], c),
      p = m(h, v[0], c),
      d = m(h, v[1], c),
      g = m(h, v[3], c),
      y = m(h, v[4], c),
      S = [];
    x(S, d, !1),
      x(S, g, !0),
      S.push(p, d, y, g),
      b(S, p),
      b(S, y),
      b(S, f),
      n.setItemLayout(c, { initBaseline: f[s], ends: S });
  }
  function m(_, w, A) {
    var I = n.get(w, A),
      T = [];
    (T[o] = _), (T[s] = I);
    var D;
    return (
      isNaN(_) || isNaN(I)
        ? (D = [NaN, NaN])
        : ((D = r.dataToPoint(T)), (D[o] += e)),
      D
    );
  }
  function x(_, w, A) {
    var I = w.slice(),
      T = w.slice();
    (I[o] += i), (T[o] -= i), A ? _.push(I, T) : _.push(T, I);
  }
  function b(_, w) {
    var A = w.slice(),
      I = w.slice();
    (A[o] -= i), (I[o] += i), _.push(A, I);
  }
}
function Rm(a, e) {
  e = e || {};
  for (
    var t = [], r = [], n = e.boundIQR, i = n === "none" || n === 0, o = 0;
    o < a.length;
    o++
  ) {
    var s = Sr(a[o].slice()),
      l = Oa(s, 0.25),
      u = Oa(s, 0.5),
      v = Oa(s, 0.75),
      c = s[0],
      h = s[s.length - 1],
      f = (n ?? 1.5) * (v - l),
      p = i ? c : Math.max(c, l - f),
      d = i ? h : Math.min(h, v + f),
      g = e.itemNameFormatter,
      y = it(g)
        ? g({ value: o })
        : et(g)
        ? g.replace("{value}", o + "")
        : o + "";
    t.push([y, p, l, u, v, d]);
    for (var S = 0; S < s.length; S++) {
      var m = s[S];
      if (m < p || m > d) {
        var x = [y, m];
        r.push(x);
      }
    }
  }
  return { boxData: t, outliers: r };
}
var Nm = {
  type: "echarts:boxplot",
  transform: function (e) {
    var t = e.upstream;
    if (t.sourceFormat !== Du) {
      var r = "";
      vt(r);
    }
    var n = Rm(t.getRawData(), e.config);
    return [
      {
        dimensions: ["ItemName", "Low", "Q1", "Q2", "Q3", "High"],
        data: n.boxData
      },
      { data: n.outliers }
    ];
  }
};
function km(a) {
  a.registerSeriesModel(Pv),
    a.registerChartView(Tm),
    a.registerLayout(Cm),
    a.registerTransform(Nm);
}
var Vm = ["itemStyle", "borderColor"],
  Gm = ["itemStyle", "borderColor0"],
  zm = ["itemStyle", "borderColorDoji"],
  Om = ["itemStyle", "color"],
  Bm = ["itemStyle", "color0"];
function Zi(a, e) {
  return e.get(a > 0 ? Om : Bm);
}
function qi(a, e) {
  return e.get(a === 0 ? zm : a > 0 ? Vm : Gm);
}
var Fm = {
    seriesType: "candlestick",
    plan: _i(),
    performRawSeries: !0,
    reset: function (a, e) {
      if (!e.isSeriesFiltered(a)) {
        var t = a.pipelineContext.large;
        return (
          !t && {
            progress: function (r, n) {
              for (var i; (i = r.next()) != null; ) {
                var o = n.getItemModel(i),
                  s = n.getItemLayout(i).sign,
                  l = o.getItemStyle();
                (l.fill = Zi(s, o)), (l.stroke = qi(s, o) || l.fill);
                var u = n.ensureUniqueItemVisual(i, "style");
                W(u, l);
              }
            }
          }
        );
      }
    }
  },
  Wm = ["color", "borderColor"],
  Hm = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        this.group.removeClipPath(),
          (this._progressiveEls = null),
          this._updateDrawMode(t),
          this._isLargeDraw ? this._renderLarge(t) : this._renderNormal(t);
      }),
      (e.prototype.incrementalPrepareRender = function (t, r, n) {
        this._clear(), this._updateDrawMode(t);
      }),
      (e.prototype.incrementalRender = function (t, r, n, i) {
        (this._progressiveEls = []),
          this._isLargeDraw
            ? this._incrementalRenderLarge(t, r)
            : this._incrementalRenderNormal(t, r);
      }),
      (e.prototype.eachRendered = function (t) {
        Ea(this._progressiveEls || this.group, t);
      }),
      (e.prototype._updateDrawMode = function (t) {
        var r = t.pipelineContext.large;
        (this._isLargeDraw == null || r !== this._isLargeDraw) &&
          ((this._isLargeDraw = r), this._clear());
      }),
      (e.prototype._renderNormal = function (t) {
        var r = t.getData(),
          n = this._data,
          i = this.group,
          o = r.getLayout("isSimpleBox"),
          s = t.get("clip", !0),
          l = t.coordinateSystem,
          u = l.getArea && l.getArea();
        this._data || i.removeAll(),
          r
            .diff(n)
            .add(function (v) {
              if (r.hasValue(v)) {
                var c = r.getItemLayout(v);
                if (s && Zs(u, c)) return;
                var h = gn(c, v, !0);
                Ct(h, { shape: { points: c.ends } }, t, v),
                  yn(h, r, v, o),
                  i.add(h),
                  r.setItemGraphicEl(v, h);
              }
            })
            .update(function (v, c) {
              var h = n.getItemGraphicEl(c);
              if (!r.hasValue(v)) {
                i.remove(h);
                return;
              }
              var f = r.getItemLayout(v);
              if (s && Zs(u, f)) {
                i.remove(h);
                return;
              }
              h
                ? (lt(h, { shape: { points: f.ends } }, t, v), Se(h))
                : (h = gn(f)),
                yn(h, r, v, o),
                i.add(h),
                r.setItemGraphicEl(v, h);
            })
            .remove(function (v) {
              var c = n.getItemGraphicEl(v);
              c && i.remove(c);
            })
            .execute(),
          (this._data = r);
      }),
      (e.prototype._renderLarge = function (t) {
        this._clear(), qs(t, this.group);
        var r = t.get("clip", !0) ? Ra(t.coordinateSystem, !1, t) : null;
        r ? this.group.setClipPath(r) : this.group.removeClipPath();
      }),
      (e.prototype._incrementalRenderNormal = function (t, r) {
        for (
          var n = r.getData(), i = n.getLayout("isSimpleBox"), o;
          (o = t.next()) != null;

        ) {
          var s = n.getItemLayout(o),
            l = gn(s);
          yn(l, n, o, i),
            (l.incremental = !0),
            this.group.add(l),
            this._progressiveEls.push(l);
        }
      }),
      (e.prototype._incrementalRenderLarge = function (t, r) {
        qs(r, this.group, this._progressiveEls, !0);
      }),
      (e.prototype.remove = function (t) {
        this._clear();
      }),
      (e.prototype._clear = function () {
        this.group.removeAll(), (this._data = null);
      }),
      (e.type = "candlestick"),
      e
    );
  })(pt),
  Um = (function () {
    function a() {}
    return a;
  })(),
  Ym = (function (a) {
    k(e, a);
    function e(t) {
      var r = a.call(this, t) || this;
      return (r.type = "normalCandlestickBox"), r;
    }
    return (
      (e.prototype.getDefaultShape = function () {
        return new Um();
      }),
      (e.prototype.buildPath = function (t, r) {
        var n = r.points;
        this.__simpleBox
          ? (t.moveTo(n[4][0], n[4][1]), t.lineTo(n[6][0], n[6][1]))
          : (t.moveTo(n[0][0], n[0][1]),
            t.lineTo(n[1][0], n[1][1]),
            t.lineTo(n[2][0], n[2][1]),
            t.lineTo(n[3][0], n[3][1]),
            t.closePath(),
            t.moveTo(n[4][0], n[4][1]),
            t.lineTo(n[5][0], n[5][1]),
            t.moveTo(n[6][0], n[6][1]),
            t.lineTo(n[7][0], n[7][1]));
      }),
      e
    );
  })(wt);
function gn(a, e, t) {
  var r = a.ends;
  return new Ym({ shape: { points: t ? $m(r, a) : r }, z2: 100 });
}
function Zs(a, e) {
  for (var t = !0, r = 0; r < e.ends.length; r++)
    if (a.contain(e.ends[r][0], e.ends[r][1])) {
      t = !1;
      break;
    }
  return t;
}
function yn(a, e, t, r) {
  var n = e.getItemModel(t);
  a.useStyle(e.getItemVisual(t, "style")),
    (a.style.strokeNoScale = !0),
    (a.__simpleBox = r),
    zt(a, n);
  var i = e.getItemLayout(t).sign;
  L(a.states, function (s, l) {
    var u = n.getModel(l),
      v = Zi(i, u),
      c = qi(i, u) || v,
      h = s.style || (s.style = {});
    v && (h.fill = v), c && (h.stroke = c);
  });
  var o = n.getModel("emphasis");
  ct(a, o.get("focus"), o.get("blurScope"), o.get("disabled"));
}
function $m(a, e) {
  return O(a, function (t) {
    return (t = t.slice()), (t[1] = e.initBaseline), t;
  });
}
var Xm = (function () {
    function a() {}
    return a;
  })(),
  mn = (function (a) {
    k(e, a);
    function e(t) {
      var r = a.call(this, t) || this;
      return (r.type = "largeCandlestickBox"), r;
    }
    return (
      (e.prototype.getDefaultShape = function () {
        return new Xm();
      }),
      (e.prototype.buildPath = function (t, r) {
        for (var n = r.points, i = 0; i < n.length; )
          if (this.__sign === n[i++]) {
            var o = n[i++];
            t.moveTo(o, n[i++]), t.lineTo(o, n[i++]);
          } else i += 3;
      }),
      e
    );
  })(wt);
function qs(a, e, t, r) {
  var n = a.getData(),
    i = n.getLayout("largePoints"),
    o = new mn({ shape: { points: i }, __sign: 1, ignoreCoarsePointer: !0 });
  e.add(o);
  var s = new mn({ shape: { points: i }, __sign: -1, ignoreCoarsePointer: !0 });
  e.add(s);
  var l = new mn({ shape: { points: i }, __sign: 0, ignoreCoarsePointer: !0 });
  e.add(l),
    Sn(1, o, a),
    Sn(-1, s, a),
    Sn(0, l, a),
    r && ((o.incremental = !0), (s.incremental = !0)),
    t && t.push(o, s);
}
function Sn(a, e, t, r) {
  var n = qi(a, t) || Zi(a, t),
    i = t.getModel("itemStyle").getItemStyle(Wm);
  e.useStyle(i), (e.style.fill = null), (e.style.stroke = n);
}
var Ev = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (
      (t.type = e.type),
      (t.defaultValueDimensions = [
        { name: "open", defaultTooltip: !0 },
        { name: "close", defaultTooltip: !0 },
        { name: "lowest", defaultTooltip: !0 },
        { name: "highest", defaultTooltip: !0 }
      ]),
      t
    );
  }
  return (
    (e.prototype.getShadowDim = function () {
      return "open";
    }),
    (e.prototype.brushSelector = function (t, r, n) {
      var i = r.getItemLayout(t);
      return i && n.rect(i.brushRect);
    }),
    (e.type = "series.candlestick"),
    (e.dependencies = ["xAxis", "yAxis", "grid"]),
    (e.defaultOption = {
      z: 2,
      coordinateSystem: "cartesian2d",
      legendHoverLink: !0,
      layout: null,
      clip: !0,
      itemStyle: {
        color: "#eb5454",
        color0: "#47b262",
        borderColor: "#eb5454",
        borderColor0: "#47b262",
        borderColorDoji: null,
        borderWidth: 1
      },
      emphasis: { itemStyle: { borderWidth: 2 } },
      barMaxWidth: null,
      barMinWidth: null,
      barWidth: null,
      large: !0,
      largeThreshold: 600,
      progressive: 3e3,
      progressiveThreshold: 1e4,
      progressiveChunkMode: "mod",
      animationEasing: "linear",
      animationDuration: 300
    }),
    e
  );
})(gt);
te(Ev, Cv, !0);
function Zm(a) {
  !a ||
    !H(a.series) ||
    L(a.series, function (e) {
      Ge(e) && e.type === "k" && (e.type = "candlestick");
    });
}
var qm = {
  seriesType: "candlestick",
  plan: _i(),
  reset: function (a) {
    var e = a.coordinateSystem,
      t = a.getData(),
      r = Km(a, t),
      n = 0,
      i = 1,
      o = ["x", "y"],
      s = t.getDimensionIndex(t.mapDimension(o[n])),
      l = O(t.mapDimensionsAll(o[i]), t.getDimensionIndex, t),
      u = l[0],
      v = l[1],
      c = l[2],
      h = l[3];
    if (
      (t.setLayout({ candleWidth: r, isSimpleBox: r <= 1.3 }),
      s < 0 || l.length < 4)
    )
      return;
    return { progress: a.pipelineContext.large ? p : f };
    function f(d, g) {
      for (var y, S = g.getStore(); (y = d.next()) != null; ) {
        var m = S.get(s, y),
          x = S.get(u, y),
          b = S.get(v, y),
          _ = S.get(c, y),
          w = S.get(h, y),
          A = Math.min(x, b),
          I = Math.max(x, b),
          T = N(A, m),
          D = N(I, m),
          M = N(_, m),
          C = N(w, m),
          P = [];
        V(P, D, 0), V(P, T, 1), P.push(B(C), B(D), B(M), B(T));
        var E = g.getItemModel(y),
          R = !!E.get(["itemStyle", "borderColorDoji"]);
        g.setItemLayout(y, {
          sign: Ks(S, y, x, b, v, R),
          initBaseline: x > b ? D[i] : T[i],
          ends: P,
          brushRect: G(_, w, m)
        });
      }
      function N(F, Y) {
        var K = [];
        return (
          (K[n] = Y),
          (K[i] = F),
          isNaN(Y) || isNaN(F) ? [NaN, NaN] : e.dataToPoint(K)
        );
      }
      function V(F, Y, K) {
        var $ = Y.slice(),
          j = Y.slice();
        ($[n] = Ba($[n] + r / 2, 1, !1)),
          (j[n] = Ba(j[n] - r / 2, 1, !0)),
          K ? F.push($, j) : F.push(j, $);
      }
      function G(F, Y, K) {
        var $ = N(F, K),
          j = N(Y, K);
        return (
          ($[n] -= r / 2),
          (j[n] -= r / 2),
          { x: $[0], y: $[1], width: r, height: j[1] - $[1] }
        );
      }
      function B(F) {
        return (F[n] = Ba(F[n], 1)), F;
      }
    }
    function p(d, g) {
      for (
        var y = Th(d.count * 4),
          S = 0,
          m,
          x = [],
          b = [],
          _,
          w = g.getStore(),
          A = !!a.get(["itemStyle", "borderColorDoji"]);
        (_ = d.next()) != null;

      ) {
        var I = w.get(s, _),
          T = w.get(u, _),
          D = w.get(v, _),
          M = w.get(c, _),
          C = w.get(h, _);
        if (isNaN(I) || isNaN(M) || isNaN(C)) {
          (y[S++] = NaN), (S += 3);
          continue;
        }
        (y[S++] = Ks(w, _, T, D, v, A)),
          (x[n] = I),
          (x[i] = M),
          (m = e.dataToPoint(x, null, b)),
          (y[S++] = m ? m[0] : NaN),
          (y[S++] = m ? m[1] : NaN),
          (x[i] = C),
          (m = e.dataToPoint(x, null, b)),
          (y[S++] = m ? m[1] : NaN);
      }
      g.setLayout("largePoints", y);
    }
  }
};
function Ks(a, e, t, r, n, i) {
  var o;
  return (
    t > r
      ? (o = -1)
      : t < r
      ? (o = 1)
      : (o = i ? 0 : e > 0 ? (a.get(n, e - 1) <= r ? 1 : -1) : 1),
    o
  );
}
function Km(a, e) {
  var t = a.getBaseAxis(),
    r,
    n =
      t.type === "category"
        ? t.getBandWidth()
        : ((r = t.getExtent()), Math.abs(r[1] - r[0]) / e.count()),
    i = z(St(a.get("barMaxWidth"), n), n),
    o = z(St(a.get("barMinWidth"), 1), n),
    s = a.get("barWidth");
  return s != null ? z(s, n) : Math.max(Math.min(n / 2, i), o);
}
function jm(a) {
  a.registerChartView(Hm),
    a.registerSeriesModel(Ev),
    a.registerPreprocessor(Zm),
    a.registerVisual(Fm),
    a.registerLayout(qm);
}
function js(a, e) {
  var t = e.rippleEffectColor || e.color;
  a.eachChild(function (r) {
    r.attr({
      z: e.z,
      zlevel: e.zlevel,
      style: {
        stroke: e.brushType === "stroke" ? t : null,
        fill: e.brushType === "fill" ? t : null
      }
    });
  });
}
var Jm = (function (a) {
    k(e, a);
    function e(t, r) {
      var n = a.call(this) || this,
        i = new yu(t, r),
        o = new Z();
      return n.add(i), n.add(o), n.updateData(t, r), n;
    }
    return (
      (e.prototype.stopEffectAnimation = function () {
        this.childAt(1).removeAll();
      }),
      (e.prototype.startEffectAnimation = function (t) {
        for (
          var r = t.symbolType,
            n = t.color,
            i = t.rippleNumber,
            o = this.childAt(1),
            s = 0;
          s < i;
          s++
        ) {
          var l = ie(r, -1, -1, 2, 2, n);
          l.attr({
            style: { strokeNoScale: !0 },
            z2: 99,
            silent: !0,
            scaleX: 0.5,
            scaleY: 0.5
          });
          var u = (-s / i) * t.period + t.effectOffset;
          l
            .animate("", !0)
            .when(t.period, {
              scaleX: t.rippleScale / 2,
              scaleY: t.rippleScale / 2
            })
            .delay(u)
            .start(),
            l.animateStyle(!0).when(t.period, { opacity: 0 }).delay(u).start(),
            o.add(l);
        }
        js(o, t);
      }),
      (e.prototype.updateEffectAnimation = function (t) {
        for (
          var r = this._effectCfg,
            n = this.childAt(1),
            i = ["symbolType", "period", "rippleScale", "rippleNumber"],
            o = 0;
          o < i.length;
          o++
        ) {
          var s = i[o];
          if (r[s] !== t[s]) {
            this.stopEffectAnimation(), this.startEffectAnimation(t);
            return;
          }
        }
        js(n, t);
      }),
      (e.prototype.highlight = function () {
        yi(this);
      }),
      (e.prototype.downplay = function () {
        mi(this);
      }),
      (e.prototype.getSymbolType = function () {
        var t = this.childAt(0);
        return t && t.getSymbolType();
      }),
      (e.prototype.updateData = function (t, r) {
        var n = this,
          i = t.hostModel;
        this.childAt(0).updateData(t, r);
        var o = this.childAt(1),
          s = t.getItemModel(r),
          l = t.getItemVisual(r, "symbol"),
          u = Dr(t.getItemVisual(r, "symbolSize")),
          v = t.getItemVisual(r, "style"),
          c = v && v.fill,
          h = s.getModel("emphasis");
        o.setScale(u),
          o.traverse(function (g) {
            g.setStyle("fill", c);
          });
        var f = Nr(t.getItemVisual(r, "symbolOffset"), u);
        f && ((o.x = f[0]), (o.y = f[1]));
        var p = t.getItemVisual(r, "symbolRotate");
        o.rotation = ((p || 0) * Math.PI) / 180 || 0;
        var d = {};
        (d.showEffectOn = i.get("showEffectOn")),
          (d.rippleScale = s.get(["rippleEffect", "scale"])),
          (d.brushType = s.get(["rippleEffect", "brushType"])),
          (d.period = s.get(["rippleEffect", "period"]) * 1e3),
          (d.effectOffset = r / t.count()),
          (d.z = i.getShallow("z") || 0),
          (d.zlevel = i.getShallow("zlevel") || 0),
          (d.symbolType = l),
          (d.color = c),
          (d.rippleEffectColor = s.get(["rippleEffect", "color"])),
          (d.rippleNumber = s.get(["rippleEffect", "number"])),
          d.showEffectOn === "render"
            ? (this._effectCfg
                ? this.updateEffectAnimation(d)
                : this.startEffectAnimation(d),
              (this._effectCfg = d))
            : ((this._effectCfg = null),
              this.stopEffectAnimation(),
              (this.onHoverStateChange = function (g) {
                g === "emphasis"
                  ? d.showEffectOn !== "render" && n.startEffectAnimation(d)
                  : g === "normal" &&
                    d.showEffectOn !== "render" &&
                    n.stopEffectAnimation();
              })),
          (this._effectCfg = d),
          ct(this, h.get("focus"), h.get("blurScope"), h.get("disabled"));
      }),
      (e.prototype.fadeOut = function (t) {
        t && t();
      }),
      e
    );
  })(Z),
  Qm = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.init = function () {
        this._symbolDraw = new Aa(Jm);
      }),
      (e.prototype.render = function (t, r, n) {
        var i = t.getData(),
          o = this._symbolDraw;
        o.updateData(i, { clipShape: this._getClipShape(t) }),
          this.group.add(o.group);
      }),
      (e.prototype._getClipShape = function (t) {
        var r = t.coordinateSystem,
          n = r && r.getArea && r.getArea();
        return t.get("clip", !0) ? n : null;
      }),
      (e.prototype.updateTransform = function (t, r, n) {
        var i = t.getData();
        this.group.dirty();
        var o = wa("").reset(t, r, n);
        o.progress &&
          o.progress({ start: 0, end: i.count(), count: i.count() }, i),
          this._symbolDraw.updateLayout();
      }),
      (e.prototype._updateGroupTransform = function (t) {
        var r = t.coordinateSystem;
        r &&
          r.getRoamTransform &&
          ((this.group.transform = Ih(r.getRoamTransform())),
          this.group.decomposeTransform());
      }),
      (e.prototype.remove = function (t, r) {
        this._symbolDraw && this._symbolDraw.remove(!0);
      }),
      (e.type = "effectScatter"),
      e
    );
  })(pt),
  t0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.hasSymbolVisual = !0), t;
    }
    return (
      (e.prototype.getInitialData = function (t, r) {
        return Je(null, this, { useEncodeDefaulter: !0 });
      }),
      (e.prototype.brushSelector = function (t, r, n) {
        return n.point(r.getItemLayout(t));
      }),
      (e.type = "series.effectScatter"),
      (e.dependencies = ["grid", "polar"]),
      (e.defaultOption = {
        coordinateSystem: "cartesian2d",
        z: 2,
        legendHoverLink: !0,
        effectType: "ripple",
        progressive: 0,
        showEffectOn: "render",
        clip: !0,
        rippleEffect: { period: 4, scale: 2.5, brushType: "fill", number: 3 },
        universalTransition: { divideShape: "clone" },
        symbolSize: 10
      }),
      e
    );
  })(gt);
function e0(a) {
  a.registerChartView(Qm),
    a.registerSeriesModel(t0),
    a.registerLayout(wa("effectScatter"));
}
var Rv = (function (a) {
    k(e, a);
    function e(t, r, n) {
      var i = a.call(this) || this;
      return i.add(i.createLine(t, r, n)), i._updateEffectSymbol(t, r), i;
    }
    return (
      (e.prototype.createLine = function (t, r, n) {
        return new $i(t, r, n);
      }),
      (e.prototype._updateEffectSymbol = function (t, r) {
        var n = t.getItemModel(r),
          i = n.getModel("effect"),
          o = i.get("symbolSize"),
          s = i.get("symbol");
        H(o) || (o = [o, o]);
        var l = t.getItemVisual(r, "style"),
          u = i.get("color") || (l && l.stroke),
          v = this.childAt(1);
        this._symbolType !== s &&
          (this.remove(v),
          (v = ie(s, -0.5, -0.5, 1, 1, u)),
          (v.z2 = 100),
          (v.culling = !0),
          this.add(v)),
          v &&
            (v.setStyle("shadowColor", u),
            v.setStyle(i.getItemStyle(["color"])),
            (v.scaleX = o[0]),
            (v.scaleY = o[1]),
            v.setColor(u),
            (this._symbolType = s),
            (this._symbolScale = o),
            this._updateEffectAnimation(t, i, r));
      }),
      (e.prototype._updateEffectAnimation = function (t, r, n) {
        var i = this.childAt(1);
        if (i) {
          var o = t.getItemLayout(n),
            s = r.get("period") * 1e3,
            l = r.get("loop"),
            u = r.get("roundTrip"),
            v = r.get("constantSpeed"),
            c = Ft(r.get("delay"), function (f) {
              return ((f / t.count()) * s) / 3;
            });
          if (
            ((i.ignore = !0),
            this._updateAnimationPoints(i, o),
            v > 0 && (s = (this._getLineLength(i) / v) * 1e3),
            s !== this._period || l !== this._loop || u !== this._roundTrip)
          ) {
            i.stopAnimation();
            var h = void 0;
            it(c) ? (h = c(n)) : (h = c),
              i.__t > 0 && (h = -s * i.__t),
              this._animateSymbol(i, s, h, l, u);
          }
          (this._period = s), (this._loop = l), (this._roundTrip = u);
        }
      }),
      (e.prototype._animateSymbol = function (t, r, n, i, o) {
        if (r > 0) {
          t.__t = 0;
          var s = this,
            l = t
              .animate("", i)
              .when(o ? r * 2 : r, { __t: o ? 2 : 1 })
              .delay(n)
              .during(function () {
                s._updateSymbolPosition(t);
              });
          i ||
            l.done(function () {
              s.remove(t);
            }),
            l.start();
        }
      }),
      (e.prototype._getLineLength = function (t) {
        return vr(t.__p1, t.__cp1) + vr(t.__cp1, t.__p2);
      }),
      (e.prototype._updateAnimationPoints = function (t, r) {
        (t.__p1 = r[0]),
          (t.__p2 = r[1]),
          (t.__cp1 = r[2] || [
            (r[0][0] + r[1][0]) / 2,
            (r[0][1] + r[1][1]) / 2
          ]);
      }),
      (e.prototype.updateData = function (t, r, n) {
        this.childAt(0).updateData(t, r, n), this._updateEffectSymbol(t, r);
      }),
      (e.prototype._updateSymbolPosition = function (t) {
        var r = t.__p1,
          n = t.__p2,
          i = t.__cp1,
          o = t.__t < 1 ? t.__t : 2 - t.__t,
          s = [t.x, t.y],
          l = s.slice(),
          u = wu,
          v = Dh;
        (s[0] = u(r[0], i[0], n[0], o)), (s[1] = u(r[1], i[1], n[1], o));
        var c = t.__t < 1 ? v(r[0], i[0], n[0], o) : v(n[0], i[0], r[0], 1 - o),
          h = t.__t < 1 ? v(r[1], i[1], n[1], o) : v(n[1], i[1], r[1], 1 - o);
        (t.rotation = -Math.atan2(h, c) - Math.PI / 2),
          (this._symbolType === "line" ||
            this._symbolType === "rect" ||
            this._symbolType === "roundRect") &&
            (t.__lastT !== void 0 && t.__lastT < t.__t
              ? ((t.scaleY = vr(l, s) * 1.05),
                o === 1 &&
                  ((s[0] = l[0] + (s[0] - l[0]) / 2),
                  (s[1] = l[1] + (s[1] - l[1]) / 2)))
              : t.__lastT === 1
              ? (t.scaleY = 2 * vr(r, s))
              : (t.scaleY = this._symbolScale[1])),
          (t.__lastT = t.__t),
          (t.ignore = !1),
          (t.x = s[0]),
          (t.y = s[1]);
      }),
      (e.prototype.updateLayout = function (t, r) {
        this.childAt(0).updateLayout(t, r);
        var n = t.getItemModel(r).getModel("effect");
        this._updateEffectAnimation(t, n, r);
      }),
      e
    );
  })(Z),
  Nv = (function (a) {
    k(e, a);
    function e(t, r, n) {
      var i = a.call(this) || this;
      return i._createPolyline(t, r, n), i;
    }
    return (
      (e.prototype._createPolyline = function (t, r, n) {
        var i = t.getItemLayout(r),
          o = new we({ shape: { points: i } });
        this.add(o), this._updateCommonStl(t, r, n);
      }),
      (e.prototype.updateData = function (t, r, n) {
        var i = t.hostModel,
          o = this.childAt(0),
          s = { shape: { points: t.getItemLayout(r) } };
        lt(o, s, i, r), this._updateCommonStl(t, r, n);
      }),
      (e.prototype._updateCommonStl = function (t, r, n) {
        var i = this.childAt(0),
          o = t.getItemModel(r),
          s = n && n.emphasisLineStyle,
          l = n && n.focus,
          u = n && n.blurScope,
          v = n && n.emphasisDisabled;
        if (!n || t.hasItemOption) {
          var c = o.getModel("emphasis");
          (s = c.getModel("lineStyle").getLineStyle()),
            (v = c.get("disabled")),
            (l = c.get("focus")),
            (u = c.get("blurScope"));
        }
        i.useStyle(t.getItemVisual(r, "style")),
          (i.style.fill = null),
          (i.style.strokeNoScale = !0);
        var h = i.ensureState("emphasis");
        (h.style = s), ct(this, l, u, v);
      }),
      (e.prototype.updateLayout = function (t, r) {
        var n = this.childAt(0);
        n.setShape("points", t.getItemLayout(r));
      }),
      e
    );
  })(Z),
  r0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t._lastFrame = 0), (t._lastFramePercent = 0), t;
    }
    return (
      (e.prototype.createLine = function (t, r, n) {
        return new Nv(t, r, n);
      }),
      (e.prototype._updateAnimationPoints = function (t, r) {
        this._points = r;
        for (var n = [0], i = 0, o = 1; o < r.length; o++) {
          var s = r[o - 1],
            l = r[o];
          (i += vr(s, l)), n.push(i);
        }
        if (i === 0) {
          this._length = 0;
          return;
        }
        for (var o = 0; o < n.length; o++) n[o] /= i;
        (this._offsets = n), (this._length = i);
      }),
      (e.prototype._getLineLength = function () {
        return this._length;
      }),
      (e.prototype._updateSymbolPosition = function (t) {
        var r = t.__t < 1 ? t.__t : 2 - t.__t,
          n = this._points,
          i = this._offsets,
          o = n.length;
        if (i) {
          var s = this._lastFrame,
            l;
          if (r < this._lastFramePercent) {
            var u = Math.min(s + 1, o - 1);
            for (l = u; l >= 0 && !(i[l] <= r); l--);
            l = Math.min(l, o - 2);
          } else {
            for (l = s; l < o && !(i[l] > r); l++);
            l = Math.min(l - 1, o - 2);
          }
          var v = (r - i[l]) / (i[l + 1] - i[l]),
            c = n[l],
            h = n[l + 1];
          (t.x = c[0] * (1 - v) + v * h[0]), (t.y = c[1] * (1 - v) + v * h[1]);
          var f = t.__t < 1 ? h[0] - c[0] : c[0] - h[0],
            p = t.__t < 1 ? h[1] - c[1] : c[1] - h[1];
          (t.rotation = -Math.atan2(p, f) - Math.PI / 2),
            (this._lastFrame = l),
            (this._lastFramePercent = r),
            (t.ignore = !1);
        }
      }),
      e
    );
  })(Rv),
  a0 = (function () {
    function a() {
      (this.polyline = !1), (this.curveness = 0), (this.segs = []);
    }
    return a;
  })(),
  n0 = (function (a) {
    k(e, a);
    function e(t) {
      var r = a.call(this, t) || this;
      return (r._off = 0), (r.hoverDataIdx = -1), r;
    }
    return (
      (e.prototype.reset = function () {
        (this.notClear = !1), (this._off = 0);
      }),
      (e.prototype.getDefaultStyle = function () {
        return { stroke: "#000", fill: null };
      }),
      (e.prototype.getDefaultShape = function () {
        return new a0();
      }),
      (e.prototype.buildPath = function (t, r) {
        var n = r.segs,
          i = r.curveness,
          o;
        if (r.polyline)
          for (o = this._off; o < n.length; ) {
            var s = n[o++];
            if (s > 0) {
              t.moveTo(n[o++], n[o++]);
              for (var l = 1; l < s; l++) t.lineTo(n[o++], n[o++]);
            }
          }
        else
          for (o = this._off; o < n.length; ) {
            var u = n[o++],
              v = n[o++],
              c = n[o++],
              h = n[o++];
            if ((t.moveTo(u, v), i > 0)) {
              var f = (u + c) / 2 - (v - h) * i,
                p = (v + h) / 2 - (c - u) * i;
              t.quadraticCurveTo(f, p, c, h);
            } else t.lineTo(c, h);
          }
        this.incremental && ((this._off = o), (this.notClear = !0));
      }),
      (e.prototype.findDataIndex = function (t, r) {
        var n = this.shape,
          i = n.segs,
          o = n.curveness,
          s = this.style.lineWidth;
        if (n.polyline)
          for (var l = 0, u = 0; u < i.length; ) {
            var v = i[u++];
            if (v > 0)
              for (var c = i[u++], h = i[u++], f = 1; f < v; f++) {
                var p = i[u++],
                  d = i[u++];
                if (wo(c, h, p, d, s, t, r)) return l;
              }
            l++;
          }
        else
          for (var l = 0, u = 0; u < i.length; ) {
            var c = i[u++],
              h = i[u++],
              p = i[u++],
              d = i[u++];
            if (o > 0) {
              var g = (c + p) / 2 - (h - d) * o,
                y = (h + d) / 2 - (p - c) * o;
              if (Lh(c, h, g, y, p, d, s, t, r)) return l;
            } else if (wo(c, h, p, d, s, t, r)) return l;
            l++;
          }
        return -1;
      }),
      (e.prototype.contain = function (t, r) {
        var n = this.transformCoordToLocal(t, r),
          i = this.getBoundingRect();
        if (((t = n[0]), (r = n[1]), i.contain(t, r))) {
          var o = (this.hoverDataIdx = this.findDataIndex(t, r));
          return o >= 0;
        }
        return (this.hoverDataIdx = -1), !1;
      }),
      (e.prototype.getBoundingRect = function () {
        var t = this._rect;
        if (!t) {
          for (
            var r = this.shape,
              n = r.segs,
              i = 1 / 0,
              o = 1 / 0,
              s = -1 / 0,
              l = -1 / 0,
              u = 0;
            u < n.length;

          ) {
            var v = n[u++],
              c = n[u++];
            (i = Math.min(v, i)),
              (s = Math.max(v, s)),
              (o = Math.min(c, o)),
              (l = Math.max(c, l));
          }
          t = this._rect = new ft(i, o, s, l);
        }
        return t;
      }),
      e
    );
  })(wt),
  i0 = (function () {
    function a() {
      this.group = new Z();
    }
    return (
      (a.prototype.updateData = function (e) {
        this._clear();
        var t = this._create();
        t.setShape({ segs: e.getLayout("linesPoints") }), this._setCommon(t, e);
      }),
      (a.prototype.incrementalPrepareUpdate = function (e) {
        this.group.removeAll(), this._clear();
      }),
      (a.prototype.incrementalUpdate = function (e, t) {
        var r = this._newAdded[0],
          n = t.getLayout("linesPoints"),
          i = r && r.shape.segs;
        if (i && i.length < 2e4) {
          var o = i.length,
            s = new Float32Array(o + n.length);
          s.set(i), s.set(n, o), r.setShape({ segs: s });
        } else {
          this._newAdded = [];
          var l = this._create();
          (l.incremental = !0),
            l.setShape({ segs: n }),
            this._setCommon(l, t),
            (l.__startIndex = e.start);
        }
      }),
      (a.prototype.remove = function () {
        this._clear();
      }),
      (a.prototype.eachRendered = function (e) {
        this._newAdded[0] && e(this._newAdded[0]);
      }),
      (a.prototype._create = function () {
        var e = new n0({ cursor: "default", ignoreCoarsePointer: !0 });
        return this._newAdded.push(e), this.group.add(e), e;
      }),
      (a.prototype._setCommon = function (e, t, r) {
        var n = t.hostModel;
        e.setShape({
          polyline: n.get("polyline"),
          curveness: n.get(["lineStyle", "curveness"])
        }),
          e.useStyle(n.getModel("lineStyle").getLineStyle()),
          (e.style.strokeNoScale = !0);
        var i = t.getVisual("style");
        i && i.stroke && e.setStyle("stroke", i.stroke),
          e.setStyle("fill", null);
        var o = nt(e);
        (o.seriesIndex = n.seriesIndex),
          e.on("mousemove", function (s) {
            o.dataIndex = null;
            var l = e.hoverDataIdx;
            l > 0 && (o.dataIndex = l + e.__startIndex);
          });
      }),
      (a.prototype._clear = function () {
        (this._newAdded = []), this.group.removeAll();
      }),
      a
    );
  })(),
  kv = {
    seriesType: "lines",
    plan: _i(),
    reset: function (a) {
      var e = a.coordinateSystem;
      if (e) {
        var t = a.get("polyline"),
          r = a.pipelineContext.large;
        return {
          progress: function (n, i) {
            var o = [];
            if (r) {
              var s = void 0,
                l = n.end - n.start;
              if (t) {
                for (var u = 0, v = n.start; v < n.end; v++)
                  u += a.getLineCoordsCount(v);
                s = new Float32Array(l + u * 2);
              } else s = new Float32Array(l * 4);
              for (var c = 0, h = [], v = n.start; v < n.end; v++) {
                var f = a.getLineCoords(v, o);
                t && (s[c++] = f);
                for (var p = 0; p < f; p++)
                  (h = e.dataToPoint(o[p], !1, h)),
                    (s[c++] = h[0]),
                    (s[c++] = h[1]);
              }
              i.setLayout("linesPoints", s);
            } else
              for (var v = n.start; v < n.end; v++) {
                var d = i.getItemModel(v),
                  f = a.getLineCoords(v, o),
                  g = [];
                if (t) for (var y = 0; y < f; y++) g.push(e.dataToPoint(o[y]));
                else {
                  (g[0] = e.dataToPoint(o[0])), (g[1] = e.dataToPoint(o[1]));
                  var S = d.get(["lineStyle", "curveness"]);
                  +S &&
                    (g[2] = [
                      (g[0][0] + g[1][0]) / 2 - (g[0][1] - g[1][1]) * S,
                      (g[0][1] + g[1][1]) / 2 - (g[1][0] - g[0][0]) * S
                    ]);
                }
                i.setItemLayout(v, g);
              }
          }
        };
      }
    }
  },
  o0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i = t.getData(),
          o = this._updateLineDraw(i, t),
          s = t.get("zlevel"),
          l = t.get(["effect", "trailLength"]),
          u = n.getZr(),
          v = u.painter.getType() === "svg";
        v || u.painter.getLayer(s).clear(!0),
          this._lastZlevel != null &&
            !v &&
            u.configLayer(this._lastZlevel, { motionBlur: !1 }),
          this._showEffect(t) &&
            l > 0 &&
            (v ||
              u.configLayer(s, {
                motionBlur: !0,
                lastFrameAlpha: Math.max(Math.min(l / 10 + 0.9, 1), 0)
              })),
          o.updateData(i);
        var c = t.get("clip", !0) && Ra(t.coordinateSystem, !1, t);
        c ? this.group.setClipPath(c) : this.group.removeClipPath(),
          (this._lastZlevel = s),
          (this._finished = !0);
      }),
      (e.prototype.incrementalPrepareRender = function (t, r, n) {
        var i = t.getData(),
          o = this._updateLineDraw(i, t);
        o.incrementalPrepareUpdate(i),
          this._clearLayer(n),
          (this._finished = !1);
      }),
      (e.prototype.incrementalRender = function (t, r, n) {
        this._lineDraw.incrementalUpdate(t, r.getData()),
          (this._finished = t.end === r.getData().count());
      }),
      (e.prototype.eachRendered = function (t) {
        this._lineDraw && this._lineDraw.eachRendered(t);
      }),
      (e.prototype.updateTransform = function (t, r, n) {
        var i = t.getData(),
          o = t.pipelineContext;
        if (!this._finished || o.large || o.progressiveRender)
          return { update: !0 };
        var s = kv.reset(t, r, n);
        s.progress &&
          s.progress({ start: 0, end: i.count(), count: i.count() }, i),
          this._lineDraw.updateLayout(),
          this._clearLayer(n);
      }),
      (e.prototype._updateLineDraw = function (t, r) {
        var n = this._lineDraw,
          i = this._showEffect(r),
          o = !!r.get("polyline"),
          s = r.pipelineContext,
          l = s.large;
        return (
          (!n ||
            i !== this._hasEffet ||
            o !== this._isPolyline ||
            l !== this._isLargeDraw) &&
            (n && n.remove(),
            (n = this._lineDraw =
              l ? new i0() : new Xi(o ? (i ? r0 : Nv) : i ? Rv : $i)),
            (this._hasEffet = i),
            (this._isPolyline = o),
            (this._isLargeDraw = l)),
          this.group.add(n.group),
          n
        );
      }),
      (e.prototype._showEffect = function (t) {
        return !!t.get(["effect", "show"]);
      }),
      (e.prototype._clearLayer = function (t) {
        var r = t.getZr(),
          n = r.painter.getType() === "svg";
        !n &&
          this._lastZlevel != null &&
          r.painter.getLayer(this._lastZlevel).clear(!0);
      }),
      (e.prototype.remove = function (t, r) {
        this._lineDraw && this._lineDraw.remove(),
          (this._lineDraw = null),
          this._clearLayer(r);
      }),
      (e.prototype.dispose = function (t, r) {
        this.remove(t, r);
      }),
      (e.type = "lines"),
      e
    );
  })(pt),
  s0 = typeof Uint32Array > "u" ? Array : Uint32Array,
  l0 = typeof Float64Array > "u" ? Array : Float64Array;
function Js(a) {
  var e = a.data;
  e &&
    e[0] &&
    e[0][0] &&
    e[0][0].coord &&
    (a.data = O(e, function (t) {
      var r = [t[0].coord, t[1].coord],
        n = { coords: r };
      return (
        t[0].name && (n.fromName = t[0].name),
        t[1].name && (n.toName = t[1].name),
        pi([n, t[0], t[1]])
      );
    }));
}
var u0 = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (
      (t.type = e.type),
      (t.visualStyleAccessPath = "lineStyle"),
      (t.visualDrawType = "stroke"),
      t
    );
  }
  return (
    (e.prototype.init = function (t) {
      (t.data = t.data || []), Js(t);
      var r = this._processFlatCoordsArray(t.data);
      (this._flatCoords = r.flatCoords),
        (this._flatCoordsOffset = r.flatCoordsOffset),
        r.flatCoords && (t.data = new Float32Array(r.count)),
        a.prototype.init.apply(this, arguments);
    }),
    (e.prototype.mergeOption = function (t) {
      if ((Js(t), t.data)) {
        var r = this._processFlatCoordsArray(t.data);
        (this._flatCoords = r.flatCoords),
          (this._flatCoordsOffset = r.flatCoordsOffset),
          r.flatCoords && (t.data = new Float32Array(r.count));
      }
      a.prototype.mergeOption.apply(this, arguments);
    }),
    (e.prototype.appendData = function (t) {
      var r = this._processFlatCoordsArray(t.data);
      r.flatCoords &&
        (this._flatCoords
          ? ((this._flatCoords = ra(this._flatCoords, r.flatCoords)),
            (this._flatCoordsOffset = ra(
              this._flatCoordsOffset,
              r.flatCoordsOffset
            )))
          : ((this._flatCoords = r.flatCoords),
            (this._flatCoordsOffset = r.flatCoordsOffset)),
        (t.data = new Float32Array(r.count))),
        this.getRawData().appendData(t.data);
    }),
    (e.prototype._getCoordsFromItemModel = function (t) {
      var r = this.getData().getItemModel(t),
        n = r.option instanceof Array ? r.option : r.getShallow("coords");
      return n;
    }),
    (e.prototype.getLineCoordsCount = function (t) {
      return this._flatCoordsOffset
        ? this._flatCoordsOffset[t * 2 + 1]
        : this._getCoordsFromItemModel(t).length;
    }),
    (e.prototype.getLineCoords = function (t, r) {
      if (this._flatCoordsOffset) {
        for (
          var n = this._flatCoordsOffset[t * 2],
            i = this._flatCoordsOffset[t * 2 + 1],
            o = 0;
          o < i;
          o++
        )
          (r[o] = r[o] || []),
            (r[o][0] = this._flatCoords[n + o * 2]),
            (r[o][1] = this._flatCoords[n + o * 2 + 1]);
        return i;
      } else {
        for (var s = this._getCoordsFromItemModel(t), o = 0; o < s.length; o++)
          (r[o] = r[o] || []), (r[o][0] = s[o][0]), (r[o][1] = s[o][1]);
        return s.length;
      }
    }),
    (e.prototype._processFlatCoordsArray = function (t) {
      var r = 0;
      if ((this._flatCoords && (r = this._flatCoords.length), Qt(t[0]))) {
        for (
          var n = t.length,
            i = new s0(n),
            o = new l0(n),
            s = 0,
            l = 0,
            u = 0,
            v = 0;
          v < n;

        ) {
          u++;
          var c = t[v++];
          (i[l++] = s + r), (i[l++] = c);
          for (var h = 0; h < c; h++) {
            var f = t[v++],
              p = t[v++];
            (o[s++] = f), (o[s++] = p);
          }
        }
        return {
          flatCoordsOffset: new Uint32Array(i.buffer, 0, l),
          flatCoords: o,
          count: u
        };
      }
      return { flatCoordsOffset: null, flatCoords: null, count: t.length };
    }),
    (e.prototype.getInitialData = function (t, r) {
      var n = new Vt(["value"], this);
      return (
        (n.hasItemOption = !1),
        n.initData(t.data, [], function (i, o, s, l) {
          if (i instanceof Array) return NaN;
          n.hasItemOption = !0;
          var u = i.value;
          if (u != null) return u instanceof Array ? u[l] : u;
        }),
        n
      );
    }),
    (e.prototype.formatTooltip = function (t, r, n) {
      var i = this.getData(),
        o = i.getItemModel(t),
        s = o.get("name");
      if (s) return s;
      var l = o.get("fromName"),
        u = o.get("toName"),
        v = [];
      return (
        l != null && v.push(l),
        u != null && v.push(u),
        Mt("nameValue", { name: v.join(" > ") })
      );
    }),
    (e.prototype.preventIncremental = function () {
      return !!this.get(["effect", "show"]);
    }),
    (e.prototype.getProgressive = function () {
      var t = this.option.progressive;
      return t ?? (this.option.large ? 1e4 : this.get("progressive"));
    }),
    (e.prototype.getProgressiveThreshold = function () {
      var t = this.option.progressiveThreshold;
      return t ?? (this.option.large ? 2e4 : this.get("progressiveThreshold"));
    }),
    (e.prototype.getZLevelKey = function () {
      var t = this.getModel("effect"),
        r = t.get("trailLength");
      return this.getData().count() > this.getProgressiveThreshold()
        ? this.id
        : t.get("show") && r > 0
        ? r + ""
        : "";
    }),
    (e.type = "series.lines"),
    (e.dependencies = ["grid", "polar", "geo", "calendar"]),
    (e.defaultOption = {
      coordinateSystem: "geo",
      z: 2,
      legendHoverLink: !0,
      xAxisIndex: 0,
      yAxisIndex: 0,
      symbol: ["none", "none"],
      symbolSize: [10, 10],
      geoIndex: 0,
      effect: {
        show: !1,
        period: 4,
        constantSpeed: 0,
        symbol: "circle",
        symbolSize: 3,
        loop: !0,
        trailLength: 0.2
      },
      large: !1,
      largeThreshold: 2e3,
      polyline: !1,
      clip: !0,
      label: { show: !1, position: "end" },
      lineStyle: { opacity: 0.5 }
    }),
    e
  );
})(gt);
function $r(a) {
  return a instanceof Array || (a = [a, a]), a;
}
var v0 = {
  seriesType: "lines",
  reset: function (a) {
    var e = $r(a.get("symbol")),
      t = $r(a.get("symbolSize")),
      r = a.getData();
    r.setVisual("fromSymbol", e && e[0]),
      r.setVisual("toSymbol", e && e[1]),
      r.setVisual("fromSymbolSize", t && t[0]),
      r.setVisual("toSymbolSize", t && t[1]);
    function n(i, o) {
      var s = i.getItemModel(o),
        l = $r(s.getShallow("symbol", !0)),
        u = $r(s.getShallow("symbolSize", !0));
      l[0] && i.setItemVisual(o, "fromSymbol", l[0]),
        l[1] && i.setItemVisual(o, "toSymbol", l[1]),
        u[0] && i.setItemVisual(o, "fromSymbolSize", u[0]),
        u[1] && i.setItemVisual(o, "toSymbolSize", u[1]);
    }
    return { dataEach: r.hasItemOption ? n : null };
  }
};
function c0(a) {
  a.registerChartView(o0),
    a.registerSeriesModel(u0),
    a.registerLayout(kv),
    a.registerVisual(v0);
}
var h0 = 256,
  f0 = (function () {
    function a() {
      (this.blurSize = 30),
        (this.pointSize = 20),
        (this.maxOpacity = 1),
        (this.minOpacity = 0),
        (this._gradientPixels = { inRange: null, outOfRange: null });
      var e = Ao.createCanvas();
      this.canvas = e;
    }
    return (
      (a.prototype.update = function (e, t, r, n, i, o) {
        var s = this._getBrush(),
          l = this._getGradient(i, "inRange"),
          u = this._getGradient(i, "outOfRange"),
          v = this.pointSize + this.blurSize,
          c = this.canvas,
          h = c.getContext("2d"),
          f = e.length;
        (c.width = t), (c.height = r);
        for (var p = 0; p < f; ++p) {
          var d = e[p],
            g = d[0],
            y = d[1],
            S = d[2],
            m = n(S);
          (h.globalAlpha = m), h.drawImage(s, g - v, y - v);
        }
        if (!c.width || !c.height) return c;
        for (
          var x = h.getImageData(0, 0, c.width, c.height),
            b = x.data,
            _ = 0,
            w = b.length,
            A = this.minOpacity,
            I = this.maxOpacity,
            T = I - A;
          _ < w;

        ) {
          var m = b[_ + 3] / 256,
            D = Math.floor(m * (h0 - 1)) * 4;
          if (m > 0) {
            var M = o(m) ? l : u;
            m > 0 && (m = m * T + A),
              (b[_++] = M[D]),
              (b[_++] = M[D + 1]),
              (b[_++] = M[D + 2]),
              (b[_++] = M[D + 3] * m * 256);
          } else _ += 4;
        }
        return h.putImageData(x, 0, 0), c;
      }),
      (a.prototype._getBrush = function () {
        var e = this._brushCanvas || (this._brushCanvas = Ao.createCanvas()),
          t = this.pointSize + this.blurSize,
          r = t * 2;
        (e.width = r), (e.height = r);
        var n = e.getContext("2d");
        return (
          n.clearRect(0, 0, r, r),
          (n.shadowOffsetX = r),
          (n.shadowBlur = this.blurSize),
          (n.shadowColor = "#000"),
          n.beginPath(),
          n.arc(-t, t, this.pointSize, 0, Math.PI * 2, !0),
          n.closePath(),
          n.fill(),
          e
        );
      }),
      (a.prototype._getGradient = function (e, t) {
        for (
          var r = this._gradientPixels,
            n = r[t] || (r[t] = new Uint8ClampedArray(256 * 4)),
            i = [0, 0, 0, 0],
            o = 0,
            s = 0;
          s < 256;
          s++
        )
          e[t](s / 255, !0, i),
            (n[o++] = i[0]),
            (n[o++] = i[1]),
            (n[o++] = i[2]),
            (n[o++] = i[3]);
        return n;
      }),
      a
    );
  })();
function p0(a, e, t) {
  var r = a[1] - a[0];
  e = O(e, function (o) {
    return {
      interval: [(o.interval[0] - a[0]) / r, (o.interval[1] - a[0]) / r]
    };
  });
  var n = e.length,
    i = 0;
  return function (o) {
    var s;
    for (s = i; s < n; s++) {
      var l = e[s].interval;
      if (l[0] <= o && o <= l[1]) {
        i = s;
        break;
      }
    }
    if (s === n)
      for (s = i - 1; s >= 0; s--) {
        var l = e[s].interval;
        if (l[0] <= o && o <= l[1]) {
          i = s;
          break;
        }
      }
    return s >= 0 && s < n && t[s];
  };
}
function d0(a, e) {
  var t = a[1] - a[0];
  return (
    (e = [(e[0] - a[0]) / t, (e[1] - a[0]) / t]),
    function (r) {
      return r >= e[0] && r <= e[1];
    }
  );
}
function Qs(a) {
  var e = a.dimensions;
  return e[0] === "lng" && e[1] === "lat";
}
var g0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i;
        r.eachComponent("visualMap", function (s) {
          s.eachTargetSeries(function (l) {
            l === t && (i = s);
          });
        }),
          (this._progressiveEls = null),
          this.group.removeAll();
        var o = t.coordinateSystem;
        o.type === "cartesian2d" || o.type === "calendar"
          ? this._renderOnCartesianAndCalendar(t, n, 0, t.getData().count())
          : Qs(o) && this._renderOnGeo(o, t, i, n);
      }),
      (e.prototype.incrementalPrepareRender = function (t, r, n) {
        this.group.removeAll();
      }),
      (e.prototype.incrementalRender = function (t, r, n, i) {
        var o = r.coordinateSystem;
        o &&
          (Qs(o)
            ? this.render(r, n, i)
            : ((this._progressiveEls = []),
              this._renderOnCartesianAndCalendar(r, i, t.start, t.end, !0)));
      }),
      (e.prototype.eachRendered = function (t) {
        Ea(this._progressiveEls || this.group, t);
      }),
      (e.prototype._renderOnCartesianAndCalendar = function (t, r, n, i, o) {
        var s = t.coordinateSystem,
          l = Na(s, "cartesian2d"),
          u,
          v,
          c,
          h;
        if (l) {
          var f = s.getAxis("x"),
            p = s.getAxis("y");
          (u = f.getBandWidth() + 0.5),
            (v = p.getBandWidth() + 0.5),
            (c = f.scale.getExtent()),
            (h = p.scale.getExtent());
        }
        for (
          var d = this.group,
            g = t.getData(),
            y = t.getModel(["emphasis", "itemStyle"]).getItemStyle(),
            S = t.getModel(["blur", "itemStyle"]).getItemStyle(),
            m = t.getModel(["select", "itemStyle"]).getItemStyle(),
            x = t.get(["itemStyle", "borderRadius"]),
            b = Pt(t),
            _ = t.getModel("emphasis"),
            w = _.get("focus"),
            A = _.get("blurScope"),
            I = _.get("disabled"),
            T = l
              ? [
                  g.mapDimension("x"),
                  g.mapDimension("y"),
                  g.mapDimension("value")
                ]
              : [g.mapDimension("time"), g.mapDimension("value")],
            D = n;
          D < i;
          D++
        ) {
          var M = void 0,
            C = g.getItemVisual(D, "style");
          if (l) {
            var P = g.get(T[0], D),
              E = g.get(T[1], D);
            if (
              isNaN(g.get(T[2], D)) ||
              isNaN(P) ||
              isNaN(E) ||
              P < c[0] ||
              P > c[1] ||
              E < h[0] ||
              E > h[1]
            )
              continue;
            var R = s.dataToPoint([P, E]);
            M = new Et({
              shape: { x: R[0] - u / 2, y: R[1] - v / 2, width: u, height: v },
              style: C
            });
          } else {
            if (isNaN(g.get(T[1], D))) continue;
            M = new Et({
              z2: 1,
              shape: s.dataToRect([g.get(T[0], D)]).contentShape,
              style: C
            });
          }
          if (g.hasItemOption) {
            var N = g.getItemModel(D),
              V = N.getModel("emphasis");
            (y = V.getModel("itemStyle").getItemStyle()),
              (S = N.getModel(["blur", "itemStyle"]).getItemStyle()),
              (m = N.getModel(["select", "itemStyle"]).getItemStyle()),
              (x = N.get(["itemStyle", "borderRadius"])),
              (w = V.get("focus")),
              (A = V.get("blurScope")),
              (I = V.get("disabled")),
              (b = Pt(N));
          }
          M.shape.r = x;
          var G = t.getRawValue(D),
            B = "-";
          G && G[2] != null && (B = G[2] + ""),
            Ut(M, b, {
              labelFetcher: t,
              labelDataIndex: D,
              defaultOpacity: C.opacity,
              defaultText: B
            }),
            (M.ensureState("emphasis").style = y),
            (M.ensureState("blur").style = S),
            (M.ensureState("select").style = m),
            ct(M, w, A, I),
            (M.incremental = o),
            o && (M.states.emphasis.hoverLayer = !0),
            d.add(M),
            g.setItemGraphicEl(D, M),
            this._progressiveEls && this._progressiveEls.push(M);
        }
      }),
      (e.prototype._renderOnGeo = function (t, r, n, i) {
        var o = n.targetVisuals.inRange,
          s = n.targetVisuals.outOfRange,
          l = r.getData(),
          u = this._hmLayer || this._hmLayer || new f0();
        (u.blurSize = r.get("blurSize")),
          (u.pointSize = r.get("pointSize")),
          (u.minOpacity = r.get("minOpacity")),
          (u.maxOpacity = r.get("maxOpacity"));
        var v = t.getViewRect().clone(),
          c = t.getRoamTransform();
        v.applyTransform(c);
        var h = Math.max(v.x, 0),
          f = Math.max(v.y, 0),
          p = Math.min(v.width + v.x, i.getWidth()),
          d = Math.min(v.height + v.y, i.getHeight()),
          g = p - h,
          y = d - f,
          S = [
            l.mapDimension("lng"),
            l.mapDimension("lat"),
            l.mapDimension("value")
          ],
          m = l.mapArray(S, function (w, A, I) {
            var T = t.dataToPoint([w, A]);
            return (T[0] -= h), (T[1] -= f), T.push(I), T;
          }),
          x = n.getExtent(),
          b =
            n.type === "visualMap.continuous"
              ? d0(x, n.option.range)
              : p0(x, n.getPieceList(), n.option.selected);
        u.update(
          m,
          g,
          y,
          o.color.getNormalizer(),
          {
            inRange: o.color.getColorMapper(),
            outOfRange: s.color.getColorMapper()
          },
          b
        );
        var _ = new _e({
          style: { width: g, height: y, x: h, y: f, image: u.canvas },
          silent: !0
        });
        this.group.add(_);
      }),
      (e.type = "heatmap"),
      e
    );
  })(pt),
  y0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.getInitialData = function (t, r) {
        return Je(null, this, { generateCoord: "value" });
      }),
      (e.prototype.preventIncremental = function () {
        var t = Au.get(this.get("coordinateSystem"));
        if (t && t.dimensions)
          return t.dimensions[0] === "lng" && t.dimensions[1] === "lat";
      }),
      (e.type = "series.heatmap"),
      (e.dependencies = ["grid", "geo", "calendar"]),
      (e.defaultOption = {
        coordinateSystem: "cartesian2d",
        z: 2,
        geoIndex: 0,
        blurSize: 30,
        pointSize: 20,
        maxOpacity: 1,
        minOpacity: 0,
        select: { itemStyle: { borderColor: "#212121" } }
      }),
      e
    );
  })(gt);
function m0(a) {
  a.registerChartView(g0), a.registerSeriesModel(y0);
}
var S0 = ["itemStyle", "borderWidth"],
  tl = [
    { xy: "x", wh: "width", index: 0, posDesc: ["left", "right"] },
    { xy: "y", wh: "height", index: 1, posDesc: ["top", "bottom"] }
  ],
  xn = new Cr(),
  x0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i = this.group,
          o = t.getData(),
          s = this._data,
          l = t.coordinateSystem,
          u = l.getBaseAxis(),
          v = u.isHorizontal(),
          c = l.master.getRect(),
          h = {
            ecSize: { width: n.getWidth(), height: n.getHeight() },
            seriesModel: t,
            coordSys: l,
            coordSysExtent: [
              [c.x, c.x + c.width],
              [c.y, c.y + c.height]
            ],
            isHorizontal: v,
            valueDim: tl[+v],
            categoryDim: tl[1 - +v]
          };
        o.diff(s)
          .add(function (p) {
            if (o.hasValue(p)) {
              var d = rl(o, p),
                g = el(o, p, d, h),
                y = al(o, h, g);
              o.setItemGraphicEl(p, y), i.add(y), il(y, h, g);
            }
          })
          .update(function (p, d) {
            var g = s.getItemGraphicEl(d);
            if (!o.hasValue(p)) {
              i.remove(g);
              return;
            }
            var y = rl(o, p),
              S = el(o, p, y, h),
              m = Fv(o, S);
            g &&
              m !== g.__pictorialShapeStr &&
              (i.remove(g), o.setItemGraphicEl(p, null), (g = null)),
              g ? D0(g, h, S) : (g = al(o, h, S, !0)),
              o.setItemGraphicEl(p, g),
              (g.__pictorialSymbolMeta = S),
              i.add(g),
              il(g, h, S);
          })
          .remove(function (p) {
            var d = s.getItemGraphicEl(p);
            d && nl(s, p, d.__pictorialSymbolMeta.animationModel, d);
          })
          .execute();
        var f = t.get("clip", !0) ? Ra(t.coordinateSystem, !1, t) : null;
        return (
          f ? i.setClipPath(f) : i.removeClipPath(),
          (this._data = o),
          this.group
        );
      }),
      (e.prototype.remove = function (t, r) {
        var n = this.group,
          i = this._data;
        t.get("animation")
          ? i &&
            i.eachItemGraphicEl(function (o) {
              nl(i, nt(o).dataIndex, t, o);
            })
          : n.removeAll();
      }),
      (e.type = "pictorialBar"),
      e
    );
  })(pt);
function el(a, e, t, r) {
  var n = a.getItemLayout(e),
    i = t.get("symbolRepeat"),
    o = t.get("symbolClip"),
    s = t.get("symbolPosition") || "start",
    l = t.get("symbolRotate"),
    u = ((l || 0) * Math.PI) / 180 || 0,
    v = t.get("symbolPatternSize") || 2,
    c = t.isAnimationEnabled(),
    h = {
      dataIndex: e,
      layout: n,
      itemModel: t,
      symbolType: a.getItemVisual(e, "symbol") || "circle",
      style: a.getItemVisual(e, "style"),
      symbolClip: o,
      symbolRepeat: i,
      symbolRepeatDirection: t.get("symbolRepeatDirection"),
      symbolPatternSize: v,
      rotation: u,
      animationModel: c ? t : null,
      hoverScale: c && t.get(["emphasis", "scale"]),
      z2: t.getShallow("z", !0) || 0
    };
  b0(t, i, n, r, h),
    _0(a, e, n, i, o, h.boundingLength, h.pxSign, v, r, h),
    w0(t, h.symbolScale, u, r, h);
  var f = h.symbolSize,
    p = Nr(t.get("symbolOffset"), f);
  return (
    A0(
      t,
      f,
      n,
      i,
      o,
      p,
      s,
      h.valueLineWidth,
      h.boundingLength,
      h.repeatCutLength,
      r,
      h
    ),
    h
  );
}
function b0(a, e, t, r, n) {
  var i = r.valueDim,
    o = a.get("symbolBoundingData"),
    s = r.coordSys.getOtherAxis(r.coordSys.getBaseAxis()),
    l = s.toGlobalCoord(s.dataToCoord(0)),
    u = 1 - +(t[i.wh] <= 0),
    v;
  if (H(o)) {
    var c = [bn(s, o[0]) - l, bn(s, o[1]) - l];
    c[1] < c[0] && c.reverse(), (v = c[u]);
  } else
    o != null
      ? (v = bn(s, o) - l)
      : e
      ? (v = r.coordSysExtent[i.index][u] - l)
      : (v = t[i.wh]);
  (n.boundingLength = v), e && (n.repeatCutLength = t[i.wh]);
  var h = i.xy === "x",
    f = s.inverse;
  n.pxSign = (h && !f) || (!h && f) ? (v >= 0 ? 1 : -1) : v > 0 ? 1 : -1;
}
function bn(a, e) {
  return a.toGlobalCoord(a.dataToCoord(a.scale.parse(e)));
}
function _0(a, e, t, r, n, i, o, s, l, u) {
  var v = l.valueDim,
    c = l.categoryDim,
    h = Math.abs(t[c.wh]),
    f = a.getItemVisual(e, "symbolSize"),
    p;
  H(f) ? (p = f.slice()) : f == null ? (p = ["100%", "100%"]) : (p = [f, f]),
    (p[c.index] = z(p[c.index], h)),
    (p[v.index] = z(p[v.index], r ? h : Math.abs(i))),
    (u.symbolSize = p);
  var d = (u.symbolScale = [p[0] / s, p[1] / s]);
  d[v.index] *= (l.isHorizontal ? -1 : 1) * o;
}
function w0(a, e, t, r, n) {
  var i = a.get(S0) || 0;
  i &&
    (xn.attr({ scaleX: e[0], scaleY: e[1], rotation: t }),
    xn.updateTransform(),
    (i /= xn.getLineScale()),
    (i *= e[r.valueDim.index])),
    (n.valueLineWidth = i || 0);
}
function A0(a, e, t, r, n, i, o, s, l, u, v, c) {
  var h = v.categoryDim,
    f = v.valueDim,
    p = c.pxSign,
    d = Math.max(e[f.index] + s, 0),
    g = d;
  if (r) {
    var y = Math.abs(l),
      S = Ft(a.get("symbolMargin"), "15%") + "",
      m = !1;
    S.lastIndexOf("!") === S.length - 1 &&
      ((m = !0), (S = S.slice(0, S.length - 1)));
    var x = z(S, e[f.index]),
      b = Math.max(d + x * 2, 0),
      _ = m ? 0 : x * 2,
      w = Ch(r),
      A = w ? r : ol((y + _) / b),
      I = y - A * d;
    (x = I / 2 / (m ? A : Math.max(A - 1, 1))),
      (b = d + x * 2),
      (_ = m ? 0 : x * 2),
      !w && r !== "fixed" && (A = u ? ol((Math.abs(u) + _) / b) : 0),
      (g = A * b - _),
      (c.repeatTimes = A),
      (c.symbolMargin = x);
  }
  var T = p * (g / 2),
    D = (c.pathPosition = []);
  (D[h.index] = t[h.wh] / 2),
    (D[f.index] = o === "start" ? T : o === "end" ? l - T : l / 2),
    i && ((D[0] += i[0]), (D[1] += i[1]));
  var M = (c.bundlePosition = []);
  (M[h.index] = t[h.xy]), (M[f.index] = t[f.xy]);
  var C = (c.barRectShape = W({}, t));
  (C[f.wh] = p * Math.max(Math.abs(t[f.wh]), Math.abs(D[f.index] + T))),
    (C[h.wh] = t[h.wh]);
  var P = (c.clipShape = {});
  (P[h.xy] = -t[h.xy]),
    (P[h.wh] = v.ecSize[h.wh]),
    (P[f.xy] = 0),
    (P[f.wh] = t[f.wh]);
}
function Vv(a) {
  var e = a.symbolPatternSize,
    t = ie(a.symbolType, -e / 2, -e / 2, e, e);
  return (
    t.attr({ culling: !0 }),
    t.type !== "image" && t.setStyle({ strokeNoScale: !0 }),
    t
  );
}
function Gv(a, e, t, r) {
  var n = a.__pictorialBundle,
    i = t.symbolSize,
    o = t.valueLineWidth,
    s = t.pathPosition,
    l = e.valueDim,
    u = t.repeatTimes || 0,
    v = 0,
    c = i[e.valueDim.index] + o + t.symbolMargin * 2;
  for (
    Ki(a, function (d) {
      (d.__pictorialAnimationIndex = v),
        (d.__pictorialRepeatTimes = u),
        v < u
          ? Ze(d, null, p(v), t, r)
          : Ze(d, null, { scaleX: 0, scaleY: 0 }, t, r, function () {
              n.remove(d);
            }),
        v++;
    });
    v < u;
    v++
  ) {
    var h = Vv(t);
    (h.__pictorialAnimationIndex = v), (h.__pictorialRepeatTimes = u), n.add(h);
    var f = p(v);
    Ze(
      h,
      { x: f.x, y: f.y, scaleX: 0, scaleY: 0 },
      { scaleX: f.scaleX, scaleY: f.scaleY, rotation: f.rotation },
      t,
      r
    );
  }
  function p(d) {
    var g = s.slice(),
      y = t.pxSign,
      S = d;
    return (
      (t.symbolRepeatDirection === "start" ? y > 0 : y < 0) && (S = u - 1 - d),
      (g[l.index] = c * (S - u / 2 + 0.5) + s[l.index]),
      {
        x: g[0],
        y: g[1],
        scaleX: t.symbolScale[0],
        scaleY: t.symbolScale[1],
        rotation: t.rotation
      }
    );
  }
}
function zv(a, e, t, r) {
  var n = a.__pictorialBundle,
    i = a.__pictorialMainPath;
  i
    ? Ze(
        i,
        null,
        {
          x: t.pathPosition[0],
          y: t.pathPosition[1],
          scaleX: t.symbolScale[0],
          scaleY: t.symbolScale[1],
          rotation: t.rotation
        },
        t,
        r
      )
    : ((i = a.__pictorialMainPath = Vv(t)),
      n.add(i),
      Ze(
        i,
        {
          x: t.pathPosition[0],
          y: t.pathPosition[1],
          scaleX: 0,
          scaleY: 0,
          rotation: t.rotation
        },
        { scaleX: t.symbolScale[0], scaleY: t.symbolScale[1] },
        t,
        r
      ));
}
function Ov(a, e, t) {
  var r = W({}, e.barRectShape),
    n = a.__pictorialBarRect;
  n
    ? Ze(n, null, { shape: r }, e, t)
    : ((n = a.__pictorialBarRect =
        new Et({
          z2: 2,
          shape: r,
          silent: !0,
          style: { stroke: "transparent", fill: "transparent", lineWidth: 0 }
        })),
      (n.disableMorphing = !0),
      a.add(n));
}
function Bv(a, e, t, r) {
  if (t.symbolClip) {
    var n = a.__pictorialClipPath,
      i = W({}, t.clipShape),
      o = e.valueDim,
      s = t.animationModel,
      l = t.dataIndex;
    if (n) lt(n, { shape: i }, s, l);
    else {
      (i[o.wh] = 0),
        (n = new Et({ shape: i })),
        a.__pictorialBundle.setClipPath(n),
        (a.__pictorialClipPath = n);
      var u = {};
      (u[o.wh] = t.clipShape[o.wh]),
        Qe[r ? "updateProps" : "initProps"](n, { shape: u }, s, l);
    }
  }
}
function rl(a, e) {
  var t = a.getItemModel(e);
  return (t.getAnimationDelayParams = T0), (t.isAnimationEnabled = I0), t;
}
function T0(a) {
  return {
    index: a.__pictorialAnimationIndex,
    count: a.__pictorialRepeatTimes
  };
}
function I0() {
  return (
    this.parentModel.isAnimationEnabled() && !!this.getShallow("animation")
  );
}
function al(a, e, t, r) {
  var n = new Z(),
    i = new Z();
  return (
    n.add(i),
    (n.__pictorialBundle = i),
    (i.x = t.bundlePosition[0]),
    (i.y = t.bundlePosition[1]),
    t.symbolRepeat ? Gv(n, e, t) : zv(n, e, t),
    Ov(n, t, r),
    Bv(n, e, t, r),
    (n.__pictorialShapeStr = Fv(a, t)),
    (n.__pictorialSymbolMeta = t),
    n
  );
}
function D0(a, e, t) {
  var r = t.animationModel,
    n = t.dataIndex,
    i = a.__pictorialBundle;
  lt(i, { x: t.bundlePosition[0], y: t.bundlePosition[1] }, r, n),
    t.symbolRepeat ? Gv(a, e, t, !0) : zv(a, e, t, !0),
    Ov(a, t, !0),
    Bv(a, e, t, !0);
}
function nl(a, e, t, r) {
  var n = r.__pictorialBarRect;
  n && n.removeTextContent();
  var i = [];
  Ki(r, function (o) {
    i.push(o);
  }),
    r.__pictorialMainPath && i.push(r.__pictorialMainPath),
    r.__pictorialClipPath && (t = null),
    L(i, function (o) {
      aa(o, { scaleX: 0, scaleY: 0 }, t, e, function () {
        r.parent && r.parent.remove(r);
      });
    }),
    a.setItemGraphicEl(e, null);
}
function Fv(a, e) {
  return [
    a.getItemVisual(e.dataIndex, "symbol") || "none",
    !!e.symbolRepeat,
    !!e.symbolClip
  ].join(":");
}
function Ki(a, e, t) {
  L(a.__pictorialBundle.children(), function (r) {
    r !== a.__pictorialBarRect && e.call(t, r);
  });
}
function Ze(a, e, t, r, n, i) {
  e && a.attr(e),
    r.symbolClip && !n
      ? t && a.attr(t)
      : t &&
        Qe[n ? "updateProps" : "initProps"](
          a,
          t,
          r.animationModel,
          r.dataIndex,
          i
        );
}
function il(a, e, t) {
  var r = t.dataIndex,
    n = t.itemModel,
    i = n.getModel("emphasis"),
    o = i.getModel("itemStyle").getItemStyle(),
    s = n.getModel(["blur", "itemStyle"]).getItemStyle(),
    l = n.getModel(["select", "itemStyle"]).getItemStyle(),
    u = n.getShallow("cursor"),
    v = i.get("focus"),
    c = i.get("blurScope"),
    h = i.get("scale");
  Ki(a, function (d) {
    if (d instanceof _e) {
      var g = d.style;
      d.useStyle(
        W(
          { image: g.image, x: g.x, y: g.y, width: g.width, height: g.height },
          t.style
        )
      );
    } else d.useStyle(t.style);
    var y = d.ensureState("emphasis");
    (y.style = o),
      h && ((y.scaleX = d.scaleX * 1.1), (y.scaleY = d.scaleY * 1.1)),
      (d.ensureState("blur").style = s),
      (d.ensureState("select").style = l),
      u && (d.cursor = u),
      (d.z2 = t.z2);
  });
  var f = e.valueDim.posDesc[+(t.boundingLength > 0)],
    p = a.__pictorialBarRect;
  (p.ignoreClip = !0),
    Ut(p, Pt(n), {
      labelFetcher: e.seriesModel,
      labelDataIndex: r,
      defaultText: Un(e.seriesModel.getData(), r),
      inheritColor: t.style.fill,
      defaultOpacity: t.style.opacity,
      defaultOutsidePosition: f
    }),
    ct(a, v, c, i.get("disabled"));
}
function ol(a) {
  var e = Math.round(a);
  return Math.abs(a - e) < 1e-4 ? e : Math.ceil(a);
}
var L0 = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (
      (t.type = e.type),
      (t.hasSymbolVisual = !0),
      (t.defaultSymbol = "roundRect"),
      t
    );
  }
  return (
    (e.prototype.getInitialData = function (t) {
      return (
        (t.stack = null), a.prototype.getInitialData.apply(this, arguments)
      );
    }),
    (e.type = "series.pictorialBar"),
    (e.dependencies = ["grid"]),
    (e.defaultOption = Lu(To.defaultOption, {
      symbol: "circle",
      symbolSize: null,
      symbolRotate: null,
      symbolPosition: null,
      symbolOffset: null,
      symbolMargin: null,
      symbolRepeat: !1,
      symbolRepeatDirection: "end",
      symbolClip: !1,
      symbolBoundingData: null,
      symbolPatternSize: 400,
      barGap: "-100%",
      clip: !1,
      progressive: 0,
      emphasis: { scale: !1 },
      select: { itemStyle: { borderColor: "#212121" } }
    })),
    e
  );
})(To);
function C0(a) {
  a.registerChartView(x0),
    a.registerSeriesModel(L0),
    a.registerLayout(a.PRIORITY.VISUAL.LAYOUT, ot(Ph, "pictorialBar")),
    a.registerLayout(a.PRIORITY.VISUAL.PROGRESSIVE_LAYOUT, Mh("pictorialBar"));
}
var P0 = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), (t._layers = []), t;
  }
  return (
    (e.prototype.render = function (t, r, n) {
      var i = t.getData(),
        o = this,
        s = this.group,
        l = t.getLayerSeries(),
        u = i.getLayout("layoutInfo"),
        v = u.rect,
        c = u.boundaryGap;
      (s.x = 0), (s.y = v.y + c[0]);
      function h(g) {
        return g.name;
      }
      var f = new je(this._layersSeries || [], l, h, h),
        p = [];
      f.add(at(d, this, "add"))
        .update(at(d, this, "update"))
        .remove(at(d, this, "remove"))
        .execute();
      function d(g, y, S) {
        var m = o._layers;
        if (g === "remove") {
          s.remove(m[y]);
          return;
        }
        for (
          var x = [], b = [], _, w = l[y].indices, A = 0;
          A < w.length;
          A++
        ) {
          var I = i.getItemLayout(w[A]),
            T = I.x,
            D = I.y0,
            M = I.y;
          x.push(T, D), b.push(T, D + M), (_ = i.getItemVisual(w[A], "style"));
        }
        var C,
          P = i.getItemLayout(w[0]),
          E = t.getModel("label"),
          R = E.get("margin"),
          N = t.getModel("emphasis");
        if (g === "add") {
          var V = (p[y] = new Z());
          (C = new Eh({
            shape: {
              points: x,
              stackedOnPoints: b,
              smooth: 0.4,
              stackedOnSmooth: 0.4,
              smoothConstraint: !1
            },
            z2: 0
          })),
            V.add(C),
            s.add(V),
            t.isAnimationEnabled() &&
              C.setClipPath(
                M0(C.getBoundingRect(), t, function () {
                  C.removeClipPath();
                })
              );
        } else {
          var V = m[S];
          (C = V.childAt(0)),
            s.add(V),
            (p[y] = V),
            lt(C, { shape: { points: x, stackedOnPoints: b } }, t),
            Se(C);
        }
        Ut(
          C,
          Pt(t),
          {
            labelDataIndex: w[A - 1],
            defaultText: i.getName(w[A - 1]),
            inheritColor: _.fill
          },
          { normal: { verticalAlign: "middle" } }
        ),
          C.setTextConfig({ position: null, local: !0 });
        var G = C.getTextContent();
        G && ((G.x = P.x - R), (G.y = P.y0 + P.y / 2)),
          C.useStyle(_),
          i.setItemGraphicEl(y, C),
          zt(C, t),
          ct(C, N.get("focus"), N.get("blurScope"), N.get("disabled"));
      }
      (this._layersSeries = l), (this._layers = p);
    }),
    (e.type = "themeRiver"),
    e
  );
})(pt);
function M0(a, e, t) {
  var r = new Et({
    shape: { x: a.x - 10, y: a.y - 10, width: 0, height: a.height + 20 }
  });
  return (
    Ct(
      r,
      { shape: { x: a.x - 50, width: a.width + 100, height: a.height + 20 } },
      e,
      t
    ),
    r
  );
}
var _n = 2,
  E0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.init = function (t) {
        a.prototype.init.apply(this, arguments),
          (this.legendVisualProvider = new Ta(
            at(this.getData, this),
            at(this.getRawData, this)
          ));
      }),
      (e.prototype.fixData = function (t) {
        var r = t.length,
          n = {},
          i = Wn(t, function (h) {
            return n.hasOwnProperty(h[0] + "") || (n[h[0] + ""] = -1), h[2];
          }),
          o = [];
        i.buckets.each(function (h, f) {
          o.push({ name: f, dataList: h });
        });
        for (var s = o.length, l = 0; l < s; ++l) {
          for (var u = o[l].name, v = 0; v < o[l].dataList.length; ++v) {
            var c = o[l].dataList[v][0] + "";
            n[c] = l;
          }
          for (var c in n)
            n.hasOwnProperty(c) &&
              n[c] !== l &&
              ((n[c] = l), (t[r] = [c, 0, u]), r++);
        }
        return t;
      }),
      (e.prototype.getInitialData = function (t, r) {
        for (
          var n = this.getReferringComponents("singleAxis", er).models[0],
            i = n.get("type"),
            o = Dt(t.data, function (p) {
              return p[2] !== void 0;
            }),
            s = this.fixData(o || []),
            l = [],
            u = (this.nameMap = tt()),
            v = 0,
            c = 0;
          c < s.length;
          ++c
        )
          l.push(s[c][_n]), u.get(s[c][_n]) || (u.set(s[c][_n], v), v++);
        var h = di(s, {
            coordDimensions: ["single"],
            dimensionsDefine: [
              { name: "time", type: Hn(i) },
              { name: "value", type: "float" },
              { name: "name", type: "ordinal" }
            ],
            encodeDefine: { single: 0, value: 1, itemName: 2 }
          }).dimensions,
          f = new Vt(h, this);
        return f.initData(s), f;
      }),
      (e.prototype.getLayerSeries = function () {
        for (var t = this.getData(), r = t.count(), n = [], i = 0; i < r; ++i)
          n[i] = i;
        var o = t.mapDimension("single"),
          s = Wn(n, function (u) {
            return t.get("name", u);
          }),
          l = [];
        return (
          s.buckets.each(function (u, v) {
            u.sort(function (c, h) {
              return t.get(o, c) - t.get(o, h);
            }),
              l.push({ name: v, indices: u });
          }),
          l
        );
      }),
      (e.prototype.getAxisTooltipData = function (t, r, n) {
        H(t) || (t = t ? [t] : []);
        for (
          var i = this.getData(),
            o = this.getLayerSeries(),
            s = [],
            l = o.length,
            u,
            v = 0;
          v < l;
          ++v
        ) {
          for (
            var c = Number.MAX_VALUE, h = -1, f = o[v].indices.length, p = 0;
            p < f;
            ++p
          ) {
            var d = i.get(t[0], o[v].indices[p]),
              g = Math.abs(d - r);
            g <= c && ((u = d), (c = g), (h = o[v].indices[p]));
          }
          s.push(h);
        }
        return { dataIndices: s, nestestValue: u };
      }),
      (e.prototype.formatTooltip = function (t, r, n) {
        var i = this.getData(),
          o = i.getName(t),
          s = i.get(i.mapDimension("value"), t);
        return Mt("nameValue", { name: o, value: s });
      }),
      (e.type = "series.themeRiver"),
      (e.dependencies = ["singleAxis"]),
      (e.defaultOption = {
        z: 2,
        colorBy: "data",
        coordinateSystem: "singleAxis",
        boundaryGap: ["10%", "10%"],
        singleAxisIndex: 0,
        animationEasing: "linear",
        label: { margin: 4, show: !0, position: "left", fontSize: 11 },
        emphasis: { label: { show: !0 } }
      }),
      e
    );
  })(gt);
function R0(a, e) {
  a.eachSeriesByType("themeRiver", function (t) {
    var r = t.getData(),
      n = t.coordinateSystem,
      i = {},
      o = n.getRect();
    i.rect = o;
    var s = t.get("boundaryGap"),
      l = n.getAxis();
    if (((i.boundaryGap = s), l.orient === "horizontal")) {
      (s[0] = z(s[0], o.height)), (s[1] = z(s[1], o.height));
      var u = o.height - s[0] - s[1];
      sl(r, t, u);
    } else {
      (s[0] = z(s[0], o.width)), (s[1] = z(s[1], o.width));
      var v = o.width - s[0] - s[1];
      sl(r, t, v);
    }
    r.setLayout("layoutInfo", i);
  });
}
function sl(a, e, t) {
  if (a.count())
    for (
      var r = e.coordinateSystem,
        n = e.getLayerSeries(),
        i = a.mapDimension("single"),
        o = a.mapDimension("value"),
        s = O(n, function (g) {
          return O(g.indices, function (y) {
            var S = r.dataToPoint(a.get(i, y));
            return (S[1] = a.get(o, y)), S;
          });
        }),
        l = N0(s),
        u = l.y0,
        v = t / l.max,
        c = n.length,
        h = n[0].indices.length,
        f,
        p = 0;
      p < h;
      ++p
    ) {
      (f = u[p] * v),
        a.setItemLayout(n[0].indices[p], {
          layerIndex: 0,
          x: s[0][p][0],
          y0: f,
          y: s[0][p][1] * v
        });
      for (var d = 1; d < c; ++d)
        (f += s[d - 1][p][1] * v),
          a.setItemLayout(n[d].indices[p], {
            layerIndex: d,
            x: s[d][p][0],
            y0: f,
            y: s[d][p][1] * v
          });
    }
}
function N0(a) {
  for (
    var e = a.length, t = a[0].length, r = [], n = [], i = 0, o = 0;
    o < t;
    ++o
  ) {
    for (var s = 0, l = 0; l < e; ++l) s += a[l][o][1];
    s > i && (i = s), r.push(s);
  }
  for (var u = 0; u < t; ++u) n[u] = (i - r[u]) / 2;
  i = 0;
  for (var v = 0; v < t; ++v) {
    var c = r[v] + n[v];
    c > i && (i = c);
  }
  return { y0: n, max: i };
}
function k0(a) {
  a.registerChartView(P0),
    a.registerSeriesModel(E0),
    a.registerLayout(R0),
    a.registerProcessor(Da("themeRiver"));
}
var V0 = 2,
  G0 = 4,
  ll = (function (a) {
    k(e, a);
    function e(t, r, n, i) {
      var o = a.call(this) || this;
      (o.z2 = V0),
        (o.textConfig = { inside: !0 }),
        (nt(o).seriesIndex = r.seriesIndex);
      var s = new kt({ z2: G0, silent: t.getModel().get(["label", "silent"]) });
      return o.setTextContent(s), o.updateData(!0, t, r, n, i), o;
    }
    return (
      (e.prototype.updateData = function (t, r, n, i, o) {
        (this.node = r),
          (r.piece = this),
          (n = n || this._seriesModel),
          (i = i || this._ecModel);
        var s = this;
        nt(s).dataIndex = r.dataIndex;
        var l = r.getModel(),
          u = l.getModel("emphasis"),
          v = r.getLayout(),
          c = W({}, v);
        c.label = null;
        var h = r.getVisual("style");
        h.lineJoin = "bevel";
        var f = r.getVisual("decal");
        f && (h.decal = fi(f, o));
        var p = Io(l.getModel("itemStyle"), c, !0);
        W(c, p),
          L(Fn, function (S) {
            var m = s.ensureState(S),
              x = l.getModel([S, "itemStyle"]);
            m.style = x.getItemStyle();
            var b = Io(x, c);
            b && (m.shape = b);
          }),
          t
            ? (s.setShape(c),
              (s.shape.r = v.r0),
              Ct(s, { shape: { r: v.r } }, n, r.dataIndex))
            : (lt(s, { shape: c }, n), Se(s)),
          s.useStyle(h),
          this._updateLabel(n);
        var d = l.getShallow("cursor");
        d && s.attr("cursor", d),
          (this._seriesModel = n || this._seriesModel),
          (this._ecModel = i || this._ecModel);
        var g = u.get("focus"),
          y =
            g === "relative"
              ? ra(r.getAncestorsIndices(), r.getDescendantIndices())
              : g === "ancestor"
              ? r.getAncestorsIndices()
              : g === "descendant"
              ? r.getDescendantIndices()
              : g;
        ct(this, y, u.get("blurScope"), u.get("disabled"));
      }),
      (e.prototype._updateLabel = function (t) {
        var r = this,
          n = this.node.getModel(),
          i = n.getModel("label"),
          o = this.node.getLayout(),
          s = o.endAngle - o.startAngle,
          l = (o.startAngle + o.endAngle) / 2,
          u = Math.cos(l),
          v = Math.sin(l),
          c = this,
          h = c.getTextContent(),
          f = this.node.dataIndex,
          p = (i.get("minAngle") / 180) * Math.PI,
          d = i.get("show") && !(p != null && Math.abs(s) < p);
        (h.ignore = !d),
          L(Nh, function (y) {
            var S =
                y === "normal" ? n.getModel("label") : n.getModel([y, "label"]),
              m = y === "normal",
              x = m ? h : h.ensureState(y),
              b = t.getFormattedLabel(f, y);
            m && (b = b || r.node.name),
              (x.style = xt(S, {}, null, y !== "normal", !0)),
              b && (x.style.text = b);
            var _ = S.get("show");
            _ != null && !m && (x.ignore = !_);
            var w = g(S, "position"),
              A = m ? c : c.states[y],
              I = A.style.fill;
            A.textConfig = {
              outsideFill: S.get("color") === "inherit" ? I : null,
              inside: w !== "outside"
            };
            var T,
              D = g(S, "distance") || 0,
              M = g(S, "align"),
              C = g(S, "rotate"),
              P = Math.PI * 0.5,
              E = Math.PI * 1.5,
              R = ur(C === "tangential" ? Math.PI / 2 - l : l),
              N = R > P && !Rh(R - P) && R < E;
            w === "outside"
              ? ((T = o.r + D), (M = N ? "right" : "left"))
              : !M || M === "center"
              ? (s === 2 * Math.PI && o.r0 === 0
                  ? (T = 0)
                  : (T = (o.r + o.r0) / 2),
                (M = "center"))
              : M === "left"
              ? ((T = o.r0 + D), (M = N ? "right" : "left"))
              : M === "right" && ((T = o.r - D), (M = N ? "left" : "right")),
              (x.style.align = M),
              (x.style.verticalAlign = g(S, "verticalAlign") || "middle"),
              (x.x = T * u + o.cx),
              (x.y = T * v + o.cy);
            var V = 0;
            C === "radial"
              ? (V = ur(-l) + (N ? Math.PI : 0))
              : C === "tangential"
              ? (V = ur(Math.PI / 2 - l) + (N ? Math.PI : 0))
              : Qt(C) && (V = (C * Math.PI) / 180),
              (x.rotation = ur(V));
          });
        function g(y, S) {
          var m = y.get(S);
          return m ?? i.get(S);
        }
        h.dirtyStyle();
      }),
      e
    );
  })(Ve),
  ri = "sunburstRootToNode",
  ul = "sunburstHighlight",
  z0 = "sunburstUnhighlight";
function O0(a) {
  a.registerAction({ type: ri, update: "updateView" }, function (e, t) {
    t.eachComponent({ mainType: "series", subType: "sunburst", query: e }, r);
    function r(n, i) {
      var o = _r(e, [ri], n);
      if (o) {
        var s = n.getViewRoot();
        s && (e.direction = Fi(s, o.node) ? "rollUp" : "drillDown"),
          n.resetViewRoot(o.node);
      }
    }
  }),
    a.registerAction({ type: ul, update: "none" }, function (e, t, r) {
      (e = W({}, e)),
        t.eachComponent(
          { mainType: "series", subType: "sunburst", query: e },
          n
        );
      function n(i) {
        var o = _r(e, [ul], i);
        o && (e.dataIndex = o.node.dataIndex);
      }
      r.dispatchAction(W(e, { type: "highlight" }));
    }),
    a.registerAction({ type: z0, update: "updateView" }, function (e, t, r) {
      (e = W({}, e)), r.dispatchAction(W(e, { type: "downplay" }));
    });
}
var B0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n, i) {
        var o = this;
        (this.seriesModel = t), (this.api = n), (this.ecModel = r);
        var s = t.getData(),
          l = s.tree.root,
          u = t.getViewRoot(),
          v = this.group,
          c = t.get("renderLabelForZeroData"),
          h = [];
        u.eachNode(function (S) {
          h.push(S);
        });
        var f = this._oldChildren || [];
        p(h, f), y(l, u), this._initEvents(), (this._oldChildren = h);
        function p(S, m) {
          if (S.length === 0 && m.length === 0) return;
          new je(m, S, x, x).add(b).update(b).remove(ot(b, null)).execute();
          function x(_) {
            return _.getId();
          }
          function b(_, w) {
            var A = _ == null ? null : S[_],
              I = w == null ? null : m[w];
            d(A, I);
          }
        }
        function d(S, m) {
          if ((!c && S && !S.getValue() && (S = null), S !== l && m !== l)) {
            if (m && m.piece)
              S
                ? (m.piece.updateData(!1, S, t, r, n),
                  s.setItemGraphicEl(S.dataIndex, m.piece))
                : g(m);
            else if (S) {
              var x = new ll(S, t, r, n);
              v.add(x), s.setItemGraphicEl(S.dataIndex, x);
            }
          }
        }
        function g(S) {
          S && S.piece && (v.remove(S.piece), (S.piece = null));
        }
        function y(S, m) {
          m.depth > 0
            ? (o.virtualPiece
                ? o.virtualPiece.updateData(!1, S, t, r, n)
                : ((o.virtualPiece = new ll(S, t, r, n)),
                  v.add(o.virtualPiece)),
              m.piece.off("click"),
              o.virtualPiece.on("click", function (x) {
                o._rootToNode(m.parentNode);
              }))
            : o.virtualPiece &&
              (v.remove(o.virtualPiece), (o.virtualPiece = null));
        }
      }),
      (e.prototype._initEvents = function () {
        var t = this;
        this.group.off("click"),
          this.group.on("click", function (r) {
            var n = !1,
              i = t.seriesModel.getViewRoot();
            i.eachNode(function (o) {
              if (!n && o.piece && o.piece === r.target) {
                var s = o.getModel().get("nodeClick");
                if (s === "rootToNode") t._rootToNode(o);
                else if (s === "link") {
                  var l = o.getModel(),
                    u = l.get("link");
                  if (u) {
                    var v = l.get("target", !0) || "_blank";
                    Su(u, v);
                  }
                }
                n = !0;
              }
            });
          });
      }),
      (e.prototype._rootToNode = function (t) {
        t !== this.seriesModel.getViewRoot() &&
          this.api.dispatchAction({
            type: ri,
            from: this.uid,
            seriesId: this.seriesModel.id,
            targetNode: t
          });
      }),
      (e.prototype.containPoint = function (t, r) {
        var n = r.getData(),
          i = n.getItemLayout(0);
        if (i) {
          var o = t[0] - i.cx,
            s = t[1] - i.cy,
            l = Math.sqrt(o * o + s * s);
          return l <= i.r && l >= i.r0;
        }
      }),
      (e.type = "sunburst"),
      e
    );
  })(pt),
  F0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.ignoreStyleOnData = !0), t;
    }
    return (
      (e.prototype.getInitialData = function (t, r) {
        var n = { name: t.name, children: t.data };
        Wv(n);
        var i = (this._levelModels = O(
            t.levels || [],
            function (l) {
              return new $t(l, this, r);
            },
            this
          )),
          o = Bi.createTree(n, this, s);
        function s(l) {
          l.wrapMethod("getItemModel", function (u, v) {
            var c = o.getNodeByDataIndex(v),
              h = i[c.depth];
            return h && (u.parentModel = h), u;
          });
        }
        return o.data;
      }),
      (e.prototype.optionUpdated = function () {
        this.resetViewRoot();
      }),
      (e.prototype.getDataParams = function (t) {
        var r = a.prototype.getDataParams.apply(this, arguments),
          n = this.getData().tree.getNodeByDataIndex(t);
        return (r.treePathInfo = Ga(n, this)), r;
      }),
      (e.prototype.getLevelModel = function (t) {
        return this._levelModels && this._levelModels[t.depth];
      }),
      (e.prototype.getViewRoot = function () {
        return this._viewRoot;
      }),
      (e.prototype.resetViewRoot = function (t) {
        t ? (this._viewRoot = t) : (t = this._viewRoot);
        var r = this.getRawData().tree.root;
        (!t || (t !== r && !r.contains(t))) && (this._viewRoot = r);
      }),
      (e.prototype.enableAriaDecal = function () {
        lv(this);
      }),
      (e.type = "series.sunburst"),
      (e.defaultOption = {
        z: 2,
        center: ["50%", "50%"],
        radius: [0, "75%"],
        clockwise: !0,
        startAngle: 90,
        minAngle: 0,
        stillShowZeroSum: !0,
        nodeClick: "rootToNode",
        renderLabelForZeroData: !1,
        label: {
          rotate: "radial",
          show: !0,
          opacity: 1,
          align: "center",
          position: "inside",
          distance: 5,
          silent: !0
        },
        itemStyle: {
          borderWidth: 1,
          borderColor: "white",
          borderType: "solid",
          shadowBlur: 0,
          shadowColor: "rgba(0, 0, 0, 0.2)",
          shadowOffsetX: 0,
          shadowOffsetY: 0,
          opacity: 1
        },
        emphasis: { focus: "descendant" },
        blur: { itemStyle: { opacity: 0.2 }, label: { opacity: 0.1 } },
        animationType: "expansion",
        animationDuration: 1e3,
        animationDurationUpdate: 500,
        data: [],
        sort: "desc"
      }),
      e
    );
  })(gt);
function Wv(a) {
  var e = 0;
  L(a.children, function (r) {
    Wv(r);
    var n = r.value;
    H(n) && (n = n[0]), (e += n);
  });
  var t = a.value;
  H(t) && (t = t[0]),
    (t == null || isNaN(t)) && (t = e),
    t < 0 && (t = 0),
    H(a.value) ? (a.value[0] = t) : (a.value = t);
}
var vl = Math.PI / 180;
function W0(a, e, t) {
  e.eachSeriesByType(a, function (r) {
    var n = r.get("center"),
      i = r.get("radius");
    H(i) || (i = [0, i]), H(n) || (n = [n, n]);
    var o = t.getWidth(),
      s = t.getHeight(),
      l = Math.min(o, s),
      u = z(n[0], o),
      v = z(n[1], s),
      c = z(i[0], l / 2),
      h = z(i[1], l / 2),
      f = -r.get("startAngle") * vl,
      p = r.get("minAngle") * vl,
      d = r.getData().tree.root,
      g = r.getViewRoot(),
      y = g.depth,
      S = r.get("sort");
    S != null && Hv(g, S);
    var m = 0;
    L(g.children, function (R) {
      !isNaN(R.getValue()) && m++;
    });
    var x = g.getValue(),
      b = (Math.PI / (x || m)) * 2,
      _ = g.depth > 0,
      w = g.height - (_ ? -1 : 1),
      A = (h - c) / (w || 1),
      I = r.get("clockwise"),
      T = r.get("stillShowZeroSum"),
      D = I ? 1 : -1,
      M = function (R, N) {
        if (R) {
          var V = N;
          if (R !== d) {
            var G = R.getValue(),
              B = x === 0 && T ? b : G * b;
            B < p && (B = p), (V = N + D * B);
            var F = R.depth - y - (_ ? -1 : 1),
              Y = c + A * F,
              K = c + A * (F + 1),
              $ = r.getLevelModel(R);
            if ($) {
              var j = $.get("r0", !0),
                yt = $.get("r", !0),
                re = $.get("radius", !0);
              re != null && ((j = re[0]), (yt = re[1])),
                j != null && (Y = z(j, l / 2)),
                yt != null && (K = z(yt, l / 2));
            }
            R.setLayout({
              angle: B,
              startAngle: N,
              endAngle: V,
              clockwise: I,
              cx: u,
              cy: v,
              r0: Y,
              r: K
            });
          }
          if (R.children && R.children.length) {
            var X = 0;
            L(R.children, function (U) {
              X += M(U, N + X);
            });
          }
          return V - N;
        }
      };
    if (_) {
      var C = c,
        P = c + A,
        E = Math.PI * 2;
      d.setLayout({
        angle: E,
        startAngle: f,
        endAngle: f + E,
        clockwise: I,
        cx: u,
        cy: v,
        r0: C,
        r: P
      });
    }
    M(g, f);
  });
}
function Hv(a, e) {
  var t = a.children || [];
  (a.children = H0(t, e)),
    t.length &&
      L(a.children, function (r) {
        Hv(r, e);
      });
}
function H0(a, e) {
  if (it(e)) {
    var t = O(a, function (n, i) {
      var o = n.getValue();
      return {
        params: {
          depth: n.depth,
          height: n.height,
          dataIndex: n.dataIndex,
          getValue: function () {
            return o;
          }
        },
        index: i
      };
    });
    return (
      t.sort(function (n, i) {
        return e(n.params, i.params);
      }),
      O(t, function (n) {
        return a[n.index];
      })
    );
  } else {
    var r = e === "asc";
    return a.sort(function (n, i) {
      var o = (n.getValue() - i.getValue()) * (r ? 1 : -1);
      return o === 0 ? (n.dataIndex - i.dataIndex) * (r ? -1 : 1) : o;
    });
  }
}
function U0(a) {
  var e = {};
  function t(r, n, i) {
    for (var o = r; o && o.depth > 1; ) o = o.parentNode;
    var s = n.getColorFromPalette(o.name || o.dataIndex + "", e);
    return (
      r.depth > 1 && et(s) && (s = kh(s, ((r.depth - 1) / (i - 1)) * 0.5)), s
    );
  }
  a.eachSeriesByType("sunburst", function (r) {
    var n = r.getData(),
      i = n.tree;
    i.eachNode(function (o) {
      var s = o.getModel(),
        l = s.getModel("itemStyle").getItemStyle();
      l.fill || (l.fill = t(o, r, i.root.height));
      var u = n.ensureUniqueItemVisual(o.dataIndex, "style");
      W(u, l);
    });
  });
}
function Y0(a) {
  a.registerChartView(B0),
    a.registerSeriesModel(F0),
    a.registerLayout(ot(W0, "sunburst")),
    a.registerProcessor(ot(Da, "sunburst")),
    a.registerVisual(U0),
    O0(a);
}
var cl = { color: "fill", borderColor: "stroke" },
  $0 = {
    symbol: 1,
    symbolSize: 1,
    symbolKeepAspect: 1,
    legendIcon: 1,
    visualMeta: 1,
    liftZ: 1,
    decal: 1
  },
  le = _t(),
  X0 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.optionUpdated = function () {
        (this.currentZLevel = this.get("zlevel", !0)),
          (this.currentZ = this.get("z", !0));
      }),
      (e.prototype.getInitialData = function (t, r) {
        return Je(null, this);
      }),
      (e.prototype.getDataParams = function (t, r, n) {
        var i = a.prototype.getDataParams.call(this, t, r);
        return n && (i.info = le(n).info), i;
      }),
      (e.type = "series.custom"),
      (e.dependencies = ["grid", "polar", "geo", "singleAxis", "calendar"]),
      (e.defaultOption = {
        coordinateSystem: "cartesian2d",
        z: 2,
        legendHoverLink: !0,
        clip: !1
      }),
      e
    );
  })(gt);
function Z0(a, e) {
  return (
    (e = e || [0, 0]),
    O(
      ["x", "y"],
      function (t, r) {
        var n = this.getAxis(t),
          i = e[r],
          o = a[r] / 2;
        return n.type === "category"
          ? n.getBandWidth()
          : Math.abs(n.dataToCoord(i - o) - n.dataToCoord(i + o));
      },
      this
    )
  );
}
function q0(a) {
  var e = a.master.getRect();
  return {
    coordSys: {
      type: "cartesian2d",
      x: e.x,
      y: e.y,
      width: e.width,
      height: e.height
    },
    api: {
      coord: function (t) {
        return a.dataToPoint(t);
      },
      size: at(Z0, a)
    }
  };
}
function K0(a, e) {
  return (
    (e = e || [0, 0]),
    O(
      [0, 1],
      function (t) {
        var r = e[t],
          n = a[t] / 2,
          i = [],
          o = [];
        return (
          (i[t] = r - n),
          (o[t] = r + n),
          (i[1 - t] = o[1 - t] = e[1 - t]),
          Math.abs(this.dataToPoint(i)[t] - this.dataToPoint(o)[t])
        );
      },
      this
    )
  );
}
function j0(a) {
  var e = a.getBoundingRect();
  return {
    coordSys: {
      type: "geo",
      x: e.x,
      y: e.y,
      width: e.width,
      height: e.height,
      zoom: a.getZoom()
    },
    api: {
      coord: function (t) {
        return a.dataToPoint(t);
      },
      size: at(K0, a)
    }
  };
}
function J0(a, e) {
  var t = this.getAxis(),
    r = e instanceof Array ? e[0] : e,
    n = (a instanceof Array ? a[0] : a) / 2;
  return t.type === "category"
    ? t.getBandWidth()
    : Math.abs(t.dataToCoord(r - n) - t.dataToCoord(r + n));
}
function Q0(a) {
  var e = a.getRect();
  return {
    coordSys: {
      type: "singleAxis",
      x: e.x,
      y: e.y,
      width: e.width,
      height: e.height
    },
    api: {
      coord: function (t) {
        return a.dataToPoint(t);
      },
      size: at(J0, a)
    }
  };
}
function tS(a, e) {
  return (
    (e = e || [0, 0]),
    O(
      ["Radius", "Angle"],
      function (t, r) {
        var n = "get" + t + "Axis",
          i = this[n](),
          o = e[r],
          s = a[r] / 2,
          l =
            i.type === "category"
              ? i.getBandWidth()
              : Math.abs(i.dataToCoord(o - s) - i.dataToCoord(o + s));
        return t === "Angle" && (l = (l * Math.PI) / 180), l;
      },
      this
    )
  );
}
function eS(a) {
  var e = a.getRadiusAxis(),
    t = a.getAngleAxis(),
    r = e.getExtent();
  return (
    r[0] > r[1] && r.reverse(),
    {
      coordSys: { type: "polar", cx: a.cx, cy: a.cy, r: r[1], r0: r[0] },
      api: {
        coord: function (n) {
          var i = e.dataToRadius(n[0]),
            o = t.dataToAngle(n[1]),
            s = a.coordToPoint([i, o]);
          return s.push(i, (o * Math.PI) / 180), s;
        },
        size: at(tS, a)
      }
    }
  );
}
function rS(a) {
  var e = a.getRect(),
    t = a.getRangeInfo();
  return {
    coordSys: {
      type: "calendar",
      x: e.x,
      y: e.y,
      width: e.width,
      height: e.height,
      cellWidth: a.getCellWidth(),
      cellHeight: a.getCellHeight(),
      rangeInfo: {
        start: t.start,
        end: t.end,
        weeks: t.weeks,
        dayCount: t.allDay
      }
    },
    api: {
      coord: function (r, n) {
        return a.dataToPoint(r, n);
      }
    }
  };
}
var ue = "emphasis",
  ge = "normal",
  ji = "blur",
  Ji = "select",
  be = [ge, ue, ji, Ji],
  wn = {
    normal: ["itemStyle"],
    emphasis: [ue, "itemStyle"],
    blur: [ji, "itemStyle"],
    select: [Ji, "itemStyle"]
  },
  An = {
    normal: ["label"],
    emphasis: [ue, "label"],
    blur: [ji, "label"],
    select: [Ji, "label"]
  },
  aS = ["x", "y"],
  nS = "e\0\0",
  Bt = { normal: {}, emphasis: {}, blur: {}, select: {} },
  iS = { cartesian2d: q0, geo: j0, single: Q0, polar: eS, calendar: rS };
function ai(a) {
  return a instanceof wt;
}
function ni(a) {
  return a instanceof mr;
}
function oS(a, e) {
  e.copyTransform(a),
    ni(e) &&
      ni(a) &&
      (e.setStyle(a.style),
      (e.z = a.z),
      (e.z2 = a.z2),
      (e.zlevel = a.zlevel),
      (e.invisible = a.invisible),
      (e.ignore = a.ignore),
      ai(e) && ai(a) && e.setShape(a.shape));
}
var sS = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), t;
  }
  return (
    (e.prototype.render = function (t, r, n, i) {
      this._progressiveEls = null;
      var o = this._data,
        s = t.getData(),
        l = this.group,
        u = hl(t, s, r, n);
      o || l.removeAll(),
        s
          .diff(o)
          .add(function (c) {
            Tn(n, null, c, u(c, i), t, l, s);
          })
          .remove(function (c) {
            var h = o.getItemGraphicEl(c);
            h && wi(h, le(h).option, t);
          })
          .update(function (c, h) {
            var f = o.getItemGraphicEl(h);
            Tn(n, f, c, u(c, i), t, l, s);
          })
          .execute();
      var v = t.get("clip", !0) ? Ra(t.coordinateSystem, !1, t) : null;
      v ? l.setClipPath(v) : l.removeClipPath(), (this._data = s);
    }),
    (e.prototype.incrementalPrepareRender = function (t, r, n) {
      this.group.removeAll(), (this._data = null);
    }),
    (e.prototype.incrementalRender = function (t, r, n, i, o) {
      var s = r.getData(),
        l = hl(r, s, n, i),
        u = (this._progressiveEls = []);
      function v(f) {
        f.isGroup ||
          ((f.incremental = !0), (f.ensureState("emphasis").hoverLayer = !0));
      }
      for (var c = t.start; c < t.end; c++) {
        var h = Tn(null, null, c, l(c, o), r, this.group, s);
        h && (h.traverse(v), u.push(h));
      }
    }),
    (e.prototype.eachRendered = function (t) {
      Ea(this._progressiveEls || this.group, t);
    }),
    (e.prototype.filterForExposedEvent = function (t, r, n, i) {
      var o = r.element;
      if (o == null || n.name === o) return !0;
      for (; (n = n.__hostTarget || n.parent) && n !== this.group; )
        if (n.name === o) return !0;
      return !1;
    }),
    (e.type = "custom"),
    e
  );
})(pt);
function Qi(a) {
  var e = a.type,
    t;
  if (e === "path") {
    var r = a.shape,
      n =
        r.width != null && r.height != null
          ? { x: r.x || 0, y: r.y || 0, width: r.width, height: r.height }
          : null,
      i = $v(r);
    (t = zh(i, null, n, r.layout || "center")), (le(t).customPathData = i);
  } else if (e === "image")
    (t = new _e({})), (le(t).customImagePath = a.style.image);
  else if (e === "text") t = new kt({});
  else if (e === "group") t = new Z();
  else {
    if (e === "compoundPath")
      throw new Error('"compoundPath" is not supported yet.');
    var o = Oh(e);
    if (!o) {
      var s = "";
      vt(s);
    }
    t = new o();
  }
  return (
    (le(t).customGraphicType = e),
    (t.name = a.name),
    (t.z2EmphasisLift = 1),
    (t.z2SelectLift = 1),
    t
  );
}
function to(a, e, t, r, n, i, o) {
  Bh(e);
  var s = n && n.normal.cfg;
  s && e.setTextConfig(s), r && r.transition == null && (r.transition = aS);
  var l = r && r.style;
  if (l) {
    if (e.type === "text") {
      var u = l;
      bt(u, "textFill") && (u.fill = u.textFill),
        bt(u, "textStroke") && (u.stroke = u.textStroke);
    }
    var v = void 0,
      c = ai(e) ? l.decal : null;
    a && c && ((c.dirty = !0), (v = fi(c, a))), (l.__decalPattern = v);
  }
  if (ni(e) && l) {
    var v = l.__decalPattern;
    v && (l.decal = v);
  }
  Fh(e, r, i, { dataIndex: t, isInit: o, clearStyle: !0 }),
    Wh(e, r.keyframeAnimation, i);
}
function Uv(a, e, t, r, n) {
  var i = e.isGroup ? null : e,
    o = n && n[a].cfg;
  if (i) {
    var s = i.ensureState(a);
    if (r === !1) {
      var l = i.getState(a);
      l && (l.style = null);
    } else s.style = r || null;
    o && (s.textConfig = o), Ke(i);
  }
}
function lS(a, e, t) {
  if (!a.isGroup) {
    var r = a,
      n = t.currentZ,
      i = t.currentZLevel;
    (r.z = n), (r.zlevel = i);
    var o = e.z2;
    o != null && (r.z2 = o || 0);
    for (var s = 0; s < be.length; s++) uS(r, e, be[s]);
  }
}
function uS(a, e, t) {
  var r = t === ge,
    n = r ? e : pa(e, t),
    i = n ? n.z2 : null,
    o;
  i != null && ((o = r ? a : a.ensureState(t)), (o.z2 = i || 0));
}
function hl(a, e, t, r) {
  var n = a.get("renderItem"),
    i = a.coordinateSystem,
    o = {};
  i && (o = i.prepareCustoms ? i.prepareCustoms(i) : iS[i.type](i));
  for (
    var s = Q(
        {
          getWidth: r.getWidth,
          getHeight: r.getHeight,
          getZr: r.getZr,
          getDevicePixelRatio: r.getDevicePixelRatio,
          value: x,
          style: _,
          ordinalRawValue: b,
          styleEmphasis: w,
          visual: T,
          barLayout: D,
          currentSeriesIndices: M,
          font: C
        },
        o.api || {}
      ),
      l = {
        context: {},
        seriesId: a.id,
        seriesName: a.name,
        seriesIndex: a.seriesIndex,
        coordSys: o.coordSys,
        dataInsideLength: e.count(),
        encode: vS(a.getData())
      },
      u,
      v,
      c = {},
      h = {},
      f = {},
      p = {},
      d = 0;
    d < be.length;
    d++
  ) {
    var g = be[d];
    (f[g] = a.getModel(wn[g])), (p[g] = a.getModel(An[g]));
  }
  function y(P) {
    return P === u ? v || (v = e.getItemModel(P)) : e.getItemModel(P);
  }
  function S(P, E) {
    return e.hasItemOption
      ? P === u
        ? c[E] || (c[E] = y(P).getModel(wn[E]))
        : y(P).getModel(wn[E])
      : f[E];
  }
  function m(P, E) {
    return e.hasItemOption
      ? P === u
        ? h[E] || (h[E] = y(P).getModel(An[E]))
        : y(P).getModel(An[E])
      : p[E];
  }
  return function (P, E) {
    return (
      (u = P),
      (v = null),
      (c = {}),
      (h = {}),
      n &&
        n(
          Q(
            {
              dataIndexInside: P,
              dataIndex: e.getRawIndex(P),
              actionType: E ? E.type : null
            },
            l
          ),
          s
        )
    );
  };
  function x(P, E) {
    return (
      E == null && (E = u), e.getStore().get(e.getDimensionIndex(P || 0), E)
    );
  }
  function b(P, E) {
    E == null && (E = u), (P = P || 0);
    var R = e.getDimensionInfo(P);
    if (!R) {
      var N = e.getDimensionIndex(P);
      return N >= 0 ? e.getStore().get(N, E) : void 0;
    }
    var V = e.get(R.name, E),
      G = R && R.ordinalMeta;
    return G ? G.categories[V] : V;
  }
  function _(P, E) {
    E == null && (E = u);
    var R = e.getItemVisual(E, "style"),
      N = R && R.fill,
      V = R && R.opacity,
      G = S(E, ge).getItemStyle();
    N != null && (G.fill = N), V != null && (G.opacity = V);
    var B = { inheritColor: et(N) ? N : "#000" },
      F = m(E, ge),
      Y = xt(F, null, B, !1, !0);
    Y.text = F.getShallow("show")
      ? St(a.getFormattedLabel(E, ge), Un(e, E))
      : null;
    var K = Do(F, B, !1);
    return I(P, G), (G = Lo(G, Y, K)), P && A(G, P), (G.legacy = !0), G;
  }
  function w(P, E) {
    E == null && (E = u);
    var R = S(E, ue).getItemStyle(),
      N = m(E, ue),
      V = xt(N, null, null, !0, !0);
    V.text = N.getShallow("show")
      ? Er(a.getFormattedLabel(E, ue), a.getFormattedLabel(E, ge), Un(e, E))
      : null;
    var G = Do(N, null, !0);
    return I(P, R), (R = Lo(R, V, G)), P && A(R, P), (R.legacy = !0), R;
  }
  function A(P, E) {
    for (var R in E) bt(E, R) && (P[R] = E[R]);
  }
  function I(P, E) {
    P &&
      (P.textFill && (E.textFill = P.textFill),
      P.textPosition && (E.textPosition = P.textPosition));
  }
  function T(P, E) {
    if ((E == null && (E = u), bt(cl, P))) {
      var R = e.getItemVisual(E, "style");
      return R ? R[cl[P]] : null;
    }
    if (bt($0, P)) return e.getItemVisual(E, P);
  }
  function D(P) {
    if (i.type === "cartesian2d") {
      var E = i.getBaseAxis();
      return Vh(Q({ axis: E }, P));
    }
  }
  function M() {
    return t.getCurrentSeriesIndices();
  }
  function C(P) {
    return Gh(P, t);
  }
}
function vS(a) {
  var e = {};
  return (
    L(a.dimensions, function (t) {
      var r = a.getDimensionInfo(t);
      if (!r.isExtraCoord) {
        var n = r.coordDim,
          i = (e[n] = e[n] || []);
        i[r.coordDimIndex] = a.getDimensionIndex(t);
      }
    }),
    e
  );
}
function Tn(a, e, t, r, n, i, o) {
  if (!r) {
    i.remove(e);
    return;
  }
  var s = eo(a, e, t, r, n, i);
  return (
    s && o.setItemGraphicEl(t, s),
    s && ct(s, r.focus, r.blurScope, r.emphasisDisabled),
    s
  );
}
function eo(a, e, t, r, n, i) {
  var o = -1,
    s = e;
  e && Yv(e, r, n) && ((o = Ht(i.childrenRef(), e)), (e = null));
  var l = !e,
    u = e;
  u ? u.clearStates() : ((u = Qi(r)), s && oS(s, u)),
    r.morph === !1
      ? (u.disableMorphing = !0)
      : u.disableMorphing && (u.disableMorphing = !1),
    (Bt.normal.cfg =
      Bt.normal.conOpt =
      Bt.emphasis.cfg =
      Bt.emphasis.conOpt =
      Bt.blur.cfg =
      Bt.blur.conOpt =
      Bt.select.cfg =
      Bt.select.conOpt =
        null),
    (Bt.isLegacy = !1),
    hS(u, t, r, n, l, Bt),
    cS(u, t, r, n, l),
    to(a, u, t, r, Bt, n, l),
    bt(r, "info") && (le(u).info = r.info);
  for (var v = 0; v < be.length; v++) {
    var c = be[v];
    if (c !== ge) {
      var h = pa(r, c),
        f = ro(r, h, c);
      Uv(c, u, h, f, Bt);
    }
  }
  return (
    lS(u, r, n),
    r.type === "group" && fS(a, u, t, r, n),
    o >= 0 ? i.replaceAt(u, o) : i.add(u),
    u
  );
}
function Yv(a, e, t) {
  var r = le(a),
    n = e.type,
    i = e.shape,
    o = e.style;
  return (
    t.isUniversalTransitionEnabled() ||
    (n != null && n !== r.customGraphicType) ||
    (n === "path" && yS(i) && $v(i) !== r.customPathData) ||
    (n === "image" && bt(o, "image") && o.image !== r.customImagePath)
  );
}
function cS(a, e, t, r, n) {
  var i = t.clipPath;
  if (i === !1) a && a.getClipPath() && a.removeClipPath();
  else if (i) {
    var o = a.getClipPath();
    o && Yv(o, i, r) && (o = null),
      o || ((o = Qi(i)), a.setClipPath(o)),
      to(null, o, e, i, null, r, n);
  }
}
function hS(a, e, t, r, n, i) {
  if (!a.isGroup) {
    fl(t, null, i), fl(t, ue, i);
    var o = i.normal.conOpt,
      s = i.emphasis.conOpt,
      l = i.blur.conOpt,
      u = i.select.conOpt;
    if (o != null || s != null || u != null || l != null) {
      var v = a.getTextContent();
      if (o === !1) v && a.removeTextContent();
      else {
        (o = i.normal.conOpt = o || { type: "text" }),
          v ? v.clearStates() : ((v = Qi(o)), a.setTextContent(v)),
          to(null, v, e, o, null, r, n);
        for (var c = o && o.style, h = 0; h < be.length; h++) {
          var f = be[h];
          if (f !== ge) {
            var p = i[f].conOpt;
            Uv(f, v, p, ro(o, p, f), null);
          }
        }
        c ? v.dirty() : v.markRedraw();
      }
    }
  }
}
function fl(a, e, t) {
  var r = e ? pa(a, e) : a,
    n = e ? ro(a, r, ue) : a.style,
    i = a.type,
    o = r ? r.textConfig : null,
    s = a.textContent,
    l = s ? (e ? pa(s, e) : s) : null;
  if (n && (t.isLegacy || Hh(n, i, !!o, !!l))) {
    t.isLegacy = !0;
    var u = Uh(n, i, !e);
    !o && u.textConfig && (o = u.textConfig),
      !l && u.textContent && (l = u.textContent);
  }
  if (!e && l) {
    var v = l;
    !v.type && (v.type = "text");
  }
  var c = e ? t[e] : t.normal;
  (c.cfg = o), (c.conOpt = l);
}
function pa(a, e) {
  return e ? (a ? a[e] : null) : a;
}
function ro(a, e, t) {
  var r = e && e.style;
  return r == null && t === ue && a && (r = a.styleEmphasis), r;
}
function fS(a, e, t, r, n) {
  var i = r.children,
    o = i ? i.length : 0,
    s = r.$mergeChildren,
    l = s === "byName" || r.diffChildrenByName,
    u = s === !1;
  if (!(!o && !l && !u)) {
    if (l) {
      dS({
        api: a,
        oldChildren: e.children() || [],
        newChildren: i || [],
        dataIndex: t,
        seriesModel: n,
        group: e
      });
      return;
    }
    u && e.removeAll();
    for (var v = 0; v < o; v++) {
      var c = i[v],
        h = e.childAt(v);
      c
        ? (c.ignore == null && (c.ignore = !1), eo(a, h, t, c, n, e))
        : (h.ignore = !0);
    }
    for (var f = e.childCount() - 1; f >= v; f--) {
      var p = e.childAt(f);
      pS(e, p, n);
    }
  }
}
function pS(a, e, t) {
  e && wi(e, le(a).option, t);
}
function dS(a) {
  new je(a.oldChildren, a.newChildren, pl, pl, a)
    .add(dl)
    .update(dl)
    .remove(gS)
    .execute();
}
function pl(a, e) {
  var t = a && a.name;
  return t ?? nS + e;
}
function dl(a, e) {
  var t = this.context,
    r = a != null ? t.newChildren[a] : null,
    n = e != null ? t.oldChildren[e] : null;
  eo(t.api, n, t.dataIndex, r, t.seriesModel, t.group);
}
function gS(a) {
  var e = this.context,
    t = e.oldChildren[a];
  t && wi(t, le(t).option, e.seriesModel);
}
function $v(a) {
  return a && (a.pathData || a.d);
}
function yS(a) {
  return a && (bt(a, "pathData") || bt(a, "d"));
}
function mS(a) {
  a.registerChartView(sS), a.registerSeriesModel(X0);
}
var SS = (function (a) {
  k(e, a);
  function e() {
    return (a !== null && a.apply(this, arguments)) || this;
  }
  return (
    (e.prototype.makeElOption = function (t, r, n, i, o) {
      var s = n.axis;
      s.dim === "angle" && (this.animationThreshold = Math.PI / 18);
      var l = s.polar,
        u = l.getOtherAxis(s),
        v = u.getExtent(),
        c = s.dataToCoord(r),
        h = i.get("type");
      if (h && h !== "none") {
        var f = Cu(i),
          p = bS[h](s, l, c, v);
        (p.style = f), (t.graphicKey = p.type), (t.pointer = p);
      }
      var d = i.get(["label", "margin"]),
        g = xS(r, n, i, l, d);
      Yh(t, n, i, o, g);
    }),
    e
  );
})(Pu);
function xS(a, e, t, r, n) {
  var i = e.axis,
    o = i.dataToCoord(a),
    s = r.getAngleAxis().getExtent()[0];
  s = (s / 180) * Math.PI;
  var l = r.getRadiusAxis().getExtent(),
    u,
    v,
    c;
  if (i.dim === "radius") {
    var h = tr();
    La(h, h, s), ke(h, h, [r.cx, r.cy]), (u = xi([o, -n], h));
    var f = e.getModel("axisLabel").get("rotate") || 0,
      p = Ne.innerTextLayout(s, (f * Math.PI) / 180, -1);
    (v = p.textAlign), (c = p.textVerticalAlign);
  } else {
    var d = l[1];
    u = r.coordToPoint([d + n, o]);
    var g = r.cx,
      y = r.cy;
    (v = Math.abs(u[0] - g) / d < 0.3 ? "center" : u[0] > g ? "left" : "right"),
      (c =
        Math.abs(u[1] - y) / d < 0.3 ? "middle" : u[1] > y ? "top" : "bottom");
  }
  return { position: u, align: v, verticalAlign: c };
}
var bS = {
    line: function (a, e, t, r) {
      return a.dim === "angle"
        ? {
            type: "Line",
            shape: Mu(e.coordToPoint([r[0], t]), e.coordToPoint([r[1], t]))
          }
        : { type: "Circle", shape: { cx: e.cx, cy: e.cy, r: t } };
    },
    shadow: function (a, e, t, r) {
      var n = Math.max(1, a.getBandWidth()),
        i = Math.PI / 180;
      return a.dim === "angle"
        ? {
            type: "Sector",
            shape: Co(
              e.cx,
              e.cy,
              r[0],
              r[1],
              (-t - n / 2) * i,
              (-t + n / 2) * i
            )
          }
        : {
            type: "Sector",
            shape: Co(e.cx, e.cy, t - n / 2, t + n / 2, 0, Math.PI * 2)
          };
    }
  },
  _S = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.findAxisModel = function (t) {
        var r,
          n = this.ecModel;
        return (
          n.eachComponent(
            t,
            function (i) {
              i.getCoordSysModel() === this && (r = i);
            },
            this
          ),
          r
        );
      }),
      (e.type = "polar"),
      (e.dependencies = ["radiusAxis", "angleAxis"]),
      (e.defaultOption = { z: 0, center: ["50%", "50%"], radius: "80%" }),
      e
    );
  })(Xt),
  ao = (function (a) {
    k(e, a);
    function e() {
      return (a !== null && a.apply(this, arguments)) || this;
    }
    return (
      (e.prototype.getCoordSysModel = function () {
        return this.getReferringComponents("polar", er).models[0];
      }),
      (e.type = "polarAxis"),
      e
    );
  })(Xt);
te(ao, Ia);
var wS = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (e.type = "angleAxis"), e;
  })(ao),
  AS = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (e.type = "radiusAxis"), e;
  })(ao),
  no = (function (a) {
    k(e, a);
    function e(t, r) {
      return a.call(this, "radius", t, r) || this;
    }
    return (
      (e.prototype.pointToData = function (t, r) {
        return this.polar.pointToData(t, r)[this.dim === "radius" ? 0 : 1];
      }),
      e
    );
  })(oe);
no.prototype.dataToRadius = oe.prototype.dataToCoord;
no.prototype.radiusToData = oe.prototype.coordToData;
var TS = _t(),
  io = (function (a) {
    k(e, a);
    function e(t, r) {
      return a.call(this, "angle", t, r || [0, 360]) || this;
    }
    return (
      (e.prototype.pointToData = function (t, r) {
        return this.polar.pointToData(t, r)[this.dim === "radius" ? 0 : 1];
      }),
      (e.prototype.calculateCategoryInterval = function () {
        var t = this,
          r = t.getLabelModel(),
          n = t.scale,
          i = n.getExtent(),
          o = n.count();
        if (i[1] - i[0] < 1) return 0;
        var s = i[0],
          l = t.dataToCoord(s + 1) - t.dataToCoord(s),
          u = Math.abs(l),
          v = $h(s == null ? "" : s + "", r.getFont(), "center", "top"),
          c = Math.max(v.height, 7),
          h = c / u;
        isNaN(h) && (h = 1 / 0);
        var f = Math.max(0, Math.floor(h)),
          p = TS(t.model),
          d = p.lastAutoInterval,
          g = p.lastTickCount;
        return (
          d != null &&
          g != null &&
          Math.abs(d - f) <= 1 &&
          Math.abs(g - o) <= 1 &&
          d > f
            ? (f = d)
            : ((p.lastTickCount = o), (p.lastAutoInterval = f)),
          f
        );
      }),
      e
    );
  })(oe);
io.prototype.dataToAngle = oe.prototype.dataToCoord;
io.prototype.angleToData = oe.prototype.coordToData;
var Xv = ["radius", "angle"],
  IS = (function () {
    function a(e) {
      (this.dimensions = Xv),
        (this.type = "polar"),
        (this.cx = 0),
        (this.cy = 0),
        (this._radiusAxis = new no()),
        (this._angleAxis = new io()),
        (this.axisPointerEnabled = !0),
        (this.name = e || ""),
        (this._radiusAxis.polar = this._angleAxis.polar = this);
    }
    return (
      (a.prototype.containPoint = function (e) {
        var t = this.pointToCoord(e);
        return this._radiusAxis.contain(t[0]) && this._angleAxis.contain(t[1]);
      }),
      (a.prototype.containData = function (e) {
        return (
          this._radiusAxis.containData(e[0]) &&
          this._angleAxis.containData(e[1])
        );
      }),
      (a.prototype.getAxis = function (e) {
        var t = "_" + e + "Axis";
        return this[t];
      }),
      (a.prototype.getAxes = function () {
        return [this._radiusAxis, this._angleAxis];
      }),
      (a.prototype.getAxesByScale = function (e) {
        var t = [],
          r = this._angleAxis,
          n = this._radiusAxis;
        return (
          r.scale.type === e && t.push(r), n.scale.type === e && t.push(n), t
        );
      }),
      (a.prototype.getAngleAxis = function () {
        return this._angleAxis;
      }),
      (a.prototype.getRadiusAxis = function () {
        return this._radiusAxis;
      }),
      (a.prototype.getOtherAxis = function (e) {
        var t = this._angleAxis;
        return e === t ? this._radiusAxis : t;
      }),
      (a.prototype.getBaseAxis = function () {
        return (
          this.getAxesByScale("ordinal")[0] ||
          this.getAxesByScale("time")[0] ||
          this.getAngleAxis()
        );
      }),
      (a.prototype.getTooltipAxes = function (e) {
        var t =
          e != null && e !== "auto" ? this.getAxis(e) : this.getBaseAxis();
        return { baseAxes: [t], otherAxes: [this.getOtherAxis(t)] };
      }),
      (a.prototype.dataToPoint = function (e, t) {
        return this.coordToPoint([
          this._radiusAxis.dataToRadius(e[0], t),
          this._angleAxis.dataToAngle(e[1], t)
        ]);
      }),
      (a.prototype.pointToData = function (e, t) {
        var r = this.pointToCoord(e);
        return [
          this._radiusAxis.radiusToData(r[0], t),
          this._angleAxis.angleToData(r[1], t)
        ];
      }),
      (a.prototype.pointToCoord = function (e) {
        var t = e[0] - this.cx,
          r = e[1] - this.cy,
          n = this.getAngleAxis(),
          i = n.getExtent(),
          o = Math.min(i[0], i[1]),
          s = Math.max(i[0], i[1]);
        n.inverse ? (o = s - 360) : (s = o + 360);
        var l = Math.sqrt(t * t + r * r);
        (t /= l), (r /= l);
        for (
          var u = (Math.atan2(-r, t) / Math.PI) * 180, v = u < o ? 1 : -1;
          u < o || u > s;

        )
          u += v * 360;
        return [l, u];
      }),
      (a.prototype.coordToPoint = function (e) {
        var t = e[0],
          r = (e[1] / 180) * Math.PI,
          n = Math.cos(r) * t + this.cx,
          i = -Math.sin(r) * t + this.cy;
        return [n, i];
      }),
      (a.prototype.getArea = function () {
        var e = this.getAngleAxis(),
          t = this.getRadiusAxis(),
          r = t.getExtent().slice();
        r[0] > r[1] && r.reverse();
        var n = e.getExtent(),
          i = Math.PI / 180,
          o = 1e-4;
        return {
          cx: this.cx,
          cy: this.cy,
          r0: r[0],
          r: r[1],
          startAngle: -n[0] * i,
          endAngle: -n[1] * i,
          clockwise: e.inverse,
          contain: function (s, l) {
            var u = s - this.cx,
              v = l - this.cy,
              c = u * u + v * v,
              h = this.r,
              f = this.r0;
            return h !== f && c - o <= h * h && c + o >= f * f;
          }
        };
      }),
      (a.prototype.convertToPixel = function (e, t, r) {
        var n = gl(t);
        return n === this ? this.dataToPoint(r) : null;
      }),
      (a.prototype.convertFromPixel = function (e, t, r) {
        var n = gl(t);
        return n === this ? this.pointToData(r) : null;
      }),
      a
    );
  })();
function gl(a) {
  var e = a.seriesModel,
    t = a.polarModel;
  return (t && t.coordinateSystem) || (e && e.coordinateSystem);
}
function DS(a, e, t) {
  var r = e.get("center"),
    n = t.getWidth(),
    i = t.getHeight();
  (a.cx = z(r[0], n)), (a.cy = z(r[1], i));
  var o = a.getRadiusAxis(),
    s = Math.min(n, i) / 2,
    l = e.get("radius");
  l == null ? (l = [0, "100%"]) : H(l) || (l = [0, l]);
  var u = [z(l[0], s), z(l[1], s)];
  o.inverse ? o.setExtent(u[1], u[0]) : o.setExtent(u[0], u[1]);
}
function LS(a, e) {
  var t = this,
    r = t.getAngleAxis(),
    n = t.getRadiusAxis();
  if (
    (r.scale.setExtent(1 / 0, -1 / 0),
    n.scale.setExtent(1 / 0, -1 / 0),
    a.eachSeries(function (s) {
      if (s.coordinateSystem === t) {
        var l = s.getData();
        L(Po(l, "radius"), function (u) {
          n.scale.unionExtentFromData(l, u);
        }),
          L(Po(l, "angle"), function (u) {
            r.scale.unionExtentFromData(l, u);
          });
      }
    }),
    na(r.scale, r.model),
    na(n.scale, n.model),
    r.type === "category" && !r.onBand)
  ) {
    var i = r.getExtent(),
      o = 360 / r.scale.count();
    r.inverse ? (i[1] += o) : (i[1] -= o), r.setExtent(i[0], i[1]);
  }
}
function CS(a) {
  return a.mainType === "angleAxis";
}
function yl(a, e) {
  var t;
  if (
    ((a.type = e.get("type")),
    (a.scale = Si(e)),
    (a.onBand = e.get("boundaryGap") && a.type === "category"),
    (a.inverse = e.get("inverse")),
    CS(e))
  ) {
    a.inverse = a.inverse !== e.get("clockwise");
    var r = e.get("startAngle"),
      n =
        (t = e.get("endAngle")) !== null && t !== void 0
          ? t
          : r + (a.inverse ? -360 : 360);
    a.setExtent(r, n);
  }
  (e.axis = a), (a.model = e);
}
var PS = {
    dimensions: Xv,
    create: function (a, e) {
      var t = [];
      return (
        a.eachComponent("polar", function (r, n) {
          var i = new IS(n + "");
          i.update = LS;
          var o = i.getRadiusAxis(),
            s = i.getAngleAxis(),
            l = r.findAxisModel("radiusAxis"),
            u = r.findAxisModel("angleAxis");
          yl(o, l),
            yl(s, u),
            DS(i, r, e),
            t.push(i),
            (r.coordinateSystem = i),
            (i.model = r);
        }),
        a.eachSeries(function (r) {
          if (r.get("coordinateSystem") === "polar") {
            var n = r.getReferringComponents("polar", er).models[0];
            r.coordinateSystem = n.coordinateSystem;
          }
        }),
        t
      );
    }
  },
  MS = [
    "axisLine",
    "axisLabel",
    "axisTick",
    "minorTick",
    "splitLine",
    "minorSplitLine",
    "splitArea"
  ];
function Xr(a, e, t) {
  e[1] > e[0] && (e = e.slice().reverse());
  var r = a.coordToPoint([e[0], t]),
    n = a.coordToPoint([e[1], t]);
  return { x1: r[0], y1: r[1], x2: n[0], y2: n[1] };
}
function Zr(a) {
  var e = a.getRadiusAxis();
  return e.inverse ? 0 : 1;
}
function ml(a) {
  var e = a[0],
    t = a[a.length - 1];
  e && t && Math.abs(Math.abs(e.coord - t.coord) - 360) < 1e-4 && a.pop();
}
var ES = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.axisPointerClass = "PolarAxisPointer"), t;
    }
    return (
      (e.prototype.render = function (t, r) {
        if ((this.group.removeAll(), !!t.get("show"))) {
          var n = t.axis,
            i = n.polar,
            o = i.getRadiusAxis().getExtent(),
            s = n.getTicksCoords(),
            l = n.getMinorTicksCoords(),
            u = O(n.getViewLabels(), function (v) {
              v = Gt(v);
              var c = n.scale,
                h =
                  c.type === "ordinal"
                    ? c.getRawOrdinalNumber(v.tickValue)
                    : v.tickValue;
              return (v.coord = n.dataToCoord(h)), v;
            });
          ml(u),
            ml(s),
            L(
              MS,
              function (v) {
                t.get([v, "show"]) &&
                  (!n.scale.isBlank() || v === "axisLine") &&
                  RS[v](this.group, t, i, s, l, o, u);
              },
              this
            );
        }
      }),
      (e.type = "angleAxis"),
      e
    );
  })(kr),
  RS = {
    axisLine: function (a, e, t, r, n, i) {
      var o = e.getModel(["axisLine", "lineStyle"]),
        s = t.getAngleAxis(),
        l = Math.PI / 180,
        u = s.getExtent(),
        v = Zr(t),
        c = v ? 0 : 1,
        h,
        f = Math.abs(u[1] - u[0]) === 360 ? "Circle" : "Arc";
      i[c] === 0
        ? (h = new Qe[f]({
            shape: {
              cx: t.cx,
              cy: t.cy,
              r: i[v],
              startAngle: -u[0] * l,
              endAngle: -u[1] * l,
              clockwise: s.inverse
            },
            style: o.getLineStyle(),
            z2: 1,
            silent: !0
          }))
        : (h = new hu({
            shape: { cx: t.cx, cy: t.cy, r: i[v], r0: i[c] },
            style: o.getLineStyle(),
            z2: 1,
            silent: !0
          })),
        (h.style.fill = null),
        a.add(h);
    },
    axisTick: function (a, e, t, r, n, i) {
      var o = e.getModel("axisTick"),
        s = (o.get("inside") ? -1 : 1) * o.get("length"),
        l = i[Zr(t)],
        u = O(r, function (v) {
          return new jt({ shape: Xr(t, [l, l + s], v.coord) });
        });
      a.add(
        Kt(u, {
          style: Q(o.getModel("lineStyle").getLineStyle(), {
            stroke: e.get(["axisLine", "lineStyle", "color"])
          })
        })
      );
    },
    minorTick: function (a, e, t, r, n, i) {
      if (n.length) {
        for (
          var o = e.getModel("axisTick"),
            s = e.getModel("minorTick"),
            l = (o.get("inside") ? -1 : 1) * s.get("length"),
            u = i[Zr(t)],
            v = [],
            c = 0;
          c < n.length;
          c++
        )
          for (var h = 0; h < n[c].length; h++)
            v.push(new jt({ shape: Xr(t, [u, u + l], n[c][h].coord) }));
        a.add(
          Kt(v, {
            style: Q(
              s.getModel("lineStyle").getLineStyle(),
              Q(o.getLineStyle(), {
                stroke: e.get(["axisLine", "lineStyle", "color"])
              })
            )
          })
        );
      }
    },
    axisLabel: function (a, e, t, r, n, i, o) {
      var s = e.getCategories(!0),
        l = e.getModel("axisLabel"),
        u = l.get("margin"),
        v = e.get("triggerEvent");
      L(
        o,
        function (c, h) {
          var f = l,
            p = c.tickValue,
            d = i[Zr(t)],
            g = t.coordToPoint([d + u, c.coord]),
            y = t.cx,
            S = t.cy,
            m =
              Math.abs(g[0] - y) / d < 0.3
                ? "center"
                : g[0] > y
                ? "left"
                : "right",
            x =
              Math.abs(g[1] - S) / d < 0.3
                ? "middle"
                : g[1] > S
                ? "top"
                : "bottom";
          if (s && s[p]) {
            var b = s[p];
            Ge(b) && b.textStyle && (f = new $t(b.textStyle, l, l.ecModel));
          }
          var _ = new kt({
            silent: Ne.isLabelSilent(e),
            style: xt(f, {
              x: g[0],
              y: g[1],
              fill:
                f.getTextColor() || e.get(["axisLine", "lineStyle", "color"]),
              text: c.formattedLabel,
              align: m,
              verticalAlign: x
            })
          });
          if ((a.add(_), v)) {
            var w = Ne.makeAxisEventDataBase(e);
            (w.targetType = "axisLabel"),
              (w.value = c.rawLabel),
              (nt(_).eventData = w);
          }
        },
        this
      );
    },
    splitLine: function (a, e, t, r, n, i) {
      var o = e.getModel("splitLine"),
        s = o.getModel("lineStyle"),
        l = s.get("color"),
        u = 0;
      l = l instanceof Array ? l : [l];
      for (var v = [], c = 0; c < r.length; c++) {
        var h = u++ % l.length;
        (v[h] = v[h] || []), v[h].push(new jt({ shape: Xr(t, i, r[c].coord) }));
      }
      for (var c = 0; c < v.length; c++)
        a.add(
          Kt(v[c], {
            style: Q({ stroke: l[c % l.length] }, s.getLineStyle()),
            silent: !0,
            z: e.get("z")
          })
        );
    },
    minorSplitLine: function (a, e, t, r, n, i) {
      if (n.length) {
        for (
          var o = e.getModel("minorSplitLine"),
            s = o.getModel("lineStyle"),
            l = [],
            u = 0;
          u < n.length;
          u++
        )
          for (var v = 0; v < n[u].length; v++)
            l.push(new jt({ shape: Xr(t, i, n[u][v].coord) }));
        a.add(Kt(l, { style: s.getLineStyle(), silent: !0, z: e.get("z") }));
      }
    },
    splitArea: function (a, e, t, r, n, i) {
      if (r.length) {
        var o = e.getModel("splitArea"),
          s = o.getModel("areaStyle"),
          l = s.get("color"),
          u = 0;
        l = l instanceof Array ? l : [l];
        for (
          var v = [],
            c = Math.PI / 180,
            h = -r[0].coord * c,
            f = Math.min(i[0], i[1]),
            p = Math.max(i[0], i[1]),
            d = e.get("clockwise"),
            g = 1,
            y = r.length;
          g <= y;
          g++
        ) {
          var S = g === y ? r[0].coord : r[g].coord,
            m = u++ % l.length;
          (v[m] = v[m] || []),
            v[m].push(
              new Ve({
                shape: {
                  cx: t.cx,
                  cy: t.cy,
                  r0: f,
                  r: p,
                  startAngle: h,
                  endAngle: -S * c,
                  clockwise: d
                },
                silent: !0
              })
            ),
            (h = -S * c);
        }
        for (var g = 0; g < v.length; g++)
          a.add(
            Kt(v[g], {
              style: Q({ fill: l[g % l.length] }, s.getAreaStyle()),
              silent: !0
            })
          );
      }
    }
  },
  NS = ["axisLine", "axisTickLabel", "axisName"],
  kS = ["splitLine", "splitArea", "minorSplitLine"],
  VS = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.axisPointerClass = "PolarAxisPointer"), t;
    }
    return (
      (e.prototype.render = function (t, r) {
        if ((this.group.removeAll(), !!t.get("show"))) {
          var n = this._axisGroup,
            i = (this._axisGroup = new Z());
          this.group.add(i);
          var o = t.axis,
            s = o.polar,
            l = s.getAngleAxis(),
            u = o.getTicksCoords(),
            v = o.getMinorTicksCoords(),
            c = l.getExtent()[0],
            h = o.getExtent(),
            f = zS(s, t, c),
            p = new Ne(t, f);
          L(NS, p.add, p),
            i.add(p.getGroup()),
            bi(n, i, t),
            L(
              kS,
              function (d) {
                t.get([d, "show"]) &&
                  !o.scale.isBlank() &&
                  GS[d](this.group, t, s, c, h, u, v);
              },
              this
            );
        }
      }),
      (e.type = "radiusAxis"),
      e
    );
  })(kr),
  GS = {
    splitLine: function (a, e, t, r, n, i) {
      var o = e.getModel("splitLine"),
        s = o.getModel("lineStyle"),
        l = s.get("color"),
        u = 0,
        v = t.getAngleAxis(),
        c = Math.PI / 180,
        h = v.getExtent(),
        f = Math.abs(h[1] - h[0]) === 360 ? "Circle" : "Arc";
      l = l instanceof Array ? l : [l];
      for (var p = [], d = 0; d < i.length; d++) {
        var g = u++ % l.length;
        (p[g] = p[g] || []),
          p[g].push(
            new Qe[f]({
              shape: {
                cx: t.cx,
                cy: t.cy,
                r: Math.max(i[d].coord, 0),
                startAngle: -h[0] * c,
                endAngle: -h[1] * c,
                clockwise: v.inverse
              }
            })
          );
      }
      for (var d = 0; d < p.length; d++)
        a.add(
          Kt(p[d], {
            style: Q({ stroke: l[d % l.length], fill: null }, s.getLineStyle()),
            silent: !0
          })
        );
    },
    minorSplitLine: function (a, e, t, r, n, i, o) {
      if (o.length) {
        for (
          var s = e.getModel("minorSplitLine"),
            l = s.getModel("lineStyle"),
            u = [],
            v = 0;
          v < o.length;
          v++
        )
          for (var c = 0; c < o[v].length; c++)
            u.push(new Cr({ shape: { cx: t.cx, cy: t.cy, r: o[v][c].coord } }));
        a.add(
          Kt(u, { style: Q({ fill: null }, l.getLineStyle()), silent: !0 })
        );
      }
    },
    splitArea: function (a, e, t, r, n, i) {
      if (i.length) {
        var o = e.getModel("splitArea"),
          s = o.getModel("areaStyle"),
          l = s.get("color"),
          u = 0;
        l = l instanceof Array ? l : [l];
        for (var v = [], c = i[0].coord, h = 1; h < i.length; h++) {
          var f = u++ % l.length;
          (v[f] = v[f] || []),
            v[f].push(
              new Ve({
                shape: {
                  cx: t.cx,
                  cy: t.cy,
                  r0: c,
                  r: i[h].coord,
                  startAngle: 0,
                  endAngle: Math.PI * 2
                },
                silent: !0
              })
            ),
            (c = i[h].coord);
        }
        for (var h = 0; h < v.length; h++)
          a.add(
            Kt(v[h], {
              style: Q({ fill: l[h % l.length] }, s.getAreaStyle()),
              silent: !0
            })
          );
      }
    }
  };
function zS(a, e, t) {
  return {
    position: [a.cx, a.cy],
    rotation: (t / 180) * Math.PI,
    labelDirection: -1,
    tickDirection: -1,
    nameDirection: 1,
    labelRotate: e.getModel("axisLabel").get("rotate"),
    z2: 1
  };
}
function Zv(a) {
  return a.get("stack") || "__ec_stack_" + a.seriesIndex;
}
function qv(a, e) {
  return e.dim + a.model.componentIndex;
}
function OS(a, e, t) {
  var r = {},
    n = BS(
      Dt(e.getSeriesByType(a), function (i) {
        return (
          !e.isSeriesFiltered(i) &&
          i.coordinateSystem &&
          i.coordinateSystem.type === "polar"
        );
      })
    );
  e.eachSeriesByType(a, function (i) {
    if (i.coordinateSystem.type === "polar") {
      var o = i.getData(),
        s = i.coordinateSystem,
        l = s.getBaseAxis(),
        u = qv(s, l),
        v = Zv(i),
        c = n[u][v],
        h = c.offset,
        f = c.width,
        p = s.getOtherAxis(l),
        d = i.coordinateSystem.cx,
        g = i.coordinateSystem.cy,
        y = i.get("barMinHeight") || 0,
        S = i.get("barMinAngle") || 0;
      r[v] = r[v] || [];
      for (
        var m = o.mapDimension(p.dim),
          x = o.mapDimension(l.dim),
          b = Eu(o, m),
          _ = l.dim !== "radius" || !i.get("roundCap", !0),
          w = p.model,
          A = w.get("startValue"),
          I = p.dataToCoord(A || 0),
          T = 0,
          D = o.count();
        T < D;
        T++
      ) {
        var M = o.get(m, T),
          C = o.get(x, T),
          P = M >= 0 ? "p" : "n",
          E = I;
        b && (r[v][C] || (r[v][C] = { p: I, n: I }), (E = r[v][C][P]));
        var R = void 0,
          N = void 0,
          V = void 0,
          G = void 0;
        if (p.dim === "radius") {
          var B = p.dataToCoord(M) - I,
            F = l.dataToCoord(C);
          Math.abs(B) < y && (B = (B < 0 ? -1 : 1) * y),
            (R = E),
            (N = E + B),
            (V = F - h),
            (G = V - f),
            b && (r[v][C][P] = N);
        } else {
          var Y = p.dataToCoord(M, _) - I,
            K = l.dataToCoord(C);
          Math.abs(Y) < S && (Y = (Y < 0 ? -1 : 1) * S),
            (R = K + h),
            (N = R + f),
            (V = E),
            (G = E + Y),
            b && (r[v][C][P] = G);
        }
        o.setItemLayout(T, {
          cx: d,
          cy: g,
          r0: R,
          r: N,
          startAngle: (-V * Math.PI) / 180,
          endAngle: (-G * Math.PI) / 180,
          clockwise: V >= G
        });
      }
    }
  });
}
function BS(a) {
  var e = {};
  L(a, function (r, n) {
    var i = r.getData(),
      o = r.coordinateSystem,
      s = o.getBaseAxis(),
      l = qv(o, s),
      u = s.getExtent(),
      v =
        s.type === "category"
          ? s.getBandWidth()
          : Math.abs(u[1] - u[0]) / i.count(),
      c = e[l] || {
        bandWidth: v,
        remainedWidth: v,
        autoWidthCount: 0,
        categoryGap: "20%",
        gap: "30%",
        stacks: {}
      },
      h = c.stacks;
    e[l] = c;
    var f = Zv(r);
    h[f] || c.autoWidthCount++, (h[f] = h[f] || { width: 0, maxWidth: 0 });
    var p = z(r.get("barWidth"), v),
      d = z(r.get("barMaxWidth"), v),
      g = r.get("barGap"),
      y = r.get("barCategoryGap");
    p &&
      !h[f].width &&
      ((p = Math.min(c.remainedWidth, p)),
      (h[f].width = p),
      (c.remainedWidth -= p)),
      d && (h[f].maxWidth = d),
      g != null && (c.gap = g),
      y != null && (c.categoryGap = y);
  });
  var t = {};
  return (
    L(e, function (r, n) {
      t[n] = {};
      var i = r.stacks,
        o = r.bandWidth,
        s = z(r.categoryGap, o),
        l = z(r.gap, 1),
        u = r.remainedWidth,
        v = r.autoWidthCount,
        c = (u - s) / (v + (v - 1) * l);
      (c = Math.max(c, 0)),
        L(i, function (d, g) {
          var y = d.maxWidth;
          y &&
            y < c &&
            ((y = Math.min(y, u)),
            d.width && (y = Math.min(y, d.width)),
            (u -= y),
            (d.width = y),
            v--);
        }),
        (c = (u - s) / (v + (v - 1) * l)),
        (c = Math.max(c, 0));
      var h = 0,
        f;
      L(i, function (d, g) {
        d.width || (d.width = c), (f = d), (h += d.width * (1 + l));
      }),
        f && (h -= f.width * l);
      var p = -h / 2;
      L(i, function (d, g) {
        (t[n][g] = t[n][g] || { offset: p, width: d.width }),
          (p += d.width * (1 + l));
      });
    }),
    t
  );
}
var FS = {
    startAngle: 90,
    clockwise: !0,
    splitNumber: 12,
    axisLabel: { rotate: 0 }
  },
  WS = { splitNumber: 5 },
  HS = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (e.type = "polar"), e;
  })(ee);
function US(a) {
  q(Ai),
    kr.registerAxisPointerClass("PolarAxisPointer", SS),
    a.registerCoordinateSystem("polar", PS),
    a.registerComponentModel(_S),
    a.registerComponentView(HS),
    ia(a, "angle", wS, FS),
    ia(a, "radius", AS, WS),
    a.registerComponentView(ES),
    a.registerComponentView(VS),
    a.registerLayout(ot(OS, "bar"));
}
function ii(a, e) {
  e = e || {};
  var t = a.coordinateSystem,
    r = a.axis,
    n = {},
    i = r.position,
    o = r.orient,
    s = t.getRect(),
    l = [s.x, s.x + s.width, s.y, s.y + s.height],
    u = {
      horizontal: { top: l[2], bottom: l[3] },
      vertical: { left: l[0], right: l[1] }
    };
  n.position = [
    o === "vertical" ? u.vertical[i] : l[0],
    o === "horizontal" ? u.horizontal[i] : l[3]
  ];
  var v = { horizontal: 0, vertical: 1 };
  n.rotation = (Math.PI / 2) * v[o];
  var c = { top: -1, bottom: 1, right: 1, left: -1 };
  (n.labelDirection = n.tickDirection = n.nameDirection = c[i]),
    a.get(["axisTick", "inside"]) && (n.tickDirection = -n.tickDirection),
    Ft(e.labelInside, a.get(["axisLabel", "inside"])) &&
      (n.labelDirection = -n.labelDirection);
  var h = e.rotate;
  return (
    h == null && (h = a.get(["axisLabel", "rotate"])),
    (n.labelRotation = i === "top" ? -h : h),
    (n.z2 = 1),
    n
  );
}
var YS = ["axisLine", "axisTickLabel", "axisName"],
  $S = ["splitArea", "splitLine"],
  XS = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.axisPointerClass = "SingleAxisPointer"), t;
    }
    return (
      (e.prototype.render = function (t, r, n, i) {
        var o = this.group;
        o.removeAll();
        var s = this._axisGroup;
        this._axisGroup = new Z();
        var l = ii(t),
          u = new Ne(t, l);
        L(YS, u.add, u),
          o.add(this._axisGroup),
          o.add(u.getGroup()),
          L(
            $S,
            function (v) {
              t.get([v, "show"]) && ZS[v](this, this.group, this._axisGroup, t);
            },
            this
          ),
          bi(s, this._axisGroup, t),
          a.prototype.render.call(this, t, r, n, i);
      }),
      (e.prototype.remove = function () {
        Xh(this);
      }),
      (e.type = "singleAxis"),
      e
    );
  })(kr),
  ZS = {
    splitLine: function (a, e, t, r) {
      var n = r.axis;
      if (!n.scale.isBlank()) {
        var i = r.getModel("splitLine"),
          o = i.getModel("lineStyle"),
          s = o.get("color");
        s = s instanceof Array ? s : [s];
        for (
          var l = o.get("width"),
            u = r.coordinateSystem.getRect(),
            v = n.isHorizontal(),
            c = [],
            h = 0,
            f = n.getTicksCoords({ tickModel: i }),
            p = [],
            d = [],
            g = 0;
          g < f.length;
          ++g
        ) {
          var y = n.toGlobalCoord(f[g].coord);
          v
            ? ((p[0] = y), (p[1] = u.y), (d[0] = y), (d[1] = u.y + u.height))
            : ((p[0] = u.x), (p[1] = y), (d[0] = u.x + u.width), (d[1] = y));
          var S = new jt({
            shape: { x1: p[0], y1: p[1], x2: d[0], y2: d[1] },
            silent: !0
          });
          qh(S.shape, l);
          var m = h++ % s.length;
          (c[m] = c[m] || []), c[m].push(S);
        }
        for (var x = o.getLineStyle(["color"]), g = 0; g < c.length; ++g)
          e.add(
            Kt(c[g], { style: Q({ stroke: s[g % s.length] }, x), silent: !0 })
          );
      }
    },
    splitArea: function (a, e, t, r) {
      Zh(a, t, r, r);
    }
  },
  ta = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.getCoordSysModel = function () {
        return this;
      }),
      (e.type = "singleAxis"),
      (e.layoutMode = "box"),
      (e.defaultOption = {
        left: "5%",
        top: "5%",
        right: "5%",
        bottom: "5%",
        type: "value",
        position: "bottom",
        orient: "horizontal",
        axisLine: { show: !0, lineStyle: { width: 1, type: "solid" } },
        tooltip: { show: !0 },
        axisTick: { show: !0, length: 6, lineStyle: { width: 1 } },
        axisLabel: { show: !0, interval: "auto" },
        splitLine: { show: !0, lineStyle: { type: "dashed", opacity: 0.2 } }
      }),
      e
    );
  })(Xt);
te(ta, Ia.prototype);
var qS = (function (a) {
    k(e, a);
    function e(t, r, n, i, o) {
      var s = a.call(this, t, r, n) || this;
      return (s.type = i || "value"), (s.position = o || "bottom"), s;
    }
    return (
      (e.prototype.isHorizontal = function () {
        var t = this.position;
        return t === "top" || t === "bottom";
      }),
      (e.prototype.pointToData = function (t, r) {
        return this.coordinateSystem.pointToData(t)[0];
      }),
      e
    );
  })(oe),
  Kv = ["single"],
  KS = (function () {
    function a(e, t, r) {
      (this.type = "single"),
        (this.dimension = "single"),
        (this.dimensions = Kv),
        (this.axisPointerEnabled = !0),
        (this.model = e),
        this._init(e, t, r);
    }
    return (
      (a.prototype._init = function (e, t, r) {
        var n = this.dimension,
          i = new qS(n, Si(e), [0, 0], e.get("type"), e.get("position")),
          o = i.type === "category";
        (i.onBand = o && e.get("boundaryGap")),
          (i.inverse = e.get("inverse")),
          (i.orient = e.get("orient")),
          (e.axis = i),
          (i.model = e),
          (i.coordinateSystem = this),
          (this._axis = i);
      }),
      (a.prototype.update = function (e, t) {
        e.eachSeries(function (r) {
          if (r.coordinateSystem === this) {
            var n = r.getData();
            L(
              n.mapDimensionsAll(this.dimension),
              function (i) {
                this._axis.scale.unionExtentFromData(n, i);
              },
              this
            ),
              na(this._axis.scale, this._axis.model);
          }
        }, this);
      }),
      (a.prototype.resize = function (e, t) {
        (this._rect = se(
          {
            left: e.get("left"),
            top: e.get("top"),
            right: e.get("right"),
            bottom: e.get("bottom"),
            width: e.get("width"),
            height: e.get("height")
          },
          { width: t.getWidth(), height: t.getHeight() }
        )),
          this._adjustAxis();
      }),
      (a.prototype.getRect = function () {
        return this._rect;
      }),
      (a.prototype._adjustAxis = function () {
        var e = this._rect,
          t = this._axis,
          r = t.isHorizontal(),
          n = r ? [0, e.width] : [0, e.height],
          i = t.inverse ? 1 : 0;
        t.setExtent(n[i], n[1 - i]),
          this._updateAxisTransform(t, r ? e.x : e.y);
      }),
      (a.prototype._updateAxisTransform = function (e, t) {
        var r = e.getExtent(),
          n = r[0] + r[1],
          i = e.isHorizontal();
        (e.toGlobalCoord = i
          ? function (o) {
              return o + t;
            }
          : function (o) {
              return n - o + t;
            }),
          (e.toLocalCoord = i
            ? function (o) {
                return o - t;
              }
            : function (o) {
                return n - o + t;
              });
      }),
      (a.prototype.getAxis = function () {
        return this._axis;
      }),
      (a.prototype.getBaseAxis = function () {
        return this._axis;
      }),
      (a.prototype.getAxes = function () {
        return [this._axis];
      }),
      (a.prototype.getTooltipAxes = function () {
        return { baseAxes: [this.getAxis()], otherAxes: [] };
      }),
      (a.prototype.containPoint = function (e) {
        var t = this.getRect(),
          r = this.getAxis(),
          n = r.orient;
        return n === "horizontal"
          ? r.contain(r.toLocalCoord(e[0])) &&
              e[1] >= t.y &&
              e[1] <= t.y + t.height
          : r.contain(r.toLocalCoord(e[1])) &&
              e[0] >= t.y &&
              e[0] <= t.y + t.height;
      }),
      (a.prototype.pointToData = function (e) {
        var t = this.getAxis();
        return [
          t.coordToData(t.toLocalCoord(e[t.orient === "horizontal" ? 0 : 1]))
        ];
      }),
      (a.prototype.dataToPoint = function (e) {
        var t = this.getAxis(),
          r = this.getRect(),
          n = [],
          i = t.orient === "horizontal" ? 0 : 1;
        return (
          e instanceof Array && (e = e[0]),
          (n[i] = t.toGlobalCoord(t.dataToCoord(+e))),
          (n[1 - i] = i === 0 ? r.y + r.height / 2 : r.x + r.width / 2),
          n
        );
      }),
      (a.prototype.convertToPixel = function (e, t, r) {
        var n = Sl(t);
        return n === this ? this.dataToPoint(r) : null;
      }),
      (a.prototype.convertFromPixel = function (e, t, r) {
        var n = Sl(t);
        return n === this ? this.pointToData(r) : null;
      }),
      a
    );
  })();
function Sl(a) {
  var e = a.seriesModel,
    t = a.singleAxisModel;
  return (t && t.coordinateSystem) || (e && e.coordinateSystem);
}
function jS(a, e) {
  var t = [];
  return (
    a.eachComponent("singleAxis", function (r, n) {
      var i = new KS(r, a, e);
      (i.name = "single_" + n),
        i.resize(r, e),
        (r.coordinateSystem = i),
        t.push(i);
    }),
    a.eachSeries(function (r) {
      if (r.get("coordinateSystem") === "singleAxis") {
        var n = r.getReferringComponents("singleAxis", er).models[0];
        r.coordinateSystem = n && n.coordinateSystem;
      }
    }),
    t
  );
}
var JS = { create: jS, dimensions: Kv },
  xl = ["x", "y"],
  QS = ["width", "height"],
  t1 = (function (a) {
    k(e, a);
    function e() {
      return (a !== null && a.apply(this, arguments)) || this;
    }
    return (
      (e.prototype.makeElOption = function (t, r, n, i, o) {
        var s = n.axis,
          l = s.coordinateSystem,
          u = In(l, 1 - da(s)),
          v = l.dataToPoint(r)[0],
          c = i.get("type");
        if (c && c !== "none") {
          var h = Cu(i),
            f = e1[c](s, v, u);
          (f.style = h), (t.graphicKey = f.type), (t.pointer = f);
        }
        var p = ii(n);
        Kh(r, t, p, n, i, o);
      }),
      (e.prototype.getHandleTransform = function (t, r, n) {
        var i = ii(r, { labelInside: !1 });
        i.labelMargin = n.get(["handle", "margin"]);
        var o = jh(r.axis, t, i);
        return {
          x: o[0],
          y: o[1],
          rotation: i.rotation + (i.labelDirection < 0 ? Math.PI : 0)
        };
      }),
      (e.prototype.updateHandleTransform = function (t, r, n, i) {
        var o = n.axis,
          s = o.coordinateSystem,
          l = da(o),
          u = In(s, l),
          v = [t.x, t.y];
        (v[l] += r[l]),
          (v[l] = Math.min(u[1], v[l])),
          (v[l] = Math.max(u[0], v[l]));
        var c = In(s, 1 - l),
          h = (c[1] + c[0]) / 2,
          f = [h, h];
        return (
          (f[l] = v[l]),
          {
            x: v[0],
            y: v[1],
            rotation: t.rotation,
            cursorPoint: f,
            tooltipOption: { verticalAlign: "middle" }
          }
        );
      }),
      e
    );
  })(Pu),
  e1 = {
    line: function (a, e, t) {
      var r = Mu([e, t[0]], [e, t[1]], da(a));
      return { type: "Line", subPixelOptimize: !0, shape: r };
    },
    shadow: function (a, e, t) {
      var r = a.getBandWidth(),
        n = t[1] - t[0];
      return { type: "Rect", shape: Jh([e - r / 2, t[0]], [r, n], da(a)) };
    }
  };
function da(a) {
  return a.isHorizontal() ? 0 : 1;
}
function In(a, e) {
  var t = a.getRect();
  return [t[xl[e]], t[xl[e]] + t[QS[e]]];
}
var r1 = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), t;
  }
  return (e.type = "single"), e;
})(ee);
function a1(a) {
  q(Ai),
    kr.registerAxisPointerClass("SingleAxisPointer", t1),
    a.registerComponentView(r1),
    a.registerComponentView(XS),
    a.registerComponentModel(ta),
    ia(a, "single", ta, ta.defaultOption),
    a.registerCoordinateSystem("single", JS);
}
var n1 = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), t;
  }
  return (
    (e.prototype.init = function (t, r, n) {
      var i = Qh(t);
      a.prototype.init.apply(this, arguments), bl(t, i);
    }),
    (e.prototype.mergeOption = function (t) {
      a.prototype.mergeOption.apply(this, arguments), bl(this.option, t);
    }),
    (e.prototype.getCellSize = function () {
      return this.option.cellSize;
    }),
    (e.type = "calendar"),
    (e.defaultOption = {
      z: 2,
      left: 80,
      top: 60,
      cellSize: 20,
      orient: "horizontal",
      splitLine: {
        show: !0,
        lineStyle: { color: "#000", width: 1, type: "solid" }
      },
      itemStyle: { color: "#fff", borderWidth: 1, borderColor: "#ccc" },
      dayLabel: {
        show: !0,
        firstDay: 0,
        position: "start",
        margin: "50%",
        color: "#000"
      },
      monthLabel: {
        show: !0,
        position: "start",
        margin: 5,
        align: "center",
        formatter: null,
        color: "#000"
      },
      yearLabel: {
        show: !0,
        position: null,
        margin: 30,
        formatter: null,
        color: "#ccc",
        fontFamily: "sans-serif",
        fontWeight: "bolder",
        fontSize: 20
      }
    }),
    e
  );
})(Xt);
function bl(a, e) {
  var t = a.cellSize,
    r;
  H(t) ? (r = t) : (r = a.cellSize = [t, t]), r.length === 1 && (r[1] = r[0]);
  var n = O([0, 1], function (i) {
    return tf(e, i) && (r[i] = "auto"), r[i] != null && r[i] !== "auto";
  });
  ef(a, e, { type: "box", ignoreSize: n });
}
var i1 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i = this.group;
        i.removeAll();
        var o = t.coordinateSystem,
          s = o.getRangeInfo(),
          l = o.getOrient(),
          u = r.getLocaleModel();
        this._renderDayRect(t, s, i),
          this._renderLines(t, s, l, i),
          this._renderYearText(t, s, l, i),
          this._renderMonthText(t, u, l, i),
          this._renderWeekText(t, u, s, l, i);
      }),
      (e.prototype._renderDayRect = function (t, r, n) {
        for (
          var i = t.coordinateSystem,
            o = t.getModel("itemStyle").getItemStyle(),
            s = i.getCellWidth(),
            l = i.getCellHeight(),
            u = r.start.time;
          u <= r.end.time;
          u = i.getNextNDay(u, 1).time
        ) {
          var v = i.dataToRect([u], !1).tl,
            c = new Et({
              shape: { x: v[0], y: v[1], width: s, height: l },
              cursor: "default",
              style: o
            });
          n.add(c);
        }
      }),
      (e.prototype._renderLines = function (t, r, n, i) {
        var o = this,
          s = t.coordinateSystem,
          l = t.getModel(["splitLine", "lineStyle"]).getLineStyle(),
          u = t.get(["splitLine", "show"]),
          v = l.lineWidth;
        (this._tlpoints = []),
          (this._blpoints = []),
          (this._firstDayOfMonth = []),
          (this._firstDayPoints = []);
        for (var c = r.start, h = 0; c.time <= r.end.time; h++) {
          p(c.formatedDate),
            h === 0 && (c = s.getDateInfo(r.start.y + "-" + r.start.m));
          var f = c.date;
          f.setMonth(f.getMonth() + 1), (c = s.getDateInfo(f));
        }
        p(s.getNextNDay(r.end.time, 1).formatedDate);
        function p(d) {
          o._firstDayOfMonth.push(s.getDateInfo(d)),
            o._firstDayPoints.push(s.dataToRect([d], !1).tl);
          var g = o._getLinePointsOfOneWeek(t, d, n);
          o._tlpoints.push(g[0]),
            o._blpoints.push(g[g.length - 1]),
            u && o._drawSplitline(g, l, i);
        }
        u && this._drawSplitline(o._getEdgesPoints(o._tlpoints, v, n), l, i),
          u && this._drawSplitline(o._getEdgesPoints(o._blpoints, v, n), l, i);
      }),
      (e.prototype._getEdgesPoints = function (t, r, n) {
        var i = [t[0].slice(), t[t.length - 1].slice()],
          o = n === "horizontal" ? 0 : 1;
        return (i[0][o] = i[0][o] - r / 2), (i[1][o] = i[1][o] + r / 2), i;
      }),
      (e.prototype._drawSplitline = function (t, r, n) {
        var i = new we({ z2: 20, shape: { points: t }, style: r });
        n.add(i);
      }),
      (e.prototype._getLinePointsOfOneWeek = function (t, r, n) {
        for (
          var i = t.coordinateSystem, o = i.getDateInfo(r), s = [], l = 0;
          l < 7;
          l++
        ) {
          var u = i.getNextNDay(o.time, l),
            v = i.dataToRect([u.time], !1);
          (s[2 * u.day] = v.tl),
            (s[2 * u.day + 1] = v[n === "horizontal" ? "bl" : "tr"]);
        }
        return s;
      }),
      (e.prototype._formatterLabel = function (t, r) {
        return et(t) && t ? rf(t, r) : it(t) ? t(r) : r.nameMap;
      }),
      (e.prototype._yearTextPositionControl = function (t, r, n, i, o) {
        var s = r[0],
          l = r[1],
          u = ["center", "bottom"];
        i === "bottom"
          ? ((l += o), (u = ["center", "top"]))
          : i === "left"
          ? (s -= o)
          : i === "right"
          ? ((s += o), (u = ["center", "top"]))
          : (l -= o);
        var v = 0;
        return (
          (i === "left" || i === "right") && (v = Math.PI / 2),
          {
            rotation: v,
            x: s,
            y: l,
            style: { align: u[0], verticalAlign: u[1] }
          }
        );
      }),
      (e.prototype._renderYearText = function (t, r, n, i) {
        var o = t.getModel("yearLabel");
        if (o.get("show")) {
          var s = o.get("margin"),
            l = o.get("position");
          l || (l = n !== "horizontal" ? "top" : "left");
          var u = [
              this._tlpoints[this._tlpoints.length - 1],
              this._blpoints[0]
            ],
            v = (u[0][0] + u[1][0]) / 2,
            c = (u[0][1] + u[1][1]) / 2,
            h = n === "horizontal" ? 0 : 1,
            f = {
              top: [v, u[h][1]],
              bottom: [v, u[1 - h][1]],
              left: [u[1 - h][0], c],
              right: [u[h][0], c]
            },
            p = r.start.y;
          +r.end.y > +r.start.y && (p = p + "-" + r.end.y);
          var d = o.get("formatter"),
            g = { start: r.start.y, end: r.end.y, nameMap: p },
            y = this._formatterLabel(d, g),
            S = new kt({
              z2: 30,
              style: xt(o, { text: y }),
              silent: o.get("silent")
            });
          S.attr(this._yearTextPositionControl(S, f[l], n, l, s)), i.add(S);
        }
      }),
      (e.prototype._monthTextPositionControl = function (t, r, n, i, o) {
        var s = "left",
          l = "top",
          u = t[0],
          v = t[1];
        return (
          n === "horizontal"
            ? ((v = v + o),
              r && (s = "center"),
              i === "start" && (l = "bottom"))
            : ((u = u + o),
              r && (l = "middle"),
              i === "start" && (s = "right")),
          { x: u, y: v, align: s, verticalAlign: l }
        );
      }),
      (e.prototype._renderMonthText = function (t, r, n, i) {
        var o = t.getModel("monthLabel");
        if (o.get("show")) {
          var s = o.get("nameMap"),
            l = o.get("margin"),
            u = o.get("position"),
            v = o.get("align"),
            c = [this._tlpoints, this._blpoints];
          (!s || et(s)) &&
            (s && (r = Mo(s) || r), (s = r.get(["time", "monthAbbr"]) || []));
          var h = u === "start" ? 0 : 1,
            f = n === "horizontal" ? 0 : 1;
          l = u === "start" ? -l : l;
          for (
            var p = v === "center", d = o.get("silent"), g = 0;
            g < c[h].length - 1;
            g++
          ) {
            var y = c[h][g].slice(),
              S = this._firstDayOfMonth[g];
            if (p) {
              var m = this._firstDayPoints[g];
              y[f] = (m[f] + c[0][g + 1][f]) / 2;
            }
            var x = o.get("formatter"),
              b = s[+S.m - 1],
              _ = {
                yyyy: S.y,
                yy: (S.y + "").slice(2),
                MM: S.m,
                M: +S.m,
                nameMap: b
              },
              w = this._formatterLabel(x, _),
              A = new kt({
                z2: 30,
                style: W(
                  xt(o, { text: w }),
                  this._monthTextPositionControl(y, p, n, u, l)
                ),
                silent: d
              });
            i.add(A);
          }
        }
      }),
      (e.prototype._weekTextPositionControl = function (t, r, n, i, o) {
        var s = "center",
          l = "middle",
          u = t[0],
          v = t[1],
          c = n === "start";
        return (
          r === "horizontal"
            ? ((u = u + i + ((c ? 1 : -1) * o[0]) / 2),
              (s = c ? "right" : "left"))
            : ((v = v + i + ((c ? 1 : -1) * o[1]) / 2),
              (l = c ? "bottom" : "top")),
          { x: u, y: v, align: s, verticalAlign: l }
        );
      }),
      (e.prototype._renderWeekText = function (t, r, n, i, o) {
        var s = t.getModel("dayLabel");
        if (s.get("show")) {
          var l = t.coordinateSystem,
            u = s.get("position"),
            v = s.get("nameMap"),
            c = s.get("margin"),
            h = l.getFirstDayOfWeek();
          if (!v || et(v)) {
            v && (r = Mo(v) || r);
            var f = r.get(["time", "dayOfWeekShort"]);
            v =
              f ||
              O(r.get(["time", "dayOfWeekAbbr"]), function (_) {
                return _[0];
              });
          }
          var p = l.getNextNDay(n.end.time, 7 - n.lweek).time,
            d = [l.getCellWidth(), l.getCellHeight()];
          (c = z(c, Math.min(d[1], d[0]))),
            u === "start" &&
              ((p = l.getNextNDay(n.start.time, -(7 + n.fweek)).time),
              (c = -c));
          for (var g = s.get("silent"), y = 0; y < 7; y++) {
            var S = l.getNextNDay(p, y),
              m = l.dataToRect([S.time], !1).center,
              x = y;
            x = Math.abs((y + h) % 7);
            var b = new kt({
              z2: 30,
              style: W(
                xt(s, { text: v[x] }),
                this._weekTextPositionControl(m, i, u, c, d)
              ),
              silent: g
            });
            o.add(b);
          }
        }
      }),
      (e.type = "calendar"),
      e
    );
  })(ee),
  Dn = 864e5,
  o1 = (function () {
    function a(e, t, r) {
      (this.type = "calendar"),
        (this.dimensions = a.dimensions),
        (this.getDimensionsInfo = a.getDimensionsInfo),
        (this._model = e);
    }
    return (
      (a.getDimensionsInfo = function () {
        return [{ name: "time", type: "time" }, "value"];
      }),
      (a.prototype.getRangeInfo = function () {
        return this._rangeInfo;
      }),
      (a.prototype.getModel = function () {
        return this._model;
      }),
      (a.prototype.getRect = function () {
        return this._rect;
      }),
      (a.prototype.getCellWidth = function () {
        return this._sw;
      }),
      (a.prototype.getCellHeight = function () {
        return this._sh;
      }),
      (a.prototype.getOrient = function () {
        return this._orient;
      }),
      (a.prototype.getFirstDayOfWeek = function () {
        return this._firstDayOfWeek;
      }),
      (a.prototype.getDateInfo = function (e) {
        e = af(e);
        var t = e.getFullYear(),
          r = e.getMonth() + 1,
          n = r < 10 ? "0" + r : "" + r,
          i = e.getDate(),
          o = i < 10 ? "0" + i : "" + i,
          s = e.getDay();
        return (
          (s = Math.abs((s + 7 - this.getFirstDayOfWeek()) % 7)),
          {
            y: t + "",
            m: n,
            d: o,
            day: s,
            time: e.getTime(),
            formatedDate: t + "-" + n + "-" + o,
            date: e
          }
        );
      }),
      (a.prototype.getNextNDay = function (e, t) {
        return (
          (t = t || 0),
          t === 0
            ? this.getDateInfo(e)
            : ((e = new Date(this.getDateInfo(e).time)),
              e.setDate(e.getDate() + t),
              this.getDateInfo(e))
        );
      }),
      (a.prototype.update = function (e, t) {
        (this._firstDayOfWeek = +this._model
          .getModel("dayLabel")
          .get("firstDay")),
          (this._orient = this._model.get("orient")),
          (this._lineWidth =
            this._model.getModel("itemStyle").getItemStyle().lineWidth || 0),
          (this._rangeInfo = this._getRangeInfo(this._initRangeOption()));
        var r = this._rangeInfo.weeks || 1,
          n = ["width", "height"],
          i = this._model.getCellSize().slice(),
          o = this._model.getBoxLayoutParams(),
          s = this._orient === "horizontal" ? [r, 7] : [7, r];
        L([0, 1], function (c) {
          v(i, c) && (o[n[c]] = i[c] * s[c]);
        });
        var l = { width: t.getWidth(), height: t.getHeight() },
          u = (this._rect = se(o, l));
        L([0, 1], function (c) {
          v(i, c) || (i[c] = u[n[c]] / s[c]);
        });
        function v(c, h) {
          return c[h] != null && c[h] !== "auto";
        }
        (this._sw = i[0]), (this._sh = i[1]);
      }),
      (a.prototype.dataToPoint = function (e, t) {
        H(e) && (e = e[0]), t == null && (t = !0);
        var r = this.getDateInfo(e),
          n = this._rangeInfo,
          i = r.formatedDate;
        if (t && !(r.time >= n.start.time && r.time < n.end.time + Dn))
          return [NaN, NaN];
        var o = r.day,
          s = this._getRangeInfo([n.start.time, i]).nthWeek;
        return this._orient === "vertical"
          ? [
              this._rect.x + o * this._sw + this._sw / 2,
              this._rect.y + s * this._sh + this._sh / 2
            ]
          : [
              this._rect.x + s * this._sw + this._sw / 2,
              this._rect.y + o * this._sh + this._sh / 2
            ];
      }),
      (a.prototype.pointToData = function (e) {
        var t = this.pointToDate(e);
        return t && t.time;
      }),
      (a.prototype.dataToRect = function (e, t) {
        var r = this.dataToPoint(e, t);
        return {
          contentShape: {
            x: r[0] - (this._sw - this._lineWidth) / 2,
            y: r[1] - (this._sh - this._lineWidth) / 2,
            width: this._sw - this._lineWidth,
            height: this._sh - this._lineWidth
          },
          center: r,
          tl: [r[0] - this._sw / 2, r[1] - this._sh / 2],
          tr: [r[0] + this._sw / 2, r[1] - this._sh / 2],
          br: [r[0] + this._sw / 2, r[1] + this._sh / 2],
          bl: [r[0] - this._sw / 2, r[1] + this._sh / 2]
        };
      }),
      (a.prototype.pointToDate = function (e) {
        var t = Math.floor((e[0] - this._rect.x) / this._sw) + 1,
          r = Math.floor((e[1] - this._rect.y) / this._sh) + 1,
          n = this._rangeInfo.range;
        return this._orient === "vertical"
          ? this._getDateByWeeksAndDay(r, t - 1, n)
          : this._getDateByWeeksAndDay(t, r - 1, n);
      }),
      (a.prototype.convertToPixel = function (e, t, r) {
        var n = _l(t);
        return n === this ? n.dataToPoint(r) : null;
      }),
      (a.prototype.convertFromPixel = function (e, t, r) {
        var n = _l(t);
        return n === this ? n.pointToData(r) : null;
      }),
      (a.prototype.containPoint = function (e) {
        return console.warn("Not implemented."), !1;
      }),
      (a.prototype._initRangeOption = function () {
        var e = this._model.get("range"),
          t;
        if ((H(e) && e.length === 1 && (e = e[0]), H(e))) t = e;
        else {
          var r = e.toString();
          if (
            (/^\d{4}$/.test(r) && (t = [r + "-01-01", r + "-12-31"]),
            /^\d{4}[\/|-]\d{1,2}$/.test(r))
          ) {
            var n = this.getDateInfo(r),
              i = n.date;
            i.setMonth(i.getMonth() + 1);
            var o = this.getNextNDay(i, -1);
            t = [n.formatedDate, o.formatedDate];
          }
          /^\d{4}[\/|-]\d{1,2}[\/|-]\d{1,2}$/.test(r) && (t = [r, r]);
        }
        if (!t) return e;
        var s = this._getRangeInfo(t);
        return s.start.time > s.end.time && t.reverse(), t;
      }),
      (a.prototype._getRangeInfo = function (e) {
        var t = [this.getDateInfo(e[0]), this.getDateInfo(e[1])],
          r;
        t[0].time > t[1].time && ((r = !0), t.reverse());
        var n = Math.floor(t[1].time / Dn) - Math.floor(t[0].time / Dn) + 1,
          i = new Date(t[0].time),
          o = i.getDate(),
          s = t[1].date.getDate();
        i.setDate(o + n - 1);
        var l = i.getDate();
        if (l !== s)
          for (
            var u = i.getTime() - t[1].time > 0 ? 1 : -1;
            (l = i.getDate()) !== s && (i.getTime() - t[1].time) * u > 0;

          )
            (n -= u), i.setDate(l - u);
        var v = Math.floor((n + t[0].day + 6) / 7),
          c = r ? -v + 1 : v - 1;
        return (
          r && t.reverse(),
          {
            range: [t[0].formatedDate, t[1].formatedDate],
            start: t[0],
            end: t[1],
            allDay: n,
            weeks: v,
            nthWeek: c,
            fweek: t[0].day,
            lweek: t[1].day
          }
        );
      }),
      (a.prototype._getDateByWeeksAndDay = function (e, t, r) {
        var n = this._getRangeInfo(r);
        if (
          e > n.weeks ||
          (e === 0 && t < n.fweek) ||
          (e === n.weeks && t > n.lweek)
        )
          return null;
        var i = (e - 1) * 7 - n.fweek + t,
          o = new Date(n.start.time);
        return o.setDate(+n.start.d + i), this.getDateInfo(o);
      }),
      (a.create = function (e, t) {
        var r = [];
        return (
          e.eachComponent("calendar", function (n) {
            var i = new a(n);
            r.push(i), (n.coordinateSystem = i);
          }),
          e.eachSeries(function (n) {
            n.get("coordinateSystem") === "calendar" &&
              (n.coordinateSystem = r[n.get("calendarIndex") || 0]);
          }),
          r
        );
      }),
      (a.dimensions = ["time", "value"]),
      a
    );
  })();
function _l(a) {
  var e = a.calendarModel,
    t = a.seriesModel,
    r = e ? e.coordinateSystem : t ? t.coordinateSystem : null;
  return r;
}
function s1(a) {
  a.registerComponentModel(n1),
    a.registerComponentView(i1),
    a.registerCoordinateSystem("calendar", o1);
}
var l1 = ["rect", "polygon", "keep", "clear"];
function u1(a, e) {
  var t = Jt(a ? a.brush : []);
  if (t.length) {
    var r = [];
    L(t, function (l) {
      var u = l.hasOwnProperty("toolbox") ? l.toolbox : [];
      u instanceof Array && (r = r.concat(u));
    });
    var n = a && a.toolbox;
    H(n) && (n = n[0]), n || ((n = { feature: {} }), (a.toolbox = [n]));
    var i = n.feature || (n.feature = {}),
      o = i.brush || (i.brush = {}),
      s = o.type || (o.type = []);
    s.push.apply(s, r), v1(s), e && !s.length && s.push.apply(s, l1);
  }
}
function v1(a) {
  var e = {};
  L(a, function (t) {
    e[t] = 1;
  }),
    (a.length = 0),
    L(e, function (t, r) {
      a.push(r);
    });
}
function c1(a) {
  var e = a.brushType,
    t = {
      point: function (r) {
        return wl[e].point(r, t, a);
      },
      rect: function (r) {
        return wl[e].rect(r, t, a);
      }
    };
  return t;
}
var wl = {
  lineX: Al(0),
  lineY: Al(1),
  rect: {
    point: function (a, e, t) {
      return a && t.boundingRect.contain(a[0], a[1]);
    },
    rect: function (a, e, t) {
      return a && t.boundingRect.intersect(a);
    }
  },
  polygon: {
    point: function (a, e, t) {
      return a && t.boundingRect.contain(a[0], a[1]) && ar(t.range, a[0], a[1]);
    },
    rect: function (a, e, t) {
      var r = t.range;
      if (!a || r.length <= 1) return !1;
      var n = a.x,
        i = a.y,
        o = a.width,
        s = a.height,
        l = r[0];
      if (
        ar(r, n, i) ||
        ar(r, n + o, i) ||
        ar(r, n, i + s) ||
        ar(r, n + o, i + s) ||
        ft.create(a).contain(l[0], l[1]) ||
        zr(n, i, n + o, i, r) ||
        zr(n, i, n, i + s, r) ||
        zr(n + o, i, n + o, i + s, r) ||
        zr(n, i + s, n + o, i + s, r)
      )
        return !0;
    }
  }
};
function Al(a) {
  var e = ["x", "y"],
    t = ["width", "height"];
  return {
    point: function (r, n, i) {
      if (r) {
        var o = i.range,
          s = r[a];
        return lr(s, o);
      }
    },
    rect: function (r, n, i) {
      if (r) {
        var o = i.range,
          s = [r[e[a]], r[e[a]] + r[t[a]]];
        return (
          s[1] < s[0] && s.reverse(),
          lr(s[0], o) || lr(s[1], o) || lr(o[0], s) || lr(o[1], s)
        );
      }
    }
  };
}
function lr(a, e) {
  return e[0] <= a && a <= e[1];
}
var Tl = ["inBrush", "outOfBrush"],
  Ln = "__ecBrushSelect",
  oi = "__ecInBrushSelectEvent";
function jv(a) {
  a.eachComponent({ mainType: "brush" }, function (e) {
    var t = (e.brushTargetManager = new sf(e.option, a));
    t.setInputRanges(e.areas, a);
  });
}
function h1(a, e, t) {
  var r = [],
    n,
    i;
  a.eachComponent({ mainType: "brush" }, function (o) {
    t &&
      t.type === "takeGlobalCursor" &&
      o.setBrushOption(t.key === "brush" ? t.brushOption : { brushType: !1 });
  }),
    jv(a),
    a.eachComponent({ mainType: "brush" }, function (o, s) {
      var l = {
        brushId: o.id,
        brushIndex: s,
        brushName: o.name,
        areas: Gt(o.areas),
        selected: []
      };
      r.push(l);
      var u = o.option,
        v = u.brushLink,
        c = [],
        h = [],
        f = [],
        p = !1;
      s || ((n = u.throttleType), (i = u.throttleDelay));
      var d = O(o.areas, function (b) {
          var _ = g1[b.brushType],
            w = Q({ boundingRect: _ ? _(b) : void 0 }, b);
          return (w.selectors = c1(w)), w;
        }),
        g = nf(o.option, Tl, function (b) {
          b.mappingMethod = "fixed";
        });
      H(v) &&
        L(v, function (b) {
          c[b] = 1;
        });
      function y(b) {
        return v === "all" || !!c[b];
      }
      function S(b) {
        return !!b.length;
      }
      a.eachSeries(function (b, _) {
        var w = (f[_] = []);
        b.subType === "parallel" ? m(b, _) : x(b, _, w);
      });
      function m(b, _) {
        var w = b.coordinateSystem;
        (p = p || w.hasAxisBrushed()),
          y(_) &&
            w.eachActiveState(b.getData(), function (A, I) {
              A === "active" && (h[I] = 1);
            });
      }
      function x(b, _, w) {
        if (
          !(!b.brushSelector || d1(o, _)) &&
          (L(d, function (I) {
            o.brushTargetManager.controlSeries(I, b, a) && w.push(I),
              (p = p || S(w));
          }),
          y(_) && S(w))
        ) {
          var A = b.getData();
          A.each(function (I) {
            Il(b, w, A, I) && (h[I] = 1);
          });
        }
      }
      a.eachSeries(function (b, _) {
        var w = {
          seriesId: b.id,
          seriesIndex: _,
          seriesName: b.name,
          dataIndex: []
        };
        l.selected.push(w);
        var A = f[_],
          I = b.getData(),
          T = y(_)
            ? function (D) {
                return h[D]
                  ? (w.dataIndex.push(I.getRawIndex(D)), "inBrush")
                  : "outOfBrush";
              }
            : function (D) {
                return Il(b, A, I, D)
                  ? (w.dataIndex.push(I.getRawIndex(D)), "inBrush")
                  : "outOfBrush";
              };
        (y(_) ? p : S(A)) && of(Tl, g, I, T);
      });
    }),
    f1(e, n, i, r, t);
}
function f1(a, e, t, r, n) {
  if (n) {
    var i = a.getZr();
    if (!i[oi]) {
      i[Ln] || (i[Ln] = p1);
      var o = Tu(i, Ln, t, e);
      o(a, r);
    }
  }
}
function p1(a, e) {
  if (!a.isDisposed()) {
    var t = a.getZr();
    (t[oi] = !0),
      a.dispatchAction({ type: "brushSelect", batch: e }),
      (t[oi] = !1);
  }
}
function Il(a, e, t, r) {
  for (var n = 0, i = e.length; n < i; n++) {
    var o = e[n];
    if (a.brushSelector(r, t, o.selectors, o)) return !0;
  }
}
function d1(a, e) {
  var t = a.option.seriesIndex;
  return t != null && t !== "all" && (H(t) ? Ht(t, e) < 0 : e !== t);
}
var g1 = {
  rect: function (a) {
    return Dl(a.range);
  },
  polygon: function (a) {
    for (var e, t = a.range, r = 0, n = t.length; r < n; r++) {
      e = e || [
        [1 / 0, -1 / 0],
        [1 / 0, -1 / 0]
      ];
      var i = t[r];
      i[0] < e[0][0] && (e[0][0] = i[0]),
        i[0] > e[0][1] && (e[0][1] = i[0]),
        i[1] < e[1][0] && (e[1][0] = i[1]),
        i[1] > e[1][1] && (e[1][1] = i[1]);
    }
    return e && Dl(e);
  }
};
function Dl(a) {
  return new ft(a[0][0], a[1][0], a[0][1] - a[0][0], a[1][1] - a[1][0]);
}
var y1 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.init = function (t, r) {
        (this.ecModel = t),
          (this.api = r),
          this.model,
          (this._brushController = new Iu(r.getZr()))
            .on("brush", at(this._onBrush, this))
            .mount();
      }),
      (e.prototype.render = function (t, r, n, i) {
        (this.model = t), this._updateController(t, r, n, i);
      }),
      (e.prototype.updateTransform = function (t, r, n, i) {
        jv(r), this._updateController(t, r, n, i);
      }),
      (e.prototype.updateVisual = function (t, r, n, i) {
        this.updateTransform(t, r, n, i);
      }),
      (e.prototype.updateView = function (t, r, n, i) {
        this._updateController(t, r, n, i);
      }),
      (e.prototype._updateController = function (t, r, n, i) {
        (!i || i.$from !== t.id) &&
          this._brushController
            .setPanels(t.brushTargetManager.makePanelOpts(n))
            .enableBrush(t.brushOption)
            .updateCovers(t.areas.slice());
      }),
      (e.prototype.dispose = function () {
        this._brushController.dispose();
      }),
      (e.prototype._onBrush = function (t) {
        var r = this.model.id,
          n = this.model.brushTargetManager.setOutputRanges(
            t.areas,
            this.ecModel
          );
        (!t.isEnd || t.removeOnClick) &&
          this.api.dispatchAction({
            type: "brush",
            brushId: r,
            areas: Gt(n),
            $from: r
          }),
          t.isEnd &&
            this.api.dispatchAction({
              type: "brushEnd",
              brushId: r,
              areas: Gt(n),
              $from: r
            });
      }),
      (e.type = "brush"),
      e
    );
  })(ee),
  m1 = "#ddd",
  S1 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.areas = []), (t.brushOption = {}), t;
    }
    return (
      (e.prototype.optionUpdated = function (t, r) {
        var n = this.option;
        !r && lf(n, t, ["inBrush", "outOfBrush"]);
        var i = (n.inBrush = n.inBrush || {});
        (n.outOfBrush = n.outOfBrush || { color: m1 }),
          i.hasOwnProperty("liftZ") || (i.liftZ = 5);
      }),
      (e.prototype.setAreas = function (t) {
        t &&
          (this.areas = O(
            t,
            function (r) {
              return Ll(this.option, r);
            },
            this
          ));
      }),
      (e.prototype.setBrushOption = function (t) {
        (this.brushOption = Ll(this.option, t)),
          (this.brushType = this.brushOption.brushType);
      }),
      (e.type = "brush"),
      (e.dependencies = [
        "geo",
        "grid",
        "xAxis",
        "yAxis",
        "parallel",
        "series"
      ]),
      (e.defaultOption = {
        seriesIndex: "all",
        brushType: "rect",
        brushMode: "single",
        transformable: !0,
        brushStyle: {
          borderWidth: 1,
          color: "rgba(210,219,238,0.3)",
          borderColor: "#D2DBEE"
        },
        throttleType: "fixRate",
        throttleDelay: 0,
        removeOnClick: !0,
        z: 1e4
      }),
      e
    );
  })(Xt);
function Ll(a, e) {
  return Yt(
    {
      brushType: a.brushType,
      brushMode: a.brushMode,
      transformable: a.transformable,
      brushStyle: new $t(a.brushStyle).getItemStyle(),
      removeOnClick: a.removeOnClick,
      z: a.z
    },
    e,
    !0
  );
}
var x1 = ["rect", "polygon", "lineX", "lineY", "keep", "clear"],
  b1 = (function (a) {
    k(e, a);
    function e() {
      return (a !== null && a.apply(this, arguments)) || this;
    }
    return (
      (e.prototype.render = function (t, r, n) {
        var i, o, s;
        r.eachComponent({ mainType: "brush" }, function (l) {
          (i = l.brushType),
            (o = l.brushOption.brushMode || "single"),
            (s = s || !!l.areas.length);
        }),
          (this._brushType = i),
          (this._brushMode = o),
          L(t.get("type", !0), function (l) {
            t.setIconStatus(
              l,
              (l === "keep" ? o === "multiple" : l === "clear" ? s : l === i)
                ? "emphasis"
                : "normal"
            );
          });
      }),
      (e.prototype.updateView = function (t, r, n) {
        this.render(t, r, n);
      }),
      (e.prototype.getIcons = function () {
        var t = this.model,
          r = t.get("icon", !0),
          n = {};
        return (
          L(t.get("type", !0), function (i) {
            r[i] && (n[i] = r[i]);
          }),
          n
        );
      }),
      (e.prototype.onclick = function (t, r, n) {
        var i = this._brushType,
          o = this._brushMode;
        n === "clear"
          ? (r.dispatchAction({ type: "axisAreaSelect", intervals: [] }),
            r.dispatchAction({ type: "brush", command: "clear", areas: [] }))
          : r.dispatchAction({
              type: "takeGlobalCursor",
              key: "brush",
              brushOption: {
                brushType: n === "keep" ? i : i === n ? !1 : n,
                brushMode:
                  n === "keep" ? (o === "multiple" ? "single" : "multiple") : o
              }
            });
      }),
      (e.getDefaultOption = function (t) {
        var r = {
          show: !0,
          type: x1.slice(),
          icon: {
            rect: "M7.3,34.7 M0.4,10V-0.2h9.8 M89.6,10V-0.2h-9.8 M0.4,60v10.2h9.8 M89.6,60v10.2h-9.8 M12.3,22.4V10.5h13.1 M33.6,10.5h7.8 M49.1,10.5h7.8 M77.5,22.4V10.5h-13 M12.3,31.1v8.2 M77.7,31.1v8.2 M12.3,47.6v11.9h13.1 M33.6,59.5h7.6 M49.1,59.5 h7.7 M77.5,47.6v11.9h-13",
            polygon:
              "M55.2,34.9c1.7,0,3.1,1.4,3.1,3.1s-1.4,3.1-3.1,3.1 s-3.1-1.4-3.1-3.1S53.5,34.9,55.2,34.9z M50.4,51c1.7,0,3.1,1.4,3.1,3.1c0,1.7-1.4,3.1-3.1,3.1c-1.7,0-3.1-1.4-3.1-3.1 C47.3,52.4,48.7,51,50.4,51z M55.6,37.1l1.5-7.8 M60.1,13.5l1.6-8.7l-7.8,4 M59,19l-1,5.3 M24,16.1l6.4,4.9l6.4-3.3 M48.5,11.6 l-5.9,3.1 M19.1,12.8L9.7,5.1l1.1,7.7 M13.4,29.8l1,7.3l6.6,1.6 M11.6,18.4l1,6.1 M32.8,41.9 M26.6,40.4 M27.3,40.2l6.1,1.6 M49.9,52.1l-5.6-7.6l-4.9-1.2",
            lineX:
              "M15.2,30 M19.7,15.6V1.9H29 M34.8,1.9H40.4 M55.3,15.6V1.9H45.9 M19.7,44.4V58.1H29 M34.8,58.1H40.4 M55.3,44.4 V58.1H45.9 M12.5,20.3l-9.4,9.6l9.6,9.8 M3.1,29.9h16.5 M62.5,20.3l9.4,9.6L62.3,39.7 M71.9,29.9H55.4",
            lineY:
              "M38.8,7.7 M52.7,12h13.2v9 M65.9,26.6V32 M52.7,46.3h13.2v-9 M24.9,12H11.8v9 M11.8,26.6V32 M24.9,46.3H11.8v-9 M48.2,5.1l-9.3-9l-9.4,9.2 M38.9-3.9V12 M48.2,53.3l-9.3,9l-9.4-9.2 M38.9,62.3V46.4",
            keep: "M4,10.5V1h10.3 M20.7,1h6.1 M33,1h6.1 M55.4,10.5V1H45.2 M4,17.3v6.6 M55.6,17.3v6.6 M4,30.5V40h10.3 M20.7,40 h6.1 M33,40h6.1 M55.4,30.5V40H45.2 M21,18.9h62.9v48.6H21V18.9z",
            clear:
              "M22,14.7l30.9,31 M52.9,14.7L22,45.7 M4.7,16.8V4.2h13.1 M26,4.2h7.8 M41.6,4.2h7.8 M70.3,16.8V4.2H57.2 M4.7,25.9v8.6 M70.3,25.9v8.6 M4.7,43.2v12.6h13.1 M26,55.8h7.8 M41.6,55.8h7.8 M70.3,43.2v12.6H57.2"
          },
          title: t.getLocaleModel().get(["toolbox", "brush", "title"])
        };
        return r;
      }),
      e
    );
  })(uf);
function _1(a) {
  a.registerComponentView(y1),
    a.registerComponentModel(S1),
    a.registerPreprocessor(u1),
    a.registerVisual(a.PRIORITY.VISUAL.BRUSH, h1),
    a.registerAction(
      { type: "brush", event: "brush", update: "updateVisual" },
      function (e, t) {
        t.eachComponent({ mainType: "brush", query: e }, function (r) {
          r.setAreas(e.areas);
        });
      }
    ),
    a.registerAction(
      { type: "brushSelect", event: "brushSelected", update: "none" },
      qe
    ),
    a.registerAction(
      { type: "brushEnd", event: "brushEnd", update: "none" },
      qe
    ),
    vf("brush", b1);
}
var Cl = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.layoutMode = "box"), t;
    }
    return (
      (e.prototype.init = function (t, r, n) {
        this.mergeDefaultAndTheme(t, n), this._initData();
      }),
      (e.prototype.mergeOption = function (t) {
        a.prototype.mergeOption.apply(this, arguments), this._initData();
      }),
      (e.prototype.setCurrentIndex = function (t) {
        t == null && (t = this.option.currentIndex);
        var r = this._data.count();
        this.option.loop
          ? (t = ((t % r) + r) % r)
          : (t >= r && (t = r - 1), t < 0 && (t = 0)),
          (this.option.currentIndex = t);
      }),
      (e.prototype.getCurrentIndex = function () {
        return this.option.currentIndex;
      }),
      (e.prototype.isIndexMax = function () {
        return this.getCurrentIndex() >= this._data.count() - 1;
      }),
      (e.prototype.setPlayState = function (t) {
        this.option.autoPlay = !!t;
      }),
      (e.prototype.getPlayState = function () {
        return !!this.option.autoPlay;
      }),
      (e.prototype._initData = function () {
        var t = this.option,
          r = t.data || [],
          n = t.axisType,
          i = (this._names = []),
          o;
        n === "category"
          ? ((o = []),
            L(r, function (u, v) {
              var c = Mr(cf(u), ""),
                h;
              Ge(u) ? ((h = Gt(u)), (h.value = v)) : (h = v),
                o.push(h),
                i.push(c);
            }))
          : (o = r);
        var s =
            { category: "ordinal", time: "time", value: "number" }[n] ||
            "number",
          l = (this._data = new Vt([{ name: "value", type: s }], this));
        l.initData(o, i);
      }),
      (e.prototype.getData = function () {
        return this._data;
      }),
      (e.prototype.getCategories = function () {
        if (this.get("axisType") === "category") return this._names.slice();
      }),
      (e.type = "timeline"),
      (e.defaultOption = {
        z: 4,
        show: !0,
        axisType: "time",
        realtime: !0,
        left: "20%",
        top: null,
        right: "20%",
        bottom: 0,
        width: null,
        height: 40,
        padding: 5,
        controlPosition: "left",
        autoPlay: !1,
        rewind: !1,
        loop: !0,
        playInterval: 2e3,
        currentIndex: 0,
        itemStyle: {},
        label: { color: "#000" },
        data: []
      }),
      e
    );
  })(Xt),
  Jv = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.type = "timeline.slider"),
      (e.defaultOption = Lu(Cl.defaultOption, {
        backgroundColor: "rgba(0,0,0,0)",
        borderColor: "#ccc",
        borderWidth: 0,
        orient: "horizontal",
        inverse: !1,
        tooltip: { trigger: "item" },
        symbol: "circle",
        symbolSize: 12,
        lineStyle: { show: !0, width: 2, color: "#DAE1F5" },
        label: {
          position: "auto",
          show: !0,
          interval: "auto",
          rotate: 0,
          color: "#A4B1D7"
        },
        itemStyle: { color: "#A4B1D7", borderWidth: 1 },
        checkpointStyle: {
          symbol: "circle",
          symbolSize: 15,
          color: "#316bf3",
          borderColor: "#fff",
          borderWidth: 2,
          shadowBlur: 2,
          shadowOffsetX: 1,
          shadowOffsetY: 1,
          shadowColor: "rgba(0, 0, 0, 0.3)",
          animation: !0,
          animationDuration: 300,
          animationEasing: "quinticInOut"
        },
        controlStyle: {
          show: !0,
          showPlayBtn: !0,
          showPrevBtn: !0,
          showNextBtn: !0,
          itemSize: 24,
          itemGap: 12,
          position: "left",
          playIcon:
            "path://M31.6,53C17.5,53,6,41.5,6,27.4S17.5,1.8,31.6,1.8C45.7,1.8,57.2,13.3,57.2,27.4S45.7,53,31.6,53z M31.6,3.3 C18.4,3.3,7.5,14.1,7.5,27.4c0,13.3,10.8,24.1,24.1,24.1C44.9,51.5,55.7,40.7,55.7,27.4C55.7,14.1,44.9,3.3,31.6,3.3z M24.9,21.3 c0-2.2,1.6-3.1,3.5-2l10.5,6.1c1.899,1.1,1.899,2.9,0,4l-10.5,6.1c-1.9,1.1-3.5,0.2-3.5-2V21.3z",
          stopIcon:
            "path://M30.9,53.2C16.8,53.2,5.3,41.7,5.3,27.6S16.8,2,30.9,2C45,2,56.4,13.5,56.4,27.6S45,53.2,30.9,53.2z M30.9,3.5C17.6,3.5,6.8,14.4,6.8,27.6c0,13.3,10.8,24.1,24.101,24.1C44.2,51.7,55,40.9,55,27.6C54.9,14.4,44.1,3.5,30.9,3.5z M36.9,35.8c0,0.601-0.4,1-0.9,1h-1.3c-0.5,0-0.9-0.399-0.9-1V19.5c0-0.6,0.4-1,0.9-1H36c0.5,0,0.9,0.4,0.9,1V35.8z M27.8,35.8 c0,0.601-0.4,1-0.9,1h-1.3c-0.5,0-0.9-0.399-0.9-1V19.5c0-0.6,0.4-1,0.9-1H27c0.5,0,0.9,0.4,0.9,1L27.8,35.8L27.8,35.8z",
          nextIcon:
            "M2,18.5A1.52,1.52,0,0,1,.92,18a1.49,1.49,0,0,1,0-2.12L7.81,9.36,1,3.11A1.5,1.5,0,1,1,3,.89l8,7.34a1.48,1.48,0,0,1,.49,1.09,1.51,1.51,0,0,1-.46,1.1L3,18.08A1.5,1.5,0,0,1,2,18.5Z",
          prevIcon:
            "M10,.5A1.52,1.52,0,0,1,11.08,1a1.49,1.49,0,0,1,0,2.12L4.19,9.64,11,15.89a1.5,1.5,0,1,1-2,2.22L1,10.77A1.48,1.48,0,0,1,.5,9.68,1.51,1.51,0,0,1,1,8.58L9,.92A1.5,1.5,0,0,1,10,.5Z",
          prevBtnSize: 18,
          nextBtnSize: 18,
          color: "#A4B1D7",
          borderColor: "#A4B1D7",
          borderWidth: 1
        },
        emphasis: {
          label: { show: !0, color: "#6f778d" },
          itemStyle: { color: "#316BF3" },
          controlStyle: {
            color: "#316BF3",
            borderColor: "#316BF3",
            borderWidth: 2
          }
        },
        progress: {
          lineStyle: { color: "#316BF3" },
          itemStyle: { color: "#316BF3" },
          label: { color: "#6f778d" }
        },
        data: []
      })),
      e
    );
  })(Cl);
te(Jv, Ti.prototype);
var w1 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (e.type = "timeline"), e;
  })(ee),
  A1 = (function (a) {
    k(e, a);
    function e(t, r, n, i) {
      var o = a.call(this, t, r, n) || this;
      return (o.type = i || "value"), o;
    }
    return (
      (e.prototype.getLabelModel = function () {
        return this.model.getModel("label");
      }),
      (e.prototype.isHorizontal = function () {
        return this.model.get("orient") === "horizontal";
      }),
      e
    );
  })(oe),
  Cn = Math.PI,
  Pl = _t(),
  T1 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.init = function (t, r) {
        this.api = r;
      }),
      (e.prototype.render = function (t, r, n) {
        if (
          ((this.model = t),
          (this.api = n),
          (this.ecModel = r),
          this.group.removeAll(),
          t.get("show", !0))
        ) {
          var i = this._layout(t, n),
            o = this._createGroup("_mainGroup"),
            s = this._createGroup("_labelGroup"),
            l = (this._axis = this._createAxis(i, t));
          (t.formatTooltip = function (u) {
            var v = l.scale.getLabel({ value: u });
            return Mt("nameValue", { noName: !0, value: v });
          }),
            L(
              ["AxisLine", "AxisTick", "Control", "CurrentPointer"],
              function (u) {
                this["_render" + u](i, o, l, t);
              },
              this
            ),
            this._renderAxisLabel(i, s, l, t),
            this._position(i, t);
        }
        this._doPlayStop(), this._updateTicksStatus();
      }),
      (e.prototype.remove = function () {
        this._clearTimer(), this.group.removeAll();
      }),
      (e.prototype.dispose = function () {
        this._clearTimer();
      }),
      (e.prototype._layout = function (t, r) {
        var n = t.get(["label", "position"]),
          i = t.get("orient"),
          o = D1(t, r),
          s;
        n == null || n === "auto"
          ? (s =
              i === "horizontal"
                ? o.y + o.height / 2 < r.getHeight() / 2
                  ? "-"
                  : "+"
                : o.x + o.width / 2 < r.getWidth() / 2
                ? "+"
                : "-")
          : et(n)
          ? (s = {
              horizontal: { top: "-", bottom: "+" },
              vertical: { left: "-", right: "+" }
            }[i][n])
          : (s = n);
        var l = {
            horizontal: "center",
            vertical: s >= 0 || s === "+" ? "left" : "right"
          },
          u = {
            horizontal: s >= 0 || s === "+" ? "top" : "bottom",
            vertical: "middle"
          },
          v = { horizontal: 0, vertical: Cn / 2 },
          c = i === "vertical" ? o.height : o.width,
          h = t.getModel("controlStyle"),
          f = h.get("show", !0),
          p = f ? h.get("itemSize") : 0,
          d = f ? h.get("itemGap") : 0,
          g = p + d,
          y = t.get(["label", "rotate"]) || 0;
        y = (y * Cn) / 180;
        var S,
          m,
          x,
          b = h.get("position", !0),
          _ = f && h.get("showPlayBtn", !0),
          w = f && h.get("showPrevBtn", !0),
          A = f && h.get("showNextBtn", !0),
          I = 0,
          T = c;
        b === "left" || b === "bottom"
          ? (_ && ((S = [0, 0]), (I += g)),
            w && ((m = [I, 0]), (I += g)),
            A && ((x = [T - p, 0]), (T -= g)))
          : (_ && ((S = [T - p, 0]), (T -= g)),
            w && ((m = [0, 0]), (I += g)),
            A && ((x = [T - p, 0]), (T -= g)));
        var D = [I, T];
        return (
          t.get("inverse") && D.reverse(),
          {
            viewRect: o,
            mainLength: c,
            orient: i,
            rotation: v[i],
            labelRotation: y,
            labelPosOpt: s,
            labelAlign: t.get(["label", "align"]) || l[i],
            labelBaseline:
              t.get(["label", "verticalAlign"]) ||
              t.get(["label", "baseline"]) ||
              u[i],
            playPosition: S,
            prevBtnPosition: m,
            nextBtnPosition: x,
            axisExtent: D,
            controlSize: p,
            controlGap: d
          }
        );
      }),
      (e.prototype._position = function (t, r) {
        var n = this._mainGroup,
          i = this._labelGroup,
          o = t.viewRect;
        if (t.orient === "vertical") {
          var s = tr(),
            l = o.x,
            u = o.y + o.height;
          ke(s, s, [-l, -u]),
            La(s, s, -Cn / 2),
            ke(s, s, [l, u]),
            (o = o.clone()),
            o.applyTransform(s);
        }
        var v = S(o),
          c = S(n.getBoundingRect()),
          h = S(i.getBoundingRect()),
          f = [n.x, n.y],
          p = [i.x, i.y];
        p[0] = f[0] = v[0][0];
        var d = t.labelPosOpt;
        if (d == null || et(d)) {
          var g = d === "+" ? 0 : 1;
          m(f, c, v, 1, g), m(p, h, v, 1, 1 - g);
        } else {
          var g = d >= 0 ? 0 : 1;
          m(f, c, v, 1, g), (p[1] = f[1] + d);
        }
        n.setPosition(f),
          i.setPosition(p),
          (n.rotation = i.rotation = t.rotation),
          y(n),
          y(i);
        function y(x) {
          (x.originX = v[0][0] - x.x), (x.originY = v[1][0] - x.y);
        }
        function S(x) {
          return [
            [x.x, x.x + x.width],
            [x.y, x.y + x.height]
          ];
        }
        function m(x, b, _, w, A) {
          x[w] += _[w][A] - b[w][A];
        }
      }),
      (e.prototype._createAxis = function (t, r) {
        var n = r.getData(),
          i = r.get("axisType"),
          o = I1(r, i);
        o.getTicks = function () {
          return n.mapArray(["value"], function (u) {
            return { value: u };
          });
        };
        var s = n.getDataExtent("value");
        o.setExtent(s[0], s[1]), o.calcNiceTicks();
        var l = new A1("value", o, t.axisExtent, i);
        return (l.model = r), l;
      }),
      (e.prototype._createGroup = function (t) {
        var r = (this[t] = new Z());
        return this.group.add(r), r;
      }),
      (e.prototype._renderAxisLine = function (t, r, n, i) {
        var o = n.getExtent();
        if (i.get(["lineStyle", "show"])) {
          var s = new jt({
            shape: { x1: o[0], y1: 0, x2: o[1], y2: 0 },
            style: W(
              { lineCap: "round" },
              i.getModel("lineStyle").getLineStyle()
            ),
            silent: !0,
            z2: 1
          });
          r.add(s);
          var l = (this._progressLine = new jt({
            shape: {
              x1: o[0],
              x2: this._currentPointer ? this._currentPointer.x : o[0],
              y1: 0,
              y2: 0
            },
            style: Q(
              { lineCap: "round", lineWidth: s.style.lineWidth },
              i.getModel(["progress", "lineStyle"]).getLineStyle()
            ),
            silent: !0,
            z2: 1
          }));
          r.add(l);
        }
      }),
      (e.prototype._renderAxisTick = function (t, r, n, i) {
        var o = this,
          s = i.getData(),
          l = n.scale.getTicks();
        (this._tickSymbols = []),
          L(l, function (u) {
            var v = n.dataToCoord(u.value),
              c = s.getItemModel(u.value),
              h = c.getModel("itemStyle"),
              f = c.getModel(["emphasis", "itemStyle"]),
              p = c.getModel(["progress", "itemStyle"]),
              d = { x: v, y: 0, onclick: at(o._changeTimeline, o, u.value) },
              g = Ml(c, h, r, d);
            (g.ensureState("emphasis").style = f.getItemStyle()),
              (g.ensureState("progress").style = p.getItemStyle()),
              Fa(g);
            var y = nt(g);
            c.get("tooltip")
              ? ((y.dataIndex = u.value), (y.dataModel = i))
              : (y.dataIndex = y.dataModel = null),
              o._tickSymbols.push(g);
          });
      }),
      (e.prototype._renderAxisLabel = function (t, r, n, i) {
        var o = this,
          s = n.getLabelModel();
        if (s.get("show")) {
          var l = i.getData(),
            u = n.getViewLabels();
          (this._tickLabels = []),
            L(u, function (v) {
              var c = v.tickValue,
                h = l.getItemModel(c),
                f = h.getModel("label"),
                p = h.getModel(["emphasis", "label"]),
                d = h.getModel(["progress", "label"]),
                g = n.dataToCoord(v.tickValue),
                y = new kt({
                  x: g,
                  y: 0,
                  rotation: t.labelRotation - t.rotation,
                  onclick: at(o._changeTimeline, o, c),
                  silent: !1,
                  style: xt(f, {
                    text: v.formattedLabel,
                    align: t.labelAlign,
                    verticalAlign: t.labelBaseline
                  })
                });
              (y.ensureState("emphasis").style = xt(p)),
                (y.ensureState("progress").style = xt(d)),
                r.add(y),
                Fa(y),
                (Pl(y).dataIndex = c),
                o._tickLabels.push(y);
            });
        }
      }),
      (e.prototype._renderControl = function (t, r, n, i) {
        var o = t.controlSize,
          s = t.rotation,
          l = i.getModel("controlStyle").getItemStyle(),
          u = i.getModel(["emphasis", "controlStyle"]).getItemStyle(),
          v = i.getPlayState(),
          c = i.get("inverse", !0);
        h(
          t.nextBtnPosition,
          "next",
          at(this._changeTimeline, this, c ? "-" : "+")
        ),
          h(
            t.prevBtnPosition,
            "prev",
            at(this._changeTimeline, this, c ? "+" : "-")
          ),
          h(
            t.playPosition,
            v ? "stop" : "play",
            at(this._handlePlayClick, this, !v),
            !0
          );
        function h(f, p, d, g) {
          if (f) {
            var y = hf(St(i.get(["controlStyle", p + "BtnSize"]), o), o),
              S = [0, -y / 2, y, y],
              m = L1(i, p + "Icon", S, {
                x: f[0],
                y: f[1],
                originX: o / 2,
                originY: 0,
                rotation: g ? -s : 0,
                rectHover: !0,
                style: l,
                onclick: d
              });
            (m.ensureState("emphasis").style = u), r.add(m), Fa(m);
          }
        }
      }),
      (e.prototype._renderCurrentPointer = function (t, r, n, i) {
        var o = i.getData(),
          s = i.getCurrentIndex(),
          l = o.getItemModel(s).getModel("checkpointStyle"),
          u = this,
          v = {
            onCreate: function (c) {
              (c.draggable = !0),
                (c.drift = at(u._handlePointerDrag, u)),
                (c.ondragend = at(u._handlePointerDragend, u)),
                El(c, u._progressLine, s, n, i, !0);
            },
            onUpdate: function (c) {
              El(c, u._progressLine, s, n, i);
            }
          };
        this._currentPointer = Ml(
          l,
          l,
          this._mainGroup,
          {},
          this._currentPointer,
          v
        );
      }),
      (e.prototype._handlePlayClick = function (t) {
        this._clearTimer(),
          this.api.dispatchAction({
            type: "timelinePlayChange",
            playState: t,
            from: this.uid
          });
      }),
      (e.prototype._handlePointerDrag = function (t, r, n) {
        this._clearTimer(), this._pointerChangeTimeline([n.offsetX, n.offsetY]);
      }),
      (e.prototype._handlePointerDragend = function (t) {
        this._pointerChangeTimeline([t.offsetX, t.offsetY], !0);
      }),
      (e.prototype._pointerChangeTimeline = function (t, r) {
        var n = this._toAxisCoord(t)[0],
          i = this._axis,
          o = Sr(i.getExtent().slice());
        n > o[1] && (n = o[1]),
          n < o[0] && (n = o[0]),
          (this._currentPointer.x = n),
          this._currentPointer.markRedraw();
        var s = this._progressLine;
        s && ((s.shape.x2 = n), s.dirty());
        var l = this._findNearestTick(n),
          u = this.model;
        (r || (l !== u.getCurrentIndex() && u.get("realtime"))) &&
          this._changeTimeline(l);
      }),
      (e.prototype._doPlayStop = function () {
        var t = this;
        this._clearTimer(),
          this.model.getPlayState() &&
            (this._timer = setTimeout(function () {
              var r = t.model;
              t._changeTimeline(
                r.getCurrentIndex() + (r.get("rewind", !0) ? -1 : 1)
              );
            }, this.model.get("playInterval")));
      }),
      (e.prototype._toAxisCoord = function (t) {
        var r = this._mainGroup.getLocalTransform();
        return xi(t, r, !0);
      }),
      (e.prototype._findNearestTick = function (t) {
        var r = this.model.getData(),
          n = 1 / 0,
          i,
          o = this._axis;
        return (
          r.each(["value"], function (s, l) {
            var u = o.dataToCoord(s),
              v = Math.abs(u - t);
            v < n && ((n = v), (i = l));
          }),
          i
        );
      }),
      (e.prototype._clearTimer = function () {
        this._timer && (clearTimeout(this._timer), (this._timer = null));
      }),
      (e.prototype._changeTimeline = function (t) {
        var r = this.model.getCurrentIndex();
        t === "+" ? (t = r + 1) : t === "-" && (t = r - 1),
          this.api.dispatchAction({
            type: "timelineChange",
            currentIndex: t,
            from: this.uid
          });
      }),
      (e.prototype._updateTicksStatus = function () {
        var t = this.model.getCurrentIndex(),
          r = this._tickSymbols,
          n = this._tickLabels;
        if (r)
          for (var i = 0; i < r.length; i++)
            r && r[i] && r[i].toggleState("progress", i < t);
        if (n)
          for (var i = 0; i < n.length; i++)
            n && n[i] && n[i].toggleState("progress", Pl(n[i]).dataIndex <= t);
      }),
      (e.type = "timeline.slider"),
      e
    );
  })(w1);
function I1(a, e) {
  if (((e = e || a.get("type")), e))
    switch (e) {
      case "category":
        return new pf({
          ordinalMeta: a.getCategories(),
          extent: [1 / 0, -1 / 0]
        });
      case "time":
        return new ff({
          locale: a.ecModel.getLocaleModel(),
          useUTC: a.ecModel.get("useUTC")
        });
      default:
        return new Vn();
    }
}
function D1(a, e) {
  return se(
    a.getBoxLayoutParams(),
    { width: e.getWidth(), height: e.getHeight() },
    a.get("padding")
  );
}
function L1(a, e, t, r) {
  var n = r.style,
    i = df(a.get(["controlStyle", e]), r || {}, new ft(t[0], t[1], t[2], t[3]));
  return n && i.setStyle(n), i;
}
function Ml(a, e, t, r, n, i) {
  var o = e.get("color");
  if (n) n.setColor(o), t.add(n), i && i.onUpdate(n);
  else {
    var s = a.get("symbol");
    (n = ie(s, -1, -1, 2, 2, o)),
      n.setStyle("strokeNoScale", !0),
      t.add(n),
      i && i.onCreate(n);
  }
  var l = e.getItemStyle(["color"]);
  n.setStyle(l), (r = Yt({ rectHover: !0, z2: 100 }, r, !0));
  var u = Dr(a.get("symbolSize"));
  (r.scaleX = u[0] / 2), (r.scaleY = u[1] / 2);
  var v = Nr(a.get("symbolOffset"), u);
  v && ((r.x = (r.x || 0) + v[0]), (r.y = (r.y || 0) + v[1]));
  var c = a.get("symbolRotate");
  return (
    (r.rotation = ((c || 0) * Math.PI) / 180 || 0),
    n.attr(r),
    n.updateTransform(),
    n
  );
}
function El(a, e, t, r, n, i) {
  if (!a.dragging) {
    var o = n.getModel("checkpointStyle"),
      s = r.dataToCoord(n.getData().get("value", t));
    if (i || !o.get("animation", !0))
      a.attr({ x: s, y: 0 }), e && e.attr({ shape: { x2: s } });
    else {
      var l = {
        duration: o.get("animationDuration", !0),
        easing: o.get("animationEasing", !0)
      };
      a.stopAnimation(null, !0),
        a.animateTo({ x: s, y: 0 }, l),
        e && e.animateTo({ shape: { x2: s } }, l);
    }
  }
}
function C1(a) {
  a.registerAction(
    {
      type: "timelineChange",
      event: "timelineChanged",
      update: "prepareAndUpdate"
    },
    function (e, t, r) {
      var n = t.getComponent("timeline");
      return (
        n &&
          e.currentIndex != null &&
          (n.setCurrentIndex(e.currentIndex),
          !n.get("loop", !0) &&
            n.isIndexMax() &&
            n.getPlayState() &&
            (n.setPlayState(!1),
            r.dispatchAction({
              type: "timelinePlayChange",
              playState: !1,
              from: e.from
            }))),
        t.resetOption("timeline", { replaceMerge: n.get("replaceMerge", !0) }),
        Q({ currentIndex: n.option.currentIndex }, e)
      );
    }
  ),
    a.registerAction(
      {
        type: "timelinePlayChange",
        event: "timelinePlayChanged",
        update: "update"
      },
      function (e, t) {
        var r = t.getComponent("timeline");
        r && e.playState != null && r.setPlayState(e.playState);
      }
    );
}
function P1(a) {
  var e = a && a.timeline;
  H(e) || (e = e ? [e] : []),
    L(e, function (t) {
      t && M1(t);
    });
}
function M1(a) {
  var e = a.type,
    t = { number: "value", time: "time" };
  if (
    (t[e] && ((a.axisType = t[e]), delete a.type),
    Rl(a),
    Re(a, "controlPosition"))
  ) {
    var r = a.controlStyle || (a.controlStyle = {});
    Re(r, "position") || (r.position = a.controlPosition),
      r.position === "none" &&
        !Re(r, "show") &&
        ((r.show = !1), delete r.position),
      delete a.controlPosition;
  }
  L(a.data || [], function (n) {
    Ge(n) &&
      !H(n) &&
      (!Re(n, "value") && Re(n, "name") && (n.value = n.name), Rl(n));
  });
}
function Rl(a) {
  var e = a.itemStyle || (a.itemStyle = {}),
    t = e.emphasis || (e.emphasis = {}),
    r = a.label || a.label || {},
    n = r.normal || (r.normal = {}),
    i = { normal: 1, emphasis: 1 };
  L(r, function (o, s) {
    !i[s] && !Re(n, s) && (n[s] = o);
  }),
    t.label && !Re(r, "emphasis") && ((r.emphasis = t.label), delete t.label);
}
function Re(a, e) {
  return a.hasOwnProperty(e);
}
function E1(a) {
  a.registerComponentModel(Jv),
    a.registerComponentView(T1),
    a.registerSubTypeDefaulter("timeline", function () {
      return "slider";
    }),
    C1(a),
    a.registerPreprocessor(P1);
}
function oo(a, e) {
  if (!a) return !1;
  for (var t = H(a) ? a : [a], r = 0; r < t.length; r++)
    if (t[r] && t[r][e]) return !0;
  return !1;
}
function qr(a) {
  Pa(a, "label", ["show"]);
}
var Kr = _t(),
  he = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), (t.createdBySelf = !1), t;
    }
    return (
      (e.prototype.init = function (t, r, n) {
        this.mergeDefaultAndTheme(t, n), this._mergeOption(t, n, !1, !0);
      }),
      (e.prototype.isAnimationEnabled = function () {
        if (gf.node) return !1;
        var t = this.__hostSeries;
        return this.getShallow("animation") && t && t.isAnimationEnabled();
      }),
      (e.prototype.mergeOption = function (t, r) {
        this._mergeOption(t, r, !1, !1);
      }),
      (e.prototype._mergeOption = function (t, r, n, i) {
        var o = this.mainType;
        n ||
          r.eachSeries(function (s) {
            var l = s.get(this.mainType, !0),
              u = Kr(s)[o];
            if (!l || !l.data) {
              Kr(s)[o] = null;
              return;
            }
            u
              ? u._mergeOption(l, r, !0)
              : (i && qr(l),
                L(l.data, function (v) {
                  v instanceof Array ? (qr(v[0]), qr(v[1])) : qr(v);
                }),
                (u = this.createMarkerModelFromSeries(l, this, r)),
                W(u, {
                  mainType: this.mainType,
                  seriesIndex: s.seriesIndex,
                  name: s.name,
                  createdBySelf: !0
                }),
                (u.__hostSeries = s)),
              (Kr(s)[o] = u);
          }, this);
      }),
      (e.prototype.formatTooltip = function (t, r, n) {
        var i = this.getData(),
          o = this.getRawValue(t),
          s = i.getName(t);
        return Mt("section", {
          header: this.name,
          blocks: [
            Mt("nameValue", {
              name: s,
              value: o,
              noName: !s,
              noValue: o == null
            })
          ]
        });
      }),
      (e.prototype.getData = function () {
        return this._data;
      }),
      (e.prototype.setData = function (t) {
        this._data = t;
      }),
      (e.prototype.getDataParams = function (t, r) {
        var n = Ti.prototype.getDataParams.call(this, t, r),
          i = this.__hostSeries;
        return (
          i &&
            ((n.seriesId = i.id),
            (n.seriesName = i.name),
            (n.seriesType = i.subType)),
          n
        );
      }),
      (e.getMarkerModelFromSeries = function (t, r) {
        return Kr(t)[r];
      }),
      (e.type = "marker"),
      (e.dependencies = ["series", "grid", "polar", "geo"]),
      e
    );
  })(Xt);
te(he, Ti.prototype);
var R1 = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), t;
  }
  return (
    (e.prototype.createMarkerModelFromSeries = function (t, r, n) {
      return new e(t, r, n);
    }),
    (e.type = "markPoint"),
    (e.defaultOption = {
      z: 5,
      symbol: "pin",
      symbolSize: 50,
      tooltip: { trigger: "item" },
      label: { show: !0, position: "inside" },
      itemStyle: { borderWidth: 2 },
      emphasis: { label: { show: !0 } }
    }),
    e
  );
})(he);
function si(a) {
  return !(isNaN(parseFloat(a.x)) && isNaN(parseFloat(a.y)));
}
function N1(a) {
  return !isNaN(parseFloat(a.x)) && !isNaN(parseFloat(a.y));
}
function jr(a, e, t, r, n, i) {
  var o = [],
    s = Eu(e, r),
    l = s ? e.getCalculationInfo("stackResultDimension") : r,
    u = so(e, l, a),
    v = e.indicesOfNearest(l, u)[0];
  (o[n] = e.get(t, v)), (o[i] = e.get(l, v));
  var c = e.get(r, v),
    h = yf(e.get(r, v));
  return (h = Math.min(h, 20)), h >= 0 && (o[i] = +o[i].toFixed(h)), [o, c];
}
var Pn = {
  min: ot(jr, "min"),
  max: ot(jr, "max"),
  average: ot(jr, "average"),
  median: ot(jr, "median")
};
function Tr(a, e) {
  if (e) {
    var t = a.getData(),
      r = a.coordinateSystem,
      n = r && r.dimensions;
    if (!N1(e) && !H(e.coord) && H(n)) {
      var i = Qv(e, t, r, a);
      if (((e = Gt(e)), e.type && Pn[e.type] && i.baseAxis && i.valueAxis)) {
        var o = Ht(n, i.baseAxis.dim),
          s = Ht(n, i.valueAxis.dim),
          l = Pn[e.type](t, i.baseDataDim, i.valueDataDim, o, s);
        (e.coord = l[0]), (e.value = l[1]);
      } else
        e.coord = [
          e.xAxis != null ? e.xAxis : e.radiusAxis,
          e.yAxis != null ? e.yAxis : e.angleAxis
        ];
    }
    if (e.coord == null || !H(n)) e.coord = [];
    else
      for (var u = e.coord, v = 0; v < 2; v++)
        Pn[u[v]] && (u[v] = so(t, t.mapDimension(n[v]), u[v]));
    return e;
  }
}
function Qv(a, e, t, r) {
  var n = {};
  return (
    a.valueIndex != null || a.valueDim != null
      ? ((n.valueDataDim =
          a.valueIndex != null ? e.getDimension(a.valueIndex) : a.valueDim),
        (n.valueAxis = t.getAxis(k1(r, n.valueDataDim))),
        (n.baseAxis = t.getOtherAxis(n.valueAxis)),
        (n.baseDataDim = e.mapDimension(n.baseAxis.dim)))
      : ((n.baseAxis = r.getBaseAxis()),
        (n.valueAxis = t.getOtherAxis(n.baseAxis)),
        (n.baseDataDim = e.mapDimension(n.baseAxis.dim)),
        (n.valueDataDim = e.mapDimension(n.valueAxis.dim))),
    n
  );
}
function k1(a, e) {
  var t = a.getData().getDimensionInfo(e);
  return t && t.coordDim;
}
function Ir(a, e) {
  return a && a.containData && e.coord && !si(e) ? a.containData(e.coord) : !0;
}
function V1(a, e, t) {
  return a && a.containZone && e.coord && t.coord && !si(e) && !si(t)
    ? a.containZone(e.coord, t.coord)
    : !0;
}
function tc(a, e) {
  return a
    ? function (t, r, n, i) {
        var o = i < 2 ? t.coord && t.coord[i] : t.value;
        return oa(o, e[i]);
      }
    : function (t, r, n, i) {
        return oa(t.value, e[i]);
      };
}
function so(a, e, t) {
  if (t === "average") {
    var r = 0,
      n = 0;
    return (
      a.each(e, function (i, o) {
        isNaN(i) || ((r += i), n++);
      }),
      r / n
    );
  } else
    return t === "median"
      ? a.getMedian(e)
      : a.getDataExtent(e)[t === "max" ? 1 : 0];
}
var Mn = _t(),
  lo = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.init = function () {
        this.markerGroupMap = tt();
      }),
      (e.prototype.render = function (t, r, n) {
        var i = this,
          o = this.markerGroupMap;
        o.each(function (s) {
          Mn(s).keep = !1;
        }),
          r.eachSeries(function (s) {
            var l = he.getMarkerModelFromSeries(s, i.type);
            l && i.renderSeries(s, l, r, n);
          }),
          o.each(function (s) {
            !Mn(s).keep && i.group.remove(s.group);
          });
      }),
      (e.prototype.markKeep = function (t) {
        Mn(t).keep = !0;
      }),
      (e.prototype.toggleBlurSeries = function (t, r) {
        var n = this;
        L(t, function (i) {
          var o = he.getMarkerModelFromSeries(i, n.type);
          if (o) {
            var s = o.getData();
            s.eachItemGraphicEl(function (l) {
              l && (r ? mf(l) : Sf(l));
            });
          }
        });
      }),
      (e.type = "marker"),
      e
    );
  })(ee);
function Nl(a, e, t) {
  var r = e.coordinateSystem;
  a.each(function (n) {
    var i = a.getItemModel(n),
      o,
      s = z(i.get("x"), t.getWidth()),
      l = z(i.get("y"), t.getHeight());
    if (!isNaN(s) && !isNaN(l)) o = [s, l];
    else if (e.getMarkerPosition)
      o = e.getMarkerPosition(a.getValues(a.dimensions, n));
    else if (r) {
      var u = a.get(r.dimensions[0], n),
        v = a.get(r.dimensions[1], n);
      o = r.dataToPoint([u, v]);
    }
    isNaN(s) || (o[0] = s), isNaN(l) || (o[1] = l), a.setItemLayout(n, o);
  });
}
var G1 = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), t;
  }
  return (
    (e.prototype.updateTransform = function (t, r, n) {
      r.eachSeries(function (i) {
        var o = he.getMarkerModelFromSeries(i, "markPoint");
        o &&
          (Nl(o.getData(), i, n), this.markerGroupMap.get(i.id).updateLayout());
      }, this);
    }),
    (e.prototype.renderSeries = function (t, r, n, i) {
      var o = t.coordinateSystem,
        s = t.id,
        l = t.getData(),
        u = this.markerGroupMap,
        v = u.get(s) || u.set(s, new Aa()),
        c = z1(o, t, r);
      r.setData(c),
        Nl(r.getData(), t, i),
        c.each(function (h) {
          var f = c.getItemModel(h),
            p = f.getShallow("symbol"),
            d = f.getShallow("symbolSize"),
            g = f.getShallow("symbolRotate"),
            y = f.getShallow("symbolOffset"),
            S = f.getShallow("symbolKeepAspect");
          if (it(p) || it(d) || it(g) || it(y)) {
            var m = r.getRawValue(h),
              x = r.getDataParams(h);
            it(p) && (p = p(m, x)),
              it(d) && (d = d(m, x)),
              it(g) && (g = g(m, x)),
              it(y) && (y = y(m, x));
          }
          var b = f.getModel("itemStyle").getItemStyle(),
            _ = Ii(l, "color");
          b.fill || (b.fill = _),
            c.setItemVisual(h, {
              symbol: p,
              symbolSize: d,
              symbolRotate: g,
              symbolOffset: y,
              symbolKeepAspect: S,
              style: b
            });
        }),
        v.updateData(c),
        this.group.add(v.group),
        c.eachItemGraphicEl(function (h) {
          h.traverse(function (f) {
            nt(f).dataModel = r;
          });
        }),
        this.markKeep(v),
        (v.group.silent = r.get("silent") || t.get("silent"));
    }),
    (e.type = "markPoint"),
    e
  );
})(lo);
function z1(a, e, t) {
  var r;
  a
    ? (r = O(a && a.dimensions, function (s) {
        var l = e.getData().getDimensionInfo(e.getData().mapDimension(s)) || {};
        return W(W({}, l), { name: s, ordinalMeta: null });
      }))
    : (r = [{ name: "value", type: "float" }]);
  var n = new Vt(r, t),
    i = O(t.get("data"), ot(Tr, e));
  a && (i = Dt(i, ot(Ir, a)));
  var o = tc(!!a, r);
  return n.initData(i, null, o), n;
}
function O1(a) {
  a.registerComponentModel(R1),
    a.registerComponentView(G1),
    a.registerPreprocessor(function (e) {
      oo(e.series, "markPoint") && (e.markPoint = e.markPoint || {});
    });
}
var B1 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.createMarkerModelFromSeries = function (t, r, n) {
        return new e(t, r, n);
      }),
      (e.type = "markLine"),
      (e.defaultOption = {
        z: 5,
        symbol: ["circle", "arrow"],
        symbolSize: [8, 16],
        symbolOffset: 0,
        precision: 2,
        tooltip: { trigger: "item" },
        label: { show: !0, position: "end", distance: 5 },
        lineStyle: { type: "dashed" },
        emphasis: { label: { show: !0 }, lineStyle: { width: 3 } },
        animationEasing: "linear"
      }),
      e
    );
  })(he),
  Jr = _t(),
  F1 = function (a, e, t, r) {
    var n = a.getData(),
      i;
    if (H(r)) i = r;
    else {
      var o = r.type;
      if (
        o === "min" ||
        o === "max" ||
        o === "average" ||
        o === "median" ||
        r.xAxis != null ||
        r.yAxis != null
      ) {
        var s = void 0,
          l = void 0;
        if (r.yAxis != null || r.xAxis != null)
          (s = e.getAxis(r.yAxis != null ? "y" : "x")),
            (l = Ft(r.yAxis, r.xAxis));
        else {
          var u = Qv(r, n, e, a);
          s = u.valueAxis;
          var v = xf(n, u.valueDataDim);
          l = so(n, v, o);
        }
        var c = s.dim === "x" ? 0 : 1,
          h = 1 - c,
          f = Gt(r),
          p = { coord: [] };
        (f.type = null),
          (f.coord = []),
          (f.coord[h] = -1 / 0),
          (p.coord[h] = 1 / 0);
        var d = t.get("precision");
        d >= 0 && Qt(l) && (l = +l.toFixed(Math.min(d, 20))),
          (f.coord[c] = p.coord[c] = l),
          (i = [f, p, { type: o, valueIndex: r.valueIndex, value: l }]);
      } else i = [];
    }
    var g = [Tr(a, i[0]), Tr(a, i[1]), W({}, i[2])];
    return (g[2].type = g[2].type || null), Yt(g[2], g[0]), Yt(g[2], g[1]), g;
  };
function ga(a) {
  return !isNaN(a) && !isFinite(a);
}
function kl(a, e, t, r) {
  var n = 1 - a,
    i = r.dimensions[a];
  return (
    ga(e[n]) && ga(t[n]) && e[a] === t[a] && r.getAxis(i).containData(e[a])
  );
}
function W1(a, e) {
  if (a.type === "cartesian2d") {
    var t = e[0].coord,
      r = e[1].coord;
    if (t && r && (kl(1, t, r, a) || kl(0, t, r, a))) return !0;
  }
  return Ir(a, e[0]) && Ir(a, e[1]);
}
function En(a, e, t, r, n) {
  var i = r.coordinateSystem,
    o = a.getItemModel(e),
    s,
    l = z(o.get("x"), n.getWidth()),
    u = z(o.get("y"), n.getHeight());
  if (!isNaN(l) && !isNaN(u)) s = [l, u];
  else {
    if (r.getMarkerPosition)
      s = r.getMarkerPosition(a.getValues(a.dimensions, e));
    else {
      var v = i.dimensions,
        c = a.get(v[0], e),
        h = a.get(v[1], e);
      s = i.dataToPoint([c, h]);
    }
    if (Na(i, "cartesian2d")) {
      var f = i.getAxis("x"),
        p = i.getAxis("y"),
        v = i.dimensions;
      ga(a.get(v[0], e))
        ? (s[0] = f.toGlobalCoord(f.getExtent()[t ? 0 : 1]))
        : ga(a.get(v[1], e)) &&
          (s[1] = p.toGlobalCoord(p.getExtent()[t ? 0 : 1]));
    }
    isNaN(l) || (s[0] = l), isNaN(u) || (s[1] = u);
  }
  a.setItemLayout(e, s);
}
var H1 = (function (a) {
  k(e, a);
  function e() {
    var t = (a !== null && a.apply(this, arguments)) || this;
    return (t.type = e.type), t;
  }
  return (
    (e.prototype.updateTransform = function (t, r, n) {
      r.eachSeries(function (i) {
        var o = he.getMarkerModelFromSeries(i, "markLine");
        if (o) {
          var s = o.getData(),
            l = Jr(o).from,
            u = Jr(o).to;
          l.each(function (v) {
            En(l, v, !0, i, n), En(u, v, !1, i, n);
          }),
            s.each(function (v) {
              s.setItemLayout(v, [l.getItemLayout(v), u.getItemLayout(v)]);
            }),
            this.markerGroupMap.get(i.id).updateLayout();
        }
      }, this);
    }),
    (e.prototype.renderSeries = function (t, r, n, i) {
      var o = t.coordinateSystem,
        s = t.id,
        l = t.getData(),
        u = this.markerGroupMap,
        v = u.get(s) || u.set(s, new Xi());
      this.group.add(v.group);
      var c = U1(o, t, r),
        h = c.from,
        f = c.to,
        p = c.line;
      (Jr(r).from = h), (Jr(r).to = f), r.setData(p);
      var d = r.get("symbol"),
        g = r.get("symbolSize"),
        y = r.get("symbolRotate"),
        S = r.get("symbolOffset");
      H(d) || (d = [d, d]),
        H(g) || (g = [g, g]),
        H(y) || (y = [y, y]),
        H(S) || (S = [S, S]),
        c.from.each(function (x) {
          m(h, x, !0), m(f, x, !1);
        }),
        p.each(function (x) {
          var b = p.getItemModel(x).getModel("lineStyle").getLineStyle();
          p.setItemLayout(x, [h.getItemLayout(x), f.getItemLayout(x)]),
            b.stroke == null && (b.stroke = h.getItemVisual(x, "style").fill),
            p.setItemVisual(x, {
              fromSymbolKeepAspect: h.getItemVisual(x, "symbolKeepAspect"),
              fromSymbolOffset: h.getItemVisual(x, "symbolOffset"),
              fromSymbolRotate: h.getItemVisual(x, "symbolRotate"),
              fromSymbolSize: h.getItemVisual(x, "symbolSize"),
              fromSymbol: h.getItemVisual(x, "symbol"),
              toSymbolKeepAspect: f.getItemVisual(x, "symbolKeepAspect"),
              toSymbolOffset: f.getItemVisual(x, "symbolOffset"),
              toSymbolRotate: f.getItemVisual(x, "symbolRotate"),
              toSymbolSize: f.getItemVisual(x, "symbolSize"),
              toSymbol: f.getItemVisual(x, "symbol"),
              style: b
            });
        }),
        v.updateData(p),
        c.line.eachItemGraphicEl(function (x) {
          (nt(x).dataModel = r),
            x.traverse(function (b) {
              nt(b).dataModel = r;
            });
        });
      function m(x, b, _) {
        var w = x.getItemModel(b);
        En(x, b, _, t, i);
        var A = w.getModel("itemStyle").getItemStyle();
        A.fill == null && (A.fill = Ii(l, "color")),
          x.setItemVisual(b, {
            symbolKeepAspect: w.get("symbolKeepAspect"),
            symbolOffset: St(w.get("symbolOffset", !0), S[_ ? 0 : 1]),
            symbolRotate: St(w.get("symbolRotate", !0), y[_ ? 0 : 1]),
            symbolSize: St(w.get("symbolSize"), g[_ ? 0 : 1]),
            symbol: St(w.get("symbol", !0), d[_ ? 0 : 1]),
            style: A
          });
      }
      this.markKeep(v), (v.group.silent = r.get("silent") || t.get("silent"));
    }),
    (e.type = "markLine"),
    e
  );
})(lo);
function U1(a, e, t) {
  var r;
  a
    ? (r = O(a && a.dimensions, function (u) {
        var v = e.getData().getDimensionInfo(e.getData().mapDimension(u)) || {};
        return W(W({}, v), { name: u, ordinalMeta: null });
      }))
    : (r = [{ name: "value", type: "float" }]);
  var n = new Vt(r, t),
    i = new Vt(r, t),
    o = new Vt([], t),
    s = O(t.get("data"), ot(F1, e, a, t));
  a && (s = Dt(s, ot(W1, a)));
  var l = tc(!!a, r);
  return (
    n.initData(
      O(s, function (u) {
        return u[0];
      }),
      null,
      l
    ),
    i.initData(
      O(s, function (u) {
        return u[1];
      }),
      null,
      l
    ),
    o.initData(
      O(s, function (u) {
        return u[2];
      })
    ),
    (o.hasItemOption = !0),
    { from: n, to: i, line: o }
  );
}
function Y1(a) {
  a.registerComponentModel(B1),
    a.registerComponentView(H1),
    a.registerPreprocessor(function (e) {
      oo(e.series, "markLine") && (e.markLine = e.markLine || {});
    });
}
var $1 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.createMarkerModelFromSeries = function (t, r, n) {
        return new e(t, r, n);
      }),
      (e.type = "markArea"),
      (e.defaultOption = {
        z: 1,
        tooltip: { trigger: "item" },
        animation: !1,
        label: { show: !0, position: "top" },
        itemStyle: { borderWidth: 0 },
        emphasis: { label: { show: !0, position: "top" } }
      }),
      e
    );
  })(he),
  Qr = _t(),
  X1 = function (a, e, t, r) {
    var n = r[0],
      i = r[1];
    if (!(!n || !i)) {
      var o = Tr(a, n),
        s = Tr(a, i),
        l = o.coord,
        u = s.coord;
      (l[0] = Ft(l[0], -1 / 0)),
        (l[1] = Ft(l[1], -1 / 0)),
        (u[0] = Ft(u[0], 1 / 0)),
        (u[1] = Ft(u[1], 1 / 0));
      var v = pi([{}, o, s]);
      return (
        (v.coord = [o.coord, s.coord]),
        (v.x0 = o.x),
        (v.y0 = o.y),
        (v.x1 = s.x),
        (v.y1 = s.y),
        v
      );
    }
  };
function ya(a) {
  return !isNaN(a) && !isFinite(a);
}
function Vl(a, e, t, r) {
  var n = 1 - a;
  return ya(e[n]) && ya(t[n]);
}
function Z1(a, e) {
  var t = e.coord[0],
    r = e.coord[1],
    n = { coord: t, x: e.x0, y: e.y0 },
    i = { coord: r, x: e.x1, y: e.y1 };
  return Na(a, "cartesian2d")
    ? t && r && (Vl(1, t, r) || Vl(0, t, r))
      ? !0
      : V1(a, n, i)
    : Ir(a, n) || Ir(a, i);
}
function Gl(a, e, t, r, n) {
  var i = r.coordinateSystem,
    o = a.getItemModel(e),
    s,
    l = z(o.get(t[0]), n.getWidth()),
    u = z(o.get(t[1]), n.getHeight());
  if (!isNaN(l) && !isNaN(u)) s = [l, u];
  else {
    if (r.getMarkerPosition) {
      var v = a.getValues(["x0", "y0"], e),
        c = a.getValues(["x1", "y1"], e),
        h = i.clampData(v),
        f = i.clampData(c),
        p = [];
      t[0] === "x0"
        ? (p[0] = h[0] > f[0] ? c[0] : v[0])
        : (p[0] = h[0] > f[0] ? v[0] : c[0]),
        t[1] === "y0"
          ? (p[1] = h[1] > f[1] ? c[1] : v[1])
          : (p[1] = h[1] > f[1] ? v[1] : c[1]),
        (s = r.getMarkerPosition(p, t, !0));
    } else {
      var d = a.get(t[0], e),
        g = a.get(t[1], e),
        y = [d, g];
      i.clampData && i.clampData(y, y), (s = i.dataToPoint(y, !0));
    }
    if (Na(i, "cartesian2d")) {
      var S = i.getAxis("x"),
        m = i.getAxis("y"),
        d = a.get(t[0], e),
        g = a.get(t[1], e);
      ya(d)
        ? (s[0] = S.toGlobalCoord(S.getExtent()[t[0] === "x0" ? 0 : 1]))
        : ya(g) &&
          (s[1] = m.toGlobalCoord(m.getExtent()[t[1] === "y0" ? 0 : 1]));
    }
    isNaN(l) || (s[0] = l), isNaN(u) || (s[1] = u);
  }
  return s;
}
var zl = [
    ["x0", "y0"],
    ["x1", "y0"],
    ["x1", "y1"],
    ["x0", "y1"]
  ],
  q1 = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = e.type), t;
    }
    return (
      (e.prototype.updateTransform = function (t, r, n) {
        r.eachSeries(function (i) {
          var o = he.getMarkerModelFromSeries(i, "markArea");
          if (o) {
            var s = o.getData();
            s.each(function (l) {
              var u = O(zl, function (c) {
                return Gl(s, l, c, i, n);
              });
              s.setItemLayout(l, u);
              var v = s.getItemGraphicEl(l);
              v.setShape("points", u);
            });
          }
        }, this);
      }),
      (e.prototype.renderSeries = function (t, r, n, i) {
        var o = t.coordinateSystem,
          s = t.id,
          l = t.getData(),
          u = this.markerGroupMap,
          v = u.get(s) || u.set(s, { group: new Z() });
        this.group.add(v.group), this.markKeep(v);
        var c = K1(o, t, r);
        r.setData(c),
          c.each(function (h) {
            var f = O(zl, function (A) {
                return Gl(c, h, A, t, i);
              }),
              p = o.getAxis("x").scale,
              d = o.getAxis("y").scale,
              g = p.getExtent(),
              y = d.getExtent(),
              S = [p.parse(c.get("x0", h)), p.parse(c.get("x1", h))],
              m = [d.parse(c.get("y0", h)), d.parse(c.get("y1", h))];
            Sr(S), Sr(m);
            var x = !(g[0] > S[1] || g[1] < S[0] || y[0] > m[1] || y[1] < m[0]),
              b = !x;
            c.setItemLayout(h, { points: f, allClipped: b });
            var _ = c.getItemModel(h).getModel("itemStyle").getItemStyle(),
              w = Ii(l, "color");
            _.fill || ((_.fill = w), et(_.fill) && (_.fill = On(_.fill, 0.4))),
              _.stroke || (_.stroke = w),
              c.setItemVisual(h, "style", _);
          }),
          c
            .diff(Qr(v).data)
            .add(function (h) {
              var f = c.getItemLayout(h);
              if (!f.allClipped) {
                var p = new ve({ shape: { points: f.points } });
                c.setItemGraphicEl(h, p), v.group.add(p);
              }
            })
            .update(function (h, f) {
              var p = Qr(v).data.getItemGraphicEl(f),
                d = c.getItemLayout(h);
              d.allClipped
                ? p && v.group.remove(p)
                : (p
                    ? lt(p, { shape: { points: d.points } }, r, h)
                    : (p = new ve({ shape: { points: d.points } })),
                  c.setItemGraphicEl(h, p),
                  v.group.add(p));
            })
            .remove(function (h) {
              var f = Qr(v).data.getItemGraphicEl(h);
              v.group.remove(f);
            })
            .execute(),
          c.eachItemGraphicEl(function (h, f) {
            var p = c.getItemModel(f),
              d = c.getItemVisual(f, "style");
            h.useStyle(c.getItemVisual(f, "style")),
              Ut(h, Pt(p), {
                labelFetcher: r,
                labelDataIndex: f,
                defaultText: c.getName(f) || "",
                inheritColor: et(d.fill) ? On(d.fill, 1) : "#000"
              }),
              zt(h, p),
              ct(h, null, null, p.get(["emphasis", "disabled"])),
              (nt(h).dataModel = r);
          }),
          (Qr(v).data = c),
          (v.group.silent = r.get("silent") || t.get("silent"));
      }),
      (e.type = "markArea"),
      e
    );
  })(lo);
function K1(a, e, t) {
  var r,
    n,
    i = ["x0", "y0", "x1", "y1"];
  if (a) {
    var o = O(a && a.dimensions, function (u) {
      var v = e.getData(),
        c = v.getDimensionInfo(v.mapDimension(u)) || {};
      return W(W({}, c), { name: u, ordinalMeta: null });
    });
    (n = O(i, function (u, v) {
      return { name: u, type: o[v % 2].type };
    })),
      (r = new Vt(n, t));
  } else (n = [{ name: "value", type: "float" }]), (r = new Vt(n, t));
  var s = O(t.get("data"), ot(X1, e, a, t));
  a && (s = Dt(s, ot(Z1, a)));
  var l = a
    ? function (u, v, c, h) {
        var f = u.coord[Math.floor(h / 2)][h % 2];
        return oa(f, n[h]);
      }
    : function (u, v, c, h) {
        return oa(u.value, n[h]);
      };
  return r.initData(s, null, l), (r.hasItemOption = !0), r;
}
function j1(a) {
  a.registerComponentModel($1),
    a.registerComponentView(q1),
    a.registerPreprocessor(function (e) {
      oo(e.series, "markArea") && (e.markArea = e.markArea || {});
    });
}
var J1 = { label: { enabled: !0 }, decal: { show: !1 } },
  Ol = _t(),
  Q1 = {};
function tx(a, e) {
  var t = a.getModel("aria");
  if (!t.get("enabled")) return;
  var r = Gt(J1);
  Yt(r.label, a.getLocaleModel().get("aria"), !1),
    Yt(t.option, r, !1),
    n(),
    i();
  function n() {
    var u = t.getModel("decal"),
      v = u.get("show");
    if (v) {
      var c = tt();
      a.eachSeries(function (h) {
        if (!h.isColorBySeries()) {
          var f = c.get(h.type);
          f || ((f = {}), c.set(h.type, f)), (Ol(h).scope = f);
        }
      }),
        a.eachRawSeries(function (h) {
          if (a.isSeriesFiltered(h)) return;
          if (it(h.enableAriaDecal)) {
            h.enableAriaDecal();
            return;
          }
          var f = h.getData();
          if (h.isColorBySeries()) {
            var S = zn(h.ecModel, h.name, Q1, a.getSeriesCount()),
              m = f.getVisual("decal");
            f.setVisual("decal", x(m, S));
          } else {
            var p = h.getRawData(),
              d = {},
              g = Ol(h).scope;
            f.each(function (b) {
              var _ = f.getRawIndex(b);
              d[_] = b;
            });
            var y = p.count();
            p.each(function (b) {
              var _ = d[b],
                w = p.getName(b) || b + "",
                A = zn(h.ecModel, w, g, y),
                I = f.getItemVisual(_, "decal");
              f.setItemVisual(_, "decal", x(I, A));
            });
          }
          function x(b, _) {
            var w = b ? W(W({}, _), b) : _;
            return (w.dirty = !0), w;
          }
        });
    }
  }
  function i() {
    var u = e.getZr().dom;
    if (u) {
      var v = a.getLocaleModel().get("aria"),
        c = t.getModel("label");
      if (((c.option = Q(c.option, v)), !!c.get("enabled"))) {
        if ((u.setAttribute("role", "img"), c.get("description"))) {
          u.setAttribute("aria-label", c.get("description"));
          return;
        }
        var h = a.getSeriesCount(),
          f = c.get(["data", "maxCount"]) || 10,
          p = c.get(["series", "maxCount"]) || 10,
          d = Math.min(h, p),
          g;
        if (!(h < 1)) {
          var y = s();
          if (y) {
            var S = c.get(["general", "withTitle"]);
            g = o(S, { title: y });
          } else g = c.get(["general", "withoutTitle"]);
          var m = [],
            x =
              h > 1
                ? c.get(["series", "multiple", "prefix"])
                : c.get(["series", "single", "prefix"]);
          (g += o(x, { seriesCount: h })),
            a.eachSeries(function (A, I) {
              if (I < d) {
                var T = void 0,
                  D = A.get("name"),
                  M = D ? "withName" : "withoutName";
                (T =
                  h > 1
                    ? c.get(["series", "multiple", M])
                    : c.get(["series", "single", M])),
                  (T = o(T, {
                    seriesId: A.seriesIndex,
                    seriesName: A.get("name"),
                    seriesType: l(A.subType)
                  }));
                var C = A.getData();
                if (C.count() > f) {
                  var P = c.get(["data", "partialData"]);
                  T += o(P, { displayCnt: f });
                } else T += c.get(["data", "allData"]);
                for (
                  var E = c.get(["data", "separator", "middle"]),
                    R = c.get(["data", "separator", "end"]),
                    N = c.get(["data", "excludeDimensionId"]),
                    V = [],
                    G = 0;
                  G < C.count();
                  G++
                )
                  if (G < f) {
                    var B = C.getName(G),
                      F = N
                        ? Dt(C.getValues(G), function (K, $) {
                            return Ht(N, $) === -1;
                          })
                        : C.getValues(G),
                      Y = c.get(["data", B ? "withName" : "withoutName"]);
                    V.push(o(Y, { name: B, value: F.join(E) }));
                  }
                (T += V.join(E) + R), m.push(T);
              }
            });
          var b = c.getModel(["series", "multiple", "separator"]),
            _ = b.get("middle"),
            w = b.get("end");
          (g += m.join(_) + w), u.setAttribute("aria-label", g);
        }
      }
    }
  }
  function o(u, v) {
    if (!et(u)) return u;
    var c = u;
    return (
      L(v, function (h, f) {
        c = c.replace(new RegExp("\\{\\s*" + f + "\\s*\\}", "g"), h);
      }),
      c
    );
  }
  function s() {
    var u = a.get("title");
    return u && u.length && (u = u[0]), u && u.text;
  }
  function l(u) {
    var v = a.getLocaleModel().get(["series", "typeNames"]);
    return v[u] || v.chart;
  }
}
function ex(a) {
  if (!(!a || !a.aria)) {
    var e = a.aria;
    e.show != null && (e.enabled = e.show),
      (e.label = e.label || {}),
      L(["description", "general", "series", "data"], function (t) {
        e[t] != null && (e.label[t] = e[t]);
      });
  }
}
function rx(a) {
  a.registerPreprocessor(ex), a.registerVisual(a.PRIORITY.VISUAL.ARIA, tx);
}
var Bl = {
    value: "eq",
    "<": "lt",
    "<=": "lte",
    ">": "gt",
    ">=": "gte",
    "=": "eq",
    "!=": "ne",
    "<>": "ne"
  },
  ax = (function () {
    function a(e) {
      var t = (this._condVal = et(e) ? new RegExp(e) : wf(e) ? e : null);
      if (t == null) {
        var r = "";
        vt(r);
      }
    }
    return (
      (a.prototype.evaluate = function (e) {
        var t = typeof e;
        return et(t)
          ? this._condVal.test(e)
          : Qt(t)
          ? this._condVal.test(e + "")
          : !1;
      }),
      a
    );
  })(),
  nx = (function () {
    function a() {}
    return (
      (a.prototype.evaluate = function () {
        return this.value;
      }),
      a
    );
  })(),
  ix = (function () {
    function a() {}
    return (
      (a.prototype.evaluate = function () {
        for (var e = this.children, t = 0; t < e.length; t++)
          if (!e[t].evaluate()) return !1;
        return !0;
      }),
      a
    );
  })(),
  ox = (function () {
    function a() {}
    return (
      (a.prototype.evaluate = function () {
        for (var e = this.children, t = 0; t < e.length; t++)
          if (e[t].evaluate()) return !0;
        return !1;
      }),
      a
    );
  })(),
  sx = (function () {
    function a() {}
    return (
      (a.prototype.evaluate = function () {
        return !this.child.evaluate();
      }),
      a
    );
  })(),
  lx = (function () {
    function a() {}
    return (
      (a.prototype.evaluate = function () {
        for (
          var e = !!this.valueParser,
            t = this.getValue,
            r = t(this.valueGetterParam),
            n = e ? this.valueParser(r) : null,
            i = 0;
          i < this.subCondList.length;
          i++
        )
          if (!this.subCondList[i].evaluate(e ? n : r)) return !1;
        return !0;
      }),
      a
    );
  })();
function uo(a, e) {
  if (a === !0 || a === !1) {
    var t = new nx();
    return (t.value = a), t;
  }
  var r = "";
  return (
    ec(a) || vt(r),
    a.and
      ? Fl("and", a, e)
      : a.or
      ? Fl("or", a, e)
      : a.not
      ? ux(a, e)
      : vx(a, e)
  );
}
function Fl(a, e, t) {
  var r = e[a],
    n = "";
  H(r) || vt(n), r.length || vt(n);
  var i = a === "and" ? new ix() : new ox();
  return (
    (i.children = O(r, function (o) {
      return uo(o, t);
    })),
    i.children.length || vt(n),
    i
  );
}
function ux(a, e) {
  var t = a.not,
    r = "";
  ec(t) || vt(r);
  var n = new sx();
  return (n.child = uo(t, e)), n.child || vt(r), n;
}
function vx(a, e) {
  for (
    var t = "",
      r = e.prepareGetValue(a),
      n = [],
      i = Lt(a),
      o = a.parser,
      s = o ? Ru(o) : null,
      l = 0;
    l < i.length;
    l++
  ) {
    var u = i[l];
    if (!(u === "parser" || e.valueGetterAttrMap.get(u))) {
      var v = bt(Bl, u) ? Bl[u] : u,
        c = a[u],
        h = s ? s(c) : c,
        f = _f(v, h) || (v === "reg" && new ax(h));
      f || vt(t), n.push(f);
    }
  }
  n.length || vt(t);
  var p = new lx();
  return (
    (p.valueGetterParam = r),
    (p.valueParser = s),
    (p.getValue = e.getValue),
    (p.subCondList = n),
    p
  );
}
function ec(a) {
  return Ge(a) && !bf(a);
}
var cx = (function () {
  function a(e, t) {
    this._cond = uo(e, t);
  }
  return (
    (a.prototype.evaluate = function () {
      return this._cond.evaluate();
    }),
    a
  );
})();
function hx(a, e) {
  return new cx(a, e);
}
var fx = {
    type: "echarts:filter",
    transform: function (a) {
      for (
        var e = a.upstream,
          t,
          r = hx(a.config, {
            valueGetterAttrMap: tt({ dimension: !0 }),
            prepareGetValue: function (s) {
              var l = "",
                u = s.dimension;
              bt(s, "dimension") || vt(l);
              var v = e.getDimensionInfo(u);
              return v || vt(l), { dimIdx: v.index };
            },
            getValue: function (s) {
              return e.retrieveValueFromItem(t, s.dimIdx);
            }
          }),
          n = [],
          i = 0,
          o = e.count();
        i < o;
        i++
      )
        (t = e.getRawDataItem(i)), r.evaluate() && n.push(t);
      return { data: n };
    }
  },
  px = {
    type: "echarts:sort",
    transform: function (a) {
      var e = a.upstream,
        t = a.config,
        r = "",
        n = Jt(t);
      n.length || vt(r);
      var i = [];
      L(n, function (v) {
        var c = v.dimension,
          h = v.order,
          f = v.parser,
          p = v.incomparable;
        if (
          (c == null && vt(r),
          h !== "asc" && h !== "desc" && vt(r),
          p && p !== "min" && p !== "max")
        ) {
          var d = "";
          vt(d);
        }
        if (h !== "asc" && h !== "desc") {
          var g = "";
          vt(g);
        }
        var y = e.getDimensionInfo(c);
        y || vt(r);
        var S = f ? Ru(f) : null;
        f && !S && vt(r),
          i.push({ dimIdx: y.index, parser: S, comparator: new Af(h, p) });
      });
      var o = e.sourceFormat;
      o !== Du && o !== Tf && vt(r);
      for (var s = [], l = 0, u = e.count(); l < u; l++)
        s.push(e.getRawDataItem(l));
      return (
        s.sort(function (v, c) {
          for (var h = 0; h < i.length; h++) {
            var f = i[h],
              p = e.retrieveValueFromItem(v, f.dimIdx),
              d = e.retrieveValueFromItem(c, f.dimIdx);
            f.parser && ((p = f.parser(p)), (d = f.parser(d)));
            var g = f.comparator.evaluate(p, d);
            if (g !== 0) return g;
          }
          return 0;
        }),
        { data: s }
      );
    }
  };
function dx(a) {
  a.registerTransform(fx), a.registerTransform(px);
}
var gx = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = "dataset"), t;
    }
    return (
      (e.prototype.init = function (t, r, n) {
        a.prototype.init.call(this, t, r, n),
          (this._sourceManager = new If(this)),
          Eo(this);
      }),
      (e.prototype.mergeOption = function (t, r) {
        a.prototype.mergeOption.call(this, t, r), Eo(this);
      }),
      (e.prototype.optionUpdated = function () {
        this._sourceManager.dirty();
      }),
      (e.prototype.getSourceManager = function () {
        return this._sourceManager;
      }),
      (e.type = "dataset"),
      (e.defaultOption = { seriesLayoutBy: Df }),
      e
    );
  })(Xt),
  yx = (function (a) {
    k(e, a);
    function e() {
      var t = (a !== null && a.apply(this, arguments)) || this;
      return (t.type = "dataset"), t;
    }
    return (e.type = "dataset"), e;
  })(ee);
function mx(a) {
  a.registerComponentModel(gx), a.registerComponentView(yx);
}
var ae = uu.CMD;
function Xe(a, e) {
  return Math.abs(a - e) < 1e-5;
}
function li(a) {
  var e = a.data,
    t = a.len(),
    r = [],
    n,
    i = 0,
    o = 0,
    s = 0,
    l = 0;
  function u(C, P) {
    n && n.length > 2 && r.push(n), (n = [C, P]);
  }
  function v(C, P, E, R) {
    (Xe(C, E) && Xe(P, R)) || n.push(C, P, E, R, E, R);
  }
  function c(C, P, E, R, N, V) {
    var G = Math.abs(P - C),
      B = (Math.tan(G / 4) * 4) / 3,
      F = P < C ? -1 : 1,
      Y = Math.cos(C),
      K = Math.sin(C),
      $ = Math.cos(P),
      j = Math.sin(P),
      yt = Y * N + E,
      re = K * V + R,
      X = $ * N + E,
      U = j * V + R,
      rt = N * B * F,
      J = V * B * F;
    n.push(yt - rt * K, re + J * Y, X + rt * j, U - J * $, X, U);
  }
  for (var h, f, p, d, g = 0; g < t; ) {
    var y = e[g++],
      S = g === 1;
    switch (
      (S &&
        ((i = e[g]),
        (o = e[g + 1]),
        (s = i),
        (l = o),
        (y === ae.L || y === ae.C || y === ae.Q) && (n = [s, l])),
      y)
    ) {
      case ae.M:
        (i = s = e[g++]), (o = l = e[g++]), u(s, l);
        break;
      case ae.L:
        (h = e[g++]), (f = e[g++]), v(i, o, h, f), (i = h), (o = f);
        break;
      case ae.C:
        n.push(e[g++], e[g++], e[g++], e[g++], (i = e[g++]), (o = e[g++]));
        break;
      case ae.Q:
        (h = e[g++]),
          (f = e[g++]),
          (p = e[g++]),
          (d = e[g++]),
          n.push(
            i + (2 / 3) * (h - i),
            o + (2 / 3) * (f - o),
            p + (2 / 3) * (h - p),
            d + (2 / 3) * (f - d),
            p,
            d
          ),
          (i = p),
          (o = d);
        break;
      case ae.A:
        var m = e[g++],
          x = e[g++],
          b = e[g++],
          _ = e[g++],
          w = e[g++],
          A = e[g++] + w;
        g += 1;
        var I = !e[g++];
        (h = Math.cos(w) * b + m),
          (f = Math.sin(w) * _ + x),
          S ? ((s = h), (l = f), u(s, l)) : v(i, o, h, f),
          (i = Math.cos(A) * b + m),
          (o = Math.sin(A) * _ + x);
        for (
          var T = ((I ? -1 : 1) * Math.PI) / 2, D = w;
          I ? D > A : D < A;
          D += T
        ) {
          var M = I ? Math.max(D + T, A) : Math.min(D + T, A);
          c(D, M, m, x, b, _);
        }
        break;
      case ae.R:
        (s = i = e[g++]),
          (l = o = e[g++]),
          (h = s + e[g++]),
          (f = l + e[g++]),
          u(h, l),
          v(h, l, h, f),
          v(h, f, s, f),
          v(s, f, s, l),
          v(s, l, h, l);
        break;
      case ae.Z:
        n && v(i, o, s, l), (i = s), (o = l);
        break;
    }
  }
  return n && n.length > 2 && r.push(n), r;
}
function ui(a, e, t, r, n, i, o, s, l, u) {
  if (Xe(a, t) && Xe(e, r) && Xe(n, o) && Xe(i, s)) {
    l.push(o, s);
    return;
  }
  var v = 2 / u,
    c = v * v,
    h = o - a,
    f = s - e,
    p = Math.sqrt(h * h + f * f);
  (h /= p), (f /= p);
  var d = t - a,
    g = r - e,
    y = n - o,
    S = i - s,
    m = d * d + g * g,
    x = y * y + S * S;
  if (m < c && x < c) {
    l.push(o, s);
    return;
  }
  var b = h * d + f * g,
    _ = -h * y - f * S,
    w = m - b * b,
    A = x - _ * _;
  if (w < c && b >= 0 && A < c && _ >= 0) {
    l.push(o, s);
    return;
  }
  var I = [],
    T = [];
  sa(a, t, n, o, 0.5, I),
    sa(e, r, i, s, 0.5, T),
    ui(I[0], T[0], I[1], T[1], I[2], T[2], I[3], T[3], l, u),
    ui(I[4], T[4], I[5], T[5], I[6], T[6], I[7], T[7], l, u);
}
function Sx(a, e) {
  var t = li(a),
    r = [];
  e = e || 1;
  for (var n = 0; n < t.length; n++) {
    var i = t[n],
      o = [],
      s = i[0],
      l = i[1];
    o.push(s, l);
    for (var u = 2; u < i.length; ) {
      var v = i[u++],
        c = i[u++],
        h = i[u++],
        f = i[u++],
        p = i[u++],
        d = i[u++];
      ui(s, l, v, c, h, f, p, d, o, e), (s = p), (l = d);
    }
    r.push(o);
  }
  return r;
}
function rc(a, e, t) {
  var r = a[e],
    n = a[1 - e],
    i = Math.abs(r / n),
    o = Math.ceil(Math.sqrt(i * t)),
    s = Math.floor(t / o);
  s === 0 && ((s = 1), (o = t));
  for (var l = [], u = 0; u < o; u++) l.push(s);
  var v = o * s,
    c = t - v;
  if (c > 0) for (var u = 0; u < c; u++) l[u % o] += 1;
  return l;
}
function Wl(a, e, t) {
  for (
    var r = a.r0,
      n = a.r,
      i = a.startAngle,
      o = a.endAngle,
      s = Math.abs(o - i),
      l = s * n,
      u = n - r,
      v = l > Math.abs(u),
      c = rc([l, u], v ? 0 : 1, e),
      h = (v ? s : u) / c.length,
      f = 0;
    f < c.length;
    f++
  )
    for (var p = (v ? u : s) / c[f], d = 0; d < c[f]; d++) {
      var g = {};
      v
        ? ((g.startAngle = i + h * f),
          (g.endAngle = i + h * (f + 1)),
          (g.r0 = r + p * d),
          (g.r = r + p * (d + 1)))
        : ((g.startAngle = i + p * d),
          (g.endAngle = i + p * (d + 1)),
          (g.r0 = r + h * f),
          (g.r = r + h * (f + 1))),
        (g.clockwise = a.clockwise),
        (g.cx = a.cx),
        (g.cy = a.cy),
        t.push(g);
    }
}
function xx(a, e, t) {
  for (
    var r = a.width,
      n = a.height,
      i = r > n,
      o = rc([r, n], i ? 0 : 1, e),
      s = i ? "width" : "height",
      l = i ? "height" : "width",
      u = i ? "x" : "y",
      v = i ? "y" : "x",
      c = a[s] / o.length,
      h = 0;
    h < o.length;
    h++
  )
    for (var f = a[l] / o[h], p = 0; p < o[h]; p++) {
      var d = {};
      (d[u] = h * c),
        (d[v] = p * f),
        (d[s] = c),
        (d[l] = f),
        (d.x += a.x),
        (d.y += a.y),
        t.push(d);
    }
}
function Hl(a, e, t, r) {
  return a * r - t * e;
}
function bx(a, e, t, r, n, i, o, s) {
  var l = t - a,
    u = r - e,
    v = o - n,
    c = s - i,
    h = Hl(v, c, l, u);
  if (Math.abs(h) < 1e-6) return null;
  var f = a - n,
    p = e - i,
    d = Hl(f, p, v, c) / h;
  return d < 0 || d > 1 ? null : new de(d * l + a, d * u + e);
}
function _x(a, e, t) {
  var r = new de();
  de.sub(r, t, e), r.normalize();
  var n = new de();
  de.sub(n, a, e);
  var i = n.dot(r);
  return i;
}
function We(a, e) {
  var t = a[a.length - 1];
  (t && t[0] === e[0] && t[1] === e[1]) || a.push(e);
}
function wx(a, e, t) {
  for (var r = a.length, n = [], i = 0; i < r; i++) {
    var o = a[i],
      s = a[(i + 1) % r],
      l = bx(o[0], o[1], s[0], s[1], e.x, e.y, t.x, t.y);
    l && n.push({ projPt: _x(l, e, t), pt: l, idx: i });
  }
  if (n.length < 2) return [{ points: a }, { points: a }];
  n.sort(function (g, y) {
    return g.projPt - y.projPt;
  });
  var u = n[0],
    v = n[n.length - 1];
  if (v.idx < u.idx) {
    var c = u;
    (u = v), (v = c);
  }
  for (
    var h = [u.pt.x, u.pt.y],
      f = [v.pt.x, v.pt.y],
      p = [h],
      d = [f],
      i = u.idx + 1;
    i <= v.idx;
    i++
  )
    We(p, a[i].slice());
  We(p, f), We(p, h);
  for (var i = v.idx + 1; i <= u.idx + r; i++) We(d, a[i % r].slice());
  return We(d, h), We(d, f), [{ points: p }, { points: d }];
}
function Ul(a) {
  var e = a.points,
    t = [],
    r = [];
  Ma(e, t, r);
  var n = new ft(t[0], t[1], r[0] - t[0], r[1] - t[1]),
    i = n.width,
    o = n.height,
    s = n.x,
    l = n.y,
    u = new de(),
    v = new de();
  return (
    i > o
      ? ((u.x = v.x = s + i / 2), (u.y = l), (v.y = l + o))
      : ((u.y = v.y = l + o / 2), (u.x = s), (v.x = s + i)),
    wx(e, u, v)
  );
}
function ma(a, e, t, r) {
  if (t === 1) r.push(e);
  else {
    var n = Math.floor(t / 2),
      i = a(e);
    ma(a, i[0], n, r), ma(a, i[1], t - n, r);
  }
  return r;
}
function Ax(a, e) {
  for (var t = [], r = 0; r < e; r++) t.push(Di(a));
  return t;
}
function Tx(a, e) {
  e.setStyle(a.style), (e.z = a.z), (e.z2 = a.z2), (e.zlevel = a.zlevel);
}
function Ix(a) {
  for (var e = [], t = 0; t < a.length; ) e.push([a[t++], a[t++]]);
  return e;
}
function Dx(a, e) {
  var t = [],
    r = a.shape,
    n;
  switch (a.type) {
    case "rect":
      xx(r, e, t), (n = Et);
      break;
    case "sector":
      Wl(r, e, t), (n = Ve);
      break;
    case "circle":
      Wl(
        {
          r0: 0,
          r: r.r,
          startAngle: 0,
          endAngle: Math.PI * 2,
          cx: r.cx,
          cy: r.cy
        },
        e,
        t
      ),
        (n = Ve);
      break;
    default:
      var i = a.getComputedTransform(),
        o = i
          ? Math.sqrt(
              Math.max(i[0] * i[0] + i[1] * i[1], i[2] * i[2] + i[3] * i[3])
            )
          : 1,
        s = O(Sx(a.getUpdatedPathProxy(), o), function (y) {
          return Ix(y);
        }),
        l = s.length;
      if (l === 0) ma(Ul, { points: s[0] }, e, t);
      else if (l === e) for (var u = 0; u < l; u++) t.push({ points: s[u] });
      else {
        var v = 0,
          c = O(s, function (y) {
            var S = [],
              m = [];
            Ma(y, S, m);
            var x = (m[1] - S[1]) * (m[0] - S[0]);
            return (v += x), { poly: y, area: x };
          });
        c.sort(function (y, S) {
          return S.area - y.area;
        });
        for (var h = e, u = 0; u < l; u++) {
          var f = c[u];
          if (h <= 0) break;
          var p = u === l - 1 ? h : Math.ceil((f.area / v) * e);
          p < 0 || (ma(Ul, { points: f.poly }, p, t), (h -= p));
        }
      }
      n = ve;
      break;
  }
  if (!n) return Ax(a, e);
  for (var d = [], u = 0; u < t.length; u++) {
    var g = new n();
    g.setShape(t[u]), Tx(a, g), d.push(g);
  }
  return d;
}
function Lx(a, e) {
  var t = a.length,
    r = e.length;
  if (t === r) return [a, e];
  for (
    var n = [],
      i = [],
      o = t < r ? a : e,
      s = Math.min(t, r),
      l = Math.abs(r - t) / 6,
      u = (s - 2) / 6,
      v = Math.ceil(l / u) + 1,
      c = [o[0], o[1]],
      h = l,
      f = 2;
    f < s;

  ) {
    var p = o[f - 2],
      d = o[f - 1],
      g = o[f++],
      y = o[f++],
      S = o[f++],
      m = o[f++],
      x = o[f++],
      b = o[f++];
    if (h <= 0) {
      c.push(g, y, S, m, x, b);
      continue;
    }
    for (var _ = Math.min(h, v - 1) + 1, w = 1; w <= _; w++) {
      var A = w / _;
      sa(p, g, S, x, A, n),
        sa(d, y, m, b, A, i),
        (p = n[3]),
        (d = i[3]),
        c.push(n[1], i[1], n[2], i[2], p, d),
        (g = n[5]),
        (y = i[5]),
        (S = n[6]),
        (m = i[6]);
    }
    h -= _ - 1;
  }
  return o === a ? [c, e] : [a, c];
}
function Yl(a, e) {
  for (
    var t = a.length, r = a[t - 2], n = a[t - 1], i = [], o = 0;
    o < e.length;

  )
    (i[o++] = r), (i[o++] = n);
  return i;
}
function Cx(a, e) {
  for (
    var t, r, n, i = [], o = [], s = 0;
    s < Math.max(a.length, e.length);
    s++
  ) {
    var l = a[s],
      u = e[s],
      v = void 0,
      c = void 0;
    l
      ? u
        ? ((t = Lx(l, u)), (v = t[0]), (c = t[1]), (r = v), (n = c))
        : ((c = Yl(n || l, l)), (v = l))
      : ((v = Yl(r || u, u)), (c = u)),
      i.push(v),
      o.push(c);
  }
  return [i, o];
}
function $l(a) {
  for (
    var e = 0, t = 0, r = 0, n = a.length, i = 0, o = n - 2;
    i < n;
    o = i, i += 2
  ) {
    var s = a[o],
      l = a[o + 1],
      u = a[i],
      v = a[i + 1],
      c = s * v - u * l;
    (e += c), (t += (s + u) * c), (r += (l + v) * c);
  }
  return e === 0 ? [a[0] || 0, a[1] || 0] : [t / e / 3, r / e / 3, e];
}
function Px(a, e, t, r) {
  for (
    var n = (a.length - 2) / 6,
      i = 1 / 0,
      o = 0,
      s = a.length,
      l = s - 2,
      u = 0;
    u < n;
    u++
  ) {
    for (var v = u * 6, c = 0, h = 0; h < s; h += 2) {
      var f = h === 0 ? v : ((v + h - 2) % l) + 2,
        p = a[f] - t[0],
        d = a[f + 1] - t[1],
        g = e[h] - r[0],
        y = e[h + 1] - r[1],
        S = g - p,
        m = y - d;
      c += S * S + m * m;
    }
    c < i && ((i = c), (o = u));
  }
  return o;
}
function Mx(a) {
  for (var e = [], t = a.length, r = 0; r < t; r += 2)
    (e[r] = a[t - r - 2]), (e[r + 1] = a[t - r - 1]);
  return e;
}
function Ex(a, e, t, r) {
  for (var n = [], i, o = 0; o < a.length; o++) {
    var s = a[o],
      l = e[o],
      u = $l(s),
      v = $l(l);
    i == null && (i = u[2] < 0 != v[2] < 0);
    var c = [],
      h = [],
      f = 0,
      p = 1 / 0,
      d = [],
      g = s.length;
    i && (s = Mx(s));
    for (var y = Px(s, l, u, v) * 6, S = g - 2, m = 0; m < S; m += 2) {
      var x = ((y + m) % S) + 2;
      (c[m + 2] = s[x] - u[0]), (c[m + 3] = s[x + 1] - u[1]);
    }
    (c[0] = s[y] - u[0]), (c[1] = s[y + 1] - u[1]);
    for (var b = r / t, _ = -r / 2; _ <= r / 2; _ += b) {
      for (
        var w = Math.sin(_), A = Math.cos(_), I = 0, m = 0;
        m < s.length;
        m += 2
      ) {
        var T = c[m],
          D = c[m + 1],
          M = l[m] - v[0],
          C = l[m + 1] - v[1],
          P = M * A - C * w,
          E = M * w + C * A;
        (d[m] = P), (d[m + 1] = E);
        var R = P - T,
          N = E - D;
        I += R * R + N * N;
      }
      if (I < p) {
        (p = I), (f = _);
        for (var V = 0; V < d.length; V++) h[V] = d[V];
      }
    }
    n.push({ from: c, to: h, fromCp: u, toCp: v, rotation: -f });
  }
  return n;
}
function Sa(a) {
  return a.__isCombineMorphing;
}
var ac = "__mOriginal_";
function xa(a, e, t) {
  var r = ac + e,
    n = a[r] || a[e];
  a[r] || (a[r] = a[e]);
  var i = t.replace,
    o = t.after,
    s = t.before;
  a[e] = function () {
    var l = arguments,
      u;
    return (
      s && s.apply(this, l),
      i ? (u = i.apply(this, l)) : (u = n.apply(this, l)),
      o && o.apply(this, l),
      u
    );
  };
}
function gr(a, e) {
  var t = ac + e;
  a[t] && ((a[e] = a[t]), (a[t] = null));
}
function Xl(a, e) {
  for (var t = 0; t < a.length; t++)
    for (var r = a[t], n = 0; n < r.length; ) {
      var i = r[n],
        o = r[n + 1];
      (r[n++] = e[0] * i + e[2] * o + e[4]),
        (r[n++] = e[1] * i + e[3] * o + e[5]);
    }
}
function nc(a, e) {
  var t = a.getUpdatedPathProxy(),
    r = e.getUpdatedPathProxy(),
    n = Cx(li(t), li(r)),
    i = n[0],
    o = n[1],
    s = a.getComputedTransform(),
    l = e.getComputedTransform();
  function u() {
    this.transform = null;
  }
  s && Xl(i, s),
    l && Xl(o, l),
    xa(e, "updateTransform", { replace: u }),
    (e.transform = null);
  var v = Ex(i, o, 10, Math.PI),
    c = [];
  xa(e, "buildPath", {
    replace: function (h) {
      for (var f = e.__morphT, p = 1 - f, d = [], g = 0; g < v.length; g++) {
        var y = v[g],
          S = y.from,
          m = y.to,
          x = y.rotation * f,
          b = y.fromCp,
          _ = y.toCp,
          w = Math.sin(x),
          A = Math.cos(x);
        Lf(d, b, _, f);
        for (var I = 0; I < S.length; I += 2) {
          var T = S[I],
            D = S[I + 1],
            M = m[I],
            C = m[I + 1],
            P = T * p + M * f,
            E = D * p + C * f;
          (c[I] = P * A - E * w + d[0]), (c[I + 1] = P * w + E * A + d[1]);
        }
        var R = c[0],
          N = c[1];
        h.moveTo(R, N);
        for (var I = 2; I < S.length; ) {
          var M = c[I++],
            C = c[I++],
            V = c[I++],
            G = c[I++],
            B = c[I++],
            F = c[I++];
          R === M && N === C && V === B && G === F
            ? h.lineTo(B, F)
            : h.bezierCurveTo(M, C, V, G, B, F),
            (R = B),
            (N = F);
        }
      }
    }
  });
}
function vo(a, e, t) {
  if (!a || !e) return e;
  var r = t.done,
    n = t.during;
  nc(a, e), (e.__morphT = 0);
  function i() {
    gr(e, "buildPath"),
      gr(e, "updateTransform"),
      (e.__morphT = -1),
      e.createPathProxy(),
      e.dirtyShape();
  }
  return (
    e.animateTo(
      { __morphT: 1 },
      Q(
        {
          during: function (o) {
            e.dirtyShape(), n && n(o);
          },
          done: function () {
            i(), r && r();
          }
        },
        t
      )
    ),
    e
  );
}
function Rx(a, e, t, r, n, i) {
  var o = 16;
  (a = n === t ? 0 : Math.round((32767 * (a - t)) / (n - t))),
    (e = i === r ? 0 : Math.round((32767 * (e - r)) / (i - r)));
  for (var s = 0, l, u = (1 << o) / 2; u > 0; u /= 2) {
    var v = 0,
      c = 0;
    (a & u) > 0 && (v = 1),
      (e & u) > 0 && (c = 1),
      (s += u * u * ((3 * v) ^ c)),
      c === 0 &&
        (v === 1 && ((a = u - 1 - a), (e = u - 1 - e)),
        (l = a),
        (a = e),
        (e = l));
  }
  return s;
}
function ba(a) {
  var e = 1 / 0,
    t = 1 / 0,
    r = -1 / 0,
    n = -1 / 0,
    i = O(a, function (s) {
      var l = s.getBoundingRect(),
        u = s.getComputedTransform(),
        v = l.x + l.width / 2 + (u ? u[4] : 0),
        c = l.y + l.height / 2 + (u ? u[5] : 0);
      return (
        (e = Math.min(v, e)),
        (t = Math.min(c, t)),
        (r = Math.max(v, r)),
        (n = Math.max(c, n)),
        [v, c]
      );
    }),
    o = O(i, function (s, l) {
      return { cp: s, z: Rx(s[0], s[1], e, t, r, n), path: a[l] };
    });
  return o
    .sort(function (s, l) {
      return s.z - l.z;
    })
    .map(function (s) {
      return s.path;
    });
}
function ic(a) {
  return Dx(a.path, a.count);
}
function vi() {
  return { fromIndividuals: [], toIndividuals: [], count: 0 };
}
function Nx(a, e, t) {
  var r = [];
  function n(b) {
    for (var _ = 0; _ < b.length; _++) {
      var w = b[_];
      Sa(w) ? n(w.childrenRef()) : w instanceof wt && r.push(w);
    }
  }
  n(a);
  var i = r.length;
  if (!i) return vi();
  var o = t.dividePath || ic,
    s = o({ path: e, count: i });
  if (s.length !== i)
    return console.error("Invalid morphing: unmatched splitted path"), vi();
  (r = ba(r)), (s = ba(s));
  for (
    var l = t.done, u = t.during, v = t.individualDelay, c = new Ye(), h = 0;
    h < i;
    h++
  ) {
    var f = r[h],
      p = s[h];
    (p.parent = e), p.copyTransform(c), v || nc(f, p);
  }
  (e.__isCombineMorphing = !0),
    (e.childrenRef = function () {
      return s;
    });
  function d(b) {
    for (var _ = 0; _ < s.length; _++) s[_].addSelfToZr(b);
  }
  xa(e, "addSelfToZr", {
    after: function (b) {
      d(b);
    }
  }),
    xa(e, "removeSelfFromZr", {
      after: function (b) {
        for (var _ = 0; _ < s.length; _++) s[_].removeSelfFromZr(b);
      }
    });
  function g() {
    (e.__isCombineMorphing = !1),
      (e.__morphT = -1),
      (e.childrenRef = null),
      gr(e, "addSelfToZr"),
      gr(e, "removeSelfFromZr");
  }
  var y = s.length;
  if (v)
    for (
      var S = y,
        m = function () {
          S--, S === 0 && (g(), l && l());
        },
        h = 0;
      h < y;
      h++
    ) {
      var x = v
        ? Q({ delay: (t.delay || 0) + v(h, y, r[h], s[h]), done: m }, t)
        : t;
      vo(r[h], s[h], x);
    }
  else
    (e.__morphT = 0),
      e.animateTo(
        { __morphT: 1 },
        Q(
          {
            during: function (b) {
              for (var _ = 0; _ < y; _++) {
                var w = s[_];
                (w.__morphT = e.__morphT), w.dirtyShape();
              }
              u && u(b);
            },
            done: function () {
              g();
              for (var b = 0; b < a.length; b++) gr(a[b], "updateTransform");
              l && l();
            }
          },
          t
        )
      );
  return (
    e.__zr && d(e.__zr), { fromIndividuals: r, toIndividuals: s, count: y }
  );
}
function kx(a, e, t) {
  var r = e.length,
    n = [],
    i = t.dividePath || ic;
  function o(f) {
    for (var p = 0; p < f.length; p++) {
      var d = f[p];
      Sa(d) ? o(d.childrenRef()) : d instanceof wt && n.push(d);
    }
  }
  if (Sa(a)) {
    o(a.childrenRef());
    var s = n.length;
    if (s < r) for (var l = 0, u = s; u < r; u++) n.push(Di(n[l++ % s]));
    n.length = r;
  } else {
    n = i({ path: a, count: r });
    for (var v = a.getComputedTransform(), u = 0; u < n.length; u++)
      n[u].setLocalTransform(v);
    if (n.length !== r)
      return console.error("Invalid morphing: unmatched splitted path"), vi();
  }
  (n = ba(n)), (e = ba(e));
  for (var c = t.individualDelay, u = 0; u < r; u++) {
    var h = c ? Q({ delay: (t.delay || 0) + c(u, r, n[u], e[u]) }, t) : t;
    vo(n[u], e[u], h);
  }
  return { fromIndividuals: n, toIndividuals: e, count: e.length };
}
function Zl(a) {
  return H(a[0]);
}
function ql(a, e) {
  for (var t = [], r = a.length, n = 0; n < r; n++)
    t.push({ one: a[n], many: [] });
  for (var n = 0; n < e.length; n++) {
    var i = e[n].length,
      o = void 0;
    for (o = 0; o < i; o++) t[o % r].many.push(e[n][o]);
  }
  for (var s = 0, n = r - 1; n >= 0; n--)
    if (!t[n].many.length) {
      var l = t[s].many;
      if (l.length <= 1)
        if (s) s = 0;
        else return t;
      var i = l.length,
        u = Math.ceil(i / 2);
      (t[n].many = l.slice(u, i)), (t[s].many = l.slice(0, u)), s++;
    }
  return t;
}
var Vx = {
  clone: function (a) {
    for (
      var e = [],
        t = 1 - Math.pow(1 - a.path.style.opacity, 1 / a.count),
        r = 0;
      r < a.count;
      r++
    ) {
      var n = Di(a.path);
      n.setStyle("opacity", t), e.push(n);
    }
    return e;
  },
  split: null
};
function Rn(a, e, t, r, n, i) {
  if (!a.length || !e.length) return;
  var o = Li("update", r, n);
  if (!(o && o.duration > 0)) return;
  var s = r.getModel("universalTransition").get("delay"),
    l = Object.assign({ setToFinal: !0 }, o),
    u,
    v;
  Zl(a) && ((u = a), (v = e)), Zl(e) && ((u = e), (v = a));
  function c(y, S, m, x, b) {
    var _ = y.many,
      w = y.one;
    if (_.length === 1 && !b) {
      var A = S ? _[0] : w,
        I = S ? w : _[0];
      if (Sa(A)) c({ many: [A], one: I }, !0, m, x, !0);
      else {
        var T = s ? Q({ delay: s(m, x) }, l) : l;
        vo(A, I, T), i(A, I, A, I, T);
      }
    } else
      for (
        var D = Q(
            {
              dividePath: Vx[t],
              individualDelay:
                s &&
                function (N, V, G, B) {
                  return s(N + m, x);
                }
            },
            l
          ),
          M = S ? Nx(_, w, D) : kx(w, _, D),
          C = M.fromIndividuals,
          P = M.toIndividuals,
          E = C.length,
          R = 0;
        R < E;
        R++
      ) {
        var T = s ? Q({ delay: s(R, E) }, l) : l;
        i(C[R], P[R], S ? _[R] : y.one, S ? y.one : _[R], T);
      }
  }
  for (
    var h = u ? u === a : a.length > e.length,
      f = u ? ql(v, u) : ql(h ? e : a, [h ? a : e]),
      p = 0,
      d = 0;
    d < f.length;
    d++
  )
    p += f[d].many.length;
  for (var g = 0, d = 0; d < f.length; d++)
    c(f[d], h, g, p), (g += f[d].many.length);
}
function Me(a) {
  if (!a) return [];
  if (H(a)) {
    for (var e = [], t = 0; t < a.length; t++) e.push(Me(a[t]));
    return e;
  }
  var r = [];
  return (
    a.traverse(function (n) {
      n instanceof wt &&
        !n.disableMorphing &&
        !n.invisible &&
        !n.ignore &&
        r.push(n);
    }),
    r
  );
}
var oc = 1e4,
  Gx = 0,
  Kl = 1,
  jl = 2,
  zx = _t();
function Ox(a, e) {
  for (var t = a.dimensions, r = 0; r < t.length; r++) {
    var n = a.getDimensionInfo(t[r]);
    if (n && n.otherDims[e] === 0) return t[r];
  }
}
function Bx(a, e, t) {
  var r = a.getDimensionInfo(t),
    n = r && r.ordinalMeta;
  if (r) {
    var i = a.get(r.name, e);
    return (n && n.categories[i]) || i + "";
  }
}
function Jl(a, e, t, r) {
  var n = r ? "itemChildGroupId" : "itemGroupId",
    i = Ox(a, n);
  if (i) {
    var o = Bx(a, e, i);
    return o;
  }
  var s = a.getRawDataItem(e),
    l = r ? "childGroupId" : "groupId";
  if (s && s[l]) return s[l] + "";
  if (!r) return t || a.getId(e);
}
function Ql(a) {
  var e = [];
  return (
    L(a, function (t) {
      var r = t.data,
        n = t.dataGroupId;
      if (!(r.count() > oc))
        for (var i = r.getIndices(), o = 0; o < i.length; o++)
          e.push({
            data: r,
            groupId: Jl(r, o, n, !1),
            childGroupId: Jl(r, o, n, !0),
            divide: t.divide,
            dataIndex: o
          });
    }),
    e
  );
}
function Nn(a, e, t) {
  a.traverse(function (r) {
    r instanceof wt &&
      Ct(r, { style: { opacity: 0 } }, e, { dataIndex: t, isFrom: !0 });
  });
}
function kn(a) {
  if (a.parent) {
    var e = a.getComputedTransform();
    a.setLocalTransform(e), a.parent.remove(a);
  }
}
function He(a) {
  a.stopAnimation(),
    a.isGroup &&
      a.traverse(function (e) {
        e.stopAnimation();
      });
}
function Fx(a, e, t) {
  var r = Li("update", t, e);
  r &&
    a.traverse(function (n) {
      if (n instanceof mr) {
        var i = Cf(n);
        i && n.animateFrom({ style: i }, r);
      }
    });
}
function Wx(a, e) {
  var t = a.length;
  if (t !== e.length) return !1;
  for (var r = 0; r < t; r++) {
    var n = a[r],
      i = e[r];
    if (n.data.getId(n.dataIndex) !== i.data.getId(i.dataIndex)) return !1;
  }
  return !0;
}
function sc(a, e, t) {
  var r = Ql(a),
    n = Ql(e);
  function i(m, x, b, _, w) {
    (b || m) &&
      x.animateFrom(
        { style: b && b !== m ? W(W({}, b.style), m.style) : m.style },
        w
      );
  }
  var o = !1,
    s = Gx,
    l = tt(),
    u = tt();
  r.forEach(function (m) {
    m.groupId && l.set(m.groupId, !0),
      m.childGroupId && u.set(m.childGroupId, !0);
  });
  for (var v = 0; v < n.length; v++) {
    var c = n[v].groupId;
    if (u.get(c)) {
      s = Kl;
      break;
    }
    var h = n[v].childGroupId;
    if (h && l.get(h)) {
      s = jl;
      break;
    }
  }
  function f(m, x) {
    return function (b) {
      var _ = b.data,
        w = b.dataIndex;
      return x
        ? _.getId(w)
        : m
        ? s === Kl
          ? b.childGroupId
          : b.groupId
        : s === jl
        ? b.childGroupId
        : b.groupId;
    };
  }
  var p = Wx(r, n),
    d = {};
  if (!p)
    for (var v = 0; v < n.length; v++) {
      var g = n[v],
        y = g.data.getItemGraphicEl(g.dataIndex);
      y && (d[y.id] = !0);
    }
  function S(m, x) {
    var b = r[x],
      _ = n[m],
      w = _.data.hostModel,
      A = b.data.getItemGraphicEl(b.dataIndex),
      I = _.data.getItemGraphicEl(_.dataIndex);
    if (A === I) {
      I && Fx(I, _.dataIndex, w);
      return;
    }
    (A && d[A.id]) ||
      (I &&
        (He(I),
        A
          ? (He(A), kn(A), (o = !0), Rn(Me(A), Me(I), _.divide, w, m, i))
          : Nn(I, w, m)));
  }
  new je(r, n, f(!0, p), f(!1, p), null, "multiple")
    .update(S)
    .updateManyToOne(function (m, x) {
      var b = n[m],
        _ = b.data,
        w = _.hostModel,
        A = _.getItemGraphicEl(b.dataIndex),
        I = Dt(
          O(x, function (T) {
            return r[T].data.getItemGraphicEl(r[T].dataIndex);
          }),
          function (T) {
            return T && T !== A && !d[T.id];
          }
        );
      A &&
        (He(A),
        I.length
          ? (L(I, function (T) {
              He(T), kn(T);
            }),
            (o = !0),
            Rn(Me(I), Me(A), b.divide, w, m, i))
          : Nn(A, w, b.dataIndex));
    })
    .updateOneToMany(function (m, x) {
      var b = r[x],
        _ = b.data.getItemGraphicEl(b.dataIndex);
      if (!(_ && d[_.id])) {
        var w = Dt(
            O(m, function (I) {
              return n[I].data.getItemGraphicEl(n[I].dataIndex);
            }),
            function (I) {
              return I && I !== _;
            }
          ),
          A = n[m[0]].data.hostModel;
        w.length &&
          (L(w, function (I) {
            return He(I);
          }),
          _
            ? (He(_), kn(_), (o = !0), Rn(Me(_), Me(w), b.divide, A, m[0], i))
            : L(w, function (I) {
                return Nn(I, A, m[0]);
              }));
      }
    })
    .updateManyToMany(function (m, x) {
      new je(
        x,
        m,
        function (b) {
          return r[b].data.getId(r[b].dataIndex);
        },
        function (b) {
          return n[b].data.getId(n[b].dataIndex);
        }
      )
        .update(function (b, _) {
          S(m[b], x[_]);
        })
        .execute();
    })
    .execute(),
    o &&
      L(e, function (m) {
        var x = m.data,
          b = x.hostModel,
          _ = b && t.getViewOfSeriesModel(b),
          w = Li("update", b, 0);
        _ &&
          b.isAnimationEnabled() &&
          w &&
          w.duration > 0 &&
          _.group.traverse(function (A) {
            A instanceof wt &&
              !A.animators.length &&
              A.animateFrom({ style: { opacity: 0 } }, w);
          });
      });
}
function tu(a) {
  var e = a.getModel("universalTransition").get("seriesKey");
  return e || a.id;
}
function eu(a) {
  return H(a) ? a.sort().join(",") : a;
}
function fe(a) {
  if (a.hostModel)
    return a.hostModel.getModel("universalTransition").get("divideShape");
}
function Hx(a, e) {
  var t = tt(),
    r = tt(),
    n = tt();
  return (
    L(a.oldSeries, function (i, o) {
      var s = a.oldDataGroupIds[o],
        l = a.oldData[o],
        u = tu(i),
        v = eu(u);
      r.set(v, { dataGroupId: s, data: l }),
        H(u) &&
          L(u, function (c) {
            n.set(c, { key: v, dataGroupId: s, data: l });
          });
    }),
    L(e.updatedSeries, function (i) {
      if (i.isUniversalTransitionEnabled() && i.isAnimationEnabled()) {
        var o = i.get("dataGroupId"),
          s = i.getData(),
          l = tu(i),
          u = eu(l),
          v = r.get(u);
        if (v)
          t.set(u, {
            oldSeries: [
              { dataGroupId: v.dataGroupId, divide: fe(v.data), data: v.data }
            ],
            newSeries: [{ dataGroupId: o, divide: fe(s), data: s }]
          });
        else if (H(l)) {
          var c = [];
          L(l, function (p) {
            var d = r.get(p);
            d.data &&
              c.push({
                dataGroupId: d.dataGroupId,
                divide: fe(d.data),
                data: d.data
              });
          }),
            c.length &&
              t.set(u, {
                oldSeries: c,
                newSeries: [{ dataGroupId: o, data: s, divide: fe(s) }]
              });
        } else {
          var h = n.get(l);
          if (h) {
            var f = t.get(h.key);
            f ||
              ((f = {
                oldSeries: [
                  {
                    dataGroupId: h.dataGroupId,
                    data: h.data,
                    divide: fe(h.data)
                  }
                ],
                newSeries: []
              }),
              t.set(h.key, f)),
              f.newSeries.push({ dataGroupId: o, data: s, divide: fe(s) });
          }
        }
      }
    }),
    t
  );
}
function ru(a, e) {
  for (var t = 0; t < a.length; t++) {
    var r =
      (e.seriesIndex != null && e.seriesIndex === a[t].seriesIndex) ||
      (e.seriesId != null && e.seriesId === a[t].id);
    if (r) return t;
  }
}
function Ux(a, e, t, r) {
  var n = [],
    i = [];
  L(Jt(a.from), function (o) {
    var s = ru(e.oldSeries, o);
    s >= 0 &&
      n.push({
        dataGroupId: e.oldDataGroupIds[s],
        data: e.oldData[s],
        divide: fe(e.oldData[s]),
        groupIdDim: o.dimension
      });
  }),
    L(Jt(a.to), function (o) {
      var s = ru(t.updatedSeries, o);
      if (s >= 0) {
        var l = t.updatedSeries[s].getData();
        i.push({
          dataGroupId: e.oldDataGroupIds[s],
          data: l,
          divide: fe(l),
          groupIdDim: o.dimension
        });
      }
    }),
    n.length > 0 && i.length > 0 && sc(n, i, r);
}
function Yx(a) {
  a.registerUpdateLifecycle("series:beforeupdate", function (e, t, r) {
    L(Jt(r.seriesTransition), function (n) {
      L(Jt(n.to), function (i) {
        for (var o = r.updatedSeries, s = 0; s < o.length; s++)
          ((i.seriesIndex != null && i.seriesIndex === o[s].seriesIndex) ||
            (i.seriesId != null && i.seriesId === o[s].id)) &&
            (o[s][Wa] = !0);
      });
    });
  }),
    a.registerUpdateLifecycle("series:transition", function (e, t, r) {
      var n = zx(t);
      if (n.oldSeries && r.updatedSeries && r.optionChanged) {
        var i = r.seriesTransition;
        if (i)
          L(Jt(i), function (f) {
            Ux(f, n, r, t);
          });
        else {
          var o = Hx(n, r);
          L(o.keys(), function (f) {
            var p = o.get(f);
            sc(p.oldSeries, p.newSeries, t);
          });
        }
        L(r.updatedSeries, function (f) {
          f[Wa] && (f[Wa] = !1);
        });
      }
      for (
        var s = e.getSeries(),
          l = (n.oldSeries = []),
          u = (n.oldDataGroupIds = []),
          v = (n.oldData = []),
          c = 0;
        c < s.length;
        c++
      ) {
        var h = s[c].getData();
        h.count() < oc &&
          (l.push(s[c]), u.push(s[c].get("dataGroupId")), v.push(h));
      }
    });
}
q([Pf]);
q([Pp]);
q([
  Mf,
  Ef,
  Rf,
  Vp,
  Xp,
  Pd,
  ng,
  Gg,
  iy,
  cy,
  xy,
  Qy,
  Am,
  km,
  jm,
  e0,
  c0,
  m0,
  C0,
  k0,
  Y0,
  mS
]);
q(Nf);
q(US);
q(av);
q(a1);
q(Dv);
q(s1);
q(kf);
q(Vf);
q(Gf);
q(Ai);
q(_1);
q(zf);
q(E1);
q(O1);
q(Y1);
q(j1);
q(Of);
q(Bf);
q(Ff);
q(Wf);
q(Hf);
q(Uf);
q(Yf);
q(rx);
q(dx);
q(mx);
q(Yx);
q(Kf);
export { Kf as a, mx as i };
