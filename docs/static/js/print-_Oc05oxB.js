const s = function (e, n) {
  if (((n = n || {}), !(this instanceof s))) return new s(e, n);
  this.conf = {
    styleStr: "",
    setDomHeightArr: [],
    printBeforeFn: null,
  ...
  for (const o in this.conf) o && n.hasOwnProperty(o) && (this.conf[o] = n[o]);
  typeof e == "string"
...
s.prototype = {
  ...
  getStyle: function () {
    let e = "";
    const n = document.querySelectorAll("style,link");
    for (let o = 0; o < n.length; o++) e += n[o].outerHTML;
    return (
  ...
  getHtml: function () {
    ...
    for (let t = 0; t < n.length; t++)
      if (n[t].type == "select-one") {
        const r = n[t].children;
        for (const i in r)
          r[i].tagName == "OPTION" &&
            (r[i].selected == !0
              ? r[i].setAttribute("selected", "selected")
              : r[i].removeAttribute("selected"));
      }
    for (let t = 0; t < c.length; t++) {
      const r = c[t].toDataURL("image/png"),
        i = document.createElement("img");
      (i.src = r),
        i.setAttribute("style", "max-width: 100%;"),
        (i.className = "isNeedRemove"),
        c[t].parentNode.insertBefore(i, c[t].nextElementSibling);
    }
  ...
  writeIframe: function (e) {
    ...
    (c.id = "myIframe"),
      ...
      (o = t.contentDocument || t.contentWindow.document),
      o.open(),
      o.write(e),
      o.close();
    const r = document.querySelectorAll(".isNeedRemove");
    for (let l = 0; l < r.length; l++) r[l].parentNode.removeChild(r[l]);
    const i = this;
    c.onload = function () {
      i.conf.printBeforeFn && i.conf.printBeforeFn({ doc: o }),
        i.toPrint(n),
        setTimeout(function () {
          document.body.removeChild(c),
            i.conf.printDoneCallBack && i.conf.printDoneCallBack();
        }, 100);
    };
  },
  toPrint: function (e) {
